...
const evidencePath = path.join(
  repoRoot,
  'docs/evidence/rpp-0896-plugin-update-hooks-topology-v5.md',
);
...
test('RPP-0896 carries release verifier through plugin update hook topology variant 5 fail-closed evidence', () => {
  const { report, text } = loadProgressReport();
  const validation = validatePluginUpdateTopologyVariant5Report(report);

  assert.equal(validation.ok, true, JSON.stringify(validation.failures));
  assert.equal(report.schemaVersion, 1);
  assert.equal(report.rppId, 'RPP-0896');
  ...
  assert.equal(report.candidateScope.status, 'plugin-update-hooks-topology-candidate-v5');
  ...
  assert.equal(report.candidateScope.hookTopologyRequirements.requiresReleaseVerifierCarryThrough, true);
  ...
  assert.ok(report.candidateScope.candidateClaims.includes('variant-5-startup-or-exact-capability-contract-recorded'));
...
test('RPP-0896 topology command starts sites or records exact unavailable capability', () => {
  ...
  assert.equal(artifact.commands.runHarness, topologyCommand);
...
test('RPP-0896 release verifier failure is carried through when Docker is unavailable', () => {
  const { report } = loadProgressReport();
  ...
  assert.equal(report.releaseVerifier.verifyReleaseFailure.exitCode, artifact.evidence.verifyReleaseFailure.exitCode);
  ...
  assert.equal(report.releaseVerifier.hookProofAccepted, false);
  ...
  assert.deepEqual(report.releaseVerifier.requiredHookProofs, pluginUpdateHookSurface);
  assert.deepEqual(report.releaseVerifier.primaryReleaseUrlEnvKeys, [
    'REPRINT_PUSH_SOURCE_URL',
    'REPRINT_PUSH_REMOTE_CHANGED_URL',
    'REPRINT_PUSH_LOCAL_URL',
  ]);
  assert.deepEqual(report.releaseVerifier.supportReleaseUrlEnvKeys, [
    'REPRINT_PUSH_APPLY_REVALIDATION_SOURCE_URL',
  ]);
  assert.equal(report.releaseVerifier.serviceDnsHostCount, 4);
});

test('RPP-0896 release movement stays NO-GO until live update hook proof passes', () => {
  const { report } = loadProgressReport();
  ...
  assert.ok(
    report.releaseReadyScope.requiredEvidence.includes('release-verifier-accepts-live-plugin-update-hook-proof'),
  );
  assert.ok(
  ...
  assert.ok(
    report.releaseReadyScope.blockers.includes('release-verifier-carry-through-is-fail-closed-not-release-ready'),
  );
...
test('RPP-0896 validation rejects ambiguous non-started topology reports', () => {
  const { report } = loadProgressReport();
  const ambiguous = {
    ...report,
    topologyCommand: {
  ...
  const validation = validatePluginUpdateTopologyVariant5Report(ambiguous);
...
test('RPP-0896 validation rejects release verifier carry-through gaps', () => {
  const { report } = loadProgressReport();
  ...
  const validation = validatePluginUpdateTopologyVariant5Report(withoutCarryThrough);

  assert.equal(validation.ok, false);
  assert.ok(validation.failures.some((failure) =>
    failure.code === 'PLUGIN_UPDATE_TOPOLOGY_RELEASE_VERIFIER_NOT_CARRIED'));
});

test('RPP-0896 validation rejects packaged fallback or release movement claims', () => {
  const { report } = loadProgressReport();
  const widened = {
    ...report,
  ...
  const validation = validatePluginUpdateTopologyVariant5Report(widened);

  assert.equal(validation.ok, false);
  assert.ok(validation.failures.some((failure) =>
    failure.code === 'PLUGIN_UPDATE_TOPOLOGY_PACKAGED_FALLBACK_NOT_DISABLED'));
  assert.ok(validation.failures.some((failure) =>
    failure.code === 'PLUGIN_UPDATE_TOPOLOGY_RELEASE_VERIFIER_NOT_FAIL_CLOSED'));
  assert.ok(validation.failures.some((failure) =>
    failure.code === 'PLUGIN_UPDATE_TOPOLOGY_RELEASE_POSTURE_NOT_NO_GO'));
});

test('RPP-0896 evidence remains hash/count/surface only', () => {
  const { report, text } = loadProgressReport();

  assert.deepEqual(findEvidenceRedactionIssues(report), []);
  assert.doesNotThrow(() =>
    assertEvidenceHasNoRawValues(report, { label: 'RPP-0896 plugin update topology progress report' }));
  ...
  assert.equal(report.invariants.releaseVerifierFailureCarriedThrough, true);
...
function loadProgressReport() {
  ...
  assert.ok(match?.groups?.json, 'RPP-0896 evidence must contain one JSON progress report block');
  return {
    text,
...
function buildMissingDockerCapabilityArtifact() {
  const plan = buildDockerTopologyPlan({
    ...
    workDir: '/tmp/rpp-0896-docker-work',
    evidenceDir: '/tmp/rpp-0896-docker-evidence',
    env: {},
  });
...
function validatePluginUpdateTopologyVariant5Report(report) {
  const failures = [];
  const topologyCommandReport = report?.topologyCommand || {};
  const capability = topologyCommandReport.exactUnavailableCapability || {};
  const hookRequirements = report?.candidateScope?.hookTopologyRequirements || {};
  const releaseVerifier = report?.releaseVerifier || {};

  if (topologyCommandReport.sitesStarted !== true && !capability.code) {
    failures.push({
      code: 'PLUGIN_UPDATE_TOPOLOGY_UNAVAILABLE_CAPABILITY_NOT_EXACT',
    });
  }

  if (topologyCommandReport.sitesStarted !== true
    && (topologyCommandReport.failClosed !== true || topologyCommandReport.acceptedForReleaseGate !== false)) {
    failures.push({
      code: 'PLUGIN_UPDATE_TOPOLOGY_NON_STARTED_REPORT_NOT_FAIL_CLOSED',
    });
  }

  if (topologyCommandReport.sitesStarted !== true && capability.code !== 'DOCKER_CLI_MISSING') {
    failures.push({
      code: 'PLUGIN_UPDATE_TOPOLOGY_UNEXPECTED_CAPABILITY_CODE',
    });
  }

  if (releaseVerifier.carriedThroughByTopologyCommand !== true
    || releaseVerifier.verifyReleaseFailure?.reason !== capability.code
    || releaseVerifier.verifyReleaseFailure?.exitCode !== topologyCommandReport.exitCode) {
    failures.push({
      code: 'PLUGIN_UPDATE_TOPOLOGY_RELEASE_VERIFIER_NOT_CARRIED',
    });
  }

  if (releaseVerifier.failClosed !== true
    || releaseVerifier.acceptedForReleaseGate !== false
    || releaseVerifier.releaseMovementAllowed !== false) {
    failures.push({
      code: 'PLUGIN_UPDATE_TOPOLOGY_RELEASE_VERIFIER_NOT_FAIL_CLOSED',
    });
  }

  if (topologyCommandReport.policy?.packagedFallbackAllowed !== false
    || topologyCommandReport.policy?.packagedFallbackObserved !== false
  ...
  if (topologyCommandReport.policy?.onlySandbox8080Ingress !== true
    || topologyCommandReport.policy?.tunnelCommandCount !== 0) {
    failures.push({
  ...
  if (!Array.isArray(releaseVerifier.requiredCarryThrough)
    || releaseVerifier.requiredCarryThrough.length !== releaseVerifierCarryThroughRequirements.length) {
    failures.push({
      code: 'PLUGIN_UPDATE_TOPOLOGY_RELEASE_VERIFIER_REQUIREMENTS_INCOMPLETE',
    });
  }

  if (report?.finalReleaseStatus !== 'NO-GO'
    || report?.integrationRecommendation !== 'NO-GO'
    || topologyCommandReport.releaseMovementAllowed !== false
    || releaseVerifier.releaseMovementAllowed !== false) {
    failures.push({
