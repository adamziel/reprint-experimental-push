...
const sessionId = 'psh_01j00000000000000000579';
const rotatedSessionId = 'psh_01j0000000000000000579r';
const idempotencyKey = 'idem-rpp-0579-credential-rotation-v4';
const resourcePath = 'wp-content/uploads/reprint-push/rpp-0579-credential-rotation.txt';
...
const credentials = {
  bound: {
    ...
    password: 'rpp-0579-bound-application-password',
  },
  rotated: {
    username: 'reprint_push_admin',
    password: 'rpp-0579-rotated-application-password',
  },
  invalidated: {
    username: 'reprint_push_admin',
    password: 'rpp-0579-invalidated-application-password',
  },
};
...
function fixtureHash(label) {
  return sha256Hex(`rpp-0579:${label}`);
}
...
function authEnvelope(kind = 'bound') {
  ...
  return {
    ...
    session: {
      ...
      applicationPasswordUuid: `app-pass-rpp-0579-${kind}`,
...
function buildReadyPlan() {
  ...
  const mutationId = 'mut_rpp_0579_apply_revalidates_live_source';
  const baseHash = fixtureHash('resource-base');
  const localHash = fixtureHash('resource-local');

  return {
    id: 'plan-rpp-0579-credential-rotation-behavior-v4',
    status: 'ready',
    summary: {
      mutations: 1,
      decisions: 0,
...
function createGeneratedCredentialRotationRoute() {
  const currentSnapshot = {
    files: { [resourcePath]: 'rpp-0579-live-source-before-accepted-apply' },
    plugins: {},
    db: {},
  };
  const state = {
  ...
  async function fetchHandler(url, options = {}) {
    ...
    throw new Error(`unexpected RPP-0579 route request: ${method} ${pathname}`);
  }
...
test('RPP-0579 v4 keeps credential checks and live-source revalidation before mutation', () => {
...
test('RPP-0579 v4 rejects rotated or invalidated apply credentials before accepted revalidation mutation', async () => {
