...
const evidencePath = path.join(
  repoRoot,
  'docs/evidence/rpp-0825-woocommerce-product-catalog-v2.md',
);
const hexSha256Pattern = /^[a-f0-9]{64}$/;
const rawCatalogNeedles = Object.freeze([
  'Reprint Proof Coffee',
  'REPRINT-PROOF-COFFEE',
  '21.00',
  'Stable production copy',
  'Complex Brewcommerce product',
  'brewcommerce-production.txt',
  'product-title',
  'product-body',
]);

test('RPP-0825 progress report records candidate versus release-ready catalog scope', () => {
  const { report, text } = loadProgressReport();

  assert.equal(report.schemaVersion, 1);
  assert.equal(report.rppId, 'RPP-0825');
  ...
  assert.equal(report.builtOn.candidateScopePattern.rppId, 'RPP-0805');
  assert.equal(report.builtOn.candidateScopePattern.recordsCandidateVersusReleaseReadyScope, true);
  assert.equal(report.builtOn.orderSafetyBoundary.rppId, 'RPP-0806');
  ...
  assert.equal(report.candidateScope.status, 'catalog-candidate-v2');
  ...
  assert.equal(report.candidateScope.sourcePattern, 'deterministic-local-woocommerce-catalog-surface-model');
...
test('RPP-0825 records explicit release-ready gaps for WooCommerce catalog release', () => {
  ...
  assert.ok(
    report.releaseReadyScope.blockers.includes('redacted-release-artifact-bundle-not-present'),
  );
...
test('RPP-0825 evidence remains hash/count/surface only', () => {
  const { report, text } = loadProgressReport();

  assert.deepEqual(findEvidenceRedactionIssues(report), []);
  assert.doesNotThrow(() =>
    assertEvidenceHasNoRawValues(report, { label: 'RPP-0825 WooCommerce catalog progress report' }));
  ...
  for (const needle of rawCatalogNeedles) {
    assert.equal(text.includes(needle), false, `RPP-0825 evidence leaked raw catalog fixture ${needle}`);
  }
  assert.doesNotMatch(text, /https?:\/\//i);
});

test('RPP-0825 evidence documents exact validation commands and results', () => {
  const { text } = loadProgressReport();

  assert.match(text, /node --check test\/rpp-0825-woocommerce-product-catalog-v2\.test\.js/);
  assert.match(text, /node --test test\/rpp-0825-woocommerce-product-catalog-v2\.test\.js/);
  assert.match(
    text,
...
function loadProgressReport() {
  ...
  assert.ok(match?.groups?.json, 'RPP-0825 evidence must contain one JSON progress report block');
  return {
    text,
