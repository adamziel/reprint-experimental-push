...
function tempJournalPath() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'reprint-rpp-0665-claim-expiry-'));
...
function baseSite() {
  ...
  for (let index = 1; index <= 5; index++) {
    files[`rpp-0665-file-${index}.txt`] = `rpp-0665-base-raw-site-value-${index}`;
  }
...
function localSite(base = baseSite()) {
  ...
  for (let index = 1; index <= 5; index++) {
    site.files[`rpp-0665-file-${index}.txt`] = `rpp-0665-local-raw-site-value-${index}`;
  }
  return site;
}
...
function rawSiteValuesFor(...sites) {
  const values = new Set([
    'rpp-0665-base-raw-site-value',
    'rpp-0665-local-raw-site-value',
  ]);
...
function assertNoRawSiteValues(value, rawSiteValues) {
  ...
  for (const rawValue of rawSiteValues) {
    assert.equal(
      ...
      false,
      `Unexpected raw site value in RPP-0665 evidence: ${rawValue}`,
    );
  }
}
  ...
  return {
    ...
    releaseProof: {
      ...
      idempotencyConflict: {
        ...
        recoveryState: {
          source: 'RPP-0665 recovery inspect after different-body conflict',
          storage: 'sqlite',
          state: 'fully-updated-remote',
...
test('RPP-0665 claim expiry policy advances once and keeps the release verifier on the same recovery path', () => {
  const filePath = tempJournalPath();
  const { plan, remote, rawSiteValues } = buildScenario();
  const artifactRefs = {
    releaseProof: 'artifact://rpp-0665-claim-expiry-policy-v4',
    recoverySupport: 'artifact://rpp-0665-local-recovery-support',
  };
  const activeClaimId = 'rpp-0665-active-claim-not-expired';
  const nonExpiredClaimId = 'rpp-0665-non-expired-competing-claim';
  const retryClaimId = 'rpp-0665-expired-claim-policy-retry';
  ...
  const oldRemoteRecovery = {
    source: 'RPP-0665 restarted file-backed claim-expiry journal',
    status: 200,
