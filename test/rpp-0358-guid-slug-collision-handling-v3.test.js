...
const fixedGeneratedHarnessNow = new Date('2026-05-31T00:00:00.000Z');
...
function generatedRawGuidSlugValues(cases) {
  const values = [];
  for (const testCase of cases) {
    const shape = assertPostGuidSlugCollisionShape(testCase, {
      staleTarget: testCase.tags.has('post-guid-slug-collision-stale'),
    });
    values.push(shape.localTitle, shape.guid, shape.slug);
    if (shape.remoteTitle) {
      values.push(shape.remoteTitle, shape.guid, shape.slug);
    }
  }
...
function assertNoRawGuidSlugValues(proof, rawValues) {
  ...
  for (const rawValue of rawValues) {
    assert.equal(
      ...
      false,
      `RPP-0358 proof leaked raw generated GUID/slug value: ${rawValue}`,
    );
  }
}

function assertCoverageShape({ coverage, report, cases, readyCases, staleCases }) {
...
function readyCaseProof(readyCase) {
  ...
  return {
    id: readyCase.id,
    tier: readyCase.tier,
    variant: 'ready-unique-guid-slug-v3',
    status: plan.status,
...
function staleCaseProof(staleCase) {
  ...
  return {
    id: staleCase.id,
    tier: staleCase.tier,
    variant: 'stale-guid-slug-collision-v3',
    status: plan.status,
    localResourceKey: shape.localResource.key,
...
function buildAggregateEvidence() {
  ...
  assertCoverageShape({ coverage, report, cases, readyCases, staleCases });

  const caseProofs = [
    ...readyCases.map(readyCaseProof),
    ...staleCases.map(staleCaseProof),
  ];
  const aggregateEvidence = {
    rpp: 'RPP-0358',
    successText: 'generated harness includes ready and stale cases',
    evidenceSource: 'generated-post-guid-slug-collision-v3',
    evidenceScope: 'local-generated-harness-support-only',
    ...
    totals: {
      ...
      readyStaleReplayRejected: caseProofs
        .filter((entry) => entry.validation?.staleReplayRejectionCode === 'PRECONDITION_FAILED').length,
      staleCollisionBlocked: caseProofs
        .filter((entry) => entry.collision?.blockerClass === 'stale-wordpress-graph-identity').length,
      staleApplyRefusedBeforeMutation: caseProofs
        .filter((entry) => entry.collision?.refusedBeforeMutation).length,
    },
    invariant: {
...
test('RPP-0358 generated harness includes GUID and slug collision ready and stale cases', () => {
  ...
  assert.deepEqual(aggregateEvidence.coverage.perTier, expectedTierCounts(2));
  assert.equal(aggregateEvidence.totals.readyApplied, 10);
  assert.equal(aggregateEvidence.totals.readyStaleReplayRejected, 10);
  ...
  assert.doesNotThrow(() =>
    assertEvidenceHasNoRawValues(aggregateEvidence, { label: 'RPP-0358 GUID/slug generated proof' }));
});
