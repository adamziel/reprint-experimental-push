...
test('production-shaped authenticated push prefers nested production recovery claim and lease-fence contracts over lab-style top-level wrappers', async () => {
  const originalFetch = global.fetch;
  const credential = {
    username: 'reprint_push_admin',
    password: 'app-password-01',
  };
  const claimId = 'production-claim-nested-01';
  const claimHash = recoveryClaimHash(claimId);
  global.fetch = async (url) => {
    const pathname = String(url);
    if (pathname.includes('/preflight')) {
    ...
    if (pathname.includes('/recovery/inspect')) {
      return new Response(JSON.stringify({
        ...
        recovery: {
          ...
          claim: {
            status: 'active',
            activeClaimId: 'lab-wrapper-claim-id',
            activeClaimHash: 'lab-wrapper-claim-hash',
            previousClaimId: null,
            previousClaimHash: null,
            sequence: 1,
            type: 'lab-wrapper-claim-opened',
          },
          leaseFence: {
            boundary: 'local-fixture-write',
            storageGuard: 'local-fixture-write',
            ...
            staleClaimRejected: false,
            writerLease: {
              strategy: 'fixture-writer',
              claimId: 'lab-wrapper-claim-id',
              claimHash: 'lab-wrapper-claim-hash',
              claimKeyUnique: false,
              storageGuard: 'local-fixture-write',
              fsyncEvidence: false,
              monotonicSequence: false,
  ...
  try {
    const summary = await runAuthenticatedHttpPush({
      ...
      username: credential.username,
      applicationPassword: credential.password,
      idempotencyKey: 'idem-recovery-nested-claim-leasefence-01',
      routeProfile: 'production-shaped',
      requireProductionAuthSession: true,
    });
    ...
    assert.equal(summary.ok, false);
    assert.equal(summary.recoveryInspect.recovery.claim.activeClaimId, claimId);
    assert.equal(summary.recoveryInspect.recovery.claim.activeClaimHash, claimHash);
    assert.equal(summary.recoveryInspect.recovery.leaseFence.boundary, 'filesystem-compare-rename');
    assert.equal(summary.recoveryInspect.recovery.leaseFence.writerLease.claimId, claimId);
    assert.equal(summary.recoveryInspect.recovery.productionJournal.claim.activeClaimId, claimId);
    assert.equal(summary.recoveryInspect.recovery.productionJournal.leaseFence.writerLease.claimId, claimId);
  } finally {
...
test('production-shaped authenticated push prefers a validated nested production recovery journal surface over misleading top-level production markers', async () => {
  const originalFetch = global.fetch;
  const seen = [];
  const credential = {
    username: 'reprint_push_admin',
  ...
  const claimId = 'production-claim-validated-nested-01';
  const claimHash = recoveryClaimHash(claimId);
  global.fetch = async (url, options) => {
    seen.push({ url: String(url), options });
    const pathname = String(url);
  ...
  try {
    const summary = await runAuthenticatedHttpPush({
      ...
      username: credential.username,
      applicationPassword: credential.password,
      idempotencyKey: 'idem-recovery-validated-nested-surface-01',
      routeProfile: 'production-shaped',
      requireProductionAuthSession: true,
    });

    assert.equal(summary.code, 'PRODUCTION_AUTH_SESSION_LIFECYCLE_REQUIRED');
    assert.equal(summary.recoveryInspect.recovery.journal.claimId, claimId);
    assert.equal(summary.recoveryInspect.recovery.journal.claimHash, claimHash);
    assert.equal(summary.recoveryInspect.recovery.claim.activeClaimId, claimId);
    assert.equal(summary.recoveryInspect.recovery.leaseFence.writerLease.claimId, claimId);
    assert.equal(summary.recoveryInspect.recovery.productionJournal.journal.claimId, claimId);
    assert.ok(seen.some(({ url }) => url.includes('/db-journal?limit=80')));
  } finally {
    global.fetch = originalFetch;
  }
...
test('production-shaped authenticated push refetches db journal when checked recovery proof drifts production adapter boundary', async () => {
  const originalFetch = global.fetch;
  const seen = [];
  let applyCount = 0;
  global.fetch = async (url, options) => {
  ...
  try {
    const summary = await runAuthenticatedHttpPush({
      ...
      username: credential.username,
      applicationPassword: credential.password,
      idempotencyKey: 'idem-inline-recovery-proof-production-adapter-gap-01',
      routeProfile: 'production-shaped',
      requireProductionAuthSession: true,
    });
    ...
    assert.equal(summary.ok, false);
    assert.equal(summary.code, 'DURABLE_JOURNAL_NOT_PROVEN');
    assert.equal(summary.recoveryInspect.recovery.dbJournal.ownership.productionAdapter, 'filesystem-compare-rename');
    assert.equal(summary.dbJournal.ownership.productionAdapter, 'wpdb-single-statement-cas');
    assert.deepEqual(summary.boundary, {
      firstRemainingProductionBoundary: 'auth/session lifecycle and durable journal semantics',
      status: 'unimplemented',
...
test('production-shaped authenticated push fails closed when fallback db journal still drifts production adapter boundary', async () => {
  const originalFetch = global.fetch;
  const seen = [];
  let applyCount = 0;
  global.fetch = async (url, options) => {
  ...
  try {
    const summary = await runAuthenticatedHttpPush({
      ...
      username: credential.username,
      applicationPassword: credential.password,
      idempotencyKey: 'idem-inline-recovery-proof-production-adapter-gap-02',
      routeProfile: 'production-shaped',
      requireProductionAuthSession: true,
    });
    ...
    assert.equal(summary.code, 'DURABLE_JOURNAL_NOT_PROVEN');
    assert.equal(summary.recoveryInspect.recovery.dbJournal.ownership.productionAdapter, 'filesystem-compare-rename');
    assert.equal(summary.dbJournal.ownership.productionAdapter, 'filesystem-compare-rename');
    assert.deepEqual(summary.boundary, {
      firstRemainingProductionBoundary: 'auth/session lifecycle and durable journal semantics',
      status: 'unimplemented',
      verdict: 'PRODUCTION_DURABLE_JOURNAL_STORAGE_REQUIRED',
...
test('production-shaped authenticated push refetches db journal when checked recovery proof drifts lease-fence stale-claim rejection', async () => {
  const originalFetch = global.fetch;
  const seen = [];
  let applyCount = 0;
  global.fetch = async (url, options) => {
  ...
  try {
    const summary = await runAuthenticatedHttpPush({
      ...
      username: credential.username,
      applicationPassword: credential.password,
      idempotencyKey: 'idem-inline-recovery-proof-lease-fence-stale-claim-drift-01',
      routeProfile: 'production-shaped',
      requireProductionAuthSession: true,
    });
    ...
    assert.equal(summary.ok, true);
    assert.equal(summary.code, undefined);
    assert.equal(summary.recoveryInspect.recovery.dbJournal.leaseFence.staleClaimRejected, false);
    assert.equal(summary.dbJournal.leaseFence.staleClaimRejected, true);
    assert.ok(seen.some(({ url }) => url.includes('/db-journal?limit=80')));
  } finally {
    global.fetch = originalFetch;
...
test('production-shaped authenticated push fails closed when fallback db journal still drifts lease-fence stale-claim rejection', async () => {
  const originalFetch = global.fetch;
  const seen = [];
  let applyCount = 0;
  global.fetch = async (url, options) => {
  ...
  try {
    const summary = await runAuthenticatedHttpPush({
      ...
      username: credential.username,
      applicationPassword: credential.password,
      idempotencyKey: 'idem-inline-recovery-proof-lease-fence-stale-claim-drift-02',
      routeProfile: 'production-shaped',
      requireProductionAuthSession: true,
    });
