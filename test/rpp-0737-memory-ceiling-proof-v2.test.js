...
const proofId = 'rpp-0737-memory-ceiling-proof-v2';
...
test('RPP-0737 variant 2 rejects stale storage state after streamed memory-ceiling temp write', () => {
  const rootDir = createFilesystemMemoryCeilingTempRoot('reprint-rpp-0737-stale-');
  const logicalPath = 'wp-content/uploads/rpp-0737/stale/reject.txt';
  writeFixture(rootDir, logicalPath, 'rpp0737-base-stale-payload');
  ...
  const stale = applyFilesystemMemoryCeilingWrite({
    ...
    createChunk: deterministicChunkSource('rpp0737-planned-stale-payload'),
    operation: 'update',
    driver: 'rpp-0737-focused-proof',
    chunkSizeBytes,
    maxBufferedBytes,
    afterTempWrite: ({ absolutePath }) => {
      afterTempWriteObserved = true;
      assert.equal(readFixture(rootDir, logicalPath), 'rpp0737-base-stale-payload');
      assert.equal(tempNamesBesideTarget(absolutePath).length, 1);
      fs.writeFileSync(absolutePath, 'rpp0737-drift-stale-payload');
    },
  });

  assert.equal(afterTempWriteObserved, true);
  assert.equal(stale.applied, false);
  assert.equal(readFixture(rootDir, logicalPath), 'rpp0737-drift-stale-payload');
...
test('RPP-0737 variant 2 proves storage performance gates and support-only NO-GO', {
  concurrency: false,
}, () => {
  const proof = buildVariant2Proof();

  assert.equal(proof.rppId, 'RPP-0737');
  ...
  assert.equal(proof.resources.storage.guardedWritesAttempted, 4);
  assert.equal(proof.resources.storage.appliedWrites, 3);
  assert.equal(proof.resources.storage.staleAtWriteWrites, 1);
  ...
  assert.equal(proof.resources.memoryCeiling.streamedWrites, 4);
  assert.equal(proof.resources.tempLeaks, 0);

  assert.equal(proof.guardedWrites.expectedWriteCount, 4);
  assert.equal(proof.guardedWrites.appliedSampleCount, 3);
  assert.equal(proof.guardedWrites.staleSampleCount, 1);
  assert.equal(proof.guardedWrites.staleWriteHashes.length, 1);
  ...
  assert.ok(proof.guardedWrites.storageGuardSamples.some((sample) => (
...
test('RPP-0737 variant 2 fails closed for stale guard, memory, and premature evidence', () => {
...
function rawFilesystemEvidencePattern() {
  return /rpp0737-(?:base|planned|drift)-stale-payload|memory-(?:base|planned|drift)-payload|filesystem memory raw fixture|wp-content\/uploads|\/tmp\/|\.tmp/i;
}

function rawFilesystemTokens(rootDir = null) {
  return [
    'rpp0737-base-stale-payload',
    'rpp0737-planned-stale-payload',
    'rpp0737-drift-stale-payload',
    'memory-base-payload',
