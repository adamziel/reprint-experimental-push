...
const evidencePath = path.join(
  repoRoot,
  'docs/evidence/rpp-0880-no-tunnel-policy-proof-v4.md',
);
const fixedNow = '2026-06-01T00:00:00.000Z';
const proofId = 'rpp-0880-no-tunnel-policy-proof-v4';
const sourcePattern = 'RPP-0860-no-tunnel-policy-pattern';
...
test('RPP-0880 progress report records candidate versus release-ready no-tunnel scope', () => {
  const { report, text } = loadProgressReport();
  const expected = buildNoTunnelPolicyProof();

  assert.deepEqual(report, expected);
  assert.equal(report.schemaVersion, 1);
  assert.equal(report.rppId, 'RPP-0880');
...
test('RPP-0880 rejects known tunnel command and domain surfaces without invoking tools', () => {
  ...
  assert.equal(report.noTunnelPolicy.sourcePattern, sourcePattern);
...
test('RPP-0880 permits only local sandbox ingress semantics', () => {
  const matrix = buildNoTunnelPolicyMatrix();
...
test('RPP-0880 keeps packaged fallback disabled and release movement at NO-GO', () => {
  const matrix = buildNoTunnelPolicyMatrix();
...
test('RPP-0880 evidence remains deterministic hash/count/surface-only', () => {
  ...
  assert.doesNotThrow(() =>
    assertEvidenceHasNoRawValues(report, { label: 'RPP-0880 no-tunnel policy proof v4' }));
  ...
  for (const needle of rawEvidenceNeedles) {
    assert.equal(text.includes(needle), false, `RPP-0880 evidence leaked raw tunnel or URL payload ${needle}`);
  }
});
...
function loadProgressReport() {
  ...
  assert.ok(match?.groups?.json, 'RPP-0880 evidence must contain one JSON progress report block');
  return {
    text,
...
function buildNoTunnelPolicyProof() {
  ...
  const candidateScope = {
    status: 'no-tunnel-policy-candidate-v4',
    sourcePattern,
    supportOnly: true,
  ...
  const proofCore = {
    ...
    proofId,
    variant: 4,
    title: 'No tunnel policy proof candidate scope variant 4',
    checkedAt: fixedNow,
...
function buildBasePlan(options = {}) {
  return buildDockerTopologyPlan({
    ...
    workDir: '/tmp/rpp-0880-docker-work',
    evidenceDir: '/tmp/rpp-0880-docker-evidence',
    env: {},
    ...options,
  });
}
