...
test('plugin-driver proof summary bundle groups stay aligned with shared scenario groups', () => {
  ...
  assert.deepEqual(
    bundleSummaryGroups['driver-receipt-credential-guards'],
...
test('plugin-driver proof summary rebuilds a reused proof when its top-level bundle view is stale against the nested mode proof', () => {
...
test('plugin-driver proof summary rebuilds a verifier alias proof when its top-level bundle view drifts from the nested modeProof', () => {
  const staleAttachedProof = {
    mode: 'driverVerifierGuardsOnly',
    canonicalMode: 'driver-verifier-guards',
    requestedScenarios: ['driverVerifierGuardsOnly'],
    requestedBundles: ['driverReceiptGuards'],
    requestedBundleStatus: 'missing',
    requestedBundleStatuses: {
  ...
  assert.notEqual(rebuiltProof, staleAttachedProof);
  assert.equal(rawSummary.pluginDriverProof, rebuiltProof);
  assert.deepEqual(rebuiltProof.requestedScenarios, ['driverVerifierGuardsOnly']);
  assert.deepEqual(rebuiltProof.requestedBundles, ['driverVerifierGuards']);
  assert.deepEqual(rebuiltProof.requestedBundleStatuses, {
    driverVerifierGuards: 'missing',
  });
  assert.equal(rebuiltProof.requestedBundlesSatisfied, false);
...
test('plugin-driver proof summary rebuilds a verifier alias proof when stale top-level legacy bundle fields drift from the nested modeProof', () => {
  const staleAttachedProof = {
    mode: 'driverVerifierGuardsOnly',
    canonicalMode: 'driver-verifier-guards',
    requestedScenarios: ['driverVerifierGuardsOnly'],
    requestedBundles: ['driverVerifierGuards'],
    selectedScenarios: Array.from(new Set(bundleSummaryGroups['driver-verifier-guards'])).sort(),
    legacyRequestedBundles: ['driverReceiptGuards'],
    ...
    modeProof: {
      ...
      requestedScenarios: ['driverVerifierGuardsOnly'],
      requestedBundles: ['driverVerifierGuards'],
      legacyRequestedBundles: ['driverVerifierGuards'],
      requestedBundleStatus: 'missing',
      requestedBundleStatuses: {
        driverVerifierGuards: 'missing',
      },
      legacyRequestedBundleStatus: 'missing',
      legacyRequestedBundleStatuses: {
        driverVerifierGuards: 'missing',
      },
      requestedBundlesSatisfied: false,
  ...
  assert.equal(rawSummary.pluginDriverProof, rebuiltProof);
  assert.deepEqual(rebuiltProof.requestedBundles, ['driverVerifierGuards']);
  assert.equal(rebuiltProof.legacyRequestedBundles, undefined);
  assert.equal(rebuiltProof.legacyRequestedBundleStatus, undefined);
  assert.equal(rebuiltProof.legacyRequestedBundleStatuses, undefined);
  assert.deepEqual(rebuiltProof.modeProof?.requestedBundles, ['driverVerifierGuards']);
  assert.deepEqual(rebuiltProof.modeProof?.legacyRequestedBundles, ['driverVerifierGuards']);
  assert.equal(rebuiltProof.modeProof?.legacyRequestedBundleStatus, 'missing');
  assert.deepEqual(rebuiltProof.modeProof?.legacyRequestedBundleStatuses, {
    driverVerifierGuards: 'missing',
  });
});
...
test('plugin-driver mode proof resolver rebuilds a stale attached top-level modeProof when bundle satisfaction drifts', () => {
  const rawSummary = {
    mode: 'driverVerifierGuardsOnly',
    canonicalMode: 'driver-verifier-guards',
    requestedScenarios: ['driverVerifierGuardsOnly'],
    ...
    modeProof: {
      ...
      marker: 'stale-top-level',
    },
  };

  const modeProof = resolveProductionPluginPackageModeProof(rawSummary, 'driverVerifierGuardsOnly', {
    requestedScenarios: ['driverVerifierGuardsOnly'],
    selectedScenarios: new Set([
      'driver-verifier-guards',
      ...scenarioGroups['driver-verifier-guards'],
  ...
  assert.notEqual(modeProof, rawSummary.modeProof);
  assert.equal(modeProof?.requestedBundleStatus, 'missing');
  assert.deepEqual(modeProof?.requestedBundleStatuses, {
    driverVerifierGuards: 'missing',
  });
  assert.equal(modeProof?.requestedBundlesSatisfied, false);
  assert.equal(rawSummary.modeProof?.requestedBundlesSatisfied, false);
});

test('plugin-driver mode proof resolver rebuilds a stale attached nested modeProof when bundle satisfaction drifts', () => {
  ...
  const rawSummary = {
    ...
    canonicalMode: 'driver-verifier-guards',
    requestedScenarios: ['driverVerifierGuardsOnly'],
    selectedScenarios: currentSelection,
    routes: {
      namespace: 'reprint/v1',
      profile: 'production-shaped',
      labNamespaceDisabled: true,
  ...
  const modeProof = resolveProductionPluginPackageModeProof(rawSummary, 'driverVerifierGuardsOnly', {
    requestedScenarios: ['driverVerifierGuardsOnly'],
    selectedScenarios: new Set(currentSelection),
    resolvedMode: 'driverVerifierGuardsOnly',
    canonicalMode: 'driver-verifier-guards',
  });

  assert.notEqual(modeProof, rawSummary.pluginDriverProof.modeProof);
  assert.equal(modeProof?.requestedBundleStatus, 'missing');
  assert.deepEqual(modeProof?.requestedBundleStatuses, {
    driverVerifierGuards: 'missing',
  });
  assert.equal(modeProof?.requestedBundlesSatisfied, false);
  assert.equal(rawSummary.pluginDriverProof.modeProof?.requestedBundlesSatisfied, true);
});
...
test('plugin-driver proof summary attach helper repairs a stale nested mode proof when the top-level cache is current', () => {
  ...
  rawSummary.pluginDriverProof = {
    ...
    modeProof: {
      ...currentProof.modeProof,
      selectedScenarios: ['driver-delete-guard'],
    },
  };
  ...
  assert.equal(rawSummary.modeProof, repairedProof.modeProof);
  assert.deepEqual(
    rawSummary.pluginDriverProof.selectedScenarios,
    Array.from(fullVerifierSelection).sort(),
  );
  assert.deepEqual(
    rawSummary.pluginDriverProof.modeProof?.selectedScenarios,
    Array.from(fullVerifierSelection).sort(),
  );
});

test('plugin-driver proof summary attach helper repairs a stale verifier alias nested mode proof when the top-level cache is current', () => {
  const fullVerifierSelection = new Set([
    'driver-verifier-guards',
    ...scenarioGroups['driver-verifier-guards'],
  ]);
...
test('plugin-driver proof summary attach helper rebuilds a stale verifier alias top-level bundle cache when the nested mode proof is current', () => {
  ...
  assert.equal(repairedProof, rawSummary.pluginDriverProof);
  assert.equal(rawSummary.modeProof, repairedProof.modeProof);
  assert.deepEqual(repairedProof.requestedBundles, ['driverVerifierGuards']);
  assert.deepEqual(repairedProof.requestedBundleStatuses, {
    driverVerifierGuards: 'missing',
  });
  assert.deepEqual(repairedProof.modeProof?.requestedBundles, ['driverVerifierGuards']);
});

test('plugin-driver proof summary attach helper repairs stale verifier alias bundle satisfaction when the nested mode proof is current', () => {
  const fullVerifierSelection = new Set([
    'driver-verifier-guards',
    ...scenarioGroups['driver-verifier-guards'],
  ]);
...
test('plugin-driver proof summary attach helper repairs stale verifier alias legacy requested bundles when the nested mode proof is current', () => {
  const fullVerifierSelection = new Set([
    'driver-verifier-guards',
    ...scenarioGroups['driver-verifier-guards'],
  ]);
  ...
  assert.equal(rawSummary.modeProof, repairedProof.modeProof);
  assert.deepEqual(repairedProof.requestedBundles, ['driverVerifierGuards']);
  assert.equal(repairedProof.legacyRequestedBundles, undefined);
  assert.deepEqual(repairedProof.modeProof?.requestedBundles, ['driverVerifierGuards']);
  assert.deepEqual(repairedProof.modeProof?.legacyRequestedBundles, ['driverVerifierGuards']);
});

test('plugin-driver proof summary attach helper repairs stale verifier alias scenario statuses when the nested mode proof is current', () => {
  const fullVerifierSelection = new Set([
    'driver-verifier-guards',
    ...scenarioGroups['driver-verifier-guards'],
  ]);
...
test('plugin-driver proof summary attach helper repairs stale receipt-registration alias scenario statuses when the nested mode proof is current', () => {
  ...
  assert.equal(repairedProof.modeProof?.requestedStatus, 'missing');
  assert.deepEqual(repairedProof.modeProof?.requestedScenarioStatuses, {
    'driver-receipt-registration-guards': 'missing',
  });
  assert.equal(repairedProof.modeProof?.requestedScenariosSatisfied, false);
});
...
test('plugin-driver proof summary attach helper repairs stale receipt-registration alias requestedSatisfied when the nested mode proof is current', () => {
  const fullReceiptRegistrationSelection = new Set([
    'driver-receipt-registration-guards',
    ...scenarioGroups['driver-receipt-registration-guards'],
  ]);
...
test('plugin-driver proof summary attach helper repairs stale callback alias requested bundles when the nested mode proof is current', () => {
  const callbackSelection = new Set([
    'driver-callback-guards',
    ...scenarioGroups['driver-callback-guards'],
  ]);
  ...
  assert.equal(repairedProof.requestedStatus, undefined);
  assert.equal(repairedProof.requestedBundleStatus, undefined);
  assert.deepEqual(repairedProof.requestedBundles, ['driverCallbackGuards']);
  assert.deepEqual(repairedProof.requestedBundleStatuses, {
    driverCallbackGuards: 'passed',
  });
  assert.equal(repairedProof.requestedBundlesSatisfied, true);
  assert.deepEqual(repairedProof.modeProof?.requestedBundles, ['driverCallbackGuards']);
  assert.equal(repairedProof.modeProof?.requestedBundleStatus, 'passed');
...
test('plugin-driver proof summary attach helper repairs stale registration-shape alias requested bundles when the nested mode proof is current', () => {
  const registrationShapeSelection = new Set([
    'driver-registration-shape-guards',
    ...scenarioGroups['driver-registration-shape-guards'],
  ]);
  ...
  rawSummary.pluginDriverProof = {
    ...
    requestedBundleStatus: 'missing',
    requestedBundleStatuses: {
      driverRegistrationShapeGuards: 'missing',
    },
  ...
  assert.equal(repairedProof.requestedStatus, undefined);
  assert.equal(repairedProof.requestedBundleStatus, undefined);
  assert.deepEqual(repairedProof.requestedBundles, ['driverRegistrationShapeGuards']);
  assert.deepEqual(repairedProof.requestedBundleStatuses, {
    driverRegistrationShapeGuards: 'passed',
  });
  assert.equal(repairedProof.requestedBundlesSatisfied, true);
  assert.deepEqual(repairedProof.modeProof?.requestedBundles, ['driverRegistrationShapeGuards']);
  assert.equal(repairedProof.modeProof?.requestedBundleStatus, 'passed');
  assert.deepEqual(repairedProof.modeProof?.requestedBundleStatuses, {
    driverRegistrationShapeGuards: 'passed',
  });
...
test('plugin-driver proof resolver repairs a stale nested mode proof when the top-level cache is current', () => {
  const fullVerifierSelection = new Set([
    'driver-verifier-guards',
    ...scenarioGroups['driver-verifier-guards'],
  ]);
  ...
  const repairedProof = resolveProductionPluginPackagePluginDriverProof(rawSummary, {
    requestedScenarios: ['driver-verifier-guards'],
    selectedScenarios: fullVerifierSelection,
    resolvedMode: 'driverVerifierGuards',
    canonicalMode: 'driver-verifier-guards',
...
test('plugin-driver proof summary reports full packaged guard coverage', () => {
  ...
  assert.equal(summary.ok, true);
  assert.equal(summary.checkedScenarioCount, 16);
  assert.equal(summary.passedScenarioCount, 16);
  assert.equal(summary.failedScenarioCount, 0);
  assert.equal(summary.skippedScenarioCount, 0);
  assert.equal(summary.checkedBundleCount, 11);
  assert.equal(summary.passedBundleCount, 11);
  ...
  assert.deepEqual(summary.bundles, {
    ...
    driverReceiptCredentialGuards: 'passed',
    driverVerifierGuards: 'passed',
    driverReceiptRegistrationGuards: 'passed',
...
test('plugin-driver proof summary marks unselected scenarios as skipped', () => {
  ...
  assert.equal(summary.failedScenarioCount, 0);
  assert.equal(summary.skippedScenarioCount, 7);
  assert.equal(summary.checkedBundleCount, 1);
  assert.equal(summary.passedBundleCount, 1);
  assert.equal(summary.failedBundleCount, 0);
  assert.equal(summary.skippedBundleCount, 10);
  ...
  assert.deepEqual(summary.bundles, {
    ...
    driverReceiptCredentialGuards: 'skipped',
...
test('plugin-driver proof summary carries receipt credential guards on dedicated modeProof', () => {
  const summary = buildProductionPluginPackageProofSummary(
    {
      driverReceiptRotatedCredentialGuard: {
        rotatedCredentialRejectedCode: 'AUTH_RECEIPT_MISMATCH',
    ...
    {
      requestedScenarios: ['driver-receipt-credential-guards'],
      selectedScenarios: new Set([
        'driver-receipt-credential-guards',
        ...scenarioGroups['driver-receipt-credential-guards'],
      ]),
      resolvedMode: 'driverReceiptCredentialGuardsOnly',
      canonicalMode: 'driver-receipt-credential-guards',
    },
  );

  assert.equal(summary.modeProof?.proof, summary.driverReceiptCredentialGuards);
  assert.equal(summary.modeProof?.proofKey, 'driverReceiptCredentialGuards');
  assert.equal(summary.modeProof?.requestedStatus, 'passed');
  assert.equal(summary.modeProof?.requestedSatisfied, true);
  assert.equal(summary.modeProof?.requestedBundlesSatisfied, true);
  assert.equal(summary.modeProof?.guardProof?.ok, true);
  assert.equal(summary.modeProof?.guardProof?.guardCount, 2);
  assert.equal(summary.modeProof?.guardProof?.passedGuardCount, 2);
  assert.deepEqual(summary.modeProof?.guardProof?.guardStatuses, {
    rotatedCredential: 'passed',
    revokedCredential: 'passed',
  });
