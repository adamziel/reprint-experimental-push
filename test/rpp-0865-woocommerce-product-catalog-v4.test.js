...
const evidencePath = path.join(
  repoRoot,
  'docs/evidence/rpp-0865-woocommerce-product-catalog-v4.md',
);
const hexSha256Pattern = /^[a-f0-9]{64}$/;
const rawCatalogNeedles = Object.freeze([
  'RPP-0865 Private Product',
  'RPP0865-PRIVATE-SKU',
  '59.00',
  'Variant 4 private catalog body',
  'variant-four-gallery.jpg',
...
test('RPP-0865 progress report records candidate versus release-ready catalog scope', () => {
  const { report, text } = loadProgressReport();

  assert.equal(report.schemaVersion, 1);
  assert.equal(report.rppId, 'RPP-0865');
  ...
  assert.equal(report.builtOn.candidateScopePattern.rppId, 'RPP-0845');
  assert.equal(report.builtOn.candidateScopePattern.variant, 3);
  assert.equal(report.builtOn.candidateScopePattern.recordsCandidateVersusReleaseReadyScope, true);
  assert.equal(report.builtOn.orderSafetyBoundary.rppId, 'RPP-0846');
  assert.equal(report.builtOn.orderSafetyBoundary.variant, 3);
  ...
  assert.equal(report.candidateScope.status, 'catalog-candidate-v4');
  ...
  assert.equal(report.candidateScope.sourcePattern, 'deterministic-local-woocommerce-catalog-surface-model-v4');
...
test('RPP-0865 records production-bound release gaps and no-go status', () => {
  ...
  assert.equal(report.productionImportExportEvidence.observedAttempt, 'not-performed-in-rpp-0865');
  assert.equal(
...
test('RPP-0865 evidence remains hash/count/surface only', () => {
  const { report, text } = loadProgressReport();

  assert.deepEqual(findEvidenceRedactionIssues(report), []);
  assert.doesNotThrow(() =>
    assertEvidenceHasNoRawValues(report, { label: 'RPP-0865 WooCommerce catalog progress report' }));
  ...
  for (const needle of rawCatalogNeedles) {
    assert.equal(text.includes(needle), false, `RPP-0865 evidence leaked raw catalog fixture ${needle}`);
  }
  assert.doesNotMatch(text, /https?:\/\//i);
});

test('RPP-0865 evidence documents exact validation commands and results', () => {
  const { text } = loadProgressReport();

  assert.match(text, /node --check test\/rpp-0865-woocommerce-product-catalog-v4\.test\.js/);
  assert.match(text, /node --test test\/rpp-0865-woocommerce-product-catalog-v4\.test\.js/);
  assert.match(
    text,
...
function loadProgressReport() {
  ...
  assert.ok(match?.groups?.json, 'RPP-0865 evidence must contain one JSON progress report block');
  return {
    text,
