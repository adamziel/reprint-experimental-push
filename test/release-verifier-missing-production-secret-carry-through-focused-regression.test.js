...
const partialSecretValue = 'RPP_0087_SHOULD_NOT_LEAK';
const secretReason = 'A live source URL is present but production credentials or an auth session source command are missing.';
const verifierReason = 'explicit live source URL is present but production credentials are missing';
const focusedCommand = 'node --test test/release-verifier-missing-production-secret-carry-through-focused-regression.test.js test/release-gate-missing-production-secret-regression.test.js test/release-gate-missing-production-secret-generated.test.js test/release-verifier-auth-source-readback-carry-through-focused-regression.test.js test/release-gates.test.js test/release-gate-cli.test.js';
const statusMarker = '[verify-release:held exit=1 reason=REPRINT_PUSH_SECRET_REQUIRED mutationAttempted=false]';
const releaseGateHeldMarker = '[release-gates-ci:held final=19/20 candidate=19/20 reason=REPRINT_PUSH_SECRET_REQUIRED]';
...
function runVerifyReleaseMissingProductionSecret() {
...
function productionSecretEvidenceFromReport(report) {
  return {
    ok: false,
    present: false,
    observed: report.boundary.authSession.observed,
    sourceUrl,
...
function writeEvidence(evidence) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'release-verifier-missing-production-secret-carry-through-'));
...
test('release verifier carries missing production secret through before mutation for RPP-0087', () => {
  const proof = runVerifyReleaseMissingProductionSecret();
  const verifyReport = extractJsonObjects(proof.stdout)
    .find((entry) => entry?.releaseProof?.code === 'REPRINT_PUSH_SECRET_REQUIRED');
  ...
  assert.ok(
    ...
    'verify:release stdout should end with the missing-secret held marker',
  );
  assert.doesNotMatch(
    `${proof.stdout}\n${proof.stderr}`,
    /Starting Playground server/,
    'missing production credentials must fail before starting live verifier servers',
  );
  assert.doesNotMatch(proof.stdout, new RegExp(partialSecretValue));
  assert.doesNotMatch(proof.stderr, new RegExp(partialSecretValue));
  ...
  const releaseGateResult = runReleaseGateCheck(completeFinalEvidence({
    productionSecret: productionSecretEvidenceFromReport(verifyReport),
    verifyReleaseFailure: verifyReleaseFailureFromReport(verifyReport),
  }));
  const releaseGateReport = parseReport(releaseGateResult);
  const secretGate = gateById(releaseGateReport, 'production-secret');
  ...
  assert.equal(releaseGateReport.primaryFailureCode, 'REPRINT_PUSH_SECRET_REQUIRED');
  ...
  assert.deepEqual(secretGate.evidence, expectedSecretEvidence);
  ...
  assert.doesNotMatch(releaseGateResult.stdout, new RegExp(partialSecretValue));
  assert.doesNotMatch(releaseGateResult.stderr, new RegExp(partialSecretValue));
});

test('release verifier missing production secret carry-through keeps credential-present positive path for RPP-0087', () => {
  const releaseGateResult = runReleaseGateCheck(completeFinalEvidence());
  const releaseGateReport = parseReport(releaseGateResult);
  const secretGate = gateById(releaseGateReport, 'production-secret');
  ...
  assert.equal(releaseGateReport.mutationAttempted, false);
  assert.equal(secretGate.status, 'passed');
  assert.equal(secretGate.code, 'OK');
  assert.equal(secretGate.evidence.required, 'production credential or auth session source command');
  assert.equal(secretGate.evidence.observed, 'production-credential-present');
  assert.equal(secretGate.evidence.scope, 'final-release');
  assert.doesNotMatch(releaseGateResult.stdout, new RegExp(partialSecretValue));
  assert.doesNotMatch(releaseGateResult.stderr, new RegExp(partialSecretValue));
});

test('release verifier missing production secret carry-through is recorded in RPP-0087 reports', () => {
  const evidenceReport = fs.readFileSync(evidenceReportPath, 'utf8');
  const progressReport = fs.readFileSync(progressReportPath, 'utf8');

  assert.ok(evidenceReport.includes('| RPP-0087 | Evidence toward release verifier missing production secret carry-through'));
  assert.ok(evidenceReport.includes(`- Command: \`${focusedCommand}\``));
  assert.ok(evidenceReport.includes(`- Observed status: \`pass\`; verifier marker: \`${statusMarker}\`; production-secret gate: \`REPRINT_PUSH_SECRET_REQUIRED\`; release marker: \`${releaseGateHeldMarker}\`.`));
  assert.ok(progressReport.includes('Release verifier missing production secret carry-through now checks `RPP-0087`'));
  assert.ok(progressReport.includes(`- Command: \`${focusedCommand}\``));
  assert.ok(progressReport.includes(`- Observed status: \`pass\`; verifier marker: \`${statusMarker}\`; production-secret gate: \`REPRINT_PUSH_SECRET_REQUIRED\`; release marker: \`${releaseGateHeldMarker}\`.`));
});
