...
test('RPP-0516 executor records same-key same-body replay proof with fresh nonce over the local fake endpoint pattern', async () => {
  ...
  try {
    ...
    assert.equal(summary.sameKeySameBodyReplay?.proven, true);
    assert.equal(summary.sameKeySameBodyReplay?.sameIdempotencyKey, true);
    assert.equal(summary.sameKeySameBodyReplay?.sameBody, true);
    assert.equal(summary.sameKeySameBodyReplay?.sameCanonicalRequest, true);
    ...
    assert.equal(summary.sameKeySameBodyReplay?.responseEquivalent, true);
    ...
    assert.match(summary.sameKeySameBodyReplay.apply.contentHash, /^[a-f0-9]{64}$/);
    assert.match(summary.sameKeySameBodyReplay.replay.idempotencyKeyHash, /^[a-f0-9]{64}$/);
  } finally {
