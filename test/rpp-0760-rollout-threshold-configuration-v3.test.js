...
const proofId = 'rpp-0760-rollout-threshold-configuration-v3';
...
const configuredThresholdsBps = Object.freeze([500, 1000, 2500, 5000, 7500, 10000]);
...
test('RPP-0760 variant 3 proves rollout thresholds gate fast-path lane updates', {
  concurrency: false,
}, () => {
  const proof = buildVariant3Proof();

  assert.equal(proof.rppId, 'RPP-0760');
  ...
  assert.equal(proof.thresholdConfiguration.schemaVersion, 3);
  assert.equal(proof.thresholdConfiguration.variant, 3);
  ...
  assert.equal(proof.thresholdConfiguration.maxP95DecisionMs, 5.5);
  ...
  assert.equal(proof.resources.performance.p95DecisionMs, 5.2);
  assert.equal(proof.resources.performance.maxDecisionMs, 5.2);
  ...
  assert.equal(proof.fastPathLaneEvidence.performance.p95DecisionMs, 5.2);
...
test('RPP-0760 variant 3 fails closed for unsafe rollout threshold evidence', () => {
  ...
  performanceThresholdExceeded.fastPathLaneEvidence.performance.p95DecisionMs = 5.8;
  ...
  mismatchedConfigurationHash.thresholdConfiguration.configHash = sha256(
    'rpp-0760-mismatched-threshold-configuration',
  );
...
function collectFastPathLaneEvidence(config) {
  ...
  const core = {
    evidenceMode: 'support-only-rollout-threshold-configuration-v3',
...
function buildStoragePerformanceSamples(config) {
  return decisionSamplesMs.map((decisionMs, index) => {
    ...
    const core = {
      sampleIndex: index,
      thresholdBps,
      resourceKeyHash: sha256(`rpp-0760-resource-${index}`),
      expectedStorageHash: sha256(`rpp-0760-storage-before-${index}`),
      actualStorageHash: sha256(`rpp-0760-storage-before-${index}`),
...
function recomputeRolloutThresholdGates(evidence, repeatedEvidence) {
  ...
  return [
    proofGate('deterministic-threshold-configuration', config.schemaVersion === 3
      && config.variant === 3
