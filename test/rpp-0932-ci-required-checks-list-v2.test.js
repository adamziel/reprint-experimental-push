...
const evidencePath = path.join(repoRoot, 'docs/evidence/rpp-0932-ci-required-checks-list-v2.md');
const fixedNowIso = '2026-06-01T02:35:00.000Z';
const auditedHead = 'e88df1517c574b6902d483f838e55603ac9ef0f7';
...
const expectedCommands = [
  ...
  'node scripts/release/check-release-gates.mjs --scope final-release --now 2026-06-01T02:35:00.000Z',
  'node --check test/rpp-0932-ci-required-checks-list-v2.test.js',
  'node --test --test-name-pattern RPP-0932 test/rpp-0932-ci-required-checks-list-v2.test.js',
  'node scripts/release/artifact-redaction-scan.mjs docs/evidence/rpp-0932-ci-required-checks-list-v2.md',
  'git diff --check',
];

test('RPP-0932 CI required checks list v2 records support-only held verdict', () => {
  const { report, text } = loadEvidenceReport();

  assert.equal(report.schemaVersion, 1);
  assert.equal(report.rppId, 'RPP-0932');
  assert.equal(report.sliceId, 'RPP-0932');
  assert.equal(report.proofId, 'rpp-0932-ci-required-checks-list-v2');
  assert.equal(report.variant, 2);
  assert.equal(report.generatedAt, fixedNowIso);
  assert.equal(report.auditedBranch, 'session/rpp-932');
  assert.equal(report.auditedLaneHeadBeforeEvidence, auditedHead);
  assert.equal(report.status, 'support-only-ci-required-checks-list-v2-recorded');
...
test('RPP-0932 every required CI check remains blocking and production-observation gated', () => {
  const { report, text } = loadEvidenceReport();
  ...
  for (const check of report.requiredChecks) {
    ...
    assert.match(rowFor(text, check.id), /\| Blocking \| Required \| Held, no movement \|/);
  }
});

test('RPP-0932 support-only observations cannot move final release', () => {
...
test('RPP-0932 final release evaluator and status row remain held without production evidence', () => {
  const { report } = loadEvidenceReport();
...
test('RPP-0932 evidence is redacted and records exact validation commands', () => {
...
function loadEvidenceReport() {
  ...
  assert.ok(match?.groups?.json, 'RPP-0932 evidence must contain one JSON audit record block');
  return {
    text,
