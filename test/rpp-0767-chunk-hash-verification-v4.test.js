...
const proofId = 'rpp-0767-chunk-hash-verification-v4';
...
test('RPP-0767 variant 4 proves generated chunk hash coverage rejects stale storage', {
  concurrency: false,
}, () => {
  const proof = buildVariant4Proof();

  assert.equal(proof.rppId, 'RPP-0767');
  ...
  assert.match(proof.determinism.firstProjectionHash, /^sha256:[a-f0-9]{64}$/);
...
test('RPP-0767 variant 4 generated chunk guards fail closed on stale storage and hash mismatch', {
  concurrency: false,
}, () => {
...
test('RPP-0767 variant 4 resolver blocks stale or incomplete chunk hash evidence', () => {
...
function buildVariant4Proof() {
  ...
  const repeated = buildRecordedEvidence();
  const safeDecision = resolveChunkHashStoragePerformanceProof(evidence);
  const unsafe = projectUnsafeDecisions(unsafeChunkHashDecisions(evidence));
  const determinism = compareDeterministicChunkHashEvidence(evidence, repeated.evidence);
  ...
  const proofGates = [
    ...
    proofGate('deterministic-local-support-evidence', determinism.sameProjection, {
      firstProjectionHash: determinism.firstProjectionHash,
      secondProjectionHash: determinism.secondProjectionHash,
      ignoredVolatileFields: determinism.ignoredVolatileFields,
    }),
...
function runUnitBenchmark() {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'reprint-rpp-0767-chunk-hash-v4-'));
  try {
