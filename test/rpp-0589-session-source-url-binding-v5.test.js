...
function fixtureHash(label) {
  return sha256Hex(`rpp-0589:${label}`);
}
...
function authEnvelope() {
  return {
    ...
    session: {
      ...
      applicationPasswordUuid: 'app-pass-rpp-0589',
...
function sourceBindingMatches(binding, currentSource) {
  ...
  for (const field of [
    ...
    if (
      ...
      || String(binding[field] ?? '') !== String(currentSource[field] ?? '')
    ) {
      return false;
    }
  }
...
function receiptWithSourceBinding(receipt, sourceBinding) {
  const nextReceipt = cloneJson(receipt);
  if (sourceBinding === undefined) {
    delete nextReceipt.authBinding.source;
  } else {
    nextReceipt.authBinding.source = sourceBinding;
  }
  delete nextReceipt.receiptHash;
  nextReceipt.receiptHash = digest(nextReceipt);
  return nextReceipt;
}
...
function acceptedApplyRevalidation({ plan, receipt, currentSource }) {
  ...
  return {
    ...
    claim: {
      activeClaimId: 'claim-rpp-0589-accepted',
...
function trustedDbJournal({ requestHash, receipt, mutationCount }) {
  const activeClaimId = 'claim-rpp-0589-accepted';
...
test('RPP-0589 v5 route revalidates live source binding before apply mutation', () => {
...
test('RPP-0589 v5 release verifier carries source-bound apply revalidation', () => {
  assert.match(verifierSource, /const proof = await runAuthenticatedHttpPush\(\{/);
  assert.match(verifierSource, /sourceUrl: liveSourceUrl/);
  assert.match(verifierSource, /requireProductionAuthSession: true/);
  assert.match(verifierSource, /proveDurableJournalBoundary: true/);
  assert.match(verifierSource, /authSessionSource/);
  assert.match(verifierSource, /same live auth\/session source at issuance and readback/);
  assert.match(verifierSource, /authSessionSourceMatchesLiveSource/);

  assert.match(
    verifierSource,
    /proof\.apply\.applyRevalidation\?\.required,\s*'fresh-live-hashes-before-first-mutation'/,
  );
  ...
  assert.match(verifierSource, /proof\.apply\.applyRevalidation\?\.receiptHash, proof\.dryRun\.receiptHash/);
  assert.match(
    verifierSource,
    /proof\.apply\.applyRevalidation\?\.verifiedCount, proof\.planObject\.mutations\.length/,
  );
  assert.match(
    verifierSource,
    /proof\.apply\.applyRevalidation\?\.verifiedResourceKeys,[\s\S]*proof\.planObject\.mutations\.map\(\(mutation\) => mutation\.resourceKey\)/,
  );
  assert.match(verifierSource, /apply revalidation must bind to an active durable claim before mutation/);
  assert.match(verifierSource, /sessionUserIdentityBinding: proof\.sessionUserIdentityBinding/);
  assert.match(verifierSource, /apply: proof\.apply/);
});

test('RPP-0589 v5 mocked verifier rejects invalid source URL binding evidence before mutation setup', async () => {
  const cases = [
    {
      name: 'missing',
      expectedCode: 'AUTH_RECEIPT_MISMATCH',
    ...
    },
    {
      ...
      receiptSource: {
        ...sourceIdentity(),
        sourceHash: 'malformed-source-hash',
        sourceUrlHash: 'not-a-sha256',
      },
    },
    {
      name: 'stale',
      expectedCode: 'AUTH_RECEIPT_MISMATCH',
      receiptSource: sourceIdentity('http://127.0.0.1:8080/rpp-0589-stale-source'),
    },
    {
      name: 'drifted',
      expectedCode: 'AUTH_SOURCE_BINDING_MISMATCH',
      receiptSource: sourceIdentity(),
      driftLiveSource: true,
    },
  ];

  for (const sourceCase of cases) {
    const originalFetch = global.fetch;
    const idempotencyKey = `idem-rpp-0589-source-binding-${sourceCase.name}`;
    const pluginName = `rpp-0589-private-plugin-${sourceCase.name}`;
    const base = {
      files: {},
      plugins: { [pluginName]: { version: `rpp-0589-private-base-${sourceCase.name}` } },
      db: {},
    };
    const local = {
      files: {},
      plugins: { [pluginName]: { version: `rpp-0589-private-local-${sourceCase.name}` } },
      db: {},
    };
    const seen = [];
    let dryRunReceipt = null;
    let applyRejection = null;
    let receiptValidationCount = 0;
    ...
    global.fetch = async (url, options = {}) => {
      ...
      if (pathname === `${routePrefix}/dry-run`) {
        ...
        dryRunReceipt = receiptWithSourceBinding(
          receiptForPlan(body.plan, idempotencyKey, contentHash),
          sourceCase.receiptSource,
        );
        return jsonResponse({
          ok: true,
          mode: 'dry-run',
      ...
      if (pathname === `${routePrefix}/apply`) {
        ...
        const receiptSource = body.receipt?.authBinding?.source;
        const receiptMatchesCurrentSource = sourceBindingMatches(receiptSource, sourceIdentity());
        if (!receiptMatchesCurrentSource) {
          applyRejection = {
            schemaVersion: 1,
            case: sourceCase.name,
            code: 'AUTH_RECEIPT_MISMATCH',
            phase: 'receipt-source-url',
            checkedAgainst: 'receipt-source-url',
            requestHash: contentHash,
            receiptHash: body.receipt?.receiptHash || fixtureHash(`${sourceCase.name}:missing-receipt-hash`),
            planHash: body.receipt?.planHash || digest(body.plan),
            currentSourceHash: sourceIdentity().sourceHash,
            currentSourceUrlHash: sourceIdentity().sourceUrlHash,
            receiptSourceHash: sha256Pattern.test(receiptSource?.sourceHash || '')
              ? receiptSource.sourceHash
              : fixtureHash(`${sourceCase.name}:receipt-source-hash`),
            receiptSourceUrlHash: sha256Pattern.test(receiptSource?.sourceUrlHash || '')
              ? receiptSource.sourceUrlHash
              : fixtureHash(`${sourceCase.name}:receipt-source-url-hash`),
            sourceBindingHash: digest({
              case: sourceCase.name,
              currentSourceHash: sourceIdentity().sourceHash,
              currentSourceUrlHash: sourceIdentity().sourceUrlHash,
          ...
          return jsonResponse({
            ...
            message: 'Receipt source binding does not match the current live source.',
            applied: 0,
            responseSchemaVersion: 1,
            auth: authEnvelope(),
            idempotency: {
        ...
        const currentSource = sourceCase.driftLiveSource
          ? sourceIdentity(driftedSourceUrl)
          : sourceIdentity();
        if (!sourceBindingMatches(receiptSource, currentSource)) {
          ...
          applyRejection = {
            ...
            planHash: body.receipt.planHash,
            currentSourceHash: currentSource.sourceHash,
            currentSourceUrlHash: currentSource.sourceUrlHash,
            receiptSourceHash: receiptSource.sourceHash,
            receiptSourceUrlHash: receiptSource.sourceUrlHash,
        ...
        throw new Error(`unexpected accepted mutation setup for RPP-0589 ${sourceCase.name}`);
      }

      mutationSetupCount += 1;
      mutationWorkCount += 1;
      throw new Error(`unexpected mutation-capable path for RPP-0589 ${sourceCase.name}: ${pathname}`);
    };
    ...
    try {
      ...
      assert.equal(summary.ok, false, sourceCase.name);
      assert.equal(summary.code, sourceCase.expectedCode, sourceCase.name);
      assert.equal(summary.apply.applied, 0, sourceCase.name);
      assert.equal(summary.apply.idempotency.freshMutationWork, false, sourceCase.name);
      assert.equal(summary.recoveryInspect, null, sourceCase.name);
      assert.equal(summary.replay, null, sourceCase.name);
      assert.equal(summary.dbJournal, null, sourceCase.name);
      assert.equal(receiptValidationCount, 1, sourceCase.name);
      assert.equal(mutationSetupCount, 0, sourceCase.name);
      assert.equal(mutationWorkCount, 0, sourceCase.name);
      assert.equal(applyRejection.code, sourceCase.expectedCode, sourceCase.name);
      if (sourceCase.driftLiveSource) {
        assert.equal(liveRevalidationCount, 1, sourceCase.name);
        ...
        assert.equal(liveRevalidationCount, 0, sourceCase.name);
        assert.equal(summary.apply.applyRevalidation, undefined, sourceCase.name);
      }
      assert.deepEqual(
      ...
      const supportEvidence = {
        schemaVersion: 1,
        rejectedInvalidSourceUrlBindingBeforeMutationSetup: {
          case: sourceCase.name,
          code: applyRejection.code,
          phase: applyRejection.phase,
          checkedAgainst: applyRejection.checkedAgainst,
          requestHash: applyRejection.requestHash,
          planHash: applyRejection.planHash,
          receiptHash: applyRejection.receiptHash,
          currentSourceHash: applyRejection.currentSourceHash,
          currentSourceUrlHash: applyRejection.currentSourceUrlHash,
      ...
      assertHashFields(supportEvidence.rejectedInvalidSourceUrlBindingBeforeMutationSetup, [
        'requestHash',
        'planHash',
        'receiptHash',
        'currentSourceHash',
      ...
      assertNoRawValues(supportEvidence, [
        ...
        sourceUrl,
        driftedSourceUrl,
        'http://127.0.0.1:8080/rpp-0589-stale-source',
        pluginName,
...
test('RPP-0589 v5 mocked apply rejects drifted source binding before mutation-capable work', async () => {
  const originalFetch = global.fetch;
  const idempotencyKey = 'idem-rpp-0589-session-source-url-binding-v5-reject';
  const pluginName = 'rpp-0589-private-plugin-reject';
  const base = {
    files: {},
    plugins: { [pluginName]: { version: 'rpp-0589-private-base-state' } },
    db: {},
  };
  const local = {
    files: {},
    plugins: { [pluginName]: { version: 'rpp-0589-private-local-state' } },
    db: {},
  };
  const seen = [];
  let dryRunPlan = null;
  ...
  global.fetch = async (url, options = {}) => {
    ...
    mutationSetupCount += 1;
    mutationWorkCount += 1;
    throw new Error(`unexpected mutation setup path for RPP-0589: ${pathname}`);
  };
...
test('RPP-0589 v5 accepted apply records hash-only live-source revalidation before mutation', async () => {
  const originalFetch = global.fetch;
  const idempotencyKey = 'idem-rpp-0589-session-source-url-binding-v5-accepted';
  const pluginName = 'rpp-0589-private-plugin-accepted';
  const base = {
    files: {},
    plugins: { [pluginName]: { version: 'rpp-0589-private-base-state' } },
    db: {},
  ...
  global.fetch = async (url, options = {}) => {
    ...
    throw new Error(`unexpected path for RPP-0589 accepted support path: ${pathname}`);
  };
