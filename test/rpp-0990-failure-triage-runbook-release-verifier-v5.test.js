...
const evidencePath = path.join(repoRoot, 'docs/evidence/rpp-0990-failure-triage-runbook-release-verifier-v5.md');
const rpp0970EvidencePath = path.join(repoRoot, 'docs/evidence/rpp-0970-failure-triage-runbook-v4.md');
const rpp0980EvidencePath = path.join(repoRoot, 'docs/evidence/rpp-0980-go-no-go-release-decision-record-v4.md');
const releaseGatesPath = path.join(repoRoot, '.agents/RELEASE_GATES.md');

const auditedHead = '653382baf23112d8ffd02ec25474f4c57a021fb4';
const fixedNowIso = '2026-06-01T05:20:00.000Z';
...
test('RPP-0990 v5 evidence anchors support-only NO-GO without release gate movement', () => {
  const { text, record } = loadEvidence(evidencePath);
  const releaseGates = readText(releaseGatesPath);

  assert.match(text, /^# RPP-0990 failure triage runbook release verifier v5 evidence$/m);
  assert.match(text, /^Date: 2026-06-01$/m);
  assert.match(text, /^Audited local branch: `session\/rpp-990`$/m);
  assert.match(text, new RegExp(`^Audited lane head before this evidence file: \`${auditedHead}\`$`, 'm'));
  assert.equal(record.schemaVersion, 1);
  assert.equal(record.rppId, 'RPP-0990');
  assert.equal(record.sliceId, 'RPP-0990');
  assert.equal(record.proofId, 'rpp-0990-failure-triage-runbook-release-verifier-v5');
  assert.equal(record.variant, 5);
  assert.equal(record.generatedAt, fixedNowIso);
  assert.equal(record.auditedBranch, 'session/rpp-990');
  assert.equal(record.auditedLaneHeadBeforeEvidence, auditedHead);
  assert.equal(record.evidencePath, 'docs/evidence/rpp-0990-failure-triage-runbook-release-verifier-v5.md');
...
test('RPP-0990 v5 carries forward the RPP-0970 v4 failure-triage contract', () => {
  const { record } = loadEvidence(evidencePath);
  const { record: rpp0970 } = loadEvidence(rpp0970EvidencePath);

  assert.equal(record.patternEvidence, 'docs/evidence/rpp-0970-failure-triage-runbook-v4.md');
  assert.equal(record.carriedForwardFailureTriageContract, 'RPP-0970 failure triage runbook v4');
  assert.equal(record.baseFailureTriageRiskEvidence, rpp0970.baseFailureTriageRiskEvidence);
  ...
  assert.deepEqual(
    ...
    rpp0970.failureTriageRiskRegister.map(projectFailureTriageRisk),
  );
...
test('RPP-0990 v5 names every remaining final-release risk and keeps it open', () => {
  const { record } = loadEvidence(evidencePath);
  const { record: rpp0970 } = loadEvidence(rpp0970EvidencePath);
  const { record: rpp0980 } = loadEvidence(rpp0980EvidencePath);

  assert.equal(record.sourceDecisionRecord, 'docs/evidence/rpp-0980-go-no-go-release-decision-record-v4.md');
  ...
  assert.deepEqual(record.finalReleaseRiskRegister, rpp0980.remainingRisks);
  assert.deepEqual(record.finalReleaseRiskRegister, rpp0970.finalReleaseRiskRegister);
  assert.deepEqual(
...
test('RPP-0990 v5 closes no risk without production-backed closure proof', () => {
  const { record } = loadEvidence(evidencePath);
  const allRisks = [
...
test('RPP-0990 v5 maps failing proof classes to safe operator actions', () => {
  const { record } = loadEvidence(evidencePath);
...
test('RPP-0990 v5 evidence is redacted and records no release-gate movement', async () => {
  const { text, record } = loadEvidence(evidencePath);
  const scan = await scanArtifacts([
    'docs/evidence/rpp-0990-failure-triage-runbook-release-verifier-v5.md',
  ], { cwd: repoRoot });

  assert.doesNotThrow(() =>
    assertEvidenceHasNoRawValues(record, { label: 'RPP-0990 failure triage release verifier v5' }));
  ...
  assert.equal(record.evidenceLimits.mode, 'support-only-release-verifier-failure-triage-v5');
  ...
  assert.equal(record.evidenceLimits.credentialsIncluded, false);
...
test('RPP-0990 v5 final-release evaluator remains held without production closure proof', () => {
  const { record } = loadEvidence(evidencePath);
  ...
  assert.equal(record.releaseGateSnapshot.exitCode, 1);
  ...
  assert.equal(record.releaseGateSnapshot.status, result.report.status);
  assert.equal(record.releaseGateSnapshot.gateState, result.report.gateState);
