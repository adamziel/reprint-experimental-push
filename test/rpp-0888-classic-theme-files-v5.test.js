...
const evidencePath = path.join(
  repoRoot,
  'docs/evidence/rpp-0888-classic-theme-files-v5.md',
);

const fixedNow = new Date('2026-06-01T00:00:00.000Z');
const proofId = 'rpp-0888-classic-theme-files-v5';
const themeSlug = 'rpp-0888-classic-v5';
...
const classicThemeSurfaceFiles = Object.freeze([
  ...
  Object.freeze({ path: `${themeRoot}/assets/classic-v5.css`, surface: 'asset', kind: 'classic-theme-static-asset', required: false }),
...
const goodEnv = Object.freeze({
  REPRINT_PUSH_SOURCE_URL: 'https://source.example.test/classic-theme-v5',
  REPRINT_PUSH_REMOTE_URL: 'https://source.example.test:443/classic-theme-v5/',
  REPRINT_PUSH_LOCAL_URL: 'https://local.example.test/classic-theme-v5',
  REPRINT_PUSH_REMOTE_CHANGED_URL: 'https://changed.example.test/classic-theme-v5',
  REPRINT_PUSH_PREFLIGHT_SOURCE_URL: 'https://source.example.test/classic-theme-v5/',
  REPRINT_PUSH_DRY_RUN_ROUTE_SOURCE_URL: 'https://source.example.test:443/classic-theme-v5',
  REPRINT_PUSH_APPLY_SOURCE_URL: 'https://source.example.test/classic-theme-v5',
  REPRINT_PUSH_JOURNAL_ROUTE_SOURCE_URL: 'https://source.example.test/classic-theme-v5',
  REPRINT_PUSH_RECOVERY_INSPECT_SOURCE_URL: 'https://source.example.test/classic-theme-v5',
  REPRINT_PUSH_USERNAME: 'rpp-0888-admin',
  REPRINT_PUSH_APPLICATION_PASSWORD: 'rpp-0888-application-password-must-not-leak',
});

const badEnv = Object.freeze({
  REPRINT_PUSH_SOURCE_URL: 'https://admin:rpp0888-secret@source.example.test/classic-theme-v5',
  REPRINT_PUSH_LOCAL_URL: 'https://local.example.test/classic-theme-v5?token=rpp0888-token',
  REPRINT_PUSH_REMOTE_CHANGED_URL: 'https://changed.ngrok-free.app/classic-theme-v5',
  REPRINT_PUSH_PACKAGE_SMOKE_MODE: 'driver-guard-only',
});

const duplicateEnv = Object.freeze({
  ...goodEnv,
  REPRINT_PUSH_LOCAL_URL: 'https://source.example.test:443/classic-theme-v5/',
});
...
const forbiddenProofNeedles = Object.freeze([
  ...
  'rpp-0888-admin',
  'rpp-0888-application-password-must-not-leak',
  'rpp-0888-source-style-private',
  'rpp-0888-local-style-private',
  'rpp-0888-local-functions-private',
  'rpp-0888-local-front-page-private',
  'rpp-0888-remote-style-private',
]);

if (process.env.RPP_0888_REPORT_JSON !== '1') {
  test('RPP-0888 captures source/local/changed role URLs and identity-checks classic theme file scope', () => {
    const proof = buildClassicThemeFilesV5Proof({ env: goodEnv });

    assert.equal(proof.schemaVersion, 1);
    assert.equal(proof.rppId, 'RPP-0888');
    ...
    assert.equal(proof.builtOn.generatedCoveragePattern.rppId, 'RPP-0828');
    assert.equal(proof.builtOn.generatedCoveragePattern.variant, 2);
    assert.equal(proof.builtOn.focusedRegressionPattern.rppId, 'RPP-0848');
    assert.equal(proof.builtOn.focusedRegressionPattern.variant, 3);
    assert.equal(proof.builtOn.releaseVerifierPattern.rppId, 'RPP-0868');
    assert.equal(proof.builtOn.releaseVerifierPattern.variant, 4);
    ...
    assert.equal(proof.urlIdentity.localLoopbackIngressOnly8080, true);
    ...
    assert.deepEqual(proof.classicTheme.localChangedFileKeys, [
      `file:${themeRoot}/assets/classic-v5.css`,
    ...
    assert.equal(proof.invariants.localLoopbackIngressOnly8080, true);
    ...
    assert.doesNotThrow(() => assertEvidenceHasNoRawValues(proof, {
      label: 'RPP-0888 classic theme files v5 proof',
    }));
  });

  test('RPP-0888 rejects tunnel and secret-shaped role URLs before accepting classic theme scope', () => {
    const proof = buildClassicThemeFilesV5Proof({ env: badEnv });
  ...
  test('RPP-0888 rejects duplicate source/local/changed role URL identities before accepting classic theme scope', () => {
    const proof = buildClassicThemeFilesV5Proof({ env: duplicateEnv });
    ...
    assert.equal(proof.finalReleaseStatus, 'NO-GO');
    assert.equal(proof.urlIdentity.sourceLocalChangedUrlsCaptured, true);
    assert.equal(proof.urlIdentity.roleIdentitiesDistinct, false);
    assert.equal(proof.urlIdentity.identityChecked, false);
    assert.equal(proof.urlIdentity.noTunnelPolicyEnforced, true);
    assert.equal(proof.urlIdentity.noSecretShapedUrlParts, true);
    assert.equal(proof.urlIdentity.packagedFallbackDisabled, true);
  ...
  test('RPP-0888 classic theme files proof is deterministic and hash/count/surface-only', () => {
    const firstProof = buildClassicThemeFilesV5Proof({ env: goodEnv });
    const secondProof = buildClassicThemeFilesV5Proof({ env: { ...goodEnv } });
  ...
  test('RPP-0888 evidence document records the same NO-GO hash/count/surface scope', () => {
    const { report, text } = loadEvidenceReport();
    const expectedReport = buildEvidenceReport();

    assert.equal(report.schemaVersion, 1);
    assert.equal(report.rppId, 'RPP-0888');
...
function buildClassicThemeFilesV5Proof({ env, now = fixedNow } = {}) {
...
function buildEvidenceReport() {
  const proof = buildClassicThemeFilesV5Proof({ env: goodEnv });
  const negativeProof = buildClassicThemeFilesV5Proof({ env: badEnv });
  const duplicateProof = buildClassicThemeFilesV5Proof({ env: duplicateEnv });
  const reportCore = {
    schemaVersion: 1,
    rppId: 'RPP-0888',
    variant: 5,
    ...
    urlIdentityScope: {
      ...
      localLoopbackIngressOnly8080: proof.urlIdentity.localLoopbackIngressOnly8080,
    ...
    negativeControls: {
      ...
      duplicateRoleUrlsRejected: duplicateProof.failures.some((failure) =>
        failure.code === 'EXTERNAL_WORDPRESS_SOURCE_LOCAL_CHANGED_URLS_NOT_DISTINCT'),
...
function classicThemeSite(role) {
  const files = {
    [`${themeRoot}/style.css`]: [
      '/*',
      'Theme Name: RPP 0888 Classic V5',
      'Template: none',
      'Version: 5.0.0',
      '*/',
      'body { --rpp-source-marker: "rpp-0888-source-style-private"; }',
    ].join('\n'),
    [`${themeRoot}/functions.php`]: '<?php function rpp_0888_classic_source() { return "source"; }',
    [`${themeRoot}/index.php`]: '<?php echo "RPP 0888 source index";',
    [`${themeRoot}/page.php`]: '<?php echo "RPP 0888 source page";',
    [`${themeRoot}/header.php`]: '<?php ?><header>RPP 0888 source header</header>',
    [`${themeRoot}/footer.php`]: '<?php ?><footer>RPP 0888 source footer</footer>',
    [`${themeRoot}/screenshot.png`]: 'rpp-0888-source-screenshot-bytes',
    [`${themeRoot}/assets/classic-v5.css`]: '.rpp-0888-source { display: block; }',
    [`${themeRoot}/assets/logo.svg`]: '<svg><title>RPP 0888 source logo</title></svg>',
  };
  ...
  if (role === 'localEdited') {
    files[`${themeRoot}/style.css`] = [
      ...
      'Version: 5.0.1-local',
      '*/',
      'body { --rpp-local-marker: "rpp-0888-local-style-private"; }',
    ].join('\n');
    files[`${themeRoot}/functions.php`] = '<?php function rpp_0888_classic_local() { return "rpp-0888-local-functions-private"; }';
    files[`${themeRoot}/front-page.php`] = '<?php echo "rpp-0888-local-front-page-private";';
    files[`${themeRoot}/page.php`] = '<?php echo "RPP 0888 local page";';
    files[`${themeRoot}/assets/classic-v5.css`] = '.rpp-0888-local { display: grid; }';
  }
  ...
  if (role === 'remoteChanged') {
    files[`${themeRoot}/style.css`] = [
      ...
      'Theme Name: RPP 0888 Classic V5',
      'Template: none',
      'Version: 5.0.1-remote',
      '*/',
      'body { --rpp-remote-marker: "rpp-0888-remote-style-private"; }',
    ].join('\n');
  }
...
function loadEvidenceReport() {
  ...
  assert.ok(match?.groups?.json, 'RPP-0888 evidence must contain one JSON report block');
  return {
    text,
