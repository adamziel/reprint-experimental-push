...
const evidencePath = path.join(
  repoRoot,
  'docs/evidence/rpp-0848-classic-theme-files-v3.md',
);

const fixedNow = new Date('2026-06-01T00:00:00.000Z');
const proofId = 'rpp-0848-classic-theme-files-v3';
const themeSlug = 'rpp-0848-classic-v3';
...
const classicThemeSurfaceFiles = Object.freeze([
  ...
  Object.freeze({ path: `${themeRoot}/assets/classic-v3.css`, surface: 'asset', kind: 'classic-theme-static-asset', required: false }),
...
const goodEnv = Object.freeze({
  REPRINT_PUSH_SOURCE_URL: 'https://source.example.test/classic-theme-v3',
  REPRINT_PUSH_REMOTE_URL: 'https://source.example.test:443/classic-theme-v3/',
  REPRINT_PUSH_LOCAL_URL: 'https://local.example.test/classic-theme-v3',
  REPRINT_PUSH_REMOTE_CHANGED_URL: 'https://changed.example.test/classic-theme-v3',
  REPRINT_PUSH_PREFLIGHT_SOURCE_URL: 'https://source.example.test/classic-theme-v3/',
  REPRINT_PUSH_DRY_RUN_ROUTE_SOURCE_URL: 'https://source.example.test:443/classic-theme-v3',
  REPRINT_PUSH_APPLY_SOURCE_URL: 'https://source.example.test/classic-theme-v3',
  REPRINT_PUSH_JOURNAL_ROUTE_SOURCE_URL: 'https://source.example.test/classic-theme-v3',
  REPRINT_PUSH_RECOVERY_INSPECT_SOURCE_URL: 'https://source.example.test/classic-theme-v3',
  REPRINT_PUSH_USERNAME: 'rpp-0848-admin',
  REPRINT_PUSH_APPLICATION_PASSWORD: 'rpp-0848-application-password-must-not-leak',
});

const badEnv = Object.freeze({
  REPRINT_PUSH_SOURCE_URL: 'https://admin:rpp0848-secret@source.example.test/classic-theme-v3',
  REPRINT_PUSH_LOCAL_URL: 'https://local.example.test/classic-theme-v3?token=rpp0848-token',
  REPRINT_PUSH_REMOTE_CHANGED_URL: 'https://changed.ngrok-free.app/classic-theme-v3',
...
const forbiddenProofNeedles = Object.freeze([
  ...
  'rpp-0848-admin',
  'rpp-0848-application-password-must-not-leak',
  'rpp-0848-source-style-private',
  'rpp-0848-local-style-private',
  'rpp-0848-local-functions-private',
  'rpp-0848-local-front-page-private',
  'rpp-0848-remote-style-private',
]);

if (process.env.RPP_0848_REPORT_JSON !== '1') {
  test('RPP-0848 captures source/local/changed role URLs and identity-checks classic theme file scope', () => {
    const proof = buildClassicThemeFilesV3Proof({ env: goodEnv });

    assert.equal(proof.schemaVersion, 1);
    assert.equal(proof.rppId, 'RPP-0848');
    ...
    assert.deepEqual(proof.classicTheme.localChangedFileKeys, [
      `file:${themeRoot}/assets/classic-v3.css`,
    ...
    assert.doesNotThrow(() => assertEvidenceHasNoRawValues(proof, {
      label: 'RPP-0848 classic theme files v3 proof',
    }));
  });

  test('RPP-0848 rejects tunnel and secret-shaped role URLs before accepting classic theme scope', () => {
    const proof = buildClassicThemeFilesV3Proof({ env: badEnv });
  ...
  test('RPP-0848 classic theme files proof is deterministic and hash/count/surface-only', () => {
    const firstProof = buildClassicThemeFilesV3Proof({ env: goodEnv });
    const secondProof = buildClassicThemeFilesV3Proof({ env: { ...goodEnv } });
  ...
  test('RPP-0848 evidence document records the same NO-GO hash/count/surface scope', () => {
    const { report, text } = loadEvidenceReport();
    const expectedReport = buildEvidenceReport();

    assert.equal(report.schemaVersion, 1);
    assert.equal(report.rppId, 'RPP-0848');
...
function buildClassicThemeFilesV3Proof({ env, now = fixedNow } = {}) {
...
function buildEvidenceReport() {
  const proof = buildClassicThemeFilesV3Proof({ env: goodEnv });
  const negativeProof = buildClassicThemeFilesV3Proof({ env: badEnv });
  const reportCore = {
    schemaVersion: 1,
    rppId: 'RPP-0848',
    variant: 3,
...
function classicThemeSite(role) {
  const files = {
    [`${themeRoot}/style.css`]: [
      '/*',
      'Theme Name: RPP 0848 Classic V3',
      'Template: none',
      'Version: 3.0.0',
      '*/',
      'body { --rpp-source-marker: "rpp-0848-source-style-private"; }',
    ].join('\n'),
    [`${themeRoot}/functions.php`]: '<?php function rpp_0848_classic_source() { return "source"; }',
    [`${themeRoot}/index.php`]: '<?php echo "RPP 0848 source index";',
    [`${themeRoot}/page.php`]: '<?php echo "RPP 0848 source page";',
    [`${themeRoot}/header.php`]: '<?php ?><header>RPP 0848 source header</header>',
    [`${themeRoot}/footer.php`]: '<?php ?><footer>RPP 0848 source footer</footer>',
    [`${themeRoot}/screenshot.png`]: 'rpp-0848-source-screenshot-bytes',
    [`${themeRoot}/assets/classic-v3.css`]: '.rpp-0848-source { display: block; }',
    [`${themeRoot}/assets/logo.svg`]: '<svg><title>RPP 0848 source logo</title></svg>',
  };
  ...
  if (role === 'localEdited') {
    files[`${themeRoot}/style.css`] = [
      ...
      'Version: 3.0.1-local',
      '*/',
      'body { --rpp-local-marker: "rpp-0848-local-style-private"; }',
    ].join('\n');
    files[`${themeRoot}/functions.php`] = '<?php function rpp_0848_classic_local() { return "rpp-0848-local-functions-private"; }';
    files[`${themeRoot}/front-page.php`] = '<?php echo "rpp-0848-local-front-page-private";';
    files[`${themeRoot}/page.php`] = '<?php echo "RPP 0848 local page";';
    files[`${themeRoot}/assets/classic-v3.css`] = '.rpp-0848-local { display: grid; }';
  }
  ...
  if (role === 'remoteChanged') {
    files[`${themeRoot}/style.css`] = [
      ...
      'Theme Name: RPP 0848 Classic V3',
      'Template: none',
      'Version: 3.0.1-remote',
      '*/',
      'body { --rpp-remote-marker: "rpp-0848-remote-style-private"; }',
    ].join('\n');
  }
...
function loadEvidenceReport() {
  ...
  assert.ok(match?.groups?.json, 'RPP-0848 evidence must contain one JSON report block');
  return {
    text,
