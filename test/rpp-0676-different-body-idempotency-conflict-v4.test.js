...
function tempSqlitePath() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'reprint-rpp-0676-sqlite-'));
...
function baseSite() {
  return {
    files: {
      'rpp-0676-a.txt': 'base-hash-only-a',
      'rpp-0676-b.txt': 'base-hash-only-b',
      'rpp-0676-c.txt': 'base-hash-only-c',
    },
    plugins: {},
    db: {},
  };
...
function localSite() {
  return {
    files: {
      'rpp-0676-a.txt': 'local-hash-only-a',
      'rpp-0676-b.txt': 'local-hash-only-b',
      'rpp-0676-c.txt': 'local-hash-only-c',
    },
    plugins: {},
    db: {},
  };
...
function buildSqliteConflictFixture() {
  ...
  const activeClaimId = 'rpp-0676-active-claim';
  const previousClaimId = 'rpp-0676-previous-claim';
  const activeClaimKeyHash = recoveryClaimHash(activeClaimId);
...
test('RPP-0676 SQLite-backed variant 4 records hash-only different-body conflict and blocks rejected-body movement', {
...
test('RPP-0676 variant 4 rejects missing malformed stale duplicated or drifted conflict evidence before proof movement', {
  skip: DatabaseSync === null ? 'node:sqlite is unavailable in this Node.js runtime' : false,
}, () => {
  const fixture = buildSqliteConflictFixture();
