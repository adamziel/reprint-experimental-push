...
const cursorPattern = /^rpp-0666:([a-f0-9]{16}):(\d+)$/;
...
const generatedPaginationCases = Object.freeze([
  {
    id: 'rpp-0666-window-two-v4',
    mutationCount: 8,
    filePageSize: 2,
    sqlitePageLimit: 5,
  },
  {
    id: 'rpp-0666-window-seven-v4',
    mutationCount: 13,
    filePageSize: 7,
    sqlitePageLimit: 4,
  },
...
function tempJournalPath() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'reprint-rpp-0666-journal-'));
  return path.join(dir, 'recovery.jsonl');
}

function tempSqlitePath() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'reprint-rpp-0666-sqlite-'));
...
function generatedSites(generatedCase) {
  ...
  for (let index = 1; index <= generatedCase.mutationCount + 1; index++) {
    base.files[`${generatedCase.id}-file-${index}.txt`] =
      `base-raw-rpp-0666-${generatedCase.id}-${index}`;
  }
  ...
  for (let index = 1; index <= generatedCase.mutationCount; index++) {
    local.files[`${generatedCase.id}-file-${index}.txt`] =
      `local-raw-rpp-0666-${generatedCase.id}-${index}`;
  }
...
function seedCompletedProductionJournal(generatedCase) {
  ...
  const artifactRefs = {
    releaseProof: `artifact://rpp-0666/${generatedCase.id}/journal-pagination-v4`,
    recoverySupport: `artifact://rpp-0666/${generatedCase.id}/local-recovery-support`,
  };
...
function assertNoRawSiteValues(value, rawSiteValues) {
  ...
  for (const rawValue of rawSiteValues) {
    assert.equal(
      ...
      false,
      `Unexpected raw site value in RPP-0666 pagination evidence: ${rawValue}`,
    );
  }
}
...
function journalCursorFor(journalPath, offset) {
  return `rpp-0666:${journalPathCursorPrefix(journalPath)}:${offset}`;
}
...
function offsetFromJournalCursor(cursor, journalPath) {
  ...
  if (!match) {
    throw cursorError(
      'RPP-0666 journal page cursor is invalid.',
      'RPP0666_JOURNAL_CURSOR_INVALID',
      { cursorHash: digest({ cursor }) },
    );
  }
  if (match[1] !== journalPathCursorPrefix(journalPath)) {
    throw cursorError(
      'RPP-0666 journal page cursor belongs to a different checked journal path.',
      'RPP0666_JOURNAL_CURSOR_STALE',
      { cursorHash: digest({ cursor }) },
    );
  }
...
function readSqliteJournalPage(database, {
  ...
  if (!Number.isInteger(limit) || limit <= 0) {
    throw cursorError(
      'RPP-0666 SQLite journal page limit must be a positive integer.',
      'RPP0666_JOURNAL_PAGE_LIMIT_INVALID',
      { limit },
    );
  }
  ...
  if (offset > restarted.records.length) {
    throw cursorError(
      'RPP-0666 journal page cursor is outside the restarted recovery journal.',
      'RPP0666_JOURNAL_CURSOR_STALE',
...
test('RPP-0666 generated file-backed journal pagination variant 4 preserves recovery state', () => {
...
test('RPP-0666 generated SQLite journal pagination variant 4 keeps restart cursors on the checked path', {
    ...
    try {
      ...
      assert.throws(
        () => readSqliteJournalPage(database, {
          journalPath: seeded.filePath,
          cursor: 'not-a-rpp-0666-cursor',
          limit: generatedCase.sqlitePageLimit,
        }),
        (error) => assertSqliteCursorFailure(error, 'RPP0666_JOURNAL_CURSOR_INVALID'),
      );
      assert.throws(
        () => readSqliteJournalPage(database, {
          journalPath: seeded.filePath,
        ...
        (error) => assertSqliteCursorFailure(error, 'RPP0666_JOURNAL_CURSOR_STALE'),
      );
      assert.throws(
        () => readSqliteJournalPage(database, {
          journalPath: seeded.filePath,
      ...
      assert.throws(
        ...
        (error) => {
          assert.equal(error.code, 'RPP0666_JOURNAL_PAGE_LIMIT_INVALID');
          return true;
        },
      );
