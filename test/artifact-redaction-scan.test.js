...
test('scans nested JSON, Markdown, and HTML artifacts in deterministic order while allowing cautious redaction docs', async (t) => {
  const root = await fixtureRoot(t, {
    'z-notes.md': [
      ...
      'Docker-only examples may use http://wp-source inside the private Compose network.',
      'Operator command examples should use REPRINT_PUSH_APPLICATION_PASSWORD=<production-application-password>.',
    ].join('\n'),
...
test('scans current evidence and reporting artifacts without blocking documented sandbox placeholders', async () => {
  const report = await scanArtifacts(['docs/evidence', 'audits', 'progress.html'], { cwd: repoRoot });

  assert.equal(report.ok, true);
  assert.deepEqual(report.rejectedFiles, []);
  assert.ok(report.scannedFiles.length >= 20, 'expected current evidence/report files to be scanned');
  assert.ok(report.scannedFiles.includes('docs/evidence/ao-docker-local-production.md'));
  assert.ok(report.scannedFiles.includes('audits/auditor-reorg-v2-20260527.md'));
  assert.ok(report.scannedFiles.includes('progress.html'));
});
