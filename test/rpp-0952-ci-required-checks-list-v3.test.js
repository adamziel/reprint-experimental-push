...
const evidencePath = path.join(repoRoot, 'docs/evidence/rpp-0952-ci-required-checks-list-v3.md');
const fixedNowIso = '2026-06-01T03:09:00.000Z';
const auditedHead = '13eae74ae2ef57694c188e009e2d3623012683a6';
...
const expectedCommands = [
  ...
  'node scripts/release/check-release-gates.mjs --scope final-release --now 2026-06-01T03:09:00.000Z',
  'node --check test/rpp-0952-ci-required-checks-list-v3.test.js',
  'node --test --test-name-pattern RPP-0952 test/rpp-0952-ci-required-checks-list-v3.test.js',
  'node scripts/release/artifact-redaction-scan.mjs docs/evidence/rpp-0952-ci-required-checks-list-v3.md',
  'git diff --check',
];

test('RPP-0952 CI required checks list v3 records support-only held verdict', () => {
  const { report, text } = loadEvidenceReport();

  assert.equal(report.schemaVersion, 1);
  assert.equal(report.rppId, 'RPP-0952');
  assert.equal(report.sliceId, 'RPP-0952');
  assert.equal(report.proofId, 'rpp-0952-ci-required-checks-list-v3');
  assert.equal(report.variant, 3);
  assert.equal(report.generatedAt, fixedNowIso);
  assert.equal(report.auditedBranch, 'session/rpp-952');
  assert.equal(report.auditedLaneHeadBeforeEvidence, auditedHead);
  assert.equal(report.status, 'support-only-ci-required-checks-list-v3-recorded');
  ...
  assert.match(text, /Missing production-backed\s+proof and failed required proof both fail closed/);
  assert.match(text, /Gate movement therefore remains held and final release remains \*\*NO-GO\*\*/);
});

test('RPP-0952 every required CI check remains blocking and production-observation gated', () => {
  const { report, text } = loadEvidenceReport();
...
test('RPP-0952 support-only observations cannot move final release', () => {
...
test('RPP-0952 failed required proof fails closed with no release-gate movement', () => {
  const { report } = loadEvidenceReport();
...
test('RPP-0952 missing production proof keeps final release evaluator and status row held', () => {
  const { report } = loadEvidenceReport();
...
test('RPP-0952 evidence is redacted and records exact validation commands', () => {
...
function loadEvidenceReport() {
  ...
  assert.ok(match?.groups?.json, 'RPP-0952 evidence must contain one JSON audit record block');
  return {
    text,
