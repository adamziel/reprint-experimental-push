...
const evidencePath = path.join(
  repoRoot,
  'docs/evidence/rpp-0829-multisite-subdirectory-topology-v2.md',
);
const variant = 'RPP-0829-variant-2';
...
test('RPP-0829 support report records fail-closed multisite subdirectory variant 2 scope', () => {
  const { report, text } = loadSupportReport();

  assert.equal(report.schemaVersion, 1);
  assert.equal(report.rppId, 'RPP-0829');
  ...
  assert.equal(report.builtOn.candidateTopology.variant, 1);
  assert.equal(report.builtOn.candidateTopology.installMode, 'multisite');
  ...
  assert.equal(report.supportReport.variantFocus, 'real-wordpress-import-export-survival-contract');
  ...
  assert.equal(
    report.candidateScope.status,
    'multisite-subdirectory-import-export-survival-support-candidate',
  );
...
test('RPP-0829 records exact unavailable capabilities and stays NO-GO', () => {
...
test('RPP-0829 accepts only production-backed real subdirectory import/export survival proof', () => {
  const proof = evaluateMultisiteSubdirectoryImportExportV2(
...
test('RPP-0829 rejects missing, playground, subdomain, and partial survival evidence', () => {
  const missingProof = evaluateMultisiteSubdirectoryImportExportV2(null);
  ...
  const proof = evaluateMultisiteSubdirectoryImportExportV2(observed);
...
test('RPP-0829 evidence remains hash/count/surface only and scoped to support', () => {
  const { report, text } = loadSupportReport();
  ...
  assert.doesNotMatch(text, /rawUrlValuesIncluded": true/);
...
function evaluateMultisiteSubdirectoryImportExportV2(observed) {
  ...
  return {
    event: 'rpp-0829-multisite-subdirectory-topology-v2',
    variant,
    ok: successContractSatisfied,
    successContractSatisfied,
    acceptedForSupportContract: successContractSatisfied,
    releaseVerifierReviewRequired: successContractSatisfied,
    finalReleaseDecision: successContractSatisfied
...
function validateObservedImportExportSurvival(observed) {
  ...
  if (observed.runtime !== 'real-wordpress-import-export' || importExport.realWordPress !== true) {
    failures.push({
      ...
      reason: 'RPP-0829 requires real WordPress import/export, not local Playground or synthetic substitute evidence.',
    });
  }
  if (importExport.productionBacked !== true) {
    failures.push({
      code: 'PRODUCTION_BACKED_ARTIFACT_REQUIRED',
      reason: 'RPP-0829 variant 2 remains support-only until production-backed proof is supplied.',
    });
  }
  if (
...
function loadSupportReport() {
  ...
  assert.ok(match?.groups?.json, 'RPP-0829 evidence must contain one JSON support report block');
  return {
    text,
...
function sampleHash(label) {
  return digest({ rpp: 'RPP-0829', label });
}
