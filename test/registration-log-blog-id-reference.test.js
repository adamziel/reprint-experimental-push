...
const sourceBlogId = 927;
const targetBlogId = 1927;
const registrationLogId = 77;
...
const registrationLogResourceKey = rowResourceKey('wp_registration_log', registrationLogRowId);

const blogDomain = 'rpp-registration-log-blog.example.test';
const remoteDriftDomain = 'remote-private-rpp-registration-log-blog.example.test';
const registeredAt = '2026-06-02 00:00:00';
...
function mappedBlogRow(blogId) {
  return {
    ...
    site_id: 1,
    domain: blogDomain,
    path: '/registration-log-reference/',
    public: 1,
    archived: 0,
    mature: 0,
    spam: 0,
...
function mappedRegistrationLogRow(blogId) {
  return {
    ID: registrationLogId,
    email: 'hash-only-registration-log@example.test',
    IP: '192.0.2.77',
    blog_id: blogId,
...
function mappedRegistrationLogSite({ staleTarget = false } = {}) {
  const base = baseSite();
  ...
  local.db.wp_registration_log[registrationLogRowId] = mappedRegistrationLogRow(sourceBlogId);
...
test('rewrites mapped wp_registration_log.blog_id payload without rewriting row id through apply', () => {
  const { base, local, remote } = mappedRegistrationLogSite();
  ...
  const mutation = mutationFor(plan, registrationLogResourceKey);
  const plannedRegistrationLog = deserializeResourceValue(mutation.value);
  ...
  assert.ok(mutation, 'missing registration log mutation');
  assert.equal(mutation.action, 'put');
  assert.equal(mutation.changeKind, 'create');
  assert.equal(mutation.resourceKey, registrationLogResourceKey);
  assert.deepEqual(plannedRegistrationLog, mappedRegistrationLogRow(targetBlogId));
  assert.ok(rewrite, 'missing registration-log-blog rewrite evidence');
  assert.equal(rewrite.relationshipKey, 'wp_registration_log.blog_id');
  assert.equal(rewrite.field, 'blog_id');
  assert.equal(rewrite.sourceResourceKey, registrationLogResourceKey);
  assert.equal(rewrite.rewrittenResourceKey, registrationLogResourceKey);
  ...
  assert.deepEqual(result.site.db.wp_registration_log[registrationLogRowId], mappedRegistrationLogRow(targetBlogId));
  assertNoRawRows({ sourceDecision, targetDecision, rewrite });
});

test('apply refuses forged wp_registration_log.blog_id rewrite payload before mutation', () => {
  const { base, local, remote } = mappedRegistrationLogSite();
  const plan = planFor(base, local, remote);
  const forgedPlan = cloneJson(plan);
  const forgedRemote = cloneJson(remote);
  const forgedRemoteBefore = JSON.stringify(forgedRemote);
  const mutation = mutationFor(forgedPlan, registrationLogResourceKey);
  ...
  assert.ok(issue, 'missing forged registration log rewrite payload issue');
  assert.equal(issue.resourceKey, registrationLogResourceKey);
  assert.equal(issue.relationshipType, 'registration-log-blog');
  ...
  assert.equal(forgedRemote.db.wp_registration_log[registrationLogRowId], undefined);
});

test('blocks stale wp_registration_log.blog_id identity maps with hash-only evidence', () => {
  const { base, local, remote } = mappedRegistrationLogSite({ staleTarget: true });
  const plan = planFor(base, local, remote);
  const sourceBlocker = blockerFor(plan, sourceBlogResourceKey);
  const registrationLogBlocker = blockerFor(plan, registrationLogResourceKey);
  const blogReference = registrationLogBlocker?.references.find((entry) =>
    entry.relationshipType === 'registration-log-blog');
  ...
  assert.equal(mutationFor(plan, sourceBlogResourceKey), undefined);
  assert.equal(mutationFor(plan, registrationLogResourceKey), undefined);
  ...
  assert.ok(registrationLogBlocker, 'missing registration log reference blocker');
  assert.equal(registrationLogBlocker.class, 'stale-wordpress-graph-identity');
  assert.match(registrationLogBlocker.reason, /without proven identity mapping or reference rewriting/);
  assert.ok(blogReference, 'missing registration-log-blog target evidence');
  assert.equal(blogReference.relationshipKey, 'wp_registration_log.blog_id');
  assert.equal(blogReference.sourceResourceKey, registrationLogResourceKey);
  ...
  for (const hash of [
    ...
    registrationLogBlocker.baseHash,
    registrationLogBlocker.localHash,
    registrationLogBlocker.remoteHash,
  ...
  assert.equal(digest(remoteBefore), beforeHash, 'blocked registration log plan must refuse before mutation');
  assert.equal(remoteBefore.db.wp_registration_log[registrationLogRowId], undefined);
