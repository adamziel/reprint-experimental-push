...
const evidenceDocPath = 'docs/evidence/rpp-0998-telemetry-free-audit-mode-release-verifier-v5.md';
const fixedNow = '2026-06-01T05:35:00.000Z';
const observedAt = '2026-06-01T05:05:00.000Z';
...
function writeObservations(t, observations, prefix = 'rpp-0998-audit-') {
...
function loadEvidenceRecord() {
  ...
  assert.ok(match?.groups?.json, 'RPP-0998 evidence must contain one JSON record block');
  return {
    text,
...
function openProductionBackedProofGaps(evidenceRecord) {
  return evidenceRecord.unresolvedProductionBackedProofGaps
    .filter((gap) => gap.status === 'open-fail-closed');
}

function finalReleaseDecision(evidenceRecord, requiredSummary) {
  const openGaps = openProductionBackedProofGaps(evidenceRecord);
  return {
    ...
    openProductionBackedProofGaps: openGaps.length,
...
test('RPP-0998 failed required proof blocks CI and reporting without telemetry fields', (t) => {
...
test('RPP-0998 release verifier fails closed when required proof is missing', (t) => {
...
test('RPP-0998 release verifier carries the RPP-0978 audit contract without status movement', () => {
  const { record } = loadEvidenceRecord();

  assert.equal(record.inheritedContract, 'RPP-0978 telemetry-free audit mode v4');
...
test('RPP-0998 support-only observations cannot close production-backed proof gaps', () => {
  const { record } = loadEvidenceRecord();
  const controlSummary = summarizeRequiredReleaseChecks({
    observations: passingObservations(),
    now: fixedNow,
  ...
  assert.equal(controlSummary.releaseReady, true);
  assert.deepEqual(openProductionBackedProofGaps(record).map((gap) => gap.id), [
    'production-backed-release-verifier-run',
    'release-gate-status-mutation-proof',
    'checked-production-required-proof-refresh',
  ]);
  assert.deepEqual(decision, {
    ...
    openProductionBackedProofGaps: 3,
  ...
  assert.equal(record.finalReleasePolicy.unresolvedProductionBackedProofGapsFailClosed, true);
  assert.equal(record.finalReleasePolicy.finalReleaseRequiredPosture, 'NO-GO');
});

test('RPP-0998 support evidence is redaction-safe and preserves final NO-GO', async () => {
  const { text, record } = loadEvidenceRecord();

  assert.equal(record.schemaVersion, 1);
  assert.equal(record.rppId, 'RPP-0998');
  assert.equal(record.workerId, 'rpp-998');
  assert.equal(record.variant, 5);
  assert.equal(record.evidenceMode, 'support-only-release-verifier-required-check-fixture');
  ...
  assert.match(text, /Mode: support-only telemetry-free audit release verifier carry-through/);
  assert.match(text, /Audited lane head: RPP-0998 telemetry-free audit mode release verifier v5/);
  ...
  assert.match(text, /RPP-0978 v4\s+telemetry-free audit mode contract/);
  ...
  assert.match(text, /cannot move final release\s+readiness/i);
  assert.match(text, /production-backed proof\s+gaps stay open and fail closed/i);
