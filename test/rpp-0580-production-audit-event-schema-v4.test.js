...
const evidencePath = path.join(repoRoot, 'docs/evidence/rpp-0580-production-audit-event-schema-v4.md');
const liveReleaseVerifierSource = readFileSync(liveReleaseVerifierPath, 'utf8');

const sourceUrl = 'https://source.example.test/rpp-0580';
...
const credential = {
  username: 'rpp_0580_admin',
  password: 'rpp-0580-application-password-should-not-leak',
};
const sessionId = 'psh_rpp_0580_raw_session_id';
const idempotencyKey = 'idem-rpp-0580-raw-idempotency-key';
const rawPayload = 'rpp-0580-private-payload-value';
const rawOptionValue = 'rpp-0580-private-option-value';
const rawJournalPayload = 'rpp-0580-private-journal-payload-value';
...
function createFocusedProductionAuditSchemaRoute({
  ...
  async function fetchHandler(url, options = {}) {
    ...
    if (request.pathname === `${routePrefix}/dry-run`) {
      return jsonResponse({
        ...
        mode: 'dry-run',
        auth,
        receipt: { receiptHash: sha256Hex('rpp-0580-dry-run-receipt') },
      });
    }
    ...
    if (request.pathname === `${routePrefix}/apply`) {
      ...
      return jsonResponse({
        ...
        auth,
        ...(replayed ? {} : { receipt: { receiptHash: sha256Hex('rpp-0580-apply-receipt') } }),
    ...
    throw new Error(`unexpected RPP-0580 fetch to ${request.pathname}`);
  }
...
function auditSchemaEvidenceCode({ schemaIssues, rowIssues }) {
...
function buildFocusedAuditSchemaEvidence({ pushSummary, journalRequest, rows }) {
  ...
  const code = ok
    ? 'LOCAL_PRODUCTION_AUDIT_EVENT_SCHEMA_V4_SUPPORT_ONLY'
    : auditSchemaEvidenceCode({ schemaIssues, rowIssues });
  const core = {
    schemaVersion: 1,
    slice: 'RPP-0580',
    proofClass: 'focused-production-audit-event-schema-v4',
    ...
    releaseMovement: {
      ...
      gates: '0/4',
      reason: ok
        ? 'focused local audit event schema proof is support-only until checked production release evidence exists'
        : 'malformed or missing audit event schema evidence blocks release movement',
    },
    boundary: {
...
function buildVerifyReleaseStyleSummary(focusedEvidence) {
  const auditEventSchema = focusedEvidence.dbJournalReadback.auditEventSchema;
  const reason = focusedEvidence.ok
    ? 'PRODUCTION_EVIDENCE_REQUIRED'
    : focusedEvidence.code;
  return {
    ok: false,
    releaseStatus: 'NO-GO',
    ...
    mutationAttempted: false,
    dbJournal: {
      status: focusedEvidence.dbJournalReadback.status,
      ok: focusedEvidence.dbJournalReadback.ok,
      requestedLimit: focusedEvidence.dbJournalReadback.requestedLimit,
      readbackPages: focusedEvidence.dbJournalReadback.readbackPages,
      paginationComplete: focusedEvidence.dbJournalReadback.paginationComplete,
      paginationTruncated: focusedEvidence.dbJournalReadback.paginationTruncated,
      rows: focusedEvidence.dbJournalReadback.rows,
      rowCount: focusedEvidence.dbJournalReadback.rowCount,
      eventCounts: focusedEvidence.dbJournalReadback.eventCounts,
      latestEventsHash: focusedEvidence.dbJournalReadback.latestEventsHash,
      journalScopeHash: focusedEvidence.dbJournalReadback.journalScopeHash,
      auditEventSchema,
    },
    productionAuditEventSchema: {
      ok: focusedEvidence.ok === true,
      ...
      routeEvidenceHash: auditEventSchema?.routeEvidence
        ...
        : null,
      auditEventRowsHash: focusedEvidence.auditEventHashEvidence.rowsHash,
      proofHash: focusedEvidence.proofHash,
      hashOnly: focusedEvidence.auditEventHashEvidence.hashOnly,
      issues: [
        ...focusedEvidence.schemaValidation.issues,
        ...focusedEvidence.auditEventHashEvidence.issues,
      ],
      required: [
      ...
      scope: focusedEvidence.evidenceScope,
    },
    releaseMovement: {
      allowed: false,
      gates: '0/4',
      reason: focusedEvidence.ok
        ? 'production-backed audit event schema route proof required before release movement'
        : 'valid hash-only audit event schema evidence is required before release movement',
    },
    boundary: {
      firstRemainingProductionBoundary: 'checked production-backed audit event schema route proof',
      status: 'blocked',
...
test('RPP-0580 v4 verify:release summary carries one production audit event schema route-evidence summary', async () => {
  const originalFetch = global.fetch;
  const route = createFocusedProductionAuditSchemaRoute();
  global.fetch = route.fetchHandler;

  try {
    const pushSummary = await runFocusedPush(route);
    const journalRequest = route.state.journalReads[0];
    const focusedEvidence = buildFocusedAuditSchemaEvidence({
      pushSummary,
      journalRequest,
      rows: route.state.rows,
    });
    const verifyReleaseSummary = buildVerifyReleaseStyleSummary(focusedEvidence);
    ...
    assert.equal(focusedEvidence.ok, true);
    assert.equal(focusedEvidence.productionBacked, false);
    assert.equal(focusedEvidence.releaseStatus, 'NO-GO');
    assert.equal(focusedEvidence.releaseMovement.allowed, false);
    assert.deepEqual(focusedEvidence.schemaValidation.issues, []);
    assert.deepEqual(focusedEvidence.auditEventHashEvidence.issues, []);
    assert.equal(focusedEvidence.auditEventHashEvidence.hashOnly, true);
    assertHashOnlyFields(focusedEvidence.sourceSummary, ['sourceUrlHash']);
    assertHashOnlyFields(focusedEvidence.authSummary, [
      'credentialHash',
      'userLoginHash',
      'sessionIdHash',
    ]);
    assertHashOnlyFields(focusedEvidence.dbJournalReadback, [
      'latestEventsHash',
      'journalScopeHash',
    ]);
    assertHashOnlyFields(focusedEvidence.auditEventHashEvidence, ['rowsHash']);
    assert.match(focusedEvidence.proofHash, hashPattern);
...
test('RPP-0580 v4 malformed audit schema evidence is rejected before release movement', async () => {
  const originalFetch = global.fetch;
  const route = createFocusedProductionAuditSchemaRoute({
  ...
  try {
    ...
    assert.equal(focusedEvidence.ok, false);
    assert.equal(focusedEvidence.code, 'PRODUCTION_AUDIT_EVENT_HASH_ONLY_REQUIRED');
    assert.equal(focusedEvidence.status, 'blocked');
    assert.equal(focusedEvidence.releaseMovement.allowed, false);
    assert.equal(focusedEvidence.boundary.verdict, 'PRODUCTION_AUDIT_EVENT_HASH_ONLY_REQUIRED');
    assert.deepEqual(focusedEvidence.auditEventHashEvidence.issues, []);
    assert.deepEqual(focusedEvidence.schemaValidation.issues, [
...
test('RPP-0580 v4 missing audit schema evidence is rejected before release movement', async () => {
  const originalFetch = global.fetch;
  const route = createFocusedProductionAuditSchemaRoute({
  ...
  try {
    ...
    assert.equal(focusedEvidence.ok, false);
    assert.equal(focusedEvidence.code, 'PRODUCTION_AUDIT_EVENT_SCHEMA_REQUIRED');
    assert.equal(focusedEvidence.status, 'blocked');
    assert.deepEqual(focusedEvidence.schemaValidation.issues, ['missing-dbJournal.auditEventSchema']);
    assert.deepEqual(focusedEvidence.auditEventHashEvidence.issues, []);
    assert.equal(focusedEvidence.releaseMovement.allowed, false);
...
test('RPP-0580 v4 combined verifier source and evidence doc preserve support-only single-summary scope', () => {
  ...
  assert.match(evidence, /^# RPP-0580 production audit event schema, variant 4$/m);
