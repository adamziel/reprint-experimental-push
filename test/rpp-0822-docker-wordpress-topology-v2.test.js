...
const evidencePath = path.join(
  repoRoot,
  'docs/evidence/rpp-0822-docker-wordpress-topology-v2.md',
);
...
test('RPP-0822 support report records Docker WordPress topology v2 scope', () => {
  const { report, text } = loadSupportReport();
  const { artifact, plan, probe } = buildMissingDockerCapabilityArtifact();
  const validation = validateDockerWordPressTopologyV2Report(report);
  ...
  assert.equal(report.rppId, 'RPP-0822');
  assert.equal(report.proofId, 'rpp-0822-docker-wordpress-topology-v2');
  ...
  assert.equal(report.builtOn.adjacentEvidence, 'RPP-0802 docker-wordpress-topology-v1');
  ...
  assert.equal(report.dockerWordPressTopologyV2.proofScope, 'docker-wordpress-topology-v2');
  assert.equal(report.dockerWordPressTopologyV2.siteRoleCount, 4);
  assert.equal(report.dockerWordPressTopologyV2.startedSiteCount, 0);
  assert.equal(report.dockerWordPressTopologyV2.startupBlockedBy, 'DOCKER_CLI_MISSING');
  assert.deepEqual(report.dockerWordPressTopologyV2.requiredServiceHosts, requiredServiceHosts);
  assert.deepEqual(
    report.dockerWordPressTopologyV2.dockerServiceSurfaces.map((entry) => entry.role),
    requiredRoleOrder,
  );
  assert.deepEqual(
    report.dockerWordPressTopologyV2.dockerServiceSurfaces.map((entry) => entry.serviceHost),
    plan.sites.map((site) => hostFromDockerUrl(site.url)),
  );
  assert.equal(report.dockerWordPressTopologyV2.network.internal, true);
  assert.equal(report.dockerWordPressTopologyV2.network.publishedHttpIngressCount, 1);
  assert.equal(report.dockerWordPressTopologyV2.network.publishedHttpIngressPort, 8080);
  assert.equal(report.dockerWordPressTopologyV2.network.publishedHttpIngressHostSurface, 'loopback-only');
  assert.equal(report.dockerWordPressTopologyV2.releaseVerifier.releaseUrlsUseDockerDns, true);
  assert.equal(report.dockerWordPressTopologyV2.releaseVerifier.releaseCommandIsVerifyRelease, true);
  assert.equal(report.dockerWordPressTopologyV2.releaseVerifier.packagedFallbackAllowed, false);
  assert.equal(report.dockerWordPressTopologyV2.releaseVerifier.packagedFallbackObserved, false);
  ...
  assert.doesNotThrow(() => assertEvidenceHasNoRawValues(report, {
    label: 'RPP-0822 Docker WordPress topology v2 support report',
  }));
});

test('RPP-0822 blocked Docker artifact fails closed without packaged fallback', () => {
...
test('RPP-0822 accepts only verify:release on Docker DNS with no packaged fallback', () => {
  const blocked = evaluateDockerWordPressTopologyV2Contract(buildMissingDockerCapabilityArtifact());
  const passed = evaluateDockerWordPressTopologyV2Contract(buildPassedDockerArtifact());
  ...
  const plan = buildDockerTopologyPlan({
    cwd: '/repo/reprint-push',
    workDir: '/tmp/rpp-0822-docker-work',
    evidenceDir: '/tmp/rpp-0822-docker-evidence',
    env: {},
  });
...
test('RPP-0822 validation rejects ambiguous, widened, or raw-scope evidence', () => {
  ...
  assertFailure(
    validateDockerWordPressTopologyV2Report(ambiguous),
    'DOCKER_WORDPRESS_TOPOLOGY_V2_UNAVAILABLE_CAPABILITY_NOT_EXACT',
  );

  const fallback = structuredClone(report);
  fallback.dockerWordPressTopologyV2.releaseVerifier.packagedFallbackObserved = true;
  assertFailure(
    validateDockerWordPressTopologyV2Report(fallback),
    'DOCKER_WORDPRESS_TOPOLOGY_V2_PACKAGED_FALLBACK_REJECTED',
  );
  ...
  assertFailure(
    validateDockerWordPressTopologyV2Report(movementClaim),
    'DOCKER_WORDPRESS_TOPOLOGY_V2_SUPPORT_ONLY_NO_GO_REQUIRED',
  );

  const rawUrl = structuredClone(report);
  rawUrl.dockerWordPressTopologyV2.dockerServiceSurfaces[0].rawUrl = 'http://wp-source';
  assertFailure(
    validateDockerWordPressTopologyV2Report(rawUrl),
    'DOCKER_WORDPRESS_TOPOLOGY_V2_RAW_URL_SURFACE_REJECTED',
  );

  const missingRole = structuredClone(report);
  missingRole.dockerWordPressTopologyV2.requiredServiceHosts = requiredServiceHosts.slice(0, 3);
  assertFailure(
    validateDockerWordPressTopologyV2Report(missingRole),
    'DOCKER_WORDPRESS_TOPOLOGY_V2_SERVICE_HOSTS_MISMATCH',
  );
});
...
function loadSupportReport() {
  ...
  assert.ok(match?.groups?.json, 'RPP-0822 evidence must contain one JSON support report block');
  return {
    text,
...
function evaluateDockerWordPressTopologyV2Contract({ artifact, plan, probe }) {
...
function validateDockerWordPressTopologyV2Report(report) {
  const failures = [];
  if (report.rppId !== 'RPP-0822'
    || report.variant !== 2
    || report.proofId !== 'rpp-0822-docker-wordpress-topology-v2') {
    failures.push({ code: 'DOCKER_WORDPRESS_TOPOLOGY_V2_IDENTITY_MISMATCH' });
  }
  if (report.supportOnly !== true
    ...
    failures.push({ code: 'DOCKER_WORDPRESS_TOPOLOGY_V2_SUPPORT_ONLY_NO_GO_REQUIRED' });
  }
  if (report.successContract?.verifyReleasePassedWithoutPackagedFallback !== true
    ...
    failures.push({ code: 'DOCKER_WORDPRESS_TOPOLOGY_V2_SUCCESS_CONTRACT_UNMET' });
  }
  if (report.topologyCommand?.status !== 'passed') {
    ...
    if (!capability?.code || !capability?.capability || !capability?.command) {
      failures.push({ code: 'DOCKER_WORDPRESS_TOPOLOGY_V2_UNAVAILABLE_CAPABILITY_NOT_EXACT' });
    }
  }
  if (!arraysEqual(report.dockerWordPressTopologyV2?.requiredServiceHosts || [], requiredServiceHosts)
    || report.dockerWordPressTopologyV2?.siteRoleCount !== requiredServiceHosts.length
    || report.dockerWordPressTopologyV2?.dockerServiceSurfaces?.length !== requiredServiceHosts.length) {
    failures.push({ code: 'DOCKER_WORDPRESS_TOPOLOGY_V2_SERVICE_HOSTS_MISMATCH' });
  }
  if (report.dockerWordPressTopologyV2?.releaseVerifier?.releaseCommandIsVerifyRelease !== true
    || report.topologyCommand?.releaseVerifierCommand !== dockerReleaseCommand.join(' ')) {
    failures.push({ code: 'DOCKER_WORDPRESS_TOPOLOGY_V2_VERIFY_RELEASE_COMMAND_REQUIRED' });
  }
  if (report.dockerWordPressTopologyV2?.releaseVerifier?.releaseUrlsUseDockerDns !== true) {
    failures.push({ code: 'DOCKER_WORDPRESS_TOPOLOGY_V2_DOCKER_DNS_REQUIRED' });
  }
  if (report.dockerWordPressTopologyV2?.releaseVerifier?.packagedFallbackAllowed !== false
    || report.dockerWordPressTopologyV2?.releaseVerifier?.packagedFallbackObserved !== false) {
    failures.push({ code: 'DOCKER_WORDPRESS_TOPOLOGY_V2_PACKAGED_FALLBACK_REJECTED' });
  }
  if (report.dockerWordPressTopologyV2?.network?.publishedHttpIngressCount !== 1
    || report.dockerWordPressTopologyV2?.network?.publishedHttpIngressPort !== 8080
    || report.dockerWordPressTopologyV2?.network?.publishedHttpIngressHostSurface !== 'loopback-only'
    || report.dockerWordPressTopologyV2?.network?.internal !== true) {
    failures.push({ code: 'DOCKER_WORDPRESS_TOPOLOGY_V2_LOCAL_ONLY_POLICY_FAILED' });
  }
  if (report.evidenceLimits?.mode !== 'hash-count-surface-only'
    ...
    failures.push({ code: 'DOCKER_WORDPRESS_TOPOLOGY_V2_EVIDENCE_LIMITS_FAILED' });
  }
  if (findRawHttpStrings(report).length > 0) {
    failures.push({ code: 'DOCKER_WORDPRESS_TOPOLOGY_V2_RAW_URL_SURFACE_REJECTED' });
  }
  if (!supportReportHashesOk(report)) {
    failures.push({ code: 'DOCKER_WORDPRESS_TOPOLOGY_V2_HASH_MISMATCH' });
  }
...
function assertSupportReportHashes(report) {
  ...
  assert.match(report.dockerWordPressTopologyV2.scopeHash, proofHashPattern);
  assert.match(report.dockerWordPressTopologyV2.serviceSurfaceDigest, proofHashPattern);
  assert.match(report.dockerWordPressTopologyV2.requirementDigest, proofHashPattern);
...
function supportReportHashesOk(report) {
  ...
  const scopeCore = {
    proofScope: report.dockerWordPressTopologyV2?.proofScope,
    status: report.status,
    successContract: report.successContract,
    topologyCommand: report.topologyCommand,
    dockerServiceSurfaces: report.dockerWordPressTopologyV2?.dockerServiceSurfaces,
    requirementSurfaces: report.dockerWordPressTopologyV2?.requirementSurfaces,
    releaseGate: report.releaseGate,
  };

  return report.dockerWordPressTopologyV2?.serviceSurfaceDigest
      === `sha256:${digest(report.dockerWordPressTopologyV2?.dockerServiceSurfaces || [])}`
    && report.dockerWordPressTopologyV2?.requirementDigest
      === `sha256:${digest(report.dockerWordPressTopologyV2?.requirementSurfaces || [])}`
    && report.dockerWordPressTopologyV2?.scopeHash === `sha256:${digest(scopeCore)}`
...
function writeTempArtifact(artifact) {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'reprint-rpp-0822-gate-artifact-'));
