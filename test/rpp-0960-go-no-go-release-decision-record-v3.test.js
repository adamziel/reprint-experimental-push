...
const evidencePath = path.join(repoRoot, 'docs/evidence/rpp-0960-go-no-go-release-decision-record-v3.md');
const fixedNowIso = '2026-06-01T03:35:00.000Z';
const auditedHeadBeforeEvidence = 'f055e243ecd8ed82033ca4fc20a114ef2f531df7';
...
test('RPP-0960 v3 evidence proves support-only NO-GO and names every remaining risk', () => {
  ...
  assert.match(evidenceText, /^# RPP-0960 go\/no-go release decision record v3 evidence$/m);
  ...
  assert.equal(evidence.rppId, 'RPP-0960');
  assert.equal(evidence.proofId, 'rpp-0960-go-no-go-release-decision-record-v3');
  assert.equal(evidence.variant, 3);
  ...
  assert.equal(evidence.auditedBranch, 'session/rpp-960');
  ...
  assert.equal(evidence.evidencePath, 'docs/evidence/rpp-0960-go-no-go-release-decision-record-v3.md');
...
test('RPP-0960 v3 risk register matches the current final release gate evaluator', () => {
...
test('RPP-0960 v3 closes no risk without production-backed proof and moves no release gate status', () => {
  const decision = loadMarkdownJson(decisionPath).report;
  const evidence = loadMarkdownJson(evidencePath).report;
...
test('RPP-0960 v3 artifacts remain redacted and do not authorize release movement', async () => {
  ...
  const scan = await scanArtifacts([
    ...
    'docs/evidence/rpp-0960-go-no-go-release-decision-record-v3.md',
  ], { cwd: repoRoot });

  assert.doesNotThrow(() =>
    assertEvidenceHasNoRawValues(decision.report, { label: 'RPP-0960 go/no-go decision record' }));
  assert.doesNotThrow(() =>
    assertEvidenceHasNoRawValues(evidence.report, { label: 'RPP-0960 go/no-go evidence' }));
