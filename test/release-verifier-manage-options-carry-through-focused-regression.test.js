...
const secretValue = 'RPP_0089_SHOULD_NOT_LEAK';
const capabilityReason = 'The checked production user does not prove manage_options capability.';
const focusedCommand = 'node --test test/release-verifier-manage-options-carry-through-focused-regression.test.js test/release-gate-manage-options-capability-regression.test.js test/release-gate-manage-options-generated.test.js test/release-verifier-application-password-binding-carry-through-focused-regression.test.js test/release-gates.test.js test/release-gate-cli.test.js';
const statusMarker = '[verify-release:held exit=1 reason=MANAGE_OPTIONS_CAPABILITY_REQUIRED mutationAttempted=false]';
const releaseGateHeldMarker = '[release-gates-ci:held final=19/20 candidate=19/20 reason=MANAGE_OPTIONS_CAPABILITY_REQUIRED]';
...
const authSessionSourceCommand = `${process.execPath} -e "process.stdout.write(JSON.stringify({sourceUrl:'${sourceUrl}',username:'${checkedUser}',applicationPassword:'${secretValue}',capabilities:{manage_options:false}}))"`;
const redactedAuthSessionSourceCommand = authSessionSourceCommand.replace(secretValue, '<redacted>');

const expectedFailedCapabilityEvidence = {
  ok: false,
...
const expectedPassedCapabilityEvidence = {
  ok: true,
...
function runVerifyReleaseManageOptionsDenial() {
...
function manageOptionsCapabilityEvidenceFromReport(report) {
  return {
    ok: false,
    hasManageOptions: false,
    observed: report.boundary.manageOptionsCapability.observed,
    checkedUser: report.boundary.manageOptionsCapability.checkedUser,
    route: report.boundary.manageOptionsCapability.route,
    method: report.boundary.manageOptionsCapability.method,
    expectedCapability: report.boundary.manageOptionsCapability.expectedCapability,
    capabilities: report.boundary.manageOptionsCapability.capabilities,
    scope: 'final-release',
  };
}
...
function writeEvidence(evidence) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'release-verifier-manage-options-carry-through-'));
...
test('release verifier carries manage_options denial before mutation for RPP-0089', () => {
  const proof = runVerifyReleaseManageOptionsDenial();
  const verifyReport = extractJsonObjects(proof.stdout)
    .find((entry) => entry?.releaseProof?.code === 'MANAGE_OPTIONS_CAPABILITY_REQUIRED');
  ...
  assert.ok(
    ...
    'verify:release stdout should end with the manage_options held marker',
  );
  assert.doesNotMatch(
    `${proof.stdout}\n${proof.stderr}`,
    /Starting Playground server/,
    'manage_options denial must fail before starting live verifier servers',
  );
  ...
  assert.deepEqual(
    ...
    },
    {
      ...
      boundary: {
        ...
        status: 'blocked',
        verdict: 'MANAGE_OPTIONS_CAPABILITY_REQUIRED',
        authSession: {
          required: 'authenticated user has manage_options on checked route',
          observed: 'subscriber-without-manage_options',
          verdict: 'MANAGE_OPTIONS_CAPABILITY_REQUIRED',
      ...
      preflight: {
        status: 0,
        authSessionType: 'subscriber-without-manage_options',
        routeProfile: 'production-shaped',
        session: {
          id: '',
          type: 'subscriber-without-manage_options',
        },
      },
      releaseProof: {
        ok: false,
      ...
      releaseMovement: {
        allowed: false,
        gates: '0/4',
        reason: 'MANAGE_OPTIONS_CAPABILITY_REQUIRED',
      },
    },
  );

  const releaseGateResult = runReleaseGateCheck(completeFinalEvidence({
    manageOptionsCapability: manageOptionsCapabilityEvidenceFromReport(verifyReport),
    verifyReleaseFailure: verifyReleaseFailureFromReport(verifyReport),
  }));
  const releaseGateReport = parseReport(releaseGateResult);
  const manageOptionsGate = gateById(releaseGateReport, 'manage-options-capability');
  ...
  assert.equal(releaseGateReport.primaryFailureCode, 'MANAGE_OPTIONS_CAPABILITY_REQUIRED');
  ...
  assert.deepEqual(manageOptionsGate.evidence, expectedFailedCapabilityEvidence);
...
test('release verifier manage_options carry-through keeps the capable positive path for RPP-0089', () => {
  const releaseGateResult = runReleaseGateCheck(completeFinalEvidence());
  const releaseGateReport = parseReport(releaseGateResult);
  const manageOptionsGate = gateById(releaseGateReport, 'manage-options-capability');
  ...
  assert.equal(releaseGateReport.mutationAttempted, false);
  assert.deepEqual(manageOptionsGate, {
    id: 'manage-options-capability',
    rpp: 'RPP-0009',
...
test('release verifier manage_options carry-through is recorded in RPP-0089 reports', () => {
  const evidenceReport = fs.readFileSync(evidenceReportPath, 'utf8');
  const progressReport = fs.readFileSync(progressReportPath, 'utf8');

  assert.ok(evidenceReport.includes('| RPP-0089 | Evidence toward release verifier manage_options capability carry-through'));
  assert.ok(evidenceReport.includes(`- Command: \`${focusedCommand}\``));
  assert.ok(evidenceReport.includes(`- Observed status: \`pass\`; verifier marker: \`${statusMarker}\`; manage-options gate: \`MANAGE_OPTIONS_CAPABILITY_REQUIRED\`; release marker: \`${releaseGateHeldMarker}\`.`));
  assert.ok(progressReport.includes('Release verifier manage_options capability carry-through now checks `RPP-0089`'));
  assert.ok(progressReport.includes(`- Command: \`${focusedCommand}\``));
  assert.ok(progressReport.includes(`- Observed status: \`pass\`; verifier marker: \`${statusMarker}\`; manage-options gate: \`MANAGE_OPTIONS_CAPABILITY_REQUIRED\`; release marker: \`${releaseGateHeldMarker}\`.`));
});
