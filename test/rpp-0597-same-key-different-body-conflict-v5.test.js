...
const signedNonce = 'rpp0597acceptednonce';
const sessionId = 'psh_01j00000000000000000597';
const invalidSessionId = 'psh_01j00000000000090597';
const idempotencyKey = 'idem-rpp-0597-same-key-different-body-v5';
...
const wrongCredential = {
  ...
  password: 'rpp-0597-wrong-application-password',
};
const auth = {
  identity: {
    userId: 597,
...
function fixtureHash(label) {
  return sha256Hex(`rpp-0597:${label}`);
}
...
function checkedJournal({ idempotencyKeyHash, requestHash, conflictRequestHash }) {
  ...
  return {
    ...
    claim: {
      ...
      previousClaimId: 'psh_01j00000000000000000596',
...
function summarizeNegativeAuthEvidence(response) {
  ...
  return {
    schemaVersion: 1,
    slice: 'RPP-0597',
    proofClass: 'same-key-different-body-conflict-v5',
...
function summarizeReleaseVerifierConflictEvidence({
  runSummary,
  applyRequest,
  replayRequest,
  conflictRequest,
  ...
  const acceptedReplay = {
    idempotencyKeyHash: runSummary.replay.idempotency.idempotencyKeyHash,
    requestHash: applyRequest.headers['x-auth-content-hash'],
    replayRequestHash: replayRequest.headers['x-auth-content-hash'],
    canonicalBodyHash: digest(replayRequest.body),
    receiptHash: runSummary.replay.receiptHash,
    replayed: runSummary.replay.idempotency.replayed,
    freshMutationWork: runSummary.replay.idempotency.freshMutationWork,
    conflict: runSummary.replay.idempotency.conflict,
    hashOnly: true,
    rawKeyIncluded: false,
    rawBodyIncluded: false,
  };
  const conflict = {
    status: runSummary.idempotencyConflict.status,
    code: runSummary.idempotencyConflict.code,
    idempotencyKeyHash: runSummary.idempotencyConflict.idempotency.idempotencyKeyHash,
    requestHash: conflictRequest.headers['x-auth-content-hash'],
    canonicalBodyHash: digest(conflictRequest.body),
    conflictingRequestHash: conflictResponse.idempotency.conflictingRequestHash,
    freshMutationWork: runSummary.idempotencyConflict.idempotency.freshMutationWork,
    mutationEventCounts: cloneJson(conflictResponse.idempotency.mutationEventCounts),
    targetSnapshotUnchanged: runSummary.idempotencyConflict.targetSnapshotUnchanged,
    finalMatchesLocal: runSummary.idempotencyConflict.finalMatchesLocal,
    hashOnly: runSummary.idempotencyConflict.hashOnly,
    rawKeyIncluded: false,
    rawBodyIncluded: false,
  };
  const summary = {
    ...
    slice: 'RPP-0597',
    proofClass: 'same-key-different-body-conflict-v5',
    verifier: 'release-verifier-support',
    evidenceScope: 'local-executor-auth-support',
    releaseStatus: 'NO-GO',
    routeEvidence: {
      method: 'POST',
      ...
      proofHash: digest({
        method: 'POST',
        endpointPath: applyEndpointPath,
        sameBodyRequestHash: acceptedReplay.requestHash,
        conflictRequestHash: conflict.requestHash,
      }),
    },
    acceptedReplay,
    conflict,
    releaseMovement: {
      ...
      reasonHash: sha256Hex(
        'same-key same-body replay and different-body conflict support evidence cannot widen release readiness',
      ),
    },
    boundary: {
      firstRemainingProductionBoundaryHash: sha256Hex(
        'checked production release verifier executor-auth same-key different-body conflict proof',
      ),
      status: 'blocked',
...
test('RPP-0597 v5 route order rejects bad auth before JSON and conflict before mutation-capable work', () => {
...
test('RPP-0597 v5 proves same-key different-body conflict preserves hash-only evidence before fresh work', async () => {
  const originalFetch = global.fetch;
  const idempotencyKeyHash = sha256Hex(idempotencyKey);
  const receiptHash = fixtureHash('dry-run-receipt');
  const base = {
    files: {
      'wp-content/uploads/reprint-push/rpp-0597.txt': 'base-body',
    },
    plugins: {},
    db: {},
  };
  const local = {
    files: {
      'wp-content/uploads/reprint-push/rpp-0597.txt': 'local-body',
    },
    plugins: {},
    db: {},
  };
  ...
  try {
    ...
    const releaseVerifierEvidence = summarizeReleaseVerifierConflictEvidence({
      runSummary: summary,
      applyRequest,
      replayRequest,
      conflictRequest,
      conflictResponse: conflictResponseBody,
    });
    assert.equal(releaseVerifierEvidence.releaseStatus, 'NO-GO');
    assert.equal(releaseVerifierEvidence.releaseMovement.allowed, false);
    assert.equal(releaseVerifierEvidence.releaseMovement.gates, '0/4');
    assert.equal(releaseVerifierEvidence.boundary.status, 'blocked');
    assert.equal(releaseVerifierEvidence.acceptedReplay.replayed, true);
    assert.equal(releaseVerifierEvidence.acceptedReplay.freshMutationWork, false);
    assert.equal(releaseVerifierEvidence.acceptedReplay.conflict, false);
    assert.equal(releaseVerifierEvidence.acceptedReplay.hashOnly, true);
    assert.equal(releaseVerifierEvidence.acceptedReplay.rawKeyIncluded, false);
    assert.equal(releaseVerifierEvidence.acceptedReplay.rawBodyIncluded, false);
    assert.equal(releaseVerifierEvidence.conflict.freshMutationWork, false);
    assert.equal(releaseVerifierEvidence.conflict.hashOnly, true);
    assert.equal(releaseVerifierEvidence.conflict.rawKeyIncluded, false);
    assert.equal(releaseVerifierEvidence.conflict.rawBodyIncluded, false);
    assertHashOnlyFields(releaseVerifierEvidence, ['proofHash']);
    assertHashOnlyFields(releaseVerifierEvidence.routeEvidence, ['endpointPathHash', 'proofHash']);
    assertHashOnlyFields(releaseVerifierEvidence.acceptedReplay, [
      'idempotencyKeyHash',
      'requestHash',
      'replayRequestHash',
      'canonicalBodyHash',
    ...
    assertHashOnlyFields(releaseVerifierEvidence.conflict, [
      'idempotencyKeyHash',
      'requestHash',
      'canonicalBodyHash',
      'conflictingRequestHash',
    ]);
    assertHashOnlyFields(releaseVerifierEvidence.releaseMovement, ['reasonHash']);
    assertHashOnlyFields(releaseVerifierEvidence.boundary, ['firstRemainingProductionBoundaryHash']);

    assertNoRawValues(conflictResponseBody.idempotency, [
      idempotencyKey,
      credential.password,
      sessionId,
      base.files['wp-content/uploads/reprint-push/rpp-0597.txt'],
      local.files['wp-content/uploads/reprint-push/rpp-0597.txt'],
    ]);
    assertNoRawValues({
    ...
    assertNoRawValues(releaseVerifierEvidence, [
      sourceUrl,
      applyEndpointPath,
      idempotencyKey,
      credential.username,
      ...
      replayRequest.rawBody,
      conflictRequest.rawBody,
      base.files['wp-content/uploads/reprint-push/rpp-0597.txt'],
      local.files['wp-content/uploads/reprint-push/rpp-0597.txt'],
      'durableJournalBoundaryProbe',
    ]);
...
test('RPP-0597 v5 negative auth cases fail before JSON parsing or mutation-capable work', async () => {
