...
const evidencePath = path.join(
  repoRoot,
  'docs/evidence/rpp-0879-sandbox-8080-ingress-rule-proof-v4.md',
);
const fixedNow = '2026-06-01T00:00:00.000Z';
const rppId = 'RPP-0879';
const proofId = 'rpp-0879-sandbox-8080-ingress-rule-proof-v4';
const variantNumber = 4;
const variant = 'RPP-0879-variant-4';
...
const rpp0879Env = Object.freeze({
...
test('RPP-0879 support report records sandbox 8080 ingress variant 4 scope', () => {
  ...
  assert.deepEqual(report.writeScope.allowedFiles, [
    'docs/evidence/rpp-0879-sandbox-8080-ingress-rule-proof-v4.md',
    'test/rpp-0879-sandbox-8080-ingress-rule-proof-v4.test.js',
  ]);
  ...
  assert.deepEqual(report.builtOn.variant4ProductionTopologyPatterns, [
  ...
  assert.equal(report.builtOn.noTunnelPolicyPattern, 'RPP-0880-no-tunnel-policy-proof-v4');
  ...
  assert.equal(report.candidateScope.status, 'sandbox-8080-ingress-rule-candidate-v4');
  ...
  assert.equal(report.productionArtifactAudit.repositoryRpp0879ProductionArtifactPresent, false);
...
test('RPP-0879 topology command fails closed while preserving 8080-only ingress', () => {
...
test('RPP-0879 accepts survival only with production-backed import/export under sandbox 8080', () => {
...
test('RPP-0879 ingress validation rejects public, non-8080, and multiple HTTP surfaces', () => {
...
test('RPP-0879 evidence remains deterministic hash/count/surface-only', () => {
  const { report, text } = loadSupportReport();

  assert.deepEqual(report, buildSupportReport());
  assert.deepEqual(findEvidenceRedactionIssues(report), []);
  assert.doesNotThrow(() =>
    assertEvidenceHasNoRawValues(report, { label: 'RPP-0879 sandbox 8080 ingress rule proof v3' }));
...
function loadSupportReport() {
  ...
  assert.ok(match?.groups?.json, 'RPP-0879 evidence must contain one JSON support report block');
  return {
    text,
...
function buildSupportReport() {
  ...
  const variant4Distinctives = {
    closestTemplate: 'RPP-0859 variant 3',
    generatedCoverageVariant: 4,
    deterministicFixtureNamespace: variant,
    productionTopologyPatternInputs: [
      'RPP-0861-three-site-local-production-topology-v4',
      'RPP-0862-docker-wordpress-topology-v4',
      'RPP-0863-external-wordpress-topology-v4',
    ],
    noTunnelPatternInput: 'RPP-0880-no-tunnel-policy-proof-v4',
  ...
  const candidateScope = {
    status: 'sandbox-8080-ingress-rule-candidate-v4',
    supportOnly: true,
    failClosed: true,
    productionBacked: false,
    releaseEligible: false,
    releaseGateMovement: 'none',
    sourcePattern: 'rpp-0859-variant-3-template-plus-rpp-0879-variant-4-production-topology-patterns',
    topologyShape: {
    ...
    candidateClaims: [
      ...
      'variant-4-plugin-and-graph-survival-contract-defined',
  ...
  const productionArtifactAudit = {
    status: 'no-accepted-production-backed-rpp0879-artifact-found',
    acceptedProductionBackedArtifactPresent: false,
    repositoryRpp0879ProductionArtifactPresent: false,
    actualProductionImportExportReadbackPresent: false,
    claimAllowed: false,
    checkedEvidencePatterns: [
      'rpp-0859-sandbox-8080-ingress-rule-proof-v3',
      'rpp-0861-three-site-local-production-topology-v4',
      'rpp-0862-docker-wordpress-topology-v4',
      'rpp-0863-external-wordpress-topology-v4',
  ...
  const reportCore = {
    ...
    title: 'Sandbox 8080 ingress rule proof support scope variant 4',
    checkedAt: fixedNow,
    ...
    builtOn: {
      ...
      noTunnelPolicyPattern: 'RPP-0880-no-tunnel-policy-proof-v4',
    },
    variant4Distinctives,
    progressReport: {
...
function buildBasePlan(options = {}) {
  return buildDockerTopologyPlan({
    ...
    workDir: '/tmp/rpp-0879-docker-work',
    evidenceDir: '/tmp/rpp-0879-docker-evidence',
    env: rpp0879Env,
    ...options,
  });
}
...
function validateObservedImportExportSurvival(observed) {
  ...
  if (observed.runtime !== 'real-wordpress-import-export' || importExport.realWordPress !== true) {
    failures.push({
      ...
      reason: 'RPP-0879 requires real WordPress import/export, not local Playground or synthetic substitute evidence.',
    });
  }
  if (importExport.productionBacked !== true) {
    failures.push({
      code: 'PRODUCTION_BACKED_IMPORT_EXPORT_REQUIRED',
      reason: 'RPP-0879 release movement requires a production-backed import/export artifact.',
    });
  }
...
function scopeComparisonInput(report) {
  return {
    ...
    variant4Distinctives: report.variant4Distinctives,
