...
const evidencePath = path.join(
  repoRoot,
  'docs/evidence/rpp-0857-rest-route-matrix-proof-v3.md',
);
...
test('RPP-0857 records REST route matrix variant 3 support evidence without release movement', () => {
  ...
  const proofValidation = validateRestRouteMatrixV3Proof(report);

  assert.equal(proofValidation.ok, true, JSON.stringify(proofValidation.failures));
  assert.equal(report.schemaVersion, 1);
  assert.equal(report.rppId, 'RPP-0857');
  ...
  assert.equal(report.builtOn.routeMatrixContract.variantLineage, 'RPP-0837-variant-2');
...
test('RPP-0857 records exact unavailable topology capability and local-only no-fallback policy', () => {
...
test('RPP-0857 fail-closed policy rejects missing live receipts, packaged fallback, and ambiguous blockers', () => {
  const { report } = loadEvidenceReport();
  ...
  const readiness = evaluateRestRouteMatrixV3Readiness({
    proof: report,
    routeValidation,
    artifact,
  });
  ...
  const ambiguousValidation = validateRestRouteMatrixV3Proof(ambiguous);
  assert.equal(ambiguousValidation.ok, false);
  assert.ok(ambiguousValidation.failures.some((failure) =>
    failure.code === 'REST_ROUTE_MATRIX_V3_UNAVAILABLE_CAPABILITY_NOT_EXACT'));
  ...
  const readbackReadiness = evaluateRestRouteMatrixV3Readiness({
  ...
  const packagedFallbackReadiness = evaluateRestRouteMatrixV3Readiness({
...
test('RPP-0857 evidence remains hash/count/surface only', () => {
  const { report, text } = loadEvidenceReport();

  assert.deepEqual(findEvidenceRedactionIssues(report), []);
  assert.doesNotThrow(() =>
    assertEvidenceHasNoRawValues(report, { label: 'RPP-0857 REST route matrix proof v3' }));
...
function loadEvidenceReport() {
  ...
  assert.ok(match?.groups?.json, 'RPP-0857 evidence must contain one JSON progress report block');
  return {
    text,
...
function buildMissingDockerCapabilityArtifact() {
  const plan = buildDockerTopologyPlan({
    ...
    workDir: '/tmp/rpp-0857-docker-work',
    evidenceDir: '/tmp/rpp-0857-docker-evidence',
    env: {},
  });
...
function validateRestRouteMatrixV3Proof(proof) {
  const failures = [];
  if (proof.rppId !== 'RPP-0857' || proof.variant !== 3) {
    failures.push({ code: 'REST_ROUTE_MATRIX_V3_IDENTITY_MISMATCH' });
  }
  if (proof.routeMatrix.validationOk !== true
    ...
    failures.push({ code: 'REST_ROUTE_MATRIX_V3_ROUTE_CONTRACT_INVALID' });
  }
  if (!proof.topologyCommand.sitesStarted) {
    ...
    if (!capability?.code || !capability?.capability || !capability?.probeCommand) {
      failures.push({ code: 'REST_ROUTE_MATRIX_V3_UNAVAILABLE_CAPABILITY_NOT_EXACT' });
    }
    if (proof.failClosed !== true
      ...
      failures.push({ code: 'REST_ROUTE_MATRIX_V3_MUST_FAIL_CLOSED_WHEN_TOPOLOGY_NOT_STARTED' });
    }
    if (proof.routeMatrix.liveRestRouteReadbackObserved !== false
      ...
      failures.push({ code: 'REST_ROUTE_MATRIX_V3_LIVE_ROUTE_READBACK_CLAIMED_WITHOUT_TOPOLOGY' });
    }
  }
  if (proof.topologyCommand.packagedFallbackObserved !== false
    || proof.builtOn.topologyCommand.packagedFallbackAllowed !== false) {
    failures.push({ code: 'REST_ROUTE_MATRIX_V3_PACKAGED_FALLBACK_REJECTED' });
  }
  if (proof.localOnlyPolicy.onlySandbox8080Ingress !== true
    ...
    failures.push({ code: 'REST_ROUTE_MATRIX_V3_LOCAL_ONLY_POLICY_FAILED' });
  }
  if (proof.evidenceLimits.mode !== 'hash-count-surface-only'
    ...
    failures.push({ code: 'REST_ROUTE_MATRIX_V3_EVIDENCE_LIMITS_FAILED' });
  }
  if (proof.releaseReadyScope.finalReleaseStatus !== 'NO-GO'
    ...
    failures.push({ code: 'REST_ROUTE_MATRIX_V3_RELEASE_MOVEMENT_FORBIDDEN' });
  }
...
function evaluateRestRouteMatrixV3Readiness({ proof, routeValidation, artifact }) {
  const blockers = [];
