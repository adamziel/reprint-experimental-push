...
const evidencePath = path.join(repoRoot, 'docs/evidence/rpp-0945-objective-audit-update-v3.md');
const fixedNowIso = '2026-06-01T02:55:00.000Z';
const auditedLaneHead = '7169b7e833f3b4877d8a7f6524170227a03f2666';
...
test('RPP-0945 evidence is support-only and keeps the final release verdict held', () => {
  const { report, text } = loadEvidenceReport();

  assert.match(text, /^# RPP-0945 objective audit update v3$/m);
  ...
  assert.equal(report.rppId, 'RPP-0945');
  assert.equal(report.proofId, 'rpp-0945-objective-audit-update-v3');
  ...
  assert.equal(report.auditedBranch, 'session/rpp-945');
  assert.equal(report.auditedLaneHeadBeforeEvidence, auditedLaneHead);
});

test('RPP-0945 final go/no-go record names every current final-release risk and closes none', () => {
...
test('RPP-0945 carries forward R1-R16 objective risks without production-backed closure proof', () => {
  const { report } = loadEvidenceReport();
...
test('RPP-0945 release-gate snapshot matches the current held final-release evaluator', () => {
  const { report } = loadEvidenceReport();
...
test('RPP-0945 evidence remains redacted and records no release-gate movement', async () => {
  const { report, text } = loadEvidenceReport();
  const scan = await scanArtifacts([
    'docs/evidence/rpp-0945-objective-audit-update-v3.md',
  ], { cwd: repoRoot });

  assert.doesNotThrow(() =>
    assertEvidenceHasNoRawValues(report, { label: 'RPP-0945 objective audit update v3' }));
...
function loadEvidenceReport() {
  ...
  assert.ok(match?.groups?.json, 'RPP-0945 evidence must contain one JSON record block');
  return {
    text,
