...
const proofId = 'rpp-0749-chunk-replay-idempotency-v3';
const fixedNow = new Date('2026-05-31T00:00:00.000Z');
const scope = {
  planId: 'plan-rpp-0749',
  resourceKey: 'file:wp-content/uploads/2026/05/rpp-0749-large.bin',
};
const expectedGateIds = Object.freeze([
  'built-on-rpp-0729-v2',
...
test('RPP-0749 variant 3 proves repeated guardedLarge chunk replay idempotency inside documented budgets', {
  concurrency: false,
}, () => {
  const report = runLargeReplayBenchmark();
  const proof = buildVariant3Proof(report);

  assert.equal(proof.rppId, 'RPP-0749');
  assert.equal(proof.proofId, proofId);
  assert.equal(proof.variant, 3);
  assert.equal(proof.status, 'passed');
  assert.equal(proof.builtOn.rppId, 'RPP-0729');
  assert.equal(proof.builtOn.proofId, 'rpp-0729-chunk-replay-idempotency-v2');
...
test('RPP-0749 variant 3 focused module keeps repeated replay idempotent and stale receipts blocked', () => {
  const entries = [
    manifestEntry(),
    manifestEntry({
      chunkIndex: 1,
  ...
  const repeatedReplays = [0, 1, 2].map(() => resolveChunkReplayAttempt({
    ...scope,
  ...
  assert.equal(evidence.attempts.attemptedReplays, 6);
  assert.equal(evidence.attempts.exactReceiptReplays, 6);
...
function runLargeReplayBenchmark() {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'reprint-rpp-0749-replay-v3-'));
  try {
...
function buildVariant3Proof(report) {
  const evidence = buildChunkReplayStoragePerformanceEvidence({ report });
  recordCorrectnessGates(evidence);
  const safeDecision = resolveChunkReplayStoragePerformanceProof(evidence);
  ...
  const proofGates = [
    ...
    proofGate('unsafe-replay-evidence-fails-closed',
  ...
  const publicProof = {
    ...
    redaction: {
      mode: 'hash-and-count-only-chunk-replay-storage-performance',
...
function buildChunkReplayStoragePerformanceEvidence({ report }) {
...
function resolveChunkReplayStoragePerformanceProof(evidence) {
...
function recomputeChunkReplayProofGates(evidence) {
  ...
  return [
    proofGate('built-on-rpp-0729-v2', evidence.builtOn?.rppId === 'RPP-0729'
...
function unsafeChunkReplayDecisions(evidence) {
  ...
  return {
    missingReceipt: resolveChunkReplayStoragePerformanceProof(missingReceipt),
    duplicateReplayWork: resolveChunkReplayStoragePerformanceProof(duplicateReplayWork),
    overBudget: resolveChunkReplayStoragePerformanceProof(overBudget),
    productionClaim: resolveChunkReplayStoragePerformanceProof(productionClaim),
    prematurePassStatus: resolveChunkReplayStoragePerformanceProof(prematurePassStatus),
  };
}
...
function rawReplayTokens(entries = []) {
  return [
    ...
    'wp-content/uploads/2026/05/rpp-0749-large.bin',
    'catalog-export.bin',
