...
const evidencePath = path.join(
  repoRoot,
  'docs/evidence/rpp-0355-nav-menu-item-fail-closed-reference-v3.md',
);
const fixedNow = new Date('2026-05-31T03:55:00.000Z');
const sha256Pattern = /^[a-f0-9]{64}$/;
const sha256EvidencePattern = /^sha256:[a-f0-9]{64}$/;

const menuItemId = 355;
const menuTermId = 9355;
const menuTermTaxonomyId = 9455;
...
const privateFixtures = Object.freeze([
  'Local Private RPP-0355 Menu Item',
  'local-private-rpp0355-menu-item',
  'local-private-rpp0355-menu-guid',
  'local-private-rpp0355-nav-menu-name',
  'local-private-rpp0355-nav-menu-slug',
  'local-private-rpp0355-nav-menu-description',
]);
...
function baseSite() {
  return {
    files: {
      'index.php': '<?php echo "base-private-rpp0355-index";',
    },
    plugins: {},
    db: {
      wp_posts: {
...
function navMenuItemFixture() {
  ...
  local.db.wp_posts[menuItemRowId] = {
    ID: menuItemId,
    post_title: 'Local Private RPP-0355 Menu Item',
    post_name: 'local-private-rpp0355-menu-item',
    post_status: 'publish',
    post_type: 'nav_menu_item',
    post_parent: 0,
    post_author: 0,
    guid: 'local-private-rpp0355-menu-guid',
  };
  ...
  local.db.wp_terms[termRowId] = {
    term_id: menuTermId,
    name: 'local-private-rpp0355-nav-menu-name',
    slug: 'local-private-rpp0355-nav-menu-slug',
    term_group: 0,
  };
  local.db.wp_term_taxonomy[taxonomyRowId] = {
    ...
    term_id: menuTermId,
    taxonomy: 'nav_menu',
    description: 'local-private-rpp0355-nav-menu-description',
    parent: 0,
    count: 1,
  };
...
function assertHashOnlyEvidence(evidence) {
  ...
  for (const privateFixture of privateFixtures) {
    assert.equal(serialized.includes(privateFixture), false, `RPP-0355 evidence leaked ${privateFixture}`);
  }
  for (const rawField of ['post_title', 'post_name', 'guid', 'meta_value', 'name', 'slug', 'description']) {
    assert.equal(serialized.includes(rawField), false, `RPP-0355 evidence exposed raw field ${rawField}`);
  }

  assert.doesNotThrow(() =>
    assertEvidenceHasNoRawValues(evidence, { label: 'RPP-0355 nav menu item v3 evidence' }));
}

test('RPP-0355 proves nav menu item graph references fail closed with generated hash-only evidence', () => {
  const { plan, evidence } = buildRpp0355Evidence();

  assert.equal(evidence.rpp, 'RPP-0355');
...
test('RPP-0355 evidence docs state remaining unmapped WordPress surfaces', () => {
  const evidenceDoc = fs.readFileSync(evidencePath, 'utf8');

  assert.match(evidenceDoc, /^# RPP-0355 nav menu item fail-closed reference v3 evidence$/m);
