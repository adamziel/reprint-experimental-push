...
test('keeps same-remote graph identity at the live release boundary while a ready delete plan preserves a matching independent edit and remote-only plugin drift', () => {
  const base = baseSite();
  base.files['about.php'] = '<?php echo "base about";';
  ...
  assert.equal(pluginFileDecision.decision, 'keep-remote');
  assertEveryMutationHasLiveRemotePrecondition(plan);
  assert.ok(plan.preconditions.some((entry) => entry.resourceKey === 'file:index.php'));
...
test('keeps remote-only plugin drift at the live release boundary while a ready delete plan preserves same-remote graph identity and refuses late drift', () => {
  const base = baseSite();
  base.files['about.php'] = '<?php echo "base about";';
  base.db.wp_posts['ID:8'] = {
    ID: 8,
    post_title: 'Base post 8',
    post_status: 'publish',
  };
  base.db.wp_options['option_name:forms_settings'] = {
    option_name: 'forms_settings',
  ...
  const deleteMutation = mutationFor(plan, 'file:index.php');
  const matchingEdit = decisionFor(plan, 'file:about.php');
  const matchingRow = decisionFor(plan, 'row:["wp_posts","ID:8"]');
  const driftedRemote = baseSite();
  driftedRemote.files['about.php'] = '<?php echo "shared about";';
  driftedRemote.db.wp_posts['ID:8'] = {
    ID: 8,
    post_title: 'Shared post 8',
    post_status: 'publish',
  };
  ...
  driftedRemote.plugins.forms.description = 'remote-only plugin drift';
  driftedRemote.files['wp-content/plugins/forms/forms.php'] = '<?php /* remote-only plugin drift */';
  driftedRemote.files['index.php'] = '<?php echo "late drift";';
  ...
  assert.equal(matchingEdit.change.remoteChange, 'update');
  assert.equal(matchingRow.decision, 'already-in-sync');
  assert.equal(matchingRow.change.localChange, 'update');
  assert.equal(matchingRow.change.remoteChange, 'update');
  assertEveryMutationHasLiveRemotePrecondition(plan);
  assert.ok(driftedError instanceof PushPlanError);
  assert.equal(driftedError.code, 'PRECONDITION_FAILED');
  assert.equal(driftedRemote.files['index.php'], '<?php echo "late drift";');
  assert.equal(driftedRemote.db.wp_posts['ID:8'].post_title, 'Shared post 8');
  assert.equal(driftedRemote.db.wp_options['option_name:forms_settings'].option_value.mode, 'shared');
  assert.equal(driftedRemote.plugins.forms.description, 'remote-only plugin drift');
  assert.equal(driftedRemote.files['wp-content/plugins/forms/forms.php'], '<?php /* remote-only plugin drift */');
});

test('refuses late drift at the live release boundary while preserving plugin-owned graph identity evidence', () => {
  const base = baseSite();
  base.files['about.php'] = '<?php echo "base about";';
  base.db.wp_posts['ID:8'] = {
    ID: 8,
  ...
  assert.equal(driftedRemote.db.wp_posts['ID:8'].post_title, 'Shared post 8');
  assert.equal(driftedRemote.db.wp_options['option_name:forms_settings'].option_value.mode, 'late drift');
  assert.equal(driftedRemote.db.wp_options['option_name:forms_settings'].__pluginOwner, 'forms');
  assert.equal(driftedRemote.plugins.forms.description, 'remote-only plugin drift');
  assert.equal(driftedRemote.files['wp-content/plugins/forms/forms.php'], '<?php /* remote-only plugin drift */');
});
...
test('keeps same-remote graph identity at the live release boundary while a delete and plugin-owned resource share live preconditions', () => {
  const base = baseSite();
  base.files['about.php'] = '<?php echo "base about";';
  base.db.wp_options['option_name:forms_settings'] = {
    option_name: 'forms_settings',
...
test('keeps same-remote graph identity at the live release boundary while a ready delete plan preserves a matching plugin-owned resource and remote-only plugin removals', () => {
  const base = baseSite();
  base.files['about.php'] = '<?php echo "base about";';
  base.db.wp_options['option_name:forms_settings'] = {
    option_name: 'forms_settings',
...
test('keeps remote-only plugin drift at the live release boundary while a ready delete plan preserves a matching independent file type swap', () => {
  const base = baseSite();
  base.files['about.php'] = '<?php echo "base about";';
  base.files['wp-content/uploads/cover'] = 'base cover bytes';
...
test('keeps remote-only plugin removals at the live release boundary while a ready delete plan preserves a matching independent file type swap', () => {
  const base = baseSite();
  base.files['about.php'] = '<?php echo "base about";';
  base.files['wp-content/uploads/cover'] = 'base cover bytes';
...
test('keeps remote-only plugin removals at the live release boundary while a ready delete plan preserves same-remote graph identity and a matching independent file type swap', () => {
  const base = baseSite();
  base.files['about.php'] = '<?php echo "base about";';
  base.files['wp-content/uploads/cover'] = 'base cover bytes';
...
test('keeps late plugin-owned drift at the live release boundary while preserving a ready delete precondition and remote-only plugin drift', () => {
  const base = baseSite();
  base.files['about.php'] = '<?php echo "base about";';
  base.db.wp_options['option_name:forms_settings'] = {
    option_name: 'forms_settings',
