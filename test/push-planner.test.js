...
test('production durable journal claims fail closed when restart-readable remote artifacts are not fenced', () => {
  const writer = {
    kind: 'production-recovery-journal',
    productionAdapter: true,
    ownsJournal: true,
...
test('production durable journal claims fail closed when the remote artifact reuses the writer journal path', () => {
  const writer = {
    kind: 'production-recovery-journal',
    productionAdapter: true,
    ownsJournal: true,
...
test('production durable journal claims fail closed when restart inspection advertises a remote artifact reference but the writer omits it', () => {
  const writer = {
    kind: 'production-recovery-journal',
    productionAdapter: true,
    ownsJournal: true,
...
test('production durable journal claims fail closed when the writer advertises a remote artifact reference with traversal segments', () => {
  const writer = {
    ...
    artifactRefs: {
      journal: '/var/lib/reprint/recovery.jsonl',
      remote: '/var/lib/reprint/../reprint/remote-a.jsonl',
    },
    schemaVersion: RECOVERY_JOURNAL_SCHEMA_VERSION,
    nextSequence: 1,
    appendEvent(type, payload) {
...
test('production durable journal claims fail closed when remote artifact references include query or fragment suffixes', () => {
  const writer = {
    ...
    artifactRefs: {
      journal: '/var/lib/reprint/recovery.jsonl',
      remote: '/var/lib/reprint/remote-a.jsonl?rev=1',
    },
    schemaVersion: RECOVERY_JOURNAL_SCHEMA_VERSION,
    nextSequence: 1,
    appendEvent(type, payload) {
    ...
    inspect() {
      return {
        ...
        artifactRefs: {
          journal: '/var/lib/reprint/recovery.jsonl',
          remote: '/var/lib/reprint/remote-a.jsonl#current',
        },
        records: [{ sequence: 1, type: 'journal-opened' }],
      };
    },
...
test('production durable journal claims fail closed when artifact paths include query or fragment suffixes', () => {
  const writer = {
    kind: 'production-recovery-journal',
    productionAdapter: true,
    ownsJournal: true,
    journalPath: '/var/lib/reprint/recovery.jsonl?rev=1',
    artifactRefs: {
      journal: '/var/lib/reprint/recovery.jsonl#current',
    },
    schemaVersion: RECOVERY_JOURNAL_SCHEMA_VERSION,
    nextSequence: 1,
    appendEvent(type, payload) {
    ...
    inspect() {
      return {
        filePath: '/var/lib/reprint/recovery.jsonl?rev=1',
        schemaVersion: RECOVERY_JOURNAL_SCHEMA_VERSION,
        artifactRefs: {
          journal: '/var/lib/reprint/recovery.jsonl#current',
        },
