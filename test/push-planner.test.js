...
test('proves wp_postmeta plugin driver semantics with local hash-redacted evidence', () => {
  const rowId = 'meta_id:42';
  const resourceKey = `row:["wp_postmeta","${rowId}"]`;
  const resource = { type: 'row', table: 'wp_postmeta', id: rowId, key: resourceKey };
  const localSecret = 'local-private-rpp-0425-postmeta';
  const baseSecret = 'base-private-rpp-0425-postmeta';

  function siteWithPostmeta(metaValue = baseSecret) {
    const site = baseSite();
    site.db.wp_postmeta = {
      [rowId]: {
        meta_id: 42,
  ...
  function localWithDriver(driver) {
    const local = siteWithPostmeta(localSecret);
    local.pushIntents = [
      {
        id: 'update-forms-postmeta',
        kind: 'plugin-data-update',
  ...
  const base = siteWithPostmeta(baseSecret);
  const remote = siteWithPostmeta(baseSecret);
  const localWithoutPolicy = siteWithPostmeta(localSecret);
  ...
  const localCanonical = localWithDriver('wp-postmeta');
  const canonicalPlan = planFor(base, localCanonical, remote);
  const localAlias = localWithDriver('wp-post-meta');
  ...
  assert.equal(wrongDriverPlan.blockers[0].resource.table, 'wp_postmeta');
  ...
  assert.equal(canonicalPlan.atomicGroups[0].id, 'update-forms-postmeta');
  ...
  assert.equal(canonicalMutation.resource.table, 'wp_postmeta');
  assert.equal(canonicalMutation.action, 'put');
  assert.equal(canonicalMutation.changeKind, 'update');
  assert.equal(canonicalMutation.atomicGroupId, 'update-forms-postmeta');
  ...
  assert.deepEqual(
    ...
    },
    {
      pluginOwner: 'forms',
      driver: 'wp-postmeta',
      policySource: 'push-intent:update-forms-postmeta',
  ...
  assert.equal(mutationFor(aliasPlan, resourceKey).pluginOwnedResource.driver, 'wp-post-meta');
  assert.equal(mutationFor(aliasPlan, resourceKey).resource.table, 'wp_postmeta');

  const applied = applyPlan(cloneJson(remote), canonicalPlan);
  assert.deepEqual(
    applied.site.db.wp_postmeta[rowId].meta_value,
  ...
  const tamperedRemote = siteWithPostmeta(baseSecret);
  ...
  assert.equal(evidence.accepted.table, 'wp_postmeta');
  assert.equal(evidence.accepted.canonicalDriver, 'wp-postmeta');
  assert.equal(evidence.accepted.aliasDriver, 'wp-post-meta');
