...
test('RPP-0446 generated wp_termmeta driver semantics are exact and redacted', () => {
  const rowId = 'meta_id:17';
  const resourceKey = `row:["wp_termmeta","${rowId}"]`;
  const privateValues = [
    'rpp-0446-private-base-termmeta',
    'rpp-0446-private-local-termmeta',
    'rpp-0446-private-missing-policy-termmeta',
    'rpp-0446-private-wrong-driver-termmeta',
    'rpp-0446-private-near-miss-table-termmeta',
    'rpp-0446-private-wrong-policy-table-termmeta',
  ];

  function termmetaSite(table, rowValue) {
    const site = baseSite();
    site.db.wp_terms = {
      'term_id:1': {
        term_id: 1,
  ...
  function planWithPolicy({
    driver,
    resourceTable = 'wp_termmeta',
    policyTable,
    baseSecret,
    localSecret,
  }) {
    const key = `row:["${resourceTable}","${rowId}"]`;
    const base = termmetaSite(resourceTable, {
      state: 'base',
      private_note: baseSecret,
    });
    const local = termmetaSite(resourceTable, {
      state: 'local',
      private_note: localSecret,
    });
    local.pushIntents = [
      {
        id: `update-forms-termmeta-${driver}-${resourceTable}`,
        kind: 'plugin-data-update',
        requireAtomic: true,
        resources: [key],
        resourcePolicy: pluginOwnedResourcePolicy(
    ...
    const remote = termmetaSite(resourceTable, {
      state: 'base',
      private_note: baseSecret,
    });
    return {
  ...
  const acceptedEvidence = [
    ...
    {
      label: 'hyphenated-alias-with-exact-table',
      driver: 'wp-term-meta',
      policyTable: 'wp_termmeta',
    },
  ].map((variant, index) => {
    ...
    assert.equal(result.appliedMutations, 1);
    assert.deepEqual(result.site.db.wp_termmeta[rowId].meta_value, proof.local.db.wp_termmeta[rowId].meta_value);
    ...
    return {
      ...
      baseRowHash: sha256Evidence(proof.base.db.wp_termmeta[rowId]),
      localRowHash: sha256Evidence(proof.local.db.wp_termmeta[rowId]),
      remoteAfterRowHash: sha256Evidence(result.site.db.wp_termmeta[rowId]),
  ...
  const missingPolicy = (() => {
    const base = termmetaSite('wp_termmeta', {
      state: 'base',
      private_note: privateValues[2],
    });
    const local = termmetaSite('wp_termmeta', {
      state: 'local',
      private_note: privateValues[2],
    });
  ...
  const refusalEvidence = [
    ...
    },
    {
      ...
      driver: 'wp-termmeta',
      resourceTable: 'wp_termmetas',
      policyTable: 'wp_termmetas',
