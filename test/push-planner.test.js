...
function rpp0212RemoteBeforeHashFixture() {
  const pluginOptionResourceKey = 'row:["wp_options","option_name:forms_settings"]';
  const base = cloneJson(baseSite());
  const local = cloneJson(base);
  const remote = cloneJson(base);
  const privateValues = [
    '<?php echo "rpp0212-local-private-file";',
    'rpp0212-local-private-row-title',
    'rpp0212-local-private-option-mode',
    'rpp0212-invalid-raw-remote-before-hash-secret',
    '<?php echo "rpp0212-stale-live-remote-file";',
  ];
...
function mergeInvariantHashOnlyPlanEvidence(plan) {
  return {
    status: plan.status,
    summary: plan.summary,
    mutations: plan.mutations.map((mutation) => ({
...
test('RPP-0221 independent local file plus remote row edit stays hash-only and unplanned-safe', () => {
  ...
  const local = cloneJson(base);
  const remote = cloneJson(base);
  const privateLocalFile = 'local-private-rpp0221-file-payload';
  const privateRemoteTitle = 'remote-private-rpp0221-row-title';
  const fileKey = 'file:wp-content/themes/theme/style.css';
  const rowKey = 'row:["wp_posts","ID:1"]';
  local.files['wp-content/themes/theme/style.css'] = privateLocalFile;
  remote.db.wp_posts['ID:1'].post_title = privateRemoteTitle;
  ...
  assert.equal(firstPlan.status, 'ready');
  assertPlannerSummaryMatchesEvidence(firstPlan, 'RPP-0221 independent file/row invariant');
  assert.deepEqual(firstPlan.summary, {
    mutations: 1,
    decisions: 1,
    conflicts: 0,
  ...
  assert.equal(rowPrecondition, undefined);
  assert.equal(firstPlan.mutations.some((mutation) => mutation.resourceKey === rowKey), false);
  assert.equal(filePrecondition?.mutationId, fileMutation.id);
  assert.equal(filePrecondition.expectedHash, fileMutation.remoteBeforeHash);
  assertEveryMutationHasLiveRemotePrecondition(firstPlan);
  assertHashOnlyEvidenceRedacted(planEvidence, [privateLocalFile, privateRemoteTitle]);
  assertHashOnlyEvidenceRedacted(durableJournal.events, [privateLocalFile, privateRemoteTitle]);
  assert.deepEqual(
    durableJournal.events
      .filter((event) => ['target-planned', 'mutation-observed'].includes(event.type))
      .map((event) => [event.type, event.resourceKey]),
    [
      ['target-planned', fileKey],
      ['mutation-observed', fileKey],
    ],
  );
  assert.equal(result.appliedMutations, 1);
  assert.equal(result.site.files['wp-content/themes/theme/style.css'], privateLocalFile);
  assert.equal(result.site.db.wp_posts['ID:1'].post_title, privateRemoteTitle);
});
...
test('RPP-0222 independent local row plus remote file edit stays hash-only and unplanned-safe', () => {
  ...
  assertHashOnlyEvidenceRedacted(staleError.details, [
    privateLocalTitle,
    privateRemoteFile,
    'stale-private-rpp0222-row-title',
  ]);
});
...
test('RPP-0223 local delete versus remote edit refuses before mutation with redacted evidence', () => {
  ...
  const rowPrecondition = firstPlan.preconditions.find((entry) => entry.resourceKey === rowKey);
  const evidence = mergeInvariantHashOnlyPlanEvidence(firstPlan);
  const durableJournal = failingDurableJournal();
  ...
  assert.deepEqual(evidence, mergeInvariantHashOnlyPlanEvidence(secondPlan));
  ...
  assert.deepEqual(durableJournal.events, []);
  assertHashOnlyEvidenceRedacted(evidence, [privateLocalFile, privateRemoteTitle]);
  assertHashOnlyEvidenceRedacted(conflict, [privateRemoteTitle]);
  assertHashOnlyEvidenceRedacted(error.details, [privateLocalFile, privateRemoteTitle]);
  assert.equal(remote.db.wp_posts['ID:1'].post_title, privateRemoteTitle);
...
test('RPP-0224 local directory delete versus remote descendant create refuses before mutation with hash-only evidence', () => {
  const base = baseSite();
  const directoryPath = 'wp-content/uploads/gallery-rpp0224-v2';
  const descendantPath = `${directoryPath}/remote-created-private.txt`;
  const independentPath = 'index.php';
  const directoryKey = `file:${directoryPath}`;
  const descendantKey = `file:${descendantPath}`;
  const independentKey = `file:${independentPath}`;
  const privateRemoteDescendant = 'remote-private-rpp0224-v2-descendant-bytes';
  const privateLocalFile = '<?php echo "local-private-rpp0224-v2-independent";';
  base.files[directoryPath] = { type: 'directory' };
  const local = cloneJson(base);
  const remote = cloneJson(base);
  delete local.files[directoryPath];
  local.files[independentPath] = privateLocalFile;
  remote.files[descendantPath] = privateRemoteDescendant;
  ...
  assert.deepEqual(
    planEvidence,
    mergeInvariantHashOnlyPlanEvidence(secondPlan),
    'RPP-0224 hash-only proof evidence changed between deterministic planning runs',
  );
  assert.equal(mutationFor(firstPlan, directoryKey), undefined, 'unsafe directory delete should not emit mutation');
  assert.equal(
    firstPlan.preconditions.some((precondition) => precondition.resourceKey === directoryKey),
    false,
    'unsafe directory delete should not emit precondition',
  );
  ...
  assertHashOnlyEvidenceRedacted(planEvidence, [privateLocalFile, privateRemoteDescendant]);
  assertHashOnlyEvidenceRedacted(conflict, [privateRemoteDescendant]);
  assertHashOnlyEvidenceRedacted(descendantDecision, [privateRemoteDescendant]);
  assertHashOnlyEvidenceRedacted(error.details, [privateLocalFile, privateRemoteDescendant]);
  assertHashOnlyEvidenceRedacted(durableJournal.events, [privateLocalFile, privateRemoteDescendant]);
});
...
test('explicit WordPress graph identity-map contract carries accepted proof through decisions and rewrites', () => {
  ...
  local.db.wp_posts['ID:2201'] = {
    ID: 2201,
    post_title: 'graph-contract-private-title',
    post_name: 'graph-contract-post',
    post_status: 'publish',
    post_type: 'page',
    post_parent: 0,
    post_author: 0,
  ...
  local.db.wp_posts['ID:2202'] = {
    ID: 2202,
    post_title: 'graph-contract-child-private-title',
    post_name: 'graph-contract-child',
    post_status: 'publish',
    post_type: 'page',
    post_parent: 2201,
    post_author: 0,
  ...
  const evidenceJson = JSON.stringify({
    sourceDecision,
    rewrite,
    graphIdentity: childMutation.wordpressGraphIdentity,
  });
  ...
  assert.equal(contractEvidence.operation, 'wordpress-graph-identity-map-contract-validation');
  ...
  assert.equal(contractEvidence.sourceResourceKey, sourcePostResourceKey);
  assert.equal(contractEvidence.targetResourceKey, targetPostResourceKey);
  ...
  assert.equal(evidenceJson.includes('graph-contract-child-private-title'), false);

  const forgedRewrite = mutationFor(forgedPlan, childPostResourceKey)
    .wordpressGraphIdentity.rewrites[0];
  forgedRewrite.relationshipContractHash = '0'.repeat(64);
  const forgedError = captureError(() => applyPlan(forgedRemote, forgedPlan));
  const forgedIssue = forgedError.details.issues.find((issue) =>
    issue.code === 'WORDPRESS_GRAPH_REWRITE_CONTRACT_HASH_MISMATCH');
...
test('unsupported explicit WordPress graph identity-map contract version fails closed before rewrite', () => {
  ...
  local.db.wp_posts['ID:2201'] = {
    ID: 2201,
    post_title: 'graph-contract-invalid-private-title',
    post_name: 'graph-contract-invalid',
    post_status: 'publish',
    post_type: 'page',
    post_parent: 0,
    post_author: 0,
  ...
  local.db.wp_posts['ID:2202'] = {
    ID: 2202,
    post_title: 'graph-contract-invalid-child-private-title',
    post_name: 'graph-contract-invalid-child',
    post_status: 'publish',
    post_type: 'page',
    post_parent: 2201,
    post_author: 0,
  ...
  assert.equal(sourceBlocker.reasonCode, 'WORDPRESS_GRAPH_IDENTITY_MAP_CONTRACT_UNSUPPORTED_VERSION');
  assert.match(sourceBlocker.reason, /invalid explicit graph identity-map contract/);
  assert.equal(evidence.outcome, 'refused-before-mutation');
  assert.equal(evidence.reasonCode, 'WORDPRESS_GRAPH_IDENTITY_MAP_CONTRACT_UNSUPPORTED_VERSION');
  ...
  assert.equal(planJson.includes('graph-contract-invalid-private-title'), false);
  assert.equal(planJson.includes('graph-contract-invalid-child-private-title'), false);
  assert.throws(() => applyPlan(remote, plan), /Refusing to apply a blocked plan/);
  assert.equal(JSON.stringify(remote), remoteBefore);
});
...
test('RPP-0302 blocks featured image attachment references when the target is not an attachment row', () => {
  ...
  base.db.wp_posts['ID:4'] = {
    ID: 4,
    post_title: 'base-private-featured-page-target',
    post_content: 'base-private-featured-page-body',
    post_status: 'publish',
    post_type: 'page',
    post_parent: 0,
  };
  ...
  local.db.wp_postmeta['meta_id:46'] = {
    ...
    meta_key: '_thumbnail_id',
    meta_value: '4',
    private_payload: 'local-private-featured-page-meta-payload',
  };
  ...
  assert.match(reference.targetSupport.reason, /not a supported attachment row/);
  assert.equal(reference.targetChange.localChange, 'unchanged');
  ...
  assertHashOnlyEvidenceRedacted({ blocker }, [
    'base-private-featured-page-target',
    'base-private-featured-page-body',
    'local-private-featured-page-meta-payload',
  ]);
  assert.throws(() => applyPlan(remote, plan), /Refusing to apply/);
});

test('RPP-0302 blocks featured image references when the attachment target uses an unsupported graph surface', () => {
  const resourceKey = 'row:["wp_postmeta","meta_id:45"]';
  const targetResourceKey = 'row:["wp_posts","ID:2"]';
  const base = baseSite();
  base.db.wp_postmeta = {};
  ...
  assert.equal(blocker.resolutionPolicy, 'preserve-remote-wordpress-graph-and-stop');
  assert.equal(targetBlocker.resolutionPolicy, 'preserve-remote-wordpress-graph-and-stop');
  assert.match(blocker.baseHash, /^[a-f0-9]{64}$/);
  assert.match(blocker.localHash, /^[a-f0-9]{64}$/);
  assert.match(blocker.remoteHash, /^[a-f0-9]{64}$/);
  assert.match(targetBlocker.baseHash, /^[a-f0-9]{64}$/);
  assert.match(targetBlocker.localHash, /^[a-f0-9]{64}$/);
  assert.match(targetBlocker.remoteHash, /^[a-f0-9]{64}$/);
  assert.match(blocker.change.local.hash, /^[a-f0-9]{64}$/);
  assert.match(targetBlocker.change.local.hash, /^[a-f0-9]{64}$/);
  assert.equal(Object.hasOwn(blocker.change.local, 'value'), false);
  assert.equal(Object.hasOwn(targetBlocker.change.local, 'value'), false);
  assert.equal(reference.relationshipKey, 'wp_postmeta.meta_value');
  assert.equal(reference.relationshipType, 'featured-image-attachment');
  assert.equal(reference.targetResourceKey, targetResourceKey);
  assert.equal(reference.targetSupport.supported, false);
  ...
  assert.equal(reference.targetChange.remoteChange, 'unchanged');
  assert.equal(Object.hasOwn(reference.targetChange.local, 'value'), false);
  assertHashOnlyEvidenceRedacted(plan, [
    'local-private-menu-target-title',
    'local-private-menu-target-body',
    'local-private-thumbnail-meta-payload',
  ]);
...
test('RPP-0212 remoteBeforeHash matches live remote for mixed resource mutations', () => {
  const fixture = rpp0212RemoteBeforeHashFixture();
  ...
  for (const mutation of ready.mutations) {
    const precondition = ready.preconditions.find((entry) => entry.mutationId === mutation.id);
    const liveRemoteHash = resourceHash(fixture.remote, mutation.resource);

    assert.match(mutation.remoteBeforeHash, /^[a-f0-9]{64}$/);
    assert.equal(mutation.remoteBeforeHash, liveRemoteHash, `${mutation.resourceKey} mutation hash`);
    assert.equal(precondition.expectedHash, liveRemoteHash, `${mutation.resourceKey} precondition hash`);
    assert.equal(precondition.checkedAgainst, 'live-remote');
  }
});
...
test('RPP-0212 executor rejects missing wrong or stale remoteBeforeHash before mutation', () => {
  ...
  const targetPrecondition = (plan, mutation = targetMutation) => plan.preconditions.find(
    (precondition) => precondition.mutationId === mutation.id,
  );
  const durableEventsFor = (events) => ({
    claimFenced: true,
  ...
  const cases = [
    {
      ...
      plan: tamperReadyPlan(ready, (plan) => {
        delete mutationFor(plan, targetMutation.resourceKey).remoteBeforeHash;
      }),
      remote: () => cloneJson(fixture.remote),
    },
    {
      name: 'malformed remoteBeforeHash',
      code: 'PLAN_INVARIANT_VIOLATION',
      issueCode: 'REMOTE_BEFORE_HASH_INVALID',
      plan: tamperReadyPlan(ready, (plan) => {
        mutationFor(plan, targetMutation.resourceKey).remoteBeforeHash = fixture.privateValues[3];
      }),
      remote: () => cloneJson(fixture.remote),
    },
    {
      name: 'mismatched remoteBeforeHash precondition',
      code: 'PLAN_INVARIANT_VIOLATION',
      issueCode: 'PRECONDITION_HASH_MISMATCH',
      plan: tamperReadyPlan(ready, (plan) => {
        mutationFor(plan, targetMutation.resourceKey).remoteBeforeHash = '0'.repeat(64);
      }),
      remote: () => cloneJson(fixture.remote),
    },
    {
      name: 'matching wrong remoteBeforeHash',
      code: 'PRECONDITION_FAILED',
      expectedHash: '0'.repeat(64),
      plan: tamperReadyPlan(ready, (plan) => {
        const forgedMutation = mutationFor(plan, targetMutation.resourceKey);
        forgedMutation.remoteBeforeHash = '0'.repeat(64);
        targetPrecondition(plan, forgedMutation).expectedHash = forgedMutation.remoteBeforeHash;
      }),
      remote: () => cloneJson(fixture.remote),
    },
    {
      name: 'stale live remote',
      code: 'PRECONDITION_FAILED',
      expectedHash: targetMutation.remoteBeforeHash,
      plan: ready,
      remote: () => {
        const staleRemote = cloneJson(fixture.remote);
        staleRemote.files['index.php'] = fixture.privateValues[4];
        return staleRemote;
      },
    },
  ];
  ...
  for (const testCase of cases) {
    const applyRemote = testCase.remote();
    const beforeRemote = JSON.stringify(applyRemote);
    const beforeRemoteHash = digest(applyRemote);
    const journalEvents = [];
    const error = captureError(() => applyPlan(applyRemote, testCase.plan, {
    ...
    if (testCase.issueCode) {
      ...
      assert.equal(error.details.resourceKey, targetMutation.resourceKey, testCase.name);
      assert.equal(error.details.expectedHash, testCase.expectedHash);
    }
    ...
    assert.equal(
      ...
      0,
      `${testCase.name} wrote target or mutation journal evidence before refusal`,
    );
...
test('RPP-0238 forged ready plans reject before mutation with hash-only evidence', () => {
  const pluginOptionResourceKey = 'row:["wp_options","option_name:forms_settings"]';
  const base = cloneJson(baseSite());
  const local = cloneJson(base);
  const remote = cloneJson(base);
  const privateValues = [
    '<?php echo "local-private-rpp0238-file";',
    'rpp0238-local-private-row-title',
    'rpp0238-local-private-option-mode',
    '<?php echo "rpp0238-stale-live-remote-file";',
  ];
  local.files['index.php'] = privateValues[0];
  local.db.wp_posts['ID:1'].post_title = privateValues[1];
  local.db.wp_options['option_name:forms_settings'].option_value.mode = privateValues[2];
  ...
  const ready = planFor(base, local, remote);
  const targetMutation = mutationFor(ready, 'file:index.php');
  const targetPrecondition = ready.preconditions.find(
    (precondition) => precondition.mutationId === targetMutation.id,
  );
  const durableEventsFor = (events) => ({
    claimFenced: true,
  ...
  const hashOnlyEvidence = (name, plan, error, journalEvents, appliedMutationCount) => ({
    name,
    status: plan.status,
    summary: plan.summary,
    mutations: plan.mutations.map((mutation) => ({
    ...
    journalEventTypes: journalEvents.map((event) => event.type),
    durableJournalMutationEvents: journalEvents.filter((event) => event.type.includes('mutation')).length,
    durableJournalTargetEvents: journalEvents.filter((event) => event.type === 'target-planned').length,
    appliedMutationCount,
  });
  ...
  assertEveryMutationHasLiveRemotePrecondition(ready);
  assertPlannerSummaryMatchesEvidence(ready, 'RPP-0238 ready baseline');

  const forgedCases = [
    {
      name: 'missing live precondition',
      issueCode: 'MISSING_LIVE_REMOTE_PRECONDITION',
      forge(plan) {
        plan.preconditions = plan.preconditions.filter(
          (precondition) => precondition.mutationId !== targetMutation.id,
    ...
    },
    {
      ...
      issueCode: 'DUPLICATE_LIVE_REMOTE_PRECONDITION',
      forge(plan) {
        plan.preconditions.push(cloneJson(targetPrecondition));
      },
    },
    {
      name: 'precondition without mutation',
      issueCode: 'PRECONDITION_WITHOUT_MUTATION',
      forge(plan) {
        plan.mutations = plan.mutations.filter((mutation) => mutation.id !== targetMutation.id);
      },
    },
    {
  ...
  for (const testCase of forgedCases) {
    ...
    const serializedEvidence = JSON.stringify({
      ...hashOnlyEvidence(testCase.name, forged, error, journalEvents, appliedMutationCount),
      evidenceHash: sha256Evidence(hashOnlyEvidence(testCase.name, forged, error, journalEvents, appliedMutationCount)),
    });
    ...
    assert.equal(digest(applyRemote), beforeRemoteHash, `${testCase.name} mutated remote before refusal`);
    ...
    for (const privateValue of privateValues) {
      assert.equal(serializedEvidence.includes(privateValue), false, `${testCase.name} leaked ${privateValue}`);
      assert.equal(JSON.stringify(error.details).includes(privateValue), false, `${testCase.name} leaked refusal details`);
    }
  }
  ...
  const staleCases = [
    {
      ...
      remote() {
        const staleRemote = cloneJson(remote);
        staleRemote.files['index.php'] = privateValues[3];
        return staleRemote;
      },
    },
    {
      name: 'forged stale remoteBeforeHash',
      plan: tamperReadyPlan(ready, (plan) => {
        const forgedMutation = mutationFor(plan, targetMutation.resourceKey);
        const forgedPrecondition = plan.preconditions.find(
          (precondition) => precondition.mutationId === forgedMutation.id,
        );
        forgedMutation.remoteBeforeHash = '0'.repeat(64);
        forgedPrecondition.expectedHash = forgedMutation.remoteBeforeHash;
      }),
      remote() {
  ...
  for (const testCase of staleCases) {
    ...
    let appliedMutationCount = 0;
    const error = captureError(() => {
      const result = applyPlan(applyRemote, testCase.plan, {
        durableJournal: durableEventsFor(journalEvents),
      });
      appliedMutationCount = result.appliedMutations;
    });
    const serializedEvidence = JSON.stringify({
      ...hashOnlyEvidence(testCase.name, testCase.plan, error, journalEvents, appliedMutationCount),
      evidenceHash: sha256Evidence(hashOnlyEvidence(testCase.name, testCase.plan, error, journalEvents, appliedMutationCount)),
    });

    assert.ok(error instanceof PushPlanError, testCase.name);
    assert.equal(error.code, 'PRECONDITION_FAILED', testCase.name);
    assert.equal(digest(applyRemote), beforeRemoteHash, `${testCase.name} mutated remote before refusal`);
    assert.equal(appliedMutationCount, 0, `${testCase.name} reported applied mutations`);
    assert.equal(
      journalEvents.filter((event) => event.type.includes('mutation')).length,
      0,
      `${testCase.name} wrote durable mutation journal evidence`,
    );
    assert.equal(
      journalEvents.filter((event) => event.type === 'target-planned').length,
      0,
      `${testCase.name} wrote target-planned journal evidence before refusal`,
    );
    for (const privateValue of privateValues) {
      assert.equal(serializedEvidence.includes(privateValue), false, `${testCase.name} leaked ${privateValue}`);
      assert.equal(JSON.stringify(error.details).includes(privateValue), false, `${testCase.name} leaked refusal details`);
