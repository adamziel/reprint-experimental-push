...
test('blocks a local usermeta reference owned by a same-plan user', () => {
  const base = baseSite();
  const local = baseSite();
  const remote = baseSite();
  ...
  assert.equal(usermetaBlocker.surface, 'users');
  assert.match(usermetaBlocker.reason, /outside the supported release-candidate slice/);
  assert.equal(JSON.stringify(usermetaBlocker).includes('local-private-user-meta'), false);
  assert.equal(JSON.stringify(usermetaBlocker).includes('remote-attachment-body'), false);
});

test('blocks a local post author reference to a same-plan user', () => {
  const base = baseSite();
  const local = baseSite();
  const remote = baseSite();
...
test('blocks a local comment parent reference owned by a same-plan comment even when an unrelated remote attachment exists', () => {
  const base = baseSite();
  const local = baseSite();
  const remote = baseSite();
...
test('blocks a local wp_navigation parent reference to a same-plan post', () => {
  ...
  local.db.wp_posts['ID:16'] = {
    ID: 16,
    post_title: 'Local navigation parent target',
    post_content: 'local-private-navigation-parent-target-body',
    post_status: 'publish',
  };
  const remote = baseSite();
  ...
  assert.equal(blocker.reason.includes(navigationResourceKey), true);
  assert.equal(JSON.stringify(blocker).includes('local-private-navigation-parent-body'), false);
  assert.equal(JSON.stringify(blocker).includes('local-private-navigation-parent-target-body'), false);
  assert.throws(() => applyPlan(remote, plan), /Refusing to apply/);
});
...
test('blocks an existing postmeta row when the same-plan post target is itself blocked by a wp_navigation parent', () => {
  const blockedParentResourceKey = 'row:["wp_posts","ID:4"]';
  const blockedPostResourceKey = 'row:["wp_posts","ID:5"]';
...
test('blocks an existing postmeta row when the same-plan post target is itself blocked by a nav_menu_item parent', () => {
  const blockedParentResourceKey = 'row:["wp_posts","ID:4"]';
  const blockedPostResourceKey = 'row:["wp_posts","ID:5"]';
  const postmetaResourceKey = 'row:["wp_postmeta","meta_id:65"]';
  const base = baseSite();
  const local = baseSite();
  const remote = baseSite();
  ...
  remote.db.wp_postmeta = {
    'meta_id:65': {
      ...base.db.wp_postmeta['meta_id:65'],
    },
  };
  local.db.wp_posts['ID:4'] = {
    ID: 4,
...
test('blocks a local termmeta reference to a same-plan term when the same plan also creates a nav menu taxonomy for that term', () => {
  ...
  assert.equal(mutationFor(plan, categoryTaxonomyResourceKey).changeKind, 'create');
...
test('blocks a local postmeta reference owned by a revision even when it targets a same-plan wp_navigation post', () => {
...
test('allows local _menu_item_menu_item_parent metadata to reference a post created by the same plan', () => {
...
test('allows an existing _menu_item_object_id metadata row to retarget to a post created by the same plan', () => {
  const resourceKey = 'row:["wp_postmeta","meta_id:461"]';
  const targetResourceKey = 'row:["wp_posts","ID:2"]';
  const base = baseSite();
  const local = baseSite();
  ...
  remote.db.wp_postmeta = {
    'meta_id:461': {
      ...base.db.wp_postmeta['meta_id:461'],
    },
  };
  local.db.wp_posts['ID:2'] = {
    ID: 2,
...
test('allows an existing _menu_item_menu_item_parent metadata row to retarget to a post created by the same plan', () => {
  const resourceKey = 'row:["wp_postmeta","meta_id:460"]';
  const targetResourceKey = 'row:["wp_posts","ID:2"]';
  const base = baseSite();
  const local = baseSite();
...
test('allows local _menu_item_object_id metadata to reference a post created by the same plan even when a remote navigation post exists', () => {
  ...
  assert.equal(reference.relationshipType, 'menu-item-object-post');
  assert.equal(reference.targetResourceKey, targetResourceKey);
  assert.equal(JSON.stringify(reference).includes('local-private-menu-object-body'), false);
...
test('blocks _menu_item_object_id metadata owned by a wp_navigation post even when it targets a same-plan post', () => {
  const resourceKey = 'row:["wp_postmeta","meta_id:461"]';
...
test('blocks _menu_item_object_id taxonomy metadata owned by an attachment even when it targets a same-plan term', () => {
  ...
  assert.equal(plan.status, 'blocked');
  assert.equal(ownerMutation.changeKind, 'update');
  assert.equal(targetMutation.changeKind, 'create');
  assert.equal(mutationFor(plan, resourceKey), undefined);
  assert.ok(blocker);
  assert.equal(blocker.class, 'unsupported-wordpress-graph-surface');
...
test('blocks _menu_item_object_id taxonomy metadata owned by a revision even when it targets a same-plan term', () => {
  const resourceKey = 'row:["wp_postmeta","meta_id:462"]';
  const ownerResourceKey = 'row:["wp_posts","ID:1"]';
  const targetResourceKey = 'row:["wp_terms","term_id:7"]';
  const base = baseSite();
...
test('blocks _menu_item_object_id taxonomy metadata owned by a wp_navigation post even when it targets a same-plan term', () => {
  const resourceKey = 'row:["wp_postmeta","meta_id:462"]';
  const ownerResourceKey = 'row:["wp_posts","ID:1"]';
  const targetResourceKey = 'row:["wp_terms","term_id:7"]';
  const base = baseSite();
...
test('allows local _menu_item_object_id taxonomy metadata to reference a term created by the same plan even when an unrelated remote attachment exists', () => {
  const resourceKey = 'row:["wp_postmeta","meta_id:462"]';
  const targetResourceKey = 'row:["wp_terms","term_id:7"]';
  const base = baseSite();
  const local = baseSite();
...
test('allows local menu item parent metadata to reference a post created by the same plan even when an unrelated remote attachment exists', () => {
  const resourceKey = 'row:["wp_postmeta","meta_id:46"]';
  const targetResourceKey = 'row:["wp_posts","ID:2"]';
  const base = baseSite();
  const local = baseSite();
...
test('blocks _menu_item_menu_item_parent metadata from referencing a same-plan attachment', () => {
  const resourceKey = 'row:["wp_postmeta","meta_id:470"]';
  const attachmentResourceKey = 'row:["wp_posts","ID:2"]';
  const base = baseSite();
  const local = baseSite();
  local.db.wp_posts['ID:2'] = {
...
test('blocks _menu_item_object_id metadata from referencing a same-plan revision', () => {
  const resourceKey = 'row:["wp_postmeta","meta_id:472"]';
  const revisionResourceKey = 'row:["wp_posts","ID:2"]';
  const base = baseSite();
  const local = baseSite();
  local.db.wp_posts['ID:2'] = {
    ID: 2,
    post_title: 'Local menu object revision',
    post_content: 'local-private-menu-object-revision-body',
    post_status: 'inherit',
    post_type: 'revision',
  };
  local.db.wp_postmeta = {
  ...
  assert.equal(reference.targetResourceKey, revisionResourceKey);
  assert.equal(
    JSON.stringify(blocker).includes('local-private-menu-object-revision-body'),
    false,
  );
  assert.throws(() => applyPlan(baseSite(), plan), /Refusing to apply/);
});

test('blocks _menu_item_object_id metadata from referencing a same-plan wp_navigation post', () => {
  const resourceKey = 'row:["wp_postmeta","meta_id:473"]';
  const navigationResourceKey = 'row:["wp_posts","ID:2"]';
  const base = baseSite();
  const local = baseSite();
  local.db.wp_posts['ID:2'] = {
    ID: 2,
    post_title: 'Local menu object navigation target',
    post_content: 'local-private-menu-object-navigation-body',
    post_status: 'publish',
    post_type: 'wp_navigation',
  };
  local.db.wp_postmeta = {
  ...
  assert.equal(reference.targetResourceKey, navigationResourceKey);
  assert.equal(
    JSON.stringify(blocker).includes('local-private-menu-object-navigation-body'),
    false,
  );
  assert.throws(() => applyPlan(baseSite(), plan), /Refusing to apply/);
});
...
test('blocks _menu_item_object_id metadata when the same-plan post target is itself blocked by a wp_navigation parent', () => {
  const navigationResourceKey = 'row:["wp_posts","ID:2"]';
  const blockedTargetResourceKey = 'row:["wp_posts","ID:3"]';
  const postmetaResourceKey = 'row:["wp_postmeta","meta_id:4741"]';
  const base = baseSite();
  const local = baseSite();
  local.db.wp_posts['ID:2'] = {
    ID: 2,
  ...
  assert.equal(targetBlocker.class, 'missing-wordpress-graph-dependency');
  assert.equal(targetBlocker.references[0].relationshipType, 'post-parent');
  assert.equal(targetBlocker.references[0].targetResourceKey, navigationResourceKey);
  assert.equal(postmetaBlocker.class, 'missing-wordpress-graph-dependency');
  assert.equal(postmetaBlocker.references[0].relationshipType, 'menu-item-object-post');
  assert.equal(postmetaBlocker.references[0].targetResourceKey, blockedTargetResourceKey);
  assert.equal(postmetaMutation.dependsOnMutationIds, undefined);
...
test('blocks _menu_item_object_id metadata when the same-plan post target is itself blocked by a nav_menu_item parent', () => {
  const menuItemResourceKey = 'row:["wp_posts","ID:2"]';
  const blockedTargetResourceKey = 'row:["wp_posts","ID:3"]';
  const postmetaResourceKey = 'row:["wp_postmeta","meta_id:4742"]';
  const base = baseSite();
  const local = baseSite();
  local.db.wp_posts['ID:2'] = {
    ID: 2,
  ...
  assert.equal(targetBlocker.class, 'missing-wordpress-graph-dependency');
  assert.equal(targetBlocker.references[0].relationshipType, 'post-parent');
  assert.equal(targetBlocker.references[0].targetResourceKey, menuItemResourceKey);
  assert.equal(postmetaBlocker.class, 'missing-wordpress-graph-dependency');
  assert.equal(postmetaBlocker.references[0].relationshipType, 'menu-item-object-post');
  assert.equal(postmetaBlocker.references[0].targetResourceKey, blockedTargetResourceKey);
  assert.equal(postmetaMutation.dependsOnMutationIds, undefined);
...
test('blocks _menu_item_object_id taxonomy metadata when its same-plan term is claimed by a nav menu taxonomy and a sibling category taxonomy depends on the same blocked term', () => {
  const targetResourceKey = 'row:["wp_terms","term_id:7"]';
  const categoryTaxonomyResourceKey = 'row:["wp_term_taxonomy","term_taxonomy_id:9"]';
  const navMenuTaxonomyResourceKey = 'row:["wp_term_taxonomy","term_taxonomy_id:20"]';
  const postmetaResourceKey = 'row:["wp_postmeta","meta_id:478"]';
  const base = baseSite();
  const local = baseSite();
  local.db.wp_terms = {
    'term_id:7': {
...
test('blocks an existing _menu_item_object_id taxonomy row when its same-plan term is claimed by a nav menu taxonomy and a sibling category taxonomy depends on the same blocked term', () => {
  const targetResourceKey = 'row:["wp_terms","term_id:7"]';
  const categoryTaxonomyResourceKey = 'row:["wp_term_taxonomy","term_taxonomy_id:9"]';
  const navMenuTaxonomyResourceKey = 'row:["wp_term_taxonomy","term_taxonomy_id:20"]';
...
test('blocks _menu_item_object_id metadata owned by an attachment even when it targets a same-plan post', () => {
  const resourceKey = 'row:["wp_postmeta","meta_id:481"]';
  const attachmentResourceKey = 'row:["wp_posts","ID:2"]';
  const targetResourceKey = 'row:["wp_posts","ID:3"]';
  const base = baseSite();
  const local = baseSite();
...
test('blocks _menu_item_object_id metadata owned by an existing revision even when it targets a same-plan post', () => {
  const resourceKey = 'row:["wp_postmeta","meta_id:487"]';
  const targetResourceKey = 'row:["wp_posts","ID:3"]';
  const base = baseSite();
  base.db.wp_posts['ID:2'] = {
    ID: 2,
    post_title: 'Existing revision owner',
    post_content: 'base-private-existing-revision-owner-body',
    post_status: 'inherit',
    post_type: 'revision',
  };
  const local = baseSite();
  ...
  assert.equal(blocker.surface, 'revision');
  assert.equal(
    JSON.stringify(blocker).includes('base-private-existing-revision-owner-body'),
    false,
  );
  assert.equal(
    JSON.stringify(blocker).includes('local-private-menu-object-post-target-body'),
...
test('blocks _menu_item_object_id metadata owned by an existing wp_navigation post even when it targets a same-plan post', () => {
  const resourceKey = 'row:["wp_postmeta","meta_id:489"]';
  const targetResourceKey = 'row:["wp_posts","ID:3"]';
  const base = baseSite();
  base.db.wp_posts['ID:2'] = {
    ID: 2,
    post_title: 'Existing wp_navigation owner',
    post_content: 'base-private-existing-wp-navigation-owner-body',
    post_status: 'publish',
    post_type: 'wp_navigation',
  };
  const local = baseSite();
  ...
  assert.equal(blocker.surface, 'wp_navigation');
  assert.equal(
    JSON.stringify(blocker).includes('base-private-existing-wp-navigation-owner-body'),
    false,
  );
  assert.equal(
    JSON.stringify(blocker).includes('local-private-menu-object-post-target-body'),
...
test('blocks _menu_item_object_id metadata owned by an existing nav_menu_item post even when it targets a same-plan post', () => {
  const resourceKey = 'row:["wp_postmeta","meta_id:497"]';
  const targetResourceKey = 'row:["wp_posts","ID:3"]';
  const base = baseSite();
  base.db.wp_posts['ID:2'] = {
    ID: 2,
...
test('blocks an existing _menu_item_object_id row when the same-plan post target is itself blocked by a revision parent', () => {
  const blockedParentResourceKey = 'row:["wp_posts","ID:4"]';
  const blockedTargetResourceKey = 'row:["wp_posts","ID:5"]';
  const postmetaResourceKey = 'row:["wp_postmeta","meta_id:4971"]';
  const base = baseSite();
  const local = baseSite();
  const remote = baseSite();
  ...
  remote.db.wp_postmeta = {
    'meta_id:4971': {
      ...base.db.wp_postmeta['meta_id:4971'],
    },
  };
  local.db.wp_posts['ID:4'] = {
    ID: 4,
...
test('blocks an existing _menu_item_object_id row when the same-plan post target is itself blocked by a wp_navigation parent', () => {
  const blockedParentResourceKey = 'row:["wp_posts","ID:4"]';
  const blockedTargetResourceKey = 'row:["wp_posts","ID:5"]';
  const postmetaResourceKey = 'row:["wp_postmeta","meta_id:4972"]';
  const base = baseSite();
  const local = baseSite();
  const remote = baseSite();
  ...
  remote.db.wp_postmeta = {
    'meta_id:4972': {
      ...base.db.wp_postmeta['meta_id:4972'],
    },
  };
  local.db.wp_posts['ID:4'] = {
    ID: 4,
  ...
  assert.equal(postmetaMutation.changeKind, 'update');
  assert.equal(parentBlocker.class, 'unsupported-wordpress-graph-surface');
  assert.equal(parentBlocker.surface, 'wp_navigation');
  assert.equal(blockedPostBlocker.class, 'missing-wordpress-graph-dependency');
  assert.equal(blockedPostBlocker.references[0].relationshipType, 'post-parent');
  assert.equal(blockedPostBlocker.references[0].targetResourceKey, blockedParentResourceKey);
  assert.equal(postmetaBlocker.class, 'missing-wordpress-graph-dependency');
...
test('blocks an existing _menu_item_object_id row when the same-plan post target is itself blocked by a nav_menu_item parent', () => {
  const blockedParentResourceKey = 'row:["wp_posts","ID:4"]';
  const blockedTargetResourceKey = 'row:["wp_posts","ID:5"]';
  const postmetaResourceKey = 'row:["wp_postmeta","meta_id:4973"]';
  const base = baseSite();
  const local = baseSite();
  const remote = baseSite();
  ...
  remote.db.wp_postmeta = {
    'meta_id:4973': {
      ...base.db.wp_postmeta['meta_id:4973'],
    },
  };
  local.db.wp_posts['ID:4'] = {
    ID: 4,
  ...
  assert.equal(postmetaMutation.changeKind, 'update');
  assert.equal(parentBlocker.class, 'unsupported-wordpress-graph-surface');
  assert.equal(parentBlocker.surface, 'nav_menu_item');
  assert.equal(blockedPostBlocker.class, 'missing-wordpress-graph-dependency');
  assert.equal(blockedPostBlocker.references[0].relationshipType, 'post-parent');
  assert.equal(blockedPostBlocker.references[0].targetResourceKey, blockedParentResourceKey);
  assert.equal(postmetaBlocker.class, 'missing-wordpress-graph-dependency');
...
test('blocks _menu_item_object_id metadata owned by an existing wp_navigation post when the same-plan post target is itself blocked by a nav_menu_item parent', () => {
  const blockedParentResourceKey = 'row:["wp_posts","ID:4"]';
  const blockedTargetResourceKey = 'row:["wp_posts","ID:5"]';
  const postmetaResourceKey = 'row:["wp_postmeta","meta_id:4974"]';
  const base = baseSite();
  const local = baseSite();
  const remote = baseSite();
  ...
  remote.db.wp_postmeta = {
    'meta_id:4974': {
      ...base.db.wp_postmeta['meta_id:4974'],
    },
  };
  local.db.wp_posts['ID:4'] = {
    ID: 4,
...
test('blocks _menu_item_object_id metadata owned by an existing nav_menu_item post when the same-plan post target is itself blocked by a wp_navigation parent', () => {
  const blockedParentResourceKey = 'row:["wp_posts","ID:4"]';
  const blockedTargetResourceKey = 'row:["wp_posts","ID:5"]';
  const postmetaResourceKey = 'row:["wp_postmeta","meta_id:4975"]';
  const base = baseSite();
  const local = baseSite();
  const remote = baseSite();
  ...
  remote.db.wp_postmeta = {
    'meta_id:4975': {
      ...base.db.wp_postmeta['meta_id:4975'],
    },
  };
  local.db.wp_posts['ID:4'] = {
    ID: 4,
...
test('blocks _menu_item_object_id metadata owned by an existing wp_navigation post when the same-plan post target is itself blocked by a revision parent', () => {
  const blockedParentResourceKey = 'row:["wp_posts","ID:4"]';
  const blockedTargetResourceKey = 'row:["wp_posts","ID:5"]';
  const postmetaResourceKey = 'row:["wp_postmeta","meta_id:49741"]';
  const base = baseSite();
  const local = baseSite();
  const remote = baseSite();
  ...
  remote.db.wp_postmeta = {
    'meta_id:49741': {
      ...base.db.wp_postmeta['meta_id:49741'],
    },
  };
  local.db.wp_posts['ID:4'] = {
    ID: 4,
...
test('blocks _menu_item_object_id metadata owned by an existing nav_menu_item post when the same-plan post target is itself blocked by a revision parent', () => {
  const blockedParentResourceKey = 'row:["wp_posts","ID:4"]';
  const blockedTargetResourceKey = 'row:["wp_posts","ID:5"]';
  const postmetaResourceKey = 'row:["wp_postmeta","meta_id:49751"]';
  const base = baseSite();
  const local = baseSite();
  const remote = baseSite();
  ...
  remote.db.wp_postmeta = {
    'meta_id:49751': {
      ...base.db.wp_postmeta['meta_id:49751'],
    },
  };
  local.db.wp_posts['ID:4'] = {
    ID: 4,
...
test('blocks _menu_item_object_id metadata owned by an existing attachment even when it targets a same-plan attachment', () => {
  const resourceKey = 'row:["wp_postmeta","meta_id:4981"]';
  const targetResourceKey = 'row:["wp_posts","ID:3"]';
  const base = baseSite();
  base.db.wp_posts['ID:2'] = {
    ID: 2,
  ...
  local.db.wp_posts['ID:3'] = {
    ID: 3,
    post_title: 'Local menu object attachment target',
    post_content: 'local-private-menu-object-attachment-target-body',
    post_status: 'inherit',
    post_type: 'attachment',
  };
  local.db.wp_postmeta = {
  ...
  assert.equal(
    JSON.stringify(blocker).includes('local-private-menu-object-attachment-target-body'),
    false,
  );
});

test('blocks _menu_item_object_id metadata owned by an existing revision even when it targets a same-plan attachment', () => {
  const resourceKey = 'row:["wp_postmeta","meta_id:4982"]';
  const targetResourceKey = 'row:["wp_posts","ID:3"]';
  const base = baseSite();
  base.db.wp_posts['ID:2'] = {
    ID: 2,
...
test('blocks _menu_item_object_id metadata owned by an existing wp_navigation post even when it targets a same-plan attachment', () => {
  const resourceKey = 'row:["wp_postmeta","meta_id:4983"]';
  const targetResourceKey = 'row:["wp_posts","ID:3"]';
  const base = baseSite();
  base.db.wp_posts['ID:2'] = {
    ID: 2,
...
test('blocks _menu_item_object_id metadata owned by an existing nav_menu_item post even when it targets a same-plan attachment', () => {
  const resourceKey = 'row:["wp_postmeta","meta_id:4984"]';
  const targetResourceKey = 'row:["wp_posts","ID:3"]';
  const base = baseSite();
  base.db.wp_posts['ID:2'] = {
    ID: 2,
...
test('blocks _menu_item_object_id taxonomy metadata owned by an existing attachment even when it targets a same-plan term and unrelated remote attachment noise exists', () => {
  const resourceKey = 'row:["wp_postmeta","meta_id:494"]';
  const targetResourceKey = 'row:["wp_terms","term_id:7"]';
  const base = baseSite();
  base.db.wp_posts['ID:2'] = {
...
test('blocks _menu_item_object_id taxonomy metadata owned by an existing revision even when it targets a same-plan term and unrelated remote attachment noise exists', () => {
  const resourceKey = 'row:["wp_postmeta","meta_id:488"]';
  const targetResourceKey = 'row:["wp_terms","term_id:7"]';
  const base = baseSite();
  base.db.wp_posts['ID:2'] = {
...
test('blocks _menu_item_object_id taxonomy metadata owned by an existing wp_navigation post even when it targets a same-plan term', () => {
  const resourceKey = 'row:["wp_postmeta","meta_id:491"]';
  const targetResourceKey = 'row:["wp_terms","term_id:7"]';
  const base = baseSite();
  base.db.wp_posts['ID:2'] = {
    ID: 2,
...
test('blocks _menu_item_object_id taxonomy metadata owned by an existing wp_navigation post even when it targets a same-plan term and unrelated remote attachment noise exists', () => {
  const resourceKey = 'row:["wp_postmeta","meta_id:491"]';
  const targetResourceKey = 'row:["wp_terms","term_id:7"]';
  const base = baseSite();
  base.db.wp_posts['ID:2'] = {
...
test('blocks _menu_item_object_id taxonomy metadata owned by an existing nav_menu_item post even when it targets a same-plan term', () => {
...
test('blocks menu item parent metadata owned by a navigation post when it targets a same-plan post', () => {
  const resourceKey = 'row:["wp_postmeta","meta_id:51"]';
  const navigationResourceKey = 'row:["wp_posts","ID:2"]';
  const targetResourceKey = 'row:["wp_posts","ID:3"]';
  const base = baseSite();
  ...
  assert.equal(JSON.stringify(navigationBlocker).includes('local-private-navigation-owner-body'), false);
  assert.equal(JSON.stringify(navigationBlocker).includes('local-private-menu-parent-target-body'), false);
  assert.equal(postmetaBlocker.class, 'unsupported-wordpress-graph-surface');
  assert.equal(postmetaBlocker.surface, 'wp_navigation');
  ...
  assert.equal(
    JSON.stringify(postmetaBlocker).includes('local-private-navigation-owner-body'),
    false,
  );
  assert.equal(
    JSON.stringify(postmetaBlocker).includes('local-private-menu-parent-target-body'),
    false,
  );
});
...
test('blocks menu item parent metadata from referencing a same-plan revision even when an unrelated remote attachment exists', () => {
  const resourceKey = 'row:["wp_postmeta","meta_id:57"]';
  const revisionResourceKey = 'row:["wp_posts","ID:5"]';
  const base = baseSite();
  const local = baseSite();
  ...
  assert.equal(blocker.references[0].targetResourceKey, revisionResourceKey);
  assert.equal(
    JSON.stringify(blocker).includes('remote-attachment-body'),
    false,
  );
  assert.equal(
    JSON.stringify(blocker).includes('local-private-revision-target-body'),
...
test('blocks menu item parent metadata when the same-plan post target is itself blocked by a wp_navigation parent', () => {
...
test('blocks menu item parent metadata when the same-plan post target is itself blocked by a nav_menu_item parent', () => {
  ...
  assert.equal(blockedTargetBlocker.class, 'missing-wordpress-graph-dependency');
  assert.equal(blockedTargetBlocker.references[0].relationshipType, 'post-parent');
  assert.equal(blockedTargetBlocker.references[0].targetResourceKey, menuItemResourceKey);
  assert.equal(postmetaBlocker.class, 'missing-wordpress-graph-dependency');
  assert.equal(postmetaBlocker.references[0].relationshipType, 'menu-item-parent-post');
  assert.equal(postmetaBlocker.references[0].targetResourceKey, blockedTargetResourceKey);
  assert.equal(postmetaMutation.dependsOnMutationIds, undefined);
...
test('blocks _menu_item_menu_item_parent metadata when the same-plan post target is itself blocked by a revision parent', () => {
  const revisionResourceKey = 'row:["wp_posts","ID:4"]';
  const blockedTargetResourceKey = 'row:["wp_posts","ID:5"]';
  const postmetaResourceKey = 'row:["wp_postmeta","meta_id:508"]';
  const base = baseSite();
  const local = baseSite();
  local.db.wp_posts['ID:4'] = {
    ID: 4,
...
test('blocks _menu_item_menu_item_parent metadata when the same-plan post target is itself blocked by a wp_navigation parent', () => {
  const navigationResourceKey = 'row:["wp_posts","ID:4"]';
  const blockedTargetResourceKey = 'row:["wp_posts","ID:5"]';
  const postmetaResourceKey = 'row:["wp_postmeta","meta_id:508"]';
  const base = baseSite();
...
test('blocks _menu_item_menu_item_parent metadata when the same-plan post target is itself blocked by a nav_menu_item parent', () => {
  const menuItemResourceKey = 'row:["wp_posts","ID:4"]';
  const blockedTargetResourceKey = 'row:["wp_posts","ID:5"]';
  const postmetaResourceKey = 'row:["wp_postmeta","meta_id:508"]';
  const base = baseSite();
...
test('allows a local post to reference a parent post created by the same plan even when an unrelated remote attachment exists', () => {
  const parentResourceKey = 'row:["wp_posts","ID:2"]';
  const childResourceKey = 'row:["wp_posts","ID:3"]';
  const base = baseSite();
  const local = baseSite();
...
test('allows a local attachment parent reference to a same-plan attachment when no remote navigation post exists', () => {
  const attachmentResourceKey = 'row:["wp_posts","ID:2"]';
  const childResourceKey = 'row:["wp_posts","ID:3"]';
  const base = baseSite();
  const local = baseSite();
  ...
  const reference = mutationFor(plan, childResourceKey).wordpressGraphReferences[0];
  ...
  assert.equal(JSON.stringify(reference).includes('local-private-attachment-child-body'), false);
  const result = applyPlan(baseSite(), plan);
  assert.equal(result.site.db.wp_posts['ID:2'].post_title, 'Local attachment parent');
  assert.equal(result.site.db.wp_posts['ID:3'].post_parent, 2);
});
...
test('allows a local thumbnail reference to an attachment created by the same plan even when a remote attachment exists', () => {
  const attachmentResourceKey = 'row:["wp_posts","ID:2"]';
...
test('allows a local thumbnail reference owned by a same-plan post to an attachment created by the same plan even when an unrelated remote attachment exists', () => {
  const postResourceKey = 'row:["wp_posts","ID:3"]';
  const attachmentResourceKey = 'row:["wp_posts","ID:2"]';
  const postmetaResourceKey = 'row:["wp_postmeta","meta_id:45"]';
  const base = baseSite();
...
test('blocks a local thumbnail reference owned by an existing nav_menu_item post even when it targets a same-plan post', () => {
...
test('blocks a local page parent reference to a same-plan attachment even when an unrelated remote attachment exists', () => {
  const attachmentResourceKey = 'row:["wp_posts","ID:2"]';
  const postResourceKey = 'row:["wp_posts","ID:3"]';
  const base = baseSite();
  const local = baseSite();
...
test('allows a local thumbnail reference from a page to a same-plan attachment', () => {
  const sourcePageResourceKey = 'row:["wp_posts","ID:2"]';
  const targetAttachmentResourceKey = 'row:["wp_posts","ID:3"]';
  const postmetaResourceKey = 'row:["wp_postmeta","meta_id:52"]';
  const base = baseSite();
  const local = baseSite();
  local.db.wp_posts['ID:2'] = {
    ID: 2,
  ...
  assert.ok(postmetaMutation.dependsOnMutationIds.includes(sourcePageMutation.id));
  assert.ok(postmetaMutation.dependsOnMutationIds.includes(targetAttachmentMutation.id));
  ...
  const result = applyPlan(baseSite(), plan);
  assert.equal(result.site.db.wp_posts['ID:2'].post_type, 'page');
...
test('allows a local attachment parent reference to a same-plan attachment even when a remote nav menu taxonomy exists', () => {
  const parentResourceKey = 'row:["wp_posts","ID:2"]';
  const attachmentResourceKey = 'row:["wp_posts","ID:3"]';
  const base = baseSite();
  const local = baseSite();
  ...
  assert.equal(result.site.db.wp_posts['ID:2'].post_title, 'Local attachment parent');
  assert.equal(result.site.db.wp_posts['ID:3'].post_parent, 2);
  assert.equal(result.site.db.wp_term_taxonomy['term_taxonomy_id:20'].taxonomy, 'nav_menu');
});
...
test('blocks a local postmeta reference to a same-plan attachment when it is not thumbnail metadata even with unrelated remote nav menu noise', () => {
  const resourceKey = 'row:["wp_postmeta","meta_id:47"]';
  const attachmentResourceKey = 'row:["wp_posts","ID:2"]';
  const base = baseSite();
  const local = baseSite();
  ...
  assert.equal(JSON.stringify(blocker).includes('local-private-attachment-note'), false);
  assert.throws(() => applyPlan(baseSite(), plan), /Refusing to apply/);
});
...
test('blocks _menu_item_menu_item_parent metadata owned by an attachment even when it targets a same-plan post', () => {
  const resourceKey = 'row:["wp_postmeta","meta_id:500"]';
  ...
  local.db.wp_posts['ID:4'] = {
    ID: 4,
    post_title: 'Local attachment alias owner',
    post_content: 'local-private-attachment-alias-owner-body',
    post_status: 'inherit',
    post_type: 'attachment',
  };
  local.db.wp_posts['ID:5'] = {
  ...
  assert.equal(blocker.surface, 'attachment');
  assert.equal(
    JSON.stringify(blocker).includes('local-private-attachment-alias-owner-body'),
    false,
  );
  assert.equal(
...
test('blocks menu item parent metadata owned by an existing attachment even when it targets a same-plan post', () => {
  const resourceKey = 'row:["wp_postmeta","meta_id:5031"]';
  const attachmentResourceKey = 'row:["wp_posts","ID:4"]';
  const targetResourceKey = 'row:["wp_posts","ID:5"]';
  const base = baseSite();
  const local = baseSite();
...
test('blocks menu item parent metadata owned by an existing revision even when it targets a same-plan post', () => {
  const resourceKey = 'row:["wp_postmeta","meta_id:504"]';
  const revisionResourceKey = 'row:["wp_posts","ID:4"]';
  const targetResourceKey = 'row:["wp_posts","ID:5"]';
  const base = baseSite();
  const local = baseSite();
  ...
  base.db.wp_posts['ID:4'] = {
    ID: 4,
    post_title: 'Existing revision alias owner',
    post_content: 'base-private-existing-revision-alias-owner-body',
    post_status: 'inherit',
    post_type: 'revision',
  };
  local.db.wp_posts['ID:4'] = {
  ...
  assert.equal(blocker.surface, 'revision');
  assert.equal(
    JSON.stringify(blocker).includes('base-private-existing-revision-alias-owner-body'),
    false,
  );
  assert.equal(
    JSON.stringify(blocker).includes('local-private-alias-menu-parent-target-body'),
...
test('blocks _menu_item_menu_item_parent metadata owned by an existing revision even when it targets a same-plan post', () => {
  const resourceKey = 'row:["wp_postmeta","meta_id:505"]';
  const revisionResourceKey = 'row:["wp_posts","ID:4"]';
  const targetResourceKey = 'row:["wp_posts","ID:5"]';
  const base = baseSite();
  const local = baseSite();
...
test('blocks menu item parent metadata owned by an existing wp_navigation post even when it targets a same-plan post', () => {
  const resourceKey = 'row:["wp_postmeta","meta_id:506"]';
  const navigationResourceKey = 'row:["wp_posts","ID:4"]';
  const targetResourceKey = 'row:["wp_posts","ID:5"]';
  const base = baseSite();
  const local = baseSite();
  ...
  base.db.wp_posts['ID:4'] = {
    ID: 4,
    post_title: 'Existing wp_navigation alias owner',
    post_content: 'base-private-existing-wp-navigation-alias-owner-body',
    post_status: 'publish',
    post_type: 'wp_navigation',
  };
  local.db.wp_posts['ID:4'] = {
  ...
  assert.equal(blocker.surface, 'wp_navigation');
  assert.equal(
    JSON.stringify(blocker).includes('base-private-existing-wp-navigation-alias-owner-body'),
    false,
  );
  assert.equal(
    JSON.stringify(blocker).includes('local-private-alias-menu-parent-target-body'),
...
test('blocks _menu_item_menu_item_parent metadata owned by an existing wp_navigation post even when it targets a same-plan post', () => {
  const resourceKey = 'row:["wp_postmeta","meta_id:507"]';
  const navigationResourceKey = 'row:["wp_posts","ID:4"]';
  const targetResourceKey = 'row:["wp_posts","ID:5"]';
  const base = baseSite();
  const local = baseSite();
...
test('blocks menu item parent metadata owned by an existing wp_navigation post when the same-plan post target is itself blocked by a nav_menu_item parent', () => {
  const sourceNavigationResourceKey = 'row:["wp_posts","ID:1"]';
  const blockedParentResourceKey = 'row:["wp_posts","ID:2"]';
  const blockedTargetResourceKey = 'row:["wp_posts","ID:3"]';
  const postmetaResourceKey = 'row:["wp_postmeta","meta_id:5073"]';
  const base = baseSite();
  const local = baseSite();
  const remote = baseSite();
  ...
  local.db.wp_posts['ID:2'] = {
    ID: 2,
    post_title: 'Local blocked nav menu item parent',
    post_content: 'local-private-blocked-nav-menu-item-parent-body',
    post_status: 'publish',
    post_type: 'nav_menu_item',
  };
  local.db.wp_posts['ID:3'] = {
  ...
  assert.equal(postmetaBlocker.surface, 'wp_navigation');
  assert.equal(
    JSON.stringify(postmetaBlocker).includes('base-private-source-navigation-body'),
    false,
  );
  assert.equal(
    JSON.stringify(postmetaBlocker).includes('local-private-blocked-nav-menu-item-parent-body'),
    false,
  );
  assert.equal(
    JSON.stringify(postmetaBlocker).includes('local-private-blocked-menu-parent-target-body'),
...
test('blocks menu item parent metadata owned by an existing nav_menu_item post when the same-plan post target is itself blocked by a wp_navigation parent', () => {
  const sourceNavMenuItemResourceKey = 'row:["wp_posts","ID:1"]';
  const blockedParentResourceKey = 'row:["wp_posts","ID:2"]';
  const blockedTargetResourceKey = 'row:["wp_posts","ID:3"]';
  const postmetaResourceKey = 'row:["wp_postmeta","meta_id:5074"]';
  const base = baseSite();
  const local = baseSite();
  const remote = baseSite();
  ...
  local.db.wp_posts['ID:2'] = {
    ID: 2,
    post_title: 'Local blocked navigation parent',
    post_content: 'local-private-blocked-navigation-parent-body',
    post_status: 'publish',
    post_type: 'wp_navigation',
  };
  local.db.wp_posts['ID:3'] = {
  ...
  assert.equal(postmetaBlocker.surface, 'nav_menu_item');
  assert.equal(
    JSON.stringify(postmetaBlocker).includes('base-private-source-nav-menu-item-body'),
    false,
  );
  assert.equal(
    JSON.stringify(postmetaBlocker).includes('local-private-blocked-navigation-parent-body'),
    false,
  );
  assert.equal(
    JSON.stringify(postmetaBlocker).includes('local-private-blocked-menu-parent-target-body'),
...
test('blocks menu item parent metadata owned by an existing wp_navigation post when the same-plan post target is itself blocked by a revision parent', () => {
  const sourceNavigationResourceKey = 'row:["wp_posts","ID:1"]';
  const blockedParentResourceKey = 'row:["wp_posts","ID:2"]';
  const blockedTargetResourceKey = 'row:["wp_posts","ID:3"]';
  const postmetaResourceKey = 'row:["wp_postmeta","meta_id:50731"]';
  const base = baseSite();
  const local = baseSite();
  const remote = baseSite();
  ...
  assert.equal(postmetaBlocker.class, 'unsupported-wordpress-graph-surface');
  assert.equal(postmetaBlocker.surface, 'wp_navigation');
  assert.equal(JSON.stringify(postmetaBlocker).includes('base-private-source-navigation-body'), false);
  assert.equal(JSON.stringify(postmetaBlocker).includes('local-private-blocked-revision-parent-body'), false);
  assert.equal(JSON.stringify(postmetaBlocker).includes('local-private-blocked-menu-parent-target-body'), false);
  assert.throws(() => applyPlan(remote, plan), /Refusing to apply/);
});

test('blocks menu item parent metadata owned by an existing nav_menu_item post when the same-plan post target is itself blocked by a revision parent', () => {
  const sourceNavMenuItemResourceKey = 'row:["wp_posts","ID:1"]';
  const blockedParentResourceKey = 'row:["wp_posts","ID:2"]';
  const blockedTargetResourceKey = 'row:["wp_posts","ID:3"]';
  const postmetaResourceKey = 'row:["wp_postmeta","meta_id:50741"]';
  const base = baseSite();
  const local = baseSite();
  const remote = baseSite();
  ...
  assert.equal(postmetaBlocker.class, 'unsupported-wordpress-graph-surface');
  assert.equal(postmetaBlocker.surface, 'nav_menu_item');
  assert.equal(JSON.stringify(postmetaBlocker).includes('base-private-source-nav-menu-item-body'), false);
  assert.equal(JSON.stringify(postmetaBlocker).includes('local-private-blocked-revision-parent-body'), false);
  assert.equal(JSON.stringify(postmetaBlocker).includes('local-private-blocked-menu-parent-target-body'), false);
  assert.throws(() => applyPlan(remote, plan), /Refusing to apply/);
});

test('blocks menu item parent metadata owned by an existing revision when the same-plan post target is itself blocked by a wp_navigation parent', () => {
  const sourceRevisionResourceKey = 'row:["wp_posts","ID:1"]';
  const blockedParentResourceKey = 'row:["wp_posts","ID:2"]';
  const blockedTargetResourceKey = 'row:["wp_posts","ID:3"]';
  const postmetaResourceKey = 'row:["wp_postmeta","meta_id:50741"]';
  ...
  assert.equal(plan.status, 'blocked');
  assert.equal(mutationFor(plan, sourceRevisionResourceKey), undefined);
  assert.equal(mutationFor(plan, blockedParentResourceKey), undefined);
  assert.equal(blockedTargetMutation.changeKind, 'create');
  assert.equal(blockedTargetMutation.dependsOnMutationIds, undefined);
  assert.equal(mutationFor(plan, postmetaResourceKey), undefined);
  ...
  assert.equal(blockedTargetBlocker.references[0].targetResourceKey, blockedParentResourceKey);
  assert.equal(postmetaBlocker.class, 'unsupported-wordpress-graph-surface');
  assert.equal(postmetaBlocker.surface, 'revision');
  assert.equal(
    JSON.stringify(postmetaBlocker).includes('base-private-source-revision-body'),
    false,
  );
  assert.equal(
    JSON.stringify(postmetaBlocker).includes('local-private-blocked-navigation-parent-body'),
...
test('blocks menu item parent metadata owned by an existing revision when the same-plan post target is itself blocked by a nav_menu_item parent', () => {
  const sourceRevisionResourceKey = 'row:["wp_posts","ID:1"]';
  const blockedParentResourceKey = 'row:["wp_posts","ID:2"]';
  const blockedTargetResourceKey = 'row:["wp_posts","ID:3"]';
  const postmetaResourceKey = 'row:["wp_postmeta","meta_id:50742"]';
  const base = baseSite();
  const local = baseSite();
  const remote = baseSite();
...
test('blocks existing menu item parent metadata when the same-plan post target is itself blocked by a revision parent', () => {
  const blockedParentResourceKey = 'row:["wp_posts","ID:4"]';
  const blockedTargetResourceKey = 'row:["wp_posts","ID:5"]';
  const postmetaResourceKey = 'row:["wp_postmeta","meta_id:5073"]';
  const base = baseSite();
  ...
  remote.db.wp_postmeta = {
    'meta_id:5073': {
      ...base.db.wp_postmeta['meta_id:5073'],
    },
  };
  local.db.wp_posts['ID:4'] = {
    ID: 4,
...
test('blocks existing menu item parent metadata when the same-plan post target is itself blocked by a wp_navigation parent', () => {
  const blockedParentResourceKey = 'row:["wp_posts","ID:4"]';
  const blockedTargetResourceKey = 'row:["wp_posts","ID:5"]';
  const postmetaResourceKey = 'row:["wp_postmeta","meta_id:5074"]';
  const base = baseSite();
  ...
  remote.db.wp_postmeta = {
    'meta_id:5074': {
      ...base.db.wp_postmeta['meta_id:5074'],
    },
  };
  local.db.wp_posts['ID:4'] = {
    ID: 4,
...
test('blocks existing menu item parent metadata when the same-plan post target is itself blocked by a nav_menu_item parent', () => {
  const blockedParentResourceKey = 'row:["wp_posts","ID:4"]';
  const blockedTargetResourceKey = 'row:["wp_posts","ID:5"]';
  const postmetaResourceKey = 'row:["wp_postmeta","meta_id:5075"]';
  const base = baseSite();
  const local = baseSite();
  const remote = baseSite();
  ...
  remote.db.wp_postmeta = {
    'meta_id:5075': {
      ...base.db.wp_postmeta['meta_id:5075'],
    },
  };
  local.db.wp_posts['ID:4'] = {
    ID: 4,
...
test('blocks existing _menu_item_menu_item_parent metadata when the same-plan post target is itself blocked by a revision parent', () => {
  const blockedParentResourceKey = 'row:["wp_posts","ID:4"]';
  const blockedTargetResourceKey = 'row:["wp_posts","ID:5"]';
  const postmetaResourceKey = 'row:["wp_postmeta","meta_id:5076"]';
  const base = baseSite();
  const local = baseSite();
  const remote = baseSite();
  ...
  remote.db.wp_postmeta = {
    'meta_id:5076': {
      ...base.db.wp_postmeta['meta_id:5076'],
    },
  };
  local.db.wp_posts['ID:4'] = {
    ID: 4,
...
test('blocks existing _menu_item_menu_item_parent metadata when the same-plan post target is itself blocked by a wp_navigation parent', () => {
  const blockedParentResourceKey = 'row:["wp_posts","ID:4"]';
  const blockedTargetResourceKey = 'row:["wp_posts","ID:5"]';
  const postmetaResourceKey = 'row:["wp_postmeta","meta_id:5077"]';
  const base = baseSite();
  const local = baseSite();
  const remote = baseSite();
  ...
  remote.db.wp_postmeta = {
    'meta_id:5077': {
      ...base.db.wp_postmeta['meta_id:5077'],
    },
  };
  local.db.wp_posts['ID:4'] = {
    ID: 4,
...
test('blocks existing _menu_item_menu_item_parent metadata when the same-plan post target is itself blocked by a nav_menu_item parent', () => {
  const blockedParentResourceKey = 'row:["wp_posts","ID:4"]';
  const blockedTargetResourceKey = 'row:["wp_posts","ID:5"]';
  const postmetaResourceKey = 'row:["wp_postmeta","meta_id:5078"]';
  const base = baseSite();
  const local = baseSite();
  const remote = baseSite();
  ...
  remote.db.wp_postmeta = {
    'meta_id:5078': {
      ...base.db.wp_postmeta['meta_id:5078'],
    },
  };
  local.db.wp_posts['ID:4'] = {
    ID: 4,
...
test('blocks _menu_item_menu_item_parent metadata owned by an existing wp_navigation post when the same-plan post target is itself blocked by a nav_menu_item parent', () => {
  const sourceNavigationResourceKey = 'row:["wp_posts","ID:1"]';
  const blockedParentResourceKey = 'row:["wp_posts","ID:2"]';
  const blockedTargetResourceKey = 'row:["wp_posts","ID:3"]';
  const postmetaResourceKey = 'row:["wp_postmeta","meta_id:5079"]';
  const base = baseSite();
  const local = baseSite();
  const remote = baseSite();
...
test('blocks _menu_item_menu_item_parent metadata owned by an existing nav_menu_item post when the same-plan post target is itself blocked by a wp_navigation parent', () => {
  const sourceNavMenuItemResourceKey = 'row:["wp_posts","ID:1"]';
  const blockedParentResourceKey = 'row:["wp_posts","ID:2"]';
  const blockedTargetResourceKey = 'row:["wp_posts","ID:3"]';
  const postmetaResourceKey = 'row:["wp_postmeta","meta_id:5080"]';
  const base = baseSite();
  const local = baseSite();
  const remote = baseSite();
...
test('blocks _menu_item_menu_item_parent metadata owned by an existing wp_navigation post when the same-plan post target is itself blocked by a revision parent', () => {
  ...
  const blockedParentResourceKey = 'row:["wp_posts","ID:2"]';
  const blockedTargetResourceKey = 'row:["wp_posts","ID:3"]';
  const postmetaResourceKey = 'row:["wp_postmeta","meta_id:50811"]';
  const base = baseSite();
  const local = baseSite();
  const remote = baseSite();
...
test('blocks _menu_item_menu_item_parent metadata owned by an existing nav_menu_item post when the same-plan post target is itself blocked by a revision parent', () => {
  const sourceNavMenuItemResourceKey = 'row:["wp_posts","ID:1"]';
  const blockedParentResourceKey = 'row:["wp_posts","ID:2"]';
  const blockedTargetResourceKey = 'row:["wp_posts","ID:3"]';
  const postmetaResourceKey = 'row:["wp_postmeta","meta_id:50812"]';
  const base = baseSite();
  const local = baseSite();
  const remote = baseSite();
...
test('blocks _menu_item_menu_item_parent metadata owned by an existing revision when the same-plan post target is itself blocked by a wp_navigation parent', () => {
  const sourceRevisionResourceKey = 'row:["wp_posts","ID:1"]';
  const blockedParentResourceKey = 'row:["wp_posts","ID:2"]';
  const blockedTargetResourceKey = 'row:["wp_posts","ID:3"]';
  const postmetaResourceKey = 'row:["wp_postmeta","meta_id:50801"]';
  const base = baseSite();
  const local = baseSite();
  const remote = baseSite();
  ...
  assert.equal(postmetaBlocker.class, 'unsupported-wordpress-graph-surface');
  assert.equal(postmetaBlocker.surface, 'revision');
  assert.equal(JSON.stringify(postmetaBlocker).includes('base-private-existing-revision-owner-body'), false);
  assert.equal(JSON.stringify(postmetaBlocker).includes('local-private-navigation-parent-body'), false);
  assert.equal(JSON.stringify(postmetaBlocker).includes('local-private-blocked-menu-parent-target-body'), false);
  assert.throws(() => applyPlan(remote, plan), /Refusing to apply/);
});

test('blocks _menu_item_menu_item_parent metadata owned by an existing revision when the same-plan post target is itself blocked by a nav_menu_item parent', () => {
  const sourceRevisionResourceKey = 'row:["wp_posts","ID:1"]';
  const blockedParentResourceKey = 'row:["wp_posts","ID:2"]';
  const blockedTargetResourceKey = 'row:["wp_posts","ID:3"]';
  const postmetaResourceKey = 'row:["wp_postmeta","meta_id:50802"]';
  const base = baseSite();
  const local = baseSite();
  const remote = baseSite();
...
test('blocks _menu_item_menu_item_parent metadata owned by a wp_navigation post even when it targets a same-plan post', () => {
  ...
  local.db.wp_posts['ID:4'] = {
    ID: 4,
    post_title: 'Local wp_navigation alias owner',
    post_content: 'local-private-wp-navigation-alias-owner-body',
    post_status: 'publish',
    post_type: 'wp_navigation',
  };
  local.db.wp_posts['ID:5'] = {
...
test('blocks menu item parent metadata owned by a revision even when it targets a same-plan post', () => {
  const resourceKey = 'row:["wp_postmeta","meta_id:53"]';
...
test('blocks a local thumbnail reference owned by an existing attachment even when it targets a same-plan attachment', () => {
  const sourceAttachmentResourceKey = 'row:["wp_posts","ID:1"]';
  const targetAttachmentResourceKey = 'row:["wp_posts","ID:2"]';
  const postmetaResourceKey = 'row:["wp_postmeta","meta_id:148"]';
  const base = baseSite();
  const local = baseSite();
  const remote = baseSite();
...
test('blocks a local thumbnail reference owned by an existing revision when the same-plan attachment target is itself blocked by a revision parent', () => {
  const sourceRevisionResourceKey = 'row:["wp_posts","ID:1"]';
  const blockedParentResourceKey = 'row:["wp_posts","ID:2"]';
  const targetAttachmentResourceKey = 'row:["wp_posts","ID:3"]';
  const postmetaResourceKey = 'row:["wp_postmeta","meta_id:156"]';
  const base = baseSite();
  const local = baseSite();
  const remote = baseSite();
  ...
  assert.equal(targetAttachmentBlocker.class, 'missing-wordpress-graph-dependency');
  assert.equal(targetAttachmentBlocker.references[0].relationshipType, 'post-parent');
  assert.equal(targetAttachmentBlocker.references[0].targetResourceKey, blockedParentResourceKey);
  assert.equal(postmetaBlocker.class, 'unsupported-wordpress-graph-surface');
  assert.equal(postmetaBlocker.surface, 'revision');
  assert.equal(
    JSON.stringify(postmetaBlocker).includes('base-private-source-revision-body'),
  ...
  assert.equal(
    JSON.stringify(postmetaBlocker).includes('local-private-blocked-revision-parent-body'),
    false,
  );
  assert.equal(
    JSON.stringify(postmetaBlocker).includes('local-private-blocked-attachment-target-body'),
...
test('blocks a local thumbnail reference owned by an existing revision when the same-plan attachment target is itself blocked by a nav_menu_item parent', () => {
  const sourceRevisionResourceKey = 'row:["wp_posts","ID:1"]';
  const blockedParentResourceKey = 'row:["wp_posts","ID:2"]';
  const targetAttachmentResourceKey = 'row:["wp_posts","ID:3"]';
  const postmetaResourceKey = 'row:["wp_postmeta","meta_id:159"]';
  const base = baseSite();
  const local = baseSite();
  const remote = baseSite();
...
test('blocks a local thumbnail reference owned by an existing revision when the same-plan attachment target is itself blocked by a wp_navigation parent', () => {
  const sourceRevisionResourceKey = 'row:["wp_posts","ID:1"]';
  const blockedParentResourceKey = 'row:["wp_posts","ID:2"]';
  const targetAttachmentResourceKey = 'row:["wp_posts","ID:3"]';
  const postmetaResourceKey = 'row:["wp_postmeta","meta_id:162"]';
  const base = baseSite();
  const local = baseSite();
  const remote = baseSite();
...
test('blocks a local thumbnail reference owned by an existing wp_navigation post even when it targets a same-plan attachment', () => {
  const sourceNavigationResourceKey = 'row:["wp_posts","ID:1"]';
  const targetAttachmentResourceKey = 'row:["wp_posts","ID:2"]';
  const postmetaResourceKey = 'row:["wp_postmeta","meta_id:151"]';
  const base = baseSite();
  const local = baseSite();
  const remote = baseSite();
...
test('blocks a local thumbnail reference owned by an existing wp_navigation post when the same-plan attachment target is itself blocked by a wp_navigation parent', () => {
  const sourceNavigationResourceKey = 'row:["wp_posts","ID:1"]';
  const blockedParentResourceKey = 'row:["wp_posts","ID:2"]';
  const targetAttachmentResourceKey = 'row:["wp_posts","ID:3"]';
  const postmetaResourceKey = 'row:["wp_postmeta","meta_id:157"]';
  const base = baseSite();
  const local = baseSite();
  const remote = baseSite();
...
test('blocks a local thumbnail reference owned by an existing wp_navigation post when the same-plan attachment target is itself blocked by a nav_menu_item parent', () => {
  const sourceNavigationResourceKey = 'row:["wp_posts","ID:1"]';
  const blockedParentResourceKey = 'row:["wp_posts","ID:2"]';
  const targetAttachmentResourceKey = 'row:["wp_posts","ID:3"]';
  const postmetaResourceKey = 'row:["wp_postmeta","meta_id:160"]';
  const base = baseSite();
  const local = baseSite();
  const remote = baseSite();
...
test('blocks a local thumbnail reference owned by an existing wp_navigation post when the same-plan attachment target is itself blocked by a revision parent', () => {
  const sourceNavigationResourceKey = 'row:["wp_posts","ID:1"]';
  const blockedParentResourceKey = 'row:["wp_posts","ID:2"]';
  const targetAttachmentResourceKey = 'row:["wp_posts","ID:3"]';
  const postmetaResourceKey = 'row:["wp_postmeta","meta_id:163"]';
  const base = baseSite();
  const local = baseSite();
  const remote = baseSite();
...
test('blocks a local thumbnail reference owned by an existing nav_menu_item post even when it targets a same-plan attachment', () => {
  const sourceNavMenuItemResourceKey = 'row:["wp_posts","ID:1"]';
  const targetAttachmentResourceKey = 'row:["wp_posts","ID:2"]';
  const postmetaResourceKey = 'row:["wp_postmeta","meta_id:153"]';
  const base = baseSite();
  const local = baseSite();
  const remote = baseSite();
...
test('blocks a local thumbnail reference owned by an existing nav_menu_item post when the same-plan attachment target is itself blocked by a nav_menu_item parent', () => {
  const sourceNavMenuItemResourceKey = 'row:["wp_posts","ID:1"]';
  const blockedParentResourceKey = 'row:["wp_posts","ID:2"]';
  const targetAttachmentResourceKey = 'row:["wp_posts","ID:3"]';
  const postmetaResourceKey = 'row:["wp_postmeta","meta_id:158"]';
  const base = baseSite();
  const local = baseSite();
  const remote = baseSite();
...
test('blocks a local thumbnail reference owned by an existing nav_menu_item post when the same-plan attachment target is itself blocked by a wp_navigation parent', () => {
  const sourceNavMenuItemResourceKey = 'row:["wp_posts","ID:1"]';
  const blockedParentResourceKey = 'row:["wp_posts","ID:2"]';
  const targetAttachmentResourceKey = 'row:["wp_posts","ID:3"]';
  const postmetaResourceKey = 'row:["wp_postmeta","meta_id:161"]';
  const base = baseSite();
  const local = baseSite();
  const remote = baseSite();
...
test('blocks a local thumbnail reference owned by an existing nav_menu_item post when the same-plan attachment target is itself blocked by a revision parent', () => {
  const sourceNavMenuItemResourceKey = 'row:["wp_posts","ID:1"]';
  const blockedParentResourceKey = 'row:["wp_posts","ID:2"]';
  const targetAttachmentResourceKey = 'row:["wp_posts","ID:3"]';
  const postmetaResourceKey = 'row:["wp_postmeta","meta_id:164"]';
  const base = baseSite();
  const local = baseSite();
  const remote = baseSite();
...
test('blocks a local thumbnail reference owned by a revision even when it targets a same-plan attachment', () => {
  ...
  local.db.wp_posts['ID:1'] = {
    ID: 1,
    post_title: 'Local source revision',
    post_content: 'local-private-source-revision-body',
    post_status: 'inherit',
    post_type: 'revision',
  };
  local.db.wp_posts['ID:2'] = {
  ...
  assert.equal(blocker.surface, 'revision');
  assert.equal(
    JSON.stringify(blocker).includes('local-private-source-revision-body'),
    false,
  );
  assert.equal(
    JSON.stringify(blocker).includes('local-private-target-attachment-body'),
...
test('blocks a local thumbnail reference owned by a wp_navigation post even when it targets a same-plan attachment', () => {
  const sourceNavigationResourceKey = 'row:["wp_posts","ID:1"]';
  const targetAttachmentResourceKey = 'row:["wp_posts","ID:2"]';
  const postmetaResourceKey = 'row:["wp_postmeta","meta_id:49"]';
  const base = baseSite();
  ...
  local.db.wp_posts['ID:1'] = {
    ID: 1,
    post_title: 'Local source navigation',
    post_content: 'local-private-source-navigation-body',
    post_status: 'publish',
    post_type: 'wp_navigation',
  };
  local.db.wp_posts['ID:2'] = {
  ...
  assert.equal(blocker.surface, 'wp_navigation');
  assert.equal(
    JSON.stringify(blocker).includes('local-private-source-navigation-body'),
    false,
  );
  assert.equal(
    JSON.stringify(blocker).includes('local-private-target-attachment-body'),
...
test('allows a local attachment parent reference to a same-plan attachment', () => {
  ...
  assert.equal(reference.relationshipType, 'post-parent');
  assert.equal(reference.targetResourceKey, targetResourceKey);
  assert.equal(JSON.stringify(reference).includes('local-private-attachment-target-body'), false);
  assert.equal(JSON.stringify(reference).includes('local-private-attachment-child-body'), false);

  const result = applyPlan(baseSite(), plan);
  assert.equal(result.site.db.wp_posts['ID:2'].post_title, 'Local attachment target');
  assert.equal(result.site.db.wp_posts['ID:3'].post_title, 'Local attachment child');
  assert.equal(result.site.db.wp_posts['ID:3'].post_parent, 2);
});
...
test('blocks an existing attachment parent reference to a same-plan post', () => {
  ...
  assert.equal(blocker.references[0].relationshipType, 'post-parent');
  assert.equal(blocker.references[0].targetResourceKey, targetResourceKey);
  assert.match(blocker.reason, /existing attachment source without a supported merge policy/);
...
test('blocks an existing attachment parent reference to a same-plan attachment', () => {
  const targetResourceKey = 'row:["wp_posts","ID:2"]';
  const attachmentResourceKey = 'row:["wp_posts","ID:3"]';
  const base = baseSite();
  const local = baseSite();
...
test('blocks an existing attachment parent reference to a same-plan revision', () => {
  const targetResourceKey = 'row:["wp_posts","ID:2"]';
  const attachmentResourceKey = 'row:["wp_posts","ID:3"]';
  const base = baseSite();
  const local = baseSite();
...
test('blocks an existing revision parent reference to a same-plan post', () => {
  const targetResourceKey = 'row:["wp_posts","ID:2"]';
  ...
  base.db.wp_posts['ID:3'] = {
    ID: 3,
    post_title: 'Existing revision child',
    post_content: 'base-private-existing-revision-child-body',
    post_status: 'inherit',
    post_type: 'revision',
    post_parent: 0,
  };
  ...
  assert.equal(blocker.surface, 'revision');
  assert.equal(
    JSON.stringify(blocker).includes('base-private-existing-revision-child-body'),
    false,
  );
  assert.equal(
    JSON.stringify(blocker).includes('local-private-parent-post-body'),
...
test('blocks an existing revision parent reference to a same-plan attachment', () => {
  const targetResourceKey = 'row:["wp_posts","ID:2"]';
  const revisionResourceKey = 'row:["wp_posts","ID:3"]';
  const base = baseSite();
  const local = baseSite();
...
test('blocks an existing revision parent reference to a same-plan revision', () => {
  const targetResourceKey = 'row:["wp_posts","ID:2"]';
  const revisionResourceKey = 'row:["wp_posts","ID:3"]';
  const base = baseSite();
  const local = baseSite();
...
test('blocks an existing revision parent reference to a same-plan wp_navigation post', () => {
  const targetResourceKey = 'row:["wp_posts","ID:2"]';
  const revisionResourceKey = 'row:["wp_posts","ID:3"]';
  const base = baseSite();
  const local = baseSite();
...
test('blocks an existing wp_navigation parent reference to a same-plan post', () => {
  const targetResourceKey = 'row:["wp_posts","ID:2"]';
...
test('blocks an existing wp_navigation parent reference to a same-plan attachment', () => {
  const targetResourceKey = 'row:["wp_posts","ID:2"]';
  const navigationResourceKey = 'row:["wp_posts","ID:3"]';
  const base = baseSite();
  const local = baseSite();
...
test('blocks an existing wp_navigation parent reference to a same-plan revision', () => {
  const targetResourceKey = 'row:["wp_posts","ID:2"]';
  const navigationResourceKey = 'row:["wp_posts","ID:3"]';
  const base = baseSite();
  const local = baseSite();
...
test('blocks an existing wp_navigation parent reference to a same-plan wp_navigation post', () => {
  const targetResourceKey = 'row:["wp_posts","ID:2"]';
  const navigationResourceKey = 'row:["wp_posts","ID:3"]';
  const base = baseSite();
  const local = baseSite();
...
test('blocks an existing nav_menu_item parent reference to a same-plan revision', () => {
  const targetResourceKey = 'row:["wp_posts","ID:2"]';
  const menuItemResourceKey = 'row:["wp_posts","ID:3"]';
  const base = baseSite();
  const local = baseSite();
...
test('blocks an existing nav_menu_item parent reference to a same-plan wp_navigation post', () => {
  const targetResourceKey = 'row:["wp_posts","ID:2"]';
  const menuItemResourceKey = 'row:["wp_posts","ID:3"]';
  const base = baseSite();
  const local = baseSite();
...
test('blocks an existing nav_menu_item parent reference to a same-plan attachment', () => {
  const targetResourceKey = 'row:["wp_posts","ID:2"]';
  const menuItemResourceKey = 'row:["wp_posts","ID:3"]';
  const base = baseSite();
  const local = baseSite();
...
test('blocks an existing attachment parent reference to a same-plan nav_menu_item post', () => {
  const targetResourceKey = 'row:["wp_posts","ID:2"]';
  const attachmentResourceKey = 'row:["wp_posts","ID:3"]';
  const base = baseSite();
  const local = baseSite();
  ...
  local.db.wp_posts['ID:2'] = {
    ID: 2,
    post_title: 'Local menu item parent',
    post_content: 'local-private-menu-item-parent-body',
    post_status: 'publish',
    post_type: 'nav_menu_item',
  };
  ...
  assert.equal(
    JSON.stringify(blocker).includes('local-private-menu-item-parent-body'),
    false,
  );
  assert.throws(() => applyPlan(remote, plan), /Refusing to apply/);
});

test('blocks an existing revision parent reference to a same-plan nav_menu_item post', () => {
  const targetResourceKey = 'row:["wp_posts","ID:2"]';
  const revisionResourceKey = 'row:["wp_posts","ID:3"]';
  const base = baseSite();
  const local = baseSite();
...
test('blocks an existing wp_navigation parent reference to a same-plan nav_menu_item post', () => {
  const targetResourceKey = 'row:["wp_posts","ID:2"]';
  const navigationResourceKey = 'row:["wp_posts","ID:3"]';
  const base = baseSite();
  const local = baseSite();
...
test('blocks an existing nav_menu_item parent reference to a same-plan nav_menu_item post', () => {
  const targetResourceKey = 'row:["wp_posts","ID:2"]';
  const menuItemResourceKey = 'row:["wp_posts","ID:3"]';
  const base = baseSite();
  const local = baseSite();
...
test('blocks a local attachment parent reference when the same-plan attachment target is itself blocked by a wp_navigation parent', () => {
  const navigationResourceKey = 'row:["wp_posts","ID:2"]';
  const targetAttachmentResourceKey = 'row:["wp_posts","ID:3"]';
  const childAttachmentResourceKey = 'row:["wp_posts","ID:4"]';
  const base = baseSite();
  ...
  assert.equal(
    JSON.stringify(childAttachmentBlocker).includes('local-private-navigation-parent-body'),
    false,
  );
  assert.equal(
    JSON.stringify(childAttachmentBlocker).includes('local-private-child-attachment-body'),
...
test('blocks a local attachment parent reference when the same-plan attachment target is itself blocked by a nav_menu_item parent', () => {
  const menuItemResourceKey = 'row:["wp_posts","ID:2"]';
  const targetAttachmentResourceKey = 'row:["wp_posts","ID:3"]';
  const childAttachmentResourceKey = 'row:["wp_posts","ID:4"]';
  const base = baseSite();
  ...
  assert.equal(
    JSON.stringify(childAttachmentBlocker).includes('local-private-nav-menu-item-parent-body'),
    false,
  );
  assert.equal(
    JSON.stringify(childAttachmentBlocker).includes('local-private-child-attachment-body'),
...
test('blocks a local attachment parent reference when the same-plan post target is itself blocked by a revision parent', () => {
  const revisionResourceKey = 'row:["wp_posts","ID:2"]';
  const blockedPostResourceKey = 'row:["wp_posts","ID:3"]';
  const childAttachmentResourceKey = 'row:["wp_posts","ID:4"]';
  const base = baseSite();
  ...
  local.db.wp_posts['ID:3'] = {
    ID: 3,
    post_title: 'Local blocked post target',
    post_content: 'local-private-blocked-post-target-body',
    post_status: 'publish',
    post_parent: 2,
  };
  local.db.wp_posts['ID:4'] = {
  ...
  assert.equal(plan.status, 'blocked');
  assert.equal(mutationFor(plan, revisionResourceKey), undefined);
  assert.equal(mutationFor(plan, blockedPostResourceKey).changeKind, 'create');
  assert.equal(childAttachmentMutation.changeKind, 'create');
  assert.equal(blockedPostBlocker.class, 'missing-wordpress-graph-dependency');
  assert.equal(blockedPostBlocker.references[0].relationshipType, 'post-parent');
  assert.equal(blockedPostBlocker.references[0].targetResourceKey, revisionResourceKey);
  assert.equal(childAttachmentBlocker.class, 'missing-wordpress-graph-dependency');
  assert.equal(childAttachmentBlocker.references[0].relationshipType, 'post-parent');
  assert.equal(childAttachmentBlocker.references[0].targetResourceKey, blockedPostResourceKey);
  assert.equal(childAttachmentMutation.dependsOnMutationIds, undefined);
  assert.equal(
    JSON.stringify(childAttachmentBlocker).includes('local-private-blocked-post-target-body'),
    false,
  );
  assert.equal(
    JSON.stringify(childAttachmentBlocker).includes('local-private-child-attachment-body'),
...
test('blocks a local thumbnail reference when the same-plan attachment target is itself blocked by a revision parent', () => {
  const revisionResourceKey = 'row:["wp_posts","ID:2"]';
  const targetAttachmentResourceKey = 'row:["wp_posts","ID:3"]';
  const postmetaResourceKey = 'row:["wp_postmeta","meta_id:45"]';
  const base = baseSite();
...
test('blocks a local thumbnail reference when the same-plan attachment target is itself blocked by a wp_navigation parent', () => {
  const navigationResourceKey = 'row:["wp_posts","ID:2"]';
  const targetAttachmentResourceKey = 'row:["wp_posts","ID:3"]';
  const postmetaResourceKey = 'row:["wp_postmeta","meta_id:46"]';
  const base = baseSite();
...
test('blocks a local post parent reference owned by a nav_menu_item post even when it targets a same-plan post', () => {
  const navigationResourceKey = 'row:["wp_posts","ID:6"]';
  ...
  assert.equal(JSON.stringify(navigationBlocker).includes('local-private-nav-menu-item-parent-body'), false);
  assert.equal(JSON.stringify(navigationBlocker).includes('local-private-post-child-body'), false);
  assert.throws(() => applyPlan(baseSite(), plan), /Refusing to apply/);
});
...
test('blocks a local post parent reference to a same-plan attachment even when an unrelated remote attachment exists', () => {
  const attachmentResourceKey = 'row:["wp_posts","ID:6"]';
  const postResourceKey = 'row:["wp_posts","ID:7"]';
  const base = baseSite();
  const local = baseSite();
...
test('blocks attachment-owned term relationships even when they target same-plan terms and unrelated remote attachment noise exists', () => {
  const attachmentResourceKey = 'row:["wp_posts","ID:3"]';
  const termResourceKey = 'row:["wp_terms","term_id:7"]';
  const taxonomyResourceKey = 'row:["wp_term_taxonomy","term_taxonomy_id:9"]';
  const relationshipResourceKey = 'row:["wp_term_relationships","object_id:3|term_taxonomy_id:9"]';
  ...
  assert.equal(relationshipBlocker.surface, 'attachment');
  assert.match(relationshipBlocker.reason, /outside the supported release-candidate slice/);
  assert.equal(JSON.stringify(relationshipBlocker).includes('local-private-attachment-owner-body'), false);
  assert.equal(JSON.stringify(relationshipBlocker).includes('remote-attachment-body'), false);
  assert.throws(() => applyPlan(remote, plan), /Refusing to apply/);
});
...
test('blocks a term taxonomy parent reference when the same-plan target term also has a nav menu taxonomy row and an unrelated remote attachment exists', () => {
  const parentTermResourceKey = 'row:["wp_terms","term_id:7"]';
  const childTaxonomyResourceKey = 'row:["wp_term_taxonomy","term_taxonomy_id:9"]';
  const navMenuTaxonomyResourceKey = 'row:["wp_term_taxonomy","term_taxonomy_id:10"]';
  const base = baseSite();
...
test('blocks a term taxonomy parent reference when the same-plan source taxonomy is nav_menu', () => {
  const parentTermResourceKey = 'row:["wp_terms","term_id:7"]';
  const navMenuTaxonomyResourceKey = 'row:["wp_term_taxonomy","term_taxonomy_id:10"]';
  const base = baseSite();
  const local = baseSite();
  ...
  local.db.wp_term_taxonomy = {
    'term_taxonomy_id:10': {
      ...
      term_id: 7,
      taxonomy: 'nav_menu',
      description: 'Local navigation menu taxonomy',
      parent: 7,
      count: 0,
    },
  };
  ...
  const plan = planFor(base, local, baseSite());
  const parentMutation = mutationFor(plan, parentTermResourceKey);
  const navMenuTaxonomyMutation = mutationFor(plan, navMenuTaxonomyResourceKey);
  const blocker = plan.blockers.find((entry) => entry.resourceKey === navMenuTaxonomyResourceKey);
  ...
  assert.equal(blocker.class, 'unsupported-wordpress-graph-surface');
  assert.equal(blocker.surface, 'nav_menu');
  assert.equal(blocker.reason.includes('nav_menu'), true);
  assert.equal(JSON.stringify(blocker).includes('Local navigation menu taxonomy'), false);
});
...
test('blocks a local term taxonomy parent reference when the same-plan term belongs to a nav menu taxonomy even with unrelated remote attachment noise', () => {
  const termResourceKey = 'row:["wp_terms","term_id:7"]';
  const taxonomyResourceKey = 'row:["wp_term_taxonomy","term_taxonomy_id:9"]';
  const base = baseSite();
  const local = baseSite();
...
test('allows an existing term taxonomy row to retarget its term reference to a same-plan term create even when an unrelated remote attachment exists', () => {
  const termResourceKey = 'row:["wp_terms","term_id:7"]';
  const taxonomyResourceKey = 'row:["wp_term_taxonomy","term_taxonomy_id:9"]';
  const base = baseSite();
  const local = baseSite();
...
test('allows an existing term taxonomy row to retarget its parent reference to a same-plan term create even when an unrelated remote attachment exists', () => {
  const termResourceKey = 'row:["wp_terms","term_id:7"]';
  const taxonomyResourceKey = 'row:["wp_term_taxonomy","term_taxonomy_id:9"]';
  const base = baseSite();
  const local = baseSite();
...
test('blocks an existing term taxonomy row when its same-plan term target is claimed by a nav menu taxonomy', () => {
  ...
  local.db.wp_terms = {
    'term_id:7': {
      term_id: 7,
      name: 'Local blocked existing taxonomy term',
      slug: 'local-blocked-existing-taxonomy-term',
    },
  };
  local.db.wp_term_taxonomy = {
    'term_taxonomy_id:9': {
  ...
  assert.equal(JSON.stringify(blocker).includes('Local blocked existing taxonomy term'), false);
  assert.throws(() => applyPlan(remote, plan), /Refusing to apply/);
});

test('blocks an existing term taxonomy row when its same-plan parent term target is claimed by a nav menu taxonomy', () => {
  const termResourceKey = 'row:["wp_terms","term_id:7"]';
  const navMenuTaxonomyResourceKey = 'row:["wp_term_taxonomy","term_taxonomy_id:10"]';
  const taxonomyResourceKey = 'row:["wp_term_taxonomy","term_taxonomy_id:9"]';
  const base = baseSite();
  ...
  assert.equal(blocker.references[0].targetResourceKey, termResourceKey);
  assert.equal(taxonomyMutation.dependsOnMutationIds, undefined);
  assert.equal(JSON.stringify(blocker).includes('Local blocked existing taxonomy parent term'), false);
  assert.throws(() => applyPlan(remote, plan), /Refusing to apply/);
});
...
test('allows a local term relationship object reference to a same-plan post even when an unrelated remote attachment exists', () => {
  const postResourceKey = 'row:["wp_posts","ID:3"]';
  const termResourceKey = 'row:["wp_terms","term_id:7"]';
  const taxonomyResourceKey = 'row:["wp_term_taxonomy","term_taxonomy_id:9"]';
  const relationshipResourceKey = 'row:["wp_term_relationships","object_id:3|term_taxonomy_id:9"]';
...
test('blocks an existing term relationship object reference when the same-plan post target is itself blocked by a wp_navigation parent', () => {
  const existingPostResourceKey = 'row:["wp_posts","ID:3"]';
  const samePlanPostResourceKey = 'row:["wp_posts","ID:4"]';
  const blockedParentResourceKey = 'row:["wp_posts","ID:5"]';
  const relationshipResourceKey = 'row:["wp_term_relationships","object_id:4|term_taxonomy_id:9"]';
...
test('blocks an existing term relationship object reference when the same-plan post target is itself blocked by a nav_menu_item parent', () => {
  ...
  assert.equal(relationshipBlocker.references[0].targetResourceKey, samePlanPostResourceKey);
  assert.equal(JSON.stringify(relationshipBlocker).includes('local-private-retagged-post-body'), false);
  assert.equal(JSON.stringify(relationshipBlocker).includes('local-private-nav-menu-item-parent-body'), false);
  assert.throws(() => applyPlan(remote, plan), /Refusing to apply/);
});
...
test('blocks a local term relationship object reference when the same-plan post target is itself blocked by a wp_navigation parent', () => {
  const navigationResourceKey = 'row:["wp_posts","ID:2"]';
  const blockedPostResourceKey = 'row:["wp_posts","ID:3"]';
  const termResourceKey = 'row:["wp_terms","term_id:7"]';
  const taxonomyResourceKey = 'row:["wp_term_taxonomy","term_taxonomy_id:9"]';
  ...
  assert.equal(blockedPostBlocker.class, 'missing-wordpress-graph-dependency');
  assert.equal(blockedPostBlocker.references[0].relationshipType, 'post-parent');
  assert.equal(blockedPostBlocker.references[0].targetResourceKey, navigationResourceKey);
  assert.equal(relationshipBlocker.class, 'missing-wordpress-graph-dependency');
  assert.equal(relationshipBlocker.references[0].relationshipType, 'term-relationship-object');
  assert.equal(relationshipBlocker.references[0].targetResourceKey, blockedPostResourceKey);
  assert.equal(relationshipMutation.dependsOnMutationIds, undefined);
...
test('blocks a local term relationship object reference when the same-plan post target is itself blocked by a nav_menu_item parent', () => {
  const menuItemResourceKey = 'row:["wp_posts","ID:2"]';
  const blockedPostResourceKey = 'row:["wp_posts","ID:3"]';
  const termResourceKey = 'row:["wp_terms","term_id:7"]';
  const taxonomyResourceKey = 'row:["wp_term_taxonomy","term_taxonomy_id:9"]';
  ...
  assert.equal(blockedPostBlocker.class, 'missing-wordpress-graph-dependency');
  assert.equal(blockedPostBlocker.references[0].relationshipType, 'post-parent');
  assert.equal(blockedPostBlocker.references[0].targetResourceKey, menuItemResourceKey);
  assert.equal(relationshipBlocker.class, 'missing-wordpress-graph-dependency');
  assert.equal(relationshipBlocker.references[0].relationshipType, 'term-relationship-object');
  assert.equal(relationshipBlocker.references[0].targetResourceKey, blockedPostResourceKey);
  assert.equal(relationshipMutation.dependsOnMutationIds, undefined);
...
test('blocks a local term relationship object reference owned by a revision even when it targets a same-plan post and unrelated remote attachment noise exists', () => {
  const revisionResourceKey = 'row:["wp_posts","ID:3"]';
  const postResourceKey = 'row:["wp_posts","ID:4"]';
  const termResourceKey = 'row:["wp_terms","term_id:7"]';
  const taxonomyResourceKey = 'row:["wp_term_taxonomy","term_taxonomy_id:9"]';
...
test('blocks a local term relationship object reference owned by an existing attachment even when it targets a same-plan post', () => {
  const attachmentResourceKey = 'row:["wp_posts","ID:3"]';
  const postResourceKey = 'row:["wp_posts","ID:4"]';
  const termResourceKey = 'row:["wp_terms","term_id:7"]';
  const taxonomyResourceKey = 'row:["wp_term_taxonomy","term_taxonomy_id:9"]';
  ...
  base.db.wp_posts['ID:3'] = {
    ID: 3,
    post_title: 'Existing attachment relationship owner',
    post_content: 'base-private-existing-attachment-relationship-owner-body',
    post_status: 'inherit',
    post_type: 'attachment',
  };
  local.db.wp_posts['ID:3'] = {
  ...
  assert.equal(blocker.surface, 'attachment');
  assert.equal(
    JSON.stringify(blocker).includes('base-private-existing-attachment-relationship-owner-body'),
    false,
  );
  assert.equal(
    JSON.stringify(blocker).includes('local-private-target-post-body'),
...
test('blocks a local term relationship object reference owned by an existing revision even when it targets a same-plan post', () => {
  const revisionResourceKey = 'row:["wp_posts","ID:3"]';
  const postResourceKey = 'row:["wp_posts","ID:4"]';
  const termResourceKey = 'row:["wp_terms","term_id:7"]';
  const taxonomyResourceKey = 'row:["wp_term_taxonomy","term_taxonomy_id:9"]';
  ...
  assert.equal(blocker.surface, 'revision');
  assert.equal(
    JSON.stringify(blocker).includes('base-private-existing-revision-relationship-owner-body'),
    false,
  );
  assert.equal(
    JSON.stringify(blocker).includes('local-private-target-post-body'),
...
test('blocks a local term relationship object reference owned by an existing nav_menu_item post even when it targets a same-plan post', () => {
  const menuItemResourceKey = 'row:["wp_posts","ID:3"]';
  const postResourceKey = 'row:["wp_posts","ID:4"]';
  const termResourceKey = 'row:["wp_terms","term_id:7"]';
  const taxonomyResourceKey = 'row:["wp_term_taxonomy","term_taxonomy_id:9"]';
  ...
  base.db.wp_posts['ID:3'] = {
    ID: 3,
    post_title: 'Existing menu item relationship owner',
    post_content: 'base-private-existing-menu-item-relationship-owner-body',
    post_status: 'publish',
    post_type: 'nav_menu_item',
  };
  local.db.wp_posts['ID:3'] = {
  ...
  assert.equal(blocker.surface, 'nav_menu_item');
  assert.equal(
    JSON.stringify(blocker).includes('base-private-existing-menu-item-relationship-owner-body'),
    false,
  );
  assert.equal(
    JSON.stringify(blocker).includes('local-private-target-post-body'),
...
test('blocks a local term relationship object reference owned by an existing wp_navigation post even when it targets a same-plan post', () => {
  const navigationResourceKey = 'row:["wp_posts","ID:3"]';
  const postResourceKey = 'row:["wp_posts","ID:4"]';
  const termResourceKey = 'row:["wp_terms","term_id:7"]';
  const taxonomyResourceKey = 'row:["wp_term_taxonomy","term_taxonomy_id:9"]';
  ...
  base.db.wp_posts['ID:3'] = {
    ID: 3,
    post_title: 'Existing navigation relationship owner',
    post_content: 'base-private-existing-navigation-relationship-owner-body',
    post_status: 'publish',
    post_type: 'wp_navigation',
  };
  local.db.wp_posts['ID:3'] = {
  ...
  assert.equal(blocker.surface, 'wp_navigation');
  assert.equal(
    JSON.stringify(blocker).includes('base-private-existing-navigation-relationship-owner-body'),
    false,
  );
  assert.equal(
    JSON.stringify(blocker).includes('local-private-target-post-body'),
...
test('blocks a local term relationship object reference to a same-plan revision', () => {
  ...
  local.db.wp_posts['ID:3'] = {
    ID: 3,
    post_title: 'Local revision object post',
    post_content: 'local-private-revision-object-body',
    post_status: 'inherit',
    post_type: 'revision',
  };
  local.db.wp_terms = {
  ...
  assert.equal(blocker.class, 'unsupported-wordpress-graph-surface');
  assert.equal(blocker.surface, 'revision');
  assert.equal(JSON.stringify(blocker).includes('local-private-revision-object-body'), false);
  assert.equal(JSON.stringify(blocker).includes('local-relationship-term'), false);
  assert.throws(() => applyPlan(baseSite(), plan), /Refusing to apply/);
});

test('blocks a local term relationship object reference to a same-plan wp_navigation post', () => {
  const sourcePostResourceKey = 'row:["wp_posts","ID:3"]';
  const targetPostResourceKey = 'row:["wp_posts","ID:4"]';
  const termResourceKey = 'row:["wp_terms","term_id:7"]';
  const taxonomyResourceKey = 'row:["wp_term_taxonomy","term_taxonomy_id:9"]';
  const relationshipResourceKey = 'row:["wp_term_relationships","object_id:3|term_taxonomy_id:9"]';
  const base = baseSite();
  ...
  local.db.wp_posts['ID:4'] = {
    ID: 4,
    post_title: 'Local navigation object post',
    post_content: 'local-private-navigation-object-body',
    post_status: 'publish',
    post_type: 'wp_navigation',
  };
  local.db.wp_terms = {
  ...
  const termMutation = mutationFor(plan, termResourceKey);
  const taxonomyMutation = mutationFor(plan, taxonomyResourceKey);
  const postBlocker = plan.blockers.find((entry) => entry.resourceKey === targetPostResourceKey);
  ...
  assert.equal(termMutation.changeKind, 'create');
  assert.equal(taxonomyMutation.changeKind, 'create');
  assert.equal(postBlocker.class, 'unsupported-wordpress-graph-surface');
  assert.equal(postBlocker.surface, 'wp_navigation');
  assert.deepEqual(relationshipMutation.dependsOnMutationIds.sort(), [sourceMutation.id, taxonomyMutation.id].sort());
  assert.equal(relationshipMutation.wordpressGraphReferences[0].targetResourceKey, sourcePostResourceKey);
  assert.equal(JSON.stringify(postBlocker).includes('local-private-navigation-object-body'), false);
});

test('blocks a local term relationship object reference to a same-plan wp_navigation post even when unrelated remote attachment noise exists', () => {
  const sourcePostResourceKey = 'row:["wp_posts","ID:3"]';
  const targetPostResourceKey = 'row:["wp_posts","ID:4"]';
  const termResourceKey = 'row:["wp_terms","term_id:7"]';
  const taxonomyResourceKey = 'row:["wp_term_taxonomy","term_taxonomy_id:9"]';
  ...
  assert.equal(relationshipMutation.wordpressGraphReferences[0].targetResourceKey, sourcePostResourceKey);
  assert.equal(JSON.stringify(postBlocker).includes('local-private-navigation-object-body'), false);
  assert.equal(JSON.stringify(postBlocker).includes('remote-attachment-noise-body'), false);
});
...
test('allows an existing term relationship taxonomy reference to retarget to a same-plan term taxonomy create even when an unrelated remote attachment exists', () => {
  const postResourceKey = 'row:["wp_posts","ID:3"]';
  const termResourceKey = 'row:["wp_terms","term_id:7"]';
  const existingTaxonomyResourceKey = 'row:["wp_term_taxonomy","term_taxonomy_id:8"]';
  const samePlanTaxonomyResourceKey = 'row:["wp_term_taxonomy","term_taxonomy_id:9"]';
...
test('blocks an existing term relationship taxonomy reference when the same-plan term taxonomy target is itself blocked by a same-plan nav menu taxonomy', () => {
  const postResourceKey = 'row:["wp_posts","ID:3"]';
  const termResourceKey = 'row:["wp_terms","term_id:7"]';
  const existingTaxonomyResourceKey = 'row:["wp_term_taxonomy","term_taxonomy_id:8"]';
  ...
  assert.equal(relationshipBlocker.references[0].targetResourceKey, samePlanTaxonomyResourceKey);
  assert.equal(JSON.stringify(relationshipBlocker).includes('local-replacement-category-term'), false);
  assert.throws(() => applyPlan(remote, plan), /Refusing to apply/);
});

test('blocks an existing term relationship taxonomy reference when the same-plan term taxonomy target is itself blocked by a blocked same-plan parent term', () => {
  const postResourceKey = 'row:["wp_posts","ID:3"]';
  ...
  assert.equal(relationshipBlocker.references[0].relationshipType, 'term-relationship-taxonomy');
  assert.equal(relationshipBlocker.references[0].targetResourceKey, samePlanTaxonomyResourceKey);
  assert.equal(JSON.stringify(relationshipBlocker).includes('local-blocked-parent-term'), false);
  assert.equal(JSON.stringify(relationshipBlocker).includes('local-replacement-category-term'), false);
  assert.throws(() => applyPlan(remote, plan), /Refusing to apply/);
});

test('blocks an existing term relationship taxonomy reference owned by an existing wp_navigation post when the same-plan term taxonomy target is itself blocked by a same-plan nav menu taxonomy', () => {
  ...
  local.db.wp_terms = {
    ...
    'term_id:7': {
      term_id: 7,
      name: 'Local replacement navigation category term',
      slug: 'local-replacement-navigation-category-term',
    },
  };
  remote.db.wp_terms = {
    'term_id:6': {
  ...
  assert.equal(mutationFor(plan, relationshipResourceKey), undefined);
  assert.equal(relationshipBlocker.class, 'unsupported-wordpress-graph-surface');
  assert.equal(relationshipBlocker.surface, 'wp_navigation');
  assert.equal(
    JSON.stringify(relationshipBlocker).includes('base-private-existing-navigation-relationship-owner-body'),
    false,
  );
  assert.equal(
    JSON.stringify(relationshipBlocker).includes('local-replacement-navigation-category-term'),
    false,
  );
  assert.throws(() => applyPlan(remote, plan), /Refusing to apply/);
});

test('blocks an existing term relationship taxonomy reference owned by an existing nav_menu_item post when the same-plan term taxonomy target is itself blocked by a same-plan nav menu taxonomy', () => {
  const navigationPostResourceKey = 'row:["wp_posts","ID:6"]';
  const termResourceKey = 'row:["wp_terms","term_id:7"]';
  const existingTaxonomyResourceKey = 'row:["wp_term_taxonomy","term_taxonomy_id:8"]';
  const blockedNavMenuTaxonomyResourceKey = 'row:["wp_term_taxonomy","term_taxonomy_id:9"]';
  ...
  base.db.wp_posts['ID:6'] = {
    ID: 6,
    post_title: 'Existing nav menu item relationship owner',
    post_content: 'base-private-existing-nav-menu-item-relationship-owner-body',
    post_status: 'publish',
    post_type: 'nav_menu_item',
  };
  local.db.wp_posts['ID:6'] = {
  ...
  local.db.wp_terms = {
    ...
    'term_id:7': {
      term_id: 7,
      name: 'Local replacement nav menu item category term',
      slug: 'local-replacement-nav-menu-item-category-term',
    },
  };
  remote.db.wp_terms = {
    'term_id:6': {
  ...
  assert.equal(
    JSON.stringify(relationshipBlocker).includes('base-private-existing-nav-menu-item-relationship-owner-body'),
    false,
  );
  assert.equal(
    JSON.stringify(relationshipBlocker).includes('local-replacement-nav-menu-item-category-term'),
    false,
  );
  assert.throws(() => applyPlan(remote, plan), /Refusing to apply/);
});

test('blocks an existing term relationship taxonomy reference owned by an existing attachment when the same-plan term taxonomy target is itself blocked by a same-plan nav menu taxonomy', () => {
  const attachmentPostResourceKey = 'row:["wp_posts","ID:6"]';
  const termResourceKey = 'row:["wp_terms","term_id:7"]';
  const existingTaxonomyResourceKey = 'row:["wp_term_taxonomy","term_taxonomy_id:8"]';
  const blockedNavMenuTaxonomyResourceKey = 'row:["wp_term_taxonomy","term_taxonomy_id:9"]';
  const samePlanTaxonomyResourceKey = 'row:["wp_term_taxonomy","term_taxonomy_id:10"]';
  ...
  local.db.wp_terms = {
    ...
    'term_id:7': {
      term_id: 7,
      name: 'Local replacement attachment category term',
      slug: 'local-replacement-attachment-category-term',
    },
  };
  remote.db.wp_terms = {
    'term_id:6': {
  ...
  assert.equal(relationshipBlocker.surface, 'attachment');
  assert.equal(
    JSON.stringify(relationshipBlocker).includes('base-private-existing-attachment-relationship-owner-body'),
    false,
  );
  assert.equal(
    JSON.stringify(relationshipBlocker).includes('local-replacement-attachment-category-term'),
    false,
  );
  assert.throws(() => applyPlan(remote, plan), /Refusing to apply/);
});

test('blocks an existing term relationship taxonomy reference owned by an existing attachment when the same-plan term taxonomy target is itself blocked by a same-plan nav menu taxonomy and unrelated remote attachment noise exists', () => {
  const attachmentPostResourceKey = 'row:["wp_posts","ID:6"]';
  const termResourceKey = 'row:["wp_terms","term_id:7"]';
  const existingTaxonomyResourceKey = 'row:["wp_term_taxonomy","term_taxonomy_id:8"]';
  const blockedNavMenuTaxonomyResourceKey = 'row:["wp_term_taxonomy","term_taxonomy_id:9"]';
...
test('blocks an existing term relationship taxonomy reference owned by an existing revision when the same-plan term taxonomy target is itself blocked by a same-plan nav menu taxonomy', () => {
  const revisionPostResourceKey = 'row:["wp_posts","ID:6"]';
  const termResourceKey = 'row:["wp_terms","term_id:7"]';
  const existingTaxonomyResourceKey = 'row:["wp_term_taxonomy","term_taxonomy_id:8"]';
  const blockedNavMenuTaxonomyResourceKey = 'row:["wp_term_taxonomy","term_taxonomy_id:9"]';
  const samePlanTaxonomyResourceKey = 'row:["wp_term_taxonomy","term_taxonomy_id:10"]';
...
test('blocks an existing term relationship taxonomy reference owned by an existing wp_navigation post when the same-plan term taxonomy target is itself blocked by a blocked same-plan parent term', () => {
  const navigationPostResourceKey = 'row:["wp_posts","ID:6"]';
  const blockedParentTermResourceKey = 'row:["wp_terms","term_id:7"]';
  const childTermResourceKey = 'row:["wp_terms","term_id:8"]';
  const existingTaxonomyResourceKey = 'row:["wp_term_taxonomy","term_taxonomy_id:8"]';
  ...
  local.db.wp_terms = {
    ...
    'term_id:7': {
      term_id: 7,
      name: 'Local blocked navigation parent term',
      slug: 'local-blocked-navigation-parent-term',
    },
    'term_id:8': {
      term_id: 8,
      name: 'Local replacement navigation category term',
  ...
  assert.equal(
    JSON.stringify(relationshipBlocker).includes('local-blocked-navigation-parent-term'),
    false,
  );
  assert.equal(
    JSON.stringify(relationshipBlocker).includes('local-replacement-navigation-category-term'),
...
test('blocks an existing term relationship taxonomy reference owned by an existing nav_menu_item post when the same-plan term taxonomy target is itself blocked by a blocked same-plan parent term', () => {
  const navigationPostResourceKey = 'row:["wp_posts","ID:6"]';
  const blockedParentTermResourceKey = 'row:["wp_terms","term_id:7"]';
  const childTermResourceKey = 'row:["wp_terms","term_id:8"]';
  const existingTaxonomyResourceKey = 'row:["wp_term_taxonomy","term_taxonomy_id:8"]';
  ...
  local.db.wp_terms = {
    ...
    'term_id:7': {
      term_id: 7,
      name: 'Local blocked nav menu item parent term',
      slug: 'local-blocked-nav-menu-item-parent-term',
    },
    'term_id:8': {
      term_id: 8,
      name: 'Local replacement nav menu item category term',
  ...
  assert.equal(
    JSON.stringify(relationshipBlocker).includes('local-blocked-nav-menu-item-parent-term'),
    false,
  );
  assert.equal(
    JSON.stringify(relationshipBlocker).includes('local-replacement-nav-menu-item-category-term'),
...
test('blocks an existing term relationship taxonomy reference owned by an existing attachment when the same-plan term taxonomy target is itself blocked by a blocked same-plan parent term', () => {
  const attachmentResourceKey = 'row:["wp_posts","ID:6"]';
  const blockedParentTermResourceKey = 'row:["wp_terms","term_id:7"]';
  const childTermResourceKey = 'row:["wp_terms","term_id:8"]';
  const existingTaxonomyResourceKey = 'row:["wp_term_taxonomy","term_taxonomy_id:8"]';
  ...
  local.db.wp_terms = {
    ...
    'term_id:7': {
      term_id: 7,
      name: 'Local blocked attachment parent term',
      slug: 'local-blocked-attachment-parent-term',
    },
    'term_id:8': {
      term_id: 8,
      name: 'Local replacement attachment category term',
  ...
  assert.equal(
    JSON.stringify(relationshipBlocker).includes('local-blocked-attachment-parent-term'),
    false,
  );
  assert.equal(
    JSON.stringify(relationshipBlocker).includes('local-replacement-attachment-category-term'),
...
test('blocks an existing term relationship taxonomy reference owned by an existing revision when the same-plan term taxonomy target is itself blocked by a blocked same-plan parent term', () => {
  const revisionResourceKey = 'row:["wp_posts","ID:6"]';
  const blockedParentTermResourceKey = 'row:["wp_terms","term_id:7"]';
  const childTermResourceKey = 'row:["wp_terms","term_id:8"]';
  const existingTaxonomyResourceKey = 'row:["wp_term_taxonomy","term_taxonomy_id:8"]';
...
test('blocks a local term relationship taxonomy reference owned by a revision even when it targets a same-plan term taxonomy and unrelated remote attachment noise exists', () => {
  const revisionResourceKey = 'row:["wp_posts","ID:3"]';
  const termResourceKey = 'row:["wp_terms","term_id:7"]';
  const taxonomyResourceKey = 'row:["wp_term_taxonomy","term_taxonomy_id:9"]';
  const relationshipResourceKey = 'row:["wp_term_relationships","object_id:3|term_taxonomy_id:9"]';
  ...
  assert.equal(blocker.surface, 'revision');
  assert.equal(
    JSON.stringify(blocker).includes('local-private-revision-relationship-owner-body'),
    false,
  );
  assert.equal(JSON.stringify(blocker).includes('remote-attachment-noise-body'), false);
});

test('blocks a local term relationship taxonomy reference owned by a nav_menu_item post even when it targets a same-plan term taxonomy', () => {
  const navMenuItemResourceKey = 'row:["wp_posts","ID:3"]';
  const termResourceKey = 'row:["wp_terms","term_id:7"]';
  const taxonomyResourceKey = 'row:["wp_term_taxonomy","term_taxonomy_id:9"]';
  const relationshipResourceKey = 'row:["wp_term_relationships","object_id:3|term_taxonomy_id:9"]';
...
test('blocks a local term relationship taxonomy reference owned by a nav_menu_item post even when it targets a same-plan term taxonomy and unrelated remote attachment noise exists', () => {
  const navMenuItemResourceKey = 'row:["wp_posts","ID:3"]';
  const termResourceKey = 'row:["wp_terms","term_id:7"]';
  const taxonomyResourceKey = 'row:["wp_term_taxonomy","term_taxonomy_id:9"]';
  const relationshipResourceKey = 'row:["wp_term_relationships","object_id:3|term_taxonomy_id:9"]';
...
test('blocks a local term relationship when the same-plan target taxonomy is nav menu even with an unrelated remote attachment', () => {
  const postResourceKey = 'row:["wp_posts","ID:3"]';
  const termResourceKey = 'row:["wp_terms","term_id:7"]';
  const taxonomyResourceKey = 'row:["wp_term_taxonomy","term_taxonomy_id:9"]';
  const relationshipResourceKey = 'row:["wp_term_relationships","object_id:3|term_taxonomy_id:9"]';
...
test('blocks an existing termmeta row when its same-plan term belongs to a nav menu taxonomy', () => {
  const termResourceKey = 'row:["wp_terms","term_id:7"]';
  const taxonomyResourceKey = 'row:["wp_term_taxonomy","term_taxonomy_id:9"]';
  const termmetaResourceKey = 'row:["wp_termmeta","meta_id:12"]';
  const base = baseSite();
...
test('blocks a local term relationship owned by a wp_navigation post even when it targets a same-plan term taxonomy', () => {
  ...
  const plan = planFor(base, local, baseSite());
  const navigationPostMutation = mutationFor(plan, navigationPostResourceKey);
  const taxonomyMutation = mutationFor(plan, taxonomyResourceKey);
  const blocker = plan.blockers.find((entry) => entry.resourceKey === relationshipResourceKey);

  assert.equal(plan.status, 'blocked');
  assert.ok(plan.summary.mutations > 0);
  assert.equal(navigationPostMutation, undefined);
  assert.equal(taxonomyMutation.changeKind, 'create');
  assert.equal(blocker.class, 'unsupported-wordpress-graph-surface');
  assert.equal(blocker.surface, 'wp_navigation');
  assert.equal(blocker.resourceKey, relationshipResourceKey);
  assert.equal(JSON.stringify(blocker).includes('local-private-navigation-post-body'), false);
});
...
test('blocks a local term relationship object owned by an attachment even when it targets a same-plan attachment', () => {
  ...
  local.db.wp_posts['ID:4'] = {
    ID: 4,
    post_title: 'Local tagged attachment',
    post_content: 'local-private-tagged-attachment-body',
    post_status: 'inherit',
    post_type: 'attachment',
  };
  local.db.wp_terms = {
  ...
  const plan = planFor(base, local, baseSite());
  const attachmentMutation = mutationFor(plan, attachmentPostResourceKey);
  const taggedAttachmentMutation = mutationFor(plan, taggedAttachmentResourceKey);
  const termMutation = mutationFor(plan, termResourceKey);
  const taxonomyMutation = mutationFor(plan, taxonomyResourceKey);
  const blocker = plan.blockers.find((entry) => entry.resourceKey === relationshipResourceKey);
  ...
  assert.equal(plan.status, 'blocked');
  assert.equal(attachmentMutation.changeKind, 'create');
  assert.equal(taggedAttachmentMutation.changeKind, 'create');
  assert.equal(termMutation.changeKind, 'create');
  assert.equal(taxonomyMutation.changeKind, 'create');
  assert.equal(blocker.class, 'unsupported-wordpress-graph-surface');
  assert.equal(blocker.surface, 'attachment');
  ...
  assert.equal(
    JSON.stringify(blocker).includes('local-private-tagged-attachment-body'),
    false,
  );
});
...
test('blocks a local term relationship owned by a revision even when it targets a same-plan term taxonomy', () => {
  const revisionPostResourceKey = 'row:["wp_posts","ID:5"]';
  const taxonomyResourceKey = 'row:["wp_term_taxonomy","term_taxonomy_id:9"]';
  const relationshipResourceKey = 'row:["wp_term_relationships","object_id:5|term_taxonomy_id:9"]';
  const base = baseSite();
  ...
  assert.equal(blocker.surface, 'revision');
  assert.equal(
    JSON.stringify(blocker).includes('local-private-revision-post-body'),
    false,
  );
});

test('blocks a local term relationship owned by an existing attachment even when it targets a same-plan term taxonomy', () => {
  const attachmentPostResourceKey = 'row:["wp_posts","ID:5"]';
  const taxonomyResourceKey = 'row:["wp_term_taxonomy","term_taxonomy_id:9"]';
  const relationshipResourceKey = 'row:["wp_term_relationships","object_id:5|term_taxonomy_id:9"]';
  const base = baseSite();
  const local = baseSite();
  ...
  base.db.wp_posts['ID:5'] = {
    ID: 5,
    post_title: 'Existing attachment post',
    post_content: 'base-private-existing-attachment-post-body',
    post_status: 'inherit',
    post_type: 'attachment',
  };
  local.db.wp_posts['ID:5'] = {
  ...
  assert.equal(blocker.surface, 'attachment');
  assert.equal(
    JSON.stringify(blocker).includes('base-private-existing-attachment-post-body'),
    false,
  );
  assert.throws(() => applyPlan(remote, plan), /Refusing to apply/);
});

test('blocks a local term relationship owned by an existing revision even when it targets a same-plan term taxonomy', () => {
  const revisionPostResourceKey = 'row:["wp_posts","ID:5"]';
  const taxonomyResourceKey = 'row:["wp_term_taxonomy","term_taxonomy_id:9"]';
  const relationshipResourceKey = 'row:["wp_term_relationships","object_id:5|term_taxonomy_id:9"]';
  const base = baseSite();
  ...
  base.db.wp_posts['ID:5'] = {
    ID: 5,
    post_title: 'Existing revision post',
    post_content: 'base-private-existing-revision-post-body',
    post_status: 'inherit',
    post_type: 'revision',
  };
  local.db.wp_posts['ID:5'] = {
  ...
  assert.equal(blocker.surface, 'revision');
  assert.equal(
    JSON.stringify(blocker).includes('base-private-existing-revision-post-body'),
    false,
  );
  assert.throws(() => applyPlan(remote, plan), /Refusing to apply/);
});

test('blocks a local term relationship owned by an existing nav_menu_item post even when it targets a same-plan term taxonomy', () => {
  const navigationPostResourceKey = 'row:["wp_posts","ID:6"]';
  const taxonomyResourceKey = 'row:["wp_term_taxonomy","term_taxonomy_id:9"]';
  ...
  base.db.wp_posts['ID:6'] = {
    ID: 6,
    post_title: 'Existing nav menu item term post',
    post_content: 'base-private-existing-nav-menu-item-term-post-body',
    post_status: 'publish',
    post_type: 'nav_menu_item',
  };
  local.db.wp_posts['ID:6'] = {
  ...
  local.db.wp_terms = {
    'term_id:7': {
      term_id: 7,
      name: 'Local term for nav menu item',
      slug: 'local-term-for-nav-menu-item',
    },
  };
  local.db.wp_term_taxonomy = {
    'term_taxonomy_id:9': {
  ...
  assert.equal(blocker.surface, 'nav_menu_item');
  assert.equal(
    JSON.stringify(blocker).includes('base-private-existing-nav-menu-item-term-post-body'),
    false,
  );
  assert.throws(() => applyPlan(remote, plan), /Refusing to apply/);
});
...
test('blocks a local term relationship owned by a navigation post even when it targets a same-plan post', () => {
  ...
  local.db.wp_posts['ID:6'] = {
    ID: 6,
    post_title: 'Local navigation relationship post',
    post_content: 'local-private-navigation-relationship-body',
    post_status: 'publish',
    post_type: 'wp_navigation',
  };
  local.db.wp_posts['ID:4'] = {
  ...
  assert.equal(relationshipBlocker.surface, 'wp_navigation');
  assert.equal(
    JSON.stringify(relationshipBlocker).includes('local-private-navigation-relationship-body'),
    false,
  );
});
...
test('blocks a local term relationship owned by an existing wp_navigation post even when it targets a same-plan term taxonomy', () => {
  const navigationPostResourceKey = 'row:["wp_posts","ID:6"]';
  const taxonomyResourceKey = 'row:["wp_term_taxonomy","term_taxonomy_id:9"]';
  const relationshipResourceKey = 'row:["wp_term_relationships","object_id:6|term_taxonomy_id:9"]';
  const base = baseSite();
...
test('allows a local termmeta reference to a term created by the same plan even when an unrelated remote attachment exists', () => {
  const termmetaResourceKey = 'row:["wp_termmeta","meta_id:12"]';
  const base = baseSite();
  const local = baseSite();
  local.db.wp_terms = {
    'term_id:5': {
      term_id: 5,
      name: 'Local termmeta term',
      slug: 'local-termmeta-term',
    },
  };
  local.db.wp_termmeta = {
    'meta_id:12': {
...
test('blocks an existing termmeta row when it retargets to a same-plan term already used by a remote nav menu taxonomy', () => {
  const termmetaResourceKey = 'row:["wp_termmeta","meta_id:14"]';
  const termResourceKey = 'row:["wp_terms","term_id:5"]';
  const base = baseSite();
  const local = baseSite();
...
test('blocks an existing termmeta row when it retargets to a same-plan term already used by a remote nav menu taxonomy and unrelated remote attachment noise exists', () => {
  const termmetaResourceKey = 'row:["wp_termmeta","meta_id:14"]';
  const termResourceKey = 'row:["wp_terms","term_id:5"]';
  const base = baseSite();
  const local = baseSite();
...
test('blocks an existing termmeta row when its same-plan term is claimed by a nav menu taxonomy and a sibling category taxonomy depends on the same blocked term', () => {
  const termResourceKey = 'row:["wp_terms","term_id:7"]';
  const navMenuTaxonomyResourceKey = 'row:["wp_term_taxonomy","term_taxonomy_id:9"]';
  const categoryTaxonomyResourceKey = 'row:["wp_term_taxonomy","term_taxonomy_id:10"]';
  const termmetaResourceKey = 'row:["wp_termmeta","meta_id:12"]';
  ...
  assert.equal(termmetaBlocker.class, 'unsupported-wordpress-graph-surface');
  assert.equal(termmetaBlocker.surface, 'nav_menu');
  assert.equal(JSON.stringify(termmetaBlocker).includes('local-private-existing-term-note'), false);
  assert.equal(JSON.stringify(termmetaBlocker).includes('local-shared-term'), false);
  assert.throws(() => applyPlan(remote, plan), /Refusing to apply/);
});
...
test('blocks a local post parent reference when the same-plan post target is itself blocked by a wp_navigation parent', () => {
  const navigationResourceKey = 'row:["wp_posts","ID:2"]';
  const blockedParentResourceKey = 'row:["wp_posts","ID:3"]';
  const childResourceKey = 'row:["wp_posts","ID:4"]';
  const base = baseSite();
...
test('blocks a local post parent reference owned by an existing revision even when it targets a same-plan post', () => {
  const revisionResourceKey = 'row:["wp_posts","ID:3"]';
  const parentResourceKey = 'row:["wp_posts","ID:4"]';
  const base = baseSite();
  const local = baseSite();
...
test('blocks a local post parent reference owned by an existing wp_navigation post even when it targets a same-plan post', () => {
  const navigationResourceKey = 'row:["wp_posts","ID:3"]';
  const parentResourceKey = 'row:["wp_posts","ID:4"]';
  const base = baseSite();
  const local = baseSite();
...
test('blocks an existing attachment parent reference when the same-plan post target is itself blocked by a wp_navigation parent', () => {
  const attachmentResourceKey = 'row:["wp_posts","ID:2"]';
  const navigationResourceKey = 'row:["wp_posts","ID:3"]';
  const blockedParentResourceKey = 'row:["wp_posts","ID:4"]';
  const base = baseSite();
...
test('blocks an existing attachment parent reference when the same-plan post target is itself blocked by a nav_menu_item parent', () => {
  const attachmentResourceKey = 'row:["wp_posts","ID:2"]';
  const menuItemResourceKey = 'row:["wp_posts","ID:3"]';
  const blockedParentResourceKey = 'row:["wp_posts","ID:4"]';
  const base = baseSite();
...
test('blocks an existing attachment parent reference when the same-plan attachment target is itself blocked by a revision parent', () => {
  const attachmentResourceKey = 'row:["wp_posts","ID:2"]';
  const revisionResourceKey = 'row:["wp_posts","ID:3"]';
...
test('blocks an existing attachment parent reference when the same-plan attachment target is itself blocked by a wp_navigation parent', () => {
  const attachmentResourceKey = 'row:["wp_posts","ID:2"]';
  const navigationResourceKey = 'row:["wp_posts","ID:3"]';
  const blockedTargetResourceKey = 'row:["wp_posts","ID:4"]';
  const base = baseSite();
...
test('blocks a local menu item parent reference owned by an attachment even when it targets a same-plan post', () => {
  const attachmentResourceKey = 'row:["wp_posts","ID:3"]';
  const parentResourceKey = 'row:["wp_posts","ID:4"]';
  const postmetaResourceKey = 'row:["wp_postmeta","meta_id:21"]';
  const base = baseSite();
  const local = baseSite();
  local.db.wp_posts['ID:3'] = {
    ID: 3,
  ...
  assert.equal(blocker.surface, 'attachment');
  assert.equal(
    JSON.stringify(blocker).includes('local-private-attachment-menu-item-body'),
    false,
  );
  assert.equal(
    JSON.stringify(blocker).includes('local-private-parent-body'),
...
test('blocks a local menu item parent reference owned by a revision even when it targets a same-plan post', () => {
  const revisionResourceKey = 'row:["wp_posts","ID:3"]';
  const parentResourceKey = 'row:["wp_posts","ID:4"]';
  const postmetaResourceKey = 'row:["wp_postmeta","meta_id:22"]';
  const base = baseSite();
  const local = baseSite();
  local.db.wp_posts['ID:3'] = {
    ID: 3,
    post_title: 'Local revision menu item',
    post_content: 'local-private-revision-menu-item-body',
    post_status: 'inherit',
    post_type: 'revision',
    post_parent: 4,
  };
  ...
  assert.equal(blocker.surface, 'revision');
  assert.equal(
    JSON.stringify(blocker).includes('local-private-revision-menu-item-body'),
    false,
  );
  assert.equal(
    JSON.stringify(blocker).includes('local-private-parent-body'),
