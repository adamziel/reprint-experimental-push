...
const evidencePath = path.join(repoRoot, 'docs/evidence/rpp-0925-objective-audit-update-v2.md');
const fixedNowIso = '2026-06-01T02:20:00.000Z';
const auditedLaneHead = 'ef7057d8fc9b09f79e08281cbbebdfda8d41db13';
...
test('RPP-0925 evidence is support-only and keeps the final release verdict held', () => {
  const { report, text } = loadEvidenceReport();

  assert.match(text, /^# RPP-0925 objective audit update v2$/m);
  ...
  assert.equal(report.rppId, 'RPP-0925');
  assert.equal(report.proofId, 'rpp-0925-objective-audit-update-v2');
  ...
  assert.equal(report.auditedBranch, 'session/rpp-925');
  assert.equal(report.auditedLaneHeadBeforeEvidence, auditedLaneHead);
});

test('RPP-0925 final go/no-go record names every current final-release risk and closes none', () => {
...
test('RPP-0925 carries forward R1-R16 objective risks without production-backed closure proof', () => {
  const { report } = loadEvidenceReport();
...
test('RPP-0925 release-gate snapshot matches the current held final-release evaluator', () => {
  const { report } = loadEvidenceReport();
...
test('RPP-0925 evidence remains redacted and records no release-gate movement', async () => {
  const { report, text } = loadEvidenceReport();
  const scan = await scanArtifacts([
    'docs/evidence/rpp-0925-objective-audit-update-v2.md',
  ], { cwd: repoRoot });

  assert.doesNotThrow(() =>
    assertEvidenceHasNoRawValues(report, { label: 'RPP-0925 objective audit update v2' }));
...
function loadEvidenceReport() {
  ...
  assert.ok(match?.groups?.json, 'RPP-0925 evidence must contain one JSON record block');
  return {
    text,
