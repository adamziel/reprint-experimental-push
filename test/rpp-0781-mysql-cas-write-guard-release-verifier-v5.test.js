...
const releaseVerifierSupportGateIds = [
...
test('RPP-0781 release verifier variant 5 benchmark command reports runtime, resources, and pass gates', () => {
  ...
  const proof = buildReleaseVerifierSupportProof(report, successIterations);
  assert.equal(proof.rppId, 'RPP-0781');
  assert.equal(proof.variant, 5);
  assert.equal(proof.evidenceSource, 'mysql-cas-write-guard-release-verifier-v5');
...
test('RPP-0781 release verifier variant 5 benchmark command exposes fail gates with runtime and resources', () => {
  const result = runBenchmarkCommand([
    '--iterations',
  ...
  assert.equal(failGateProof.rppId, 'RPP-0781');
  assert.equal(failGateProof.variant, 5);
  assert.equal(failGateProof.evidenceSource, 'mysql-cas-write-guard-release-verifier-v5');
  assert.deepEqual(failGateProof.failGateIds, ['runtime-resource-budget']);
  ...
  assertNoRawOrPrivateEvidence(report);
  assertNoRawOrPrivateEvidence(failGateProof);
});
...
function buildReleaseVerifierSupportProof(report, iterations) {
  ...
  const proof = {
    rppId: 'RPP-0781',
    variant: 5,
    evidenceSource: 'mysql-cas-write-guard-release-verifier-v5',
    supportOnly: true,
    ...
    releaseVerifier: {
      ...
      mode: report.mode,
      command: 'node scripts/bench/mysql-cas-write-guard.js',
...
function buildFailGateReleaseVerifierProof(report) {
  return {
    ...
    failGateIds: report.gates.filter((gate) => gate.status === 'fail').map((gate) => gate.id),
    failedBudgetGate: {
      id: 'runtime-resource-budget',
      heapUsedBytes: gateById(report, 'runtime-resource-budget').metrics.heapUsedBytes,
      maxHeapUsedBytes: gateById(report, 'runtime-resource-budget').metrics.maxHeapUsedBytes,
      durationMs: gateById(report, 'runtime-resource-budget').metrics.durationMs,
    },
  };
}
