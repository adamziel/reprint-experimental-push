...
function baseSite(optionMode = 'rpp-0459-base-plugin-owned-option') {
  return {
    files: {
      [formsPluginFilePath]: '<?php /* rpp-0459 forms plugin base */',
    },
    plugins: {
      forms: {
        version: '1.0.0',
...
function assertNoRawOptionPayloads(value, rawValues, label) {
  ...
  for (const rawValue of rawValues) {
    assert.equal(serialized.includes(rawValue), false, `${label} leaked raw payload ${rawValue}`);
  }
  assert.equal(serialized.includes('option_value'), false, `${label} must not include raw option_value fields`);
  assert.equal(serialized.includes('__pluginOwner'), false, `${label} must not include raw owner marker fields`);
}
...
test('RPP-0459 remote owner-context drift preserves plugin-owned remote data with hash-only audit evidence', () => {
  const baseRaw = 'rpp-0459-owner-context-base-option';
  const localRaw = 'rpp-0459-owner-context-local-option';
  const remotePluginFileDrift = '<?php /* rpp-0459 remote owner context drift */';
  ...
  const rowHashBefore = `sha256:${resourceHash(remote, formsOptionResource())}`;
  ...
  const proof = {
    rpp: 'RPP-0459',
    evidenceSource: 'local-plugin-driver-audit-evidence-redaction-v3-owner-context-drift',
    ...
    blockedOwnerContextDrift: {
      ...
      decision: blocker.driverAuditEvidence.decision,
      driverDecisionEvidenceHash: sha256Evidence(blocker.driverAuditEvidence),
      ...
      applyRefusalCode: applyError.code,
      applyRefusalDetailsHash: sha256Evidence(applyError.details),
      beforeMutationCalls,
      rowHashBefore,
      rowHashAfter: `sha256:${resourceHash(remote, blocker.resource)}`,
      remoteHashBefore,
      remoteHashAfter: sha256Evidence(remote),
      remoteDataPreserved: JSON.stringify(remote) === remoteBeforeJson,
    },
  };
  proof.proofHash = sha256Evidence(proof.blockedOwnerContextDrift);
  ...
  assertNoRawOptionPayloads(blocker, rawValues, 'RPP-0459 owner-context blocker');
  assertNoRawOptionPayloads(applyError.details, rawValues, 'RPP-0459 owner-context apply refusal');
  assertNoRawOptionPayloads(proof, rawValues, 'RPP-0459 owner-context proof');
  assert.doesNotThrow(() => assertEvidenceHasNoRawValues(blocker, { label: 'RPP-0459 owner-context blocker evidence' }));
  assert.doesNotThrow(() => assertEvidenceHasNoRawValues(applyError.details, { label: 'RPP-0459 owner-context apply refusal' }));
  assert.doesNotThrow(() => assertEvidenceHasNoRawValues(proof, { label: 'RPP-0459 owner-context proof' }));
});

test('RPP-0459 stale remote row drift preserves plugin-owned remote data while audit evidence stays redacted', () => {
  const baseRaw = 'rpp-0459-stale-base-option';
  const localRaw = 'rpp-0459-stale-local-option';
  const remoteDriftRaw = 'rpp-0459-stale-remote-drift-option';
  const remoteOnlyRaw = 'rpp-0459-stale-remote-only-field';
  ...
  const proof = {
    rpp: 'RPP-0459',
    evidenceSource: 'local-plugin-driver-audit-evidence-redaction-v3-row-drift',
    productionBacked: false,
    releaseGate: 'NO-GO',
    rawValuesIncluded: false,
    acceptedDriverAudit: {
    ...
    staleRemotePreservation: {
      ...
      actualHashMatchesRowBefore: staleError.details.actualHash === driftedRowBeforeHash,
      ...
      remoteDataPreserved: JSON.stringify(driftedRemote) === driftedRemoteBeforeJson,
    },
  };
  ...
  assert.match(proof.acceptedDriverAudit.mutationHash, sha256EvidencePattern);
  assert.match(proof.staleRemotePreservation.preconditionDetailsHash, sha256EvidencePattern);
  assert.match(proof.proofHash, sha256EvidencePattern);
  assertNoRawOptionPayloads(plannerAuditEvidence, rawValues, 'RPP-0459 planner audit evidence');
  assertNoRawOptionPayloads(driverDecisionEvidence, rawValues, 'RPP-0459 driver decision evidence');
  assertNoRawOptionPayloads(staleError.details, rawValues, 'RPP-0459 stale precondition details');
  assertNoRawOptionPayloads(proof, rawValues, 'RPP-0459 stale preservation proof');
  assert.doesNotThrow(() => assertEvidenceHasNoRawValues(plannerAuditEvidence, { label: 'RPP-0459 planner audit evidence' }));
  assert.doesNotThrow(() => assertEvidenceHasNoRawValues(driverDecisionEvidence, { label: 'RPP-0459 driver decision evidence' }));
  assert.doesNotThrow(() => assertEvidenceHasNoRawValues(staleError.details, { label: 'RPP-0459 stale precondition details' }));
  assert.doesNotThrow(() => assertEvidenceHasNoRawValues(proof, { label: 'RPP-0459 stale preservation proof' }));
});
