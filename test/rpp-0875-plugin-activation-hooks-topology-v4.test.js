...
const evidencePath = path.join(
  repoRoot,
  'docs/evidence/rpp-0875-plugin-activation-hooks-topology-v4.md',
);
...
const rawNeedles = Object.freeze([
  'rpp-0875-unproven-private-hook-state',
  'rpp-0875-driver-proofed-private-hook-state',
...
test('RPP-0875 progress report records candidate versus release-ready activation hook scope', () => {
  const { report, text } = loadProgressReport();

  assert.equal(report.schemaVersion, 1);
  assert.equal(report.rppId, 'RPP-0875');
  ...
  assert.equal(report.candidateScope.status, 'activation-hook-topology-candidate-v4');
  assert.equal(report.candidateScope.sourcePattern, 'rpp-0855-candidate-scope');
...
test('RPP-0875 activation-hook surfaces are deterministic surface/count/hash evidence', () => {
...
test('RPP-0875 dependency and side-effect boundaries match existing guardrails', () => {
  const { report } = loadProgressReport();
...
test('RPP-0875 existing activation-hook guardrails stay support-only and no-go', () => {
  const { report } = loadProgressReport();
...
test('RPP-0875 evidence remains hash/count/surface only', () => {
  const { report, text } = loadProgressReport();

  assert.deepEqual(findEvidenceRedactionIssues(report), []);
  assert.doesNotThrow(() =>
    assertEvidenceHasNoRawValues(report, { label: 'RPP-0875 activation hook topology progress report' }));
  ...
  for (const needle of rawNeedles) {
    assert.equal(text.includes(needle), false, `RPP-0875 evidence leaked raw fixture value ${needle}`);
  }
...
function loadProgressReport() {
  ...
  assert.ok(match?.groups?.json, 'RPP-0875 evidence must contain one JSON progress report block');
  return {
    text,
...
function buildActivePluginsDirectMutationSummary() {
  ...
  plan.mutations.push({
    id: 'mutation-rpp-0875-active-plugins-direct',
...
function productionPluginDriverSnapshot(mode, version, marker) {
  ...
  allowlistEntry.registeredDriverProvenanceEvidence = pluginOwnedRowDriverRegistrationProvenanceEvidence(
    allowlistEntry,
    {
      source: 'rpp-0875-release-state-driver-registry',
      evidenceScope: 'activation-hook-topology-v4',
    },
  );
...
function addActivationHookSideEffectMutation(plan, { withDriverProof }) {
  plan.mutations.push({
    id: `mutation-rpp-0875-activation-hook-side-effect-${plan.mutations.length + 1}`,
    resourceKey: activationHookSideEffectResourceKey,
    resource: {
      type: 'row',
      table: 'wp_options',
    ...
    pluginOwnedResource: {
      pluginOwner: productionPluginDriverBoundary.owner,
      driver: 'wp-option',
      policySource: 'rpp-0875-activation-hook-side-effect-proof',
      supportsDelete: false,
