...
const rawFixtures = Object.freeze({
  basePluginFile: 'rpp-0846-base-woocommerce-plugin-file',
  baseLegacyTitle: 'rpp-0846-base-private-legacy-title',
  localLegacyTitle: 'rpp-0846-local-private-legacy-title',
  baseBillingEmail: 'rpp-0846-base-buyer@example.test',
  localBillingEmail: 'rpp-0846-local-buyer@example.test',
  baseOrderKey: 'wc_order_rpp_0846_base_private_key',
  localOrderKey: 'wc_order_rpp_0846_local_private_key',
  baseAddress: 'rpp-0846-base-private-address',
  localAddress: 'rpp-0846-local-private-address',
  baseLineItem: 'rpp-0846-base-private-line-item',
  localLineItem: 'rpp-0846-local-private-line-item',
  baseLineMeta: 'rpp-0846-base-private-line-meta',
  localLineMeta: 'rpp-0846-local-private-line-meta',
  baseHposMeta: 'rpp-0846-base-private-hpos-meta',
  localHposMeta: 'rpp-0846-local-private-hpos-meta',
});

test('RPP-0846 builds hash-count-surface-only WooCommerce order refusal proof', () => {
  ...
  assert.equal(proof.rppId, 'RPP-0846');
  ...
  for (const raw of Object.values(rawFixtures)) {
    assert.equal(serializedProof.includes(raw), false, `RPP-0846 proof leaked raw order fixture ${raw}`);
  }
  assert.doesNotThrow(() => assertEvidenceHasNoRawValues(proof, {
    label: 'RPP-0846 WooCommerce order refusal proof',
  }));
});

test('RPP-0846 refuses every forged WooCommerce order mutation before mutation-capable work', () => {
  const base = baseSite();
  ...
  for (const { surfaceEntry, forgedPlan, refusal } of results) {
    ...
    assert.doesNotThrow(() => assertEvidenceHasNoRawValues(refusal.error.details, {
      label: `RPP-0846 forged ${surfaceEntry.surfaceId} refusal details`,
    }));
  }
});

test('RPP-0846 topology command artifact records exact unavailable capability fail-closed', () => {
  ...
  assert.doesNotThrow(() => assertEvidenceHasNoRawValues(proof, {
    label: 'RPP-0846 topology unavailable-capability proof',
  }));
});

test('RPP-0846 validation rejects non-started topology without an exact capability code', () => {
  ...
  assert.ok(validation.failures.some((failure) =>
    failure.code === 'RPP_0846_TOPOLOGY_UNAVAILABLE_CAPABILITY_NOT_EXACT'));
});
...
function forgedReadyOrderPlan(base, local, remote, surfaceEntry) {
  ...
  return {
    schemaVersion: 1,
    id: `plan-rpp-0846-forged-${surfaceEntry.surfaceId}`,
...
function buildWooCommerceOrderSafetyProof() {
  ...
  const proofCore = {
    ...
    rppId: 'RPP-0846',
    variant: 3,
    proofId: 'rpp-0846-woocommerce-order-safety-refusal-v3',
    checkedAt: fixedNow,
...
function buildMissingDockerCapabilityArtifact() {
  const plan = buildDockerTopologyPlan({
    ...
    workDir: '/tmp/rpp-0846-docker-work',
    evidenceDir: '/tmp/rpp-0846-docker-evidence',
    env: {},
  });
...
function validateWooCommerceOrderSafetyProof(proof) {
  const failures = [];
  if (proof.rppId && proof.rppId !== 'RPP-0846') {
    failures.push({ code: 'RPP_0846_ID_MISMATCH' });
  }
  if (proof.variant && proof.variant !== 3) {
    failures.push({ code: 'RPP_0846_VARIANT_MISMATCH' });
  }
  if (proof.supportOnly === false || proof.productionBacked === true || proof.releaseEligible === true) {
    failures.push({ code: 'RPP_0846_SUPPORT_ONLY_RELEASE_STATE_INVALID' });
  }
  ...
  if (orderSafety.surfaceCount !== expectedSurfaceCount) {
    failures.push({ code: 'RPP_0846_ORDER_SURFACE_COUNT_MISMATCH' });
  }
  ...
  if (counts.base !== expectedSurfaceCount || counts.local !== expectedSurfaceCount || counts.remote !== expectedSurfaceCount) {
    failures.push({ code: 'RPP_0846_ORDER_ROWS_NOT_PLUGIN_OWNED' });
  }
  ...
  if (plan.status !== 'blocked'
    ...
    failures.push({ code: 'RPP_0846_ORDER_PLAN_NOT_FAIL_CLOSED' });
  }
  ...
  if (blockers.count !== expectedSurfaceCount
    ...
    failures.push({ code: 'RPP_0846_ORDER_BLOCKERS_NOT_EXACT' });
  }
  if (!Array.isArray(blockers.bySurface)
    || blockers.bySurface.some((entry) =>
      ...
      || !sha256EvidencePattern.test(entry.blockerHash || '')
      || !sha256EvidencePattern.test(entry.refusalEvidenceHash || ''))) {
    failures.push({ code: 'RPP_0846_ORDER_BLOCKER_EVIDENCE_NOT_HASH_ONLY' });
  }
  if (proof.applyRefusal?.code !== 'PLAN_NOT_READY'
    || proof.applyRefusal?.beforeMutationCalls !== 0
    || proof.applyRefusal?.remoteUnchanged !== true) {
    failures.push({ code: 'RPP_0846_BLOCKED_PLAN_APPLY_NOT_REFUSED_PRE_MUTATION' });
  }
  ...
  if (forged.count !== expectedSurfaceCount
    ...
    failures.push({ code: 'RPP_0846_FORGED_MUTATIONS_NOT_REFUSED_PRE_MUTATION' });
  }
  ...
  for (const key of [
    ...
    if (invariants[key] !== true) {
      failures.push({ code: `RPP_0846_INVARIANT_FAILED_${key}` });
    }
  }
