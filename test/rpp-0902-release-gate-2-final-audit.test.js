...
const evidencePath = path.join(repoRoot, 'docs/evidence/rpp-0902-release-gate-2-final-audit.md');
const releaseGatesPath = path.join(repoRoot, '.agents/RELEASE_GATES.md');
const fixedNowIso = '2026-06-01T01:26:00.000Z';
const auditedHead = '959e89d8354463cab70116195c92374e826bf839';
const supportBoundaryVerdict = 'PACKAGED_RELEASE_BOUNDARY_OK';
const liveBoundaryVerdict = 'LIVE_RELEASE_BOUNDARY_OK';

test('RPP-0902 audit report records GATE-2 support_only and final release NO-GO', () => {
  const { report, text } = loadAuditReport();

  assert.deepEqual(report, expectedAuditReport());
  assert.equal(report.rppId, 'RPP-0902');
...
test('RPP-0902 current release gate row keeps durable recovery journal support_only', () => {
...
test('RPP-0902 final release evaluator fails closed without production-backed durable recovery evidence', () => {
...
test('RPP-0902 durable recovery proof only reports proven on a production-backed live boundary', () => {
...
function loadAuditReport() {
  ...
  assert.ok(match?.groups?.json, 'RPP-0902 evidence must contain one JSON audit report block');
  return {
    text,
...
function buildReleaseSummary(boundaryVerdict) {
  const activeClaimId = 'claim-active-rpp-0902';
  const previousClaimId = 'claim-previous-rpp-0902';
