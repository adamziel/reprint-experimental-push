...
const evidencePath = path.join(
  repoRoot,
  'docs/evidence/rpp-0835-plugin-activation-hooks-topology-v2.md',
);
...
const rawNeedles = Object.freeze([
  'rpp-0835-unproven-private-hook-state',
  'rpp-0835-driver-proofed-private-hook-state',
...
test('RPP-0835 progress report records candidate versus release-ready activation hook scope', () => {
  const { report, text } = loadProgressReport();

  assert.equal(report.schemaVersion, 1);
  assert.equal(report.rppId, 'RPP-0835');
  ...
  assert.equal(report.candidateScope.status, 'activation-hook-topology-candidate-v2');
  assert.equal(report.candidateScope.sourcePattern, 'rpp-0815-candidate-scope');
...
test('RPP-0835 activation-hook surfaces are deterministic surface/count/hash evidence', () => {
...
test('RPP-0835 dependency and side-effect boundaries match existing guardrails', () => {
  const { report } = loadProgressReport();
...
test('RPP-0835 existing activation-hook guardrails stay support-only and no-go', () => {
  const { report } = loadProgressReport();
...
test('RPP-0835 evidence remains hash/count/surface only', () => {
  const { report, text } = loadProgressReport();

  assert.deepEqual(findEvidenceRedactionIssues(report), []);
  assert.doesNotThrow(() =>
    assertEvidenceHasNoRawValues(report, { label: 'RPP-0835 activation hook topology progress report' }));
  ...
  for (const needle of rawNeedles) {
    assert.equal(text.includes(needle), false, `RPP-0835 evidence leaked raw fixture value ${needle}`);
  }
...
function loadProgressReport() {
  ...
  assert.ok(match?.groups?.json, 'RPP-0835 evidence must contain one JSON progress report block');
  return {
    text,
...
function buildActivePluginsDirectMutationSummary() {
  ...
  plan.mutations.push({
    id: 'mutation-rpp-0835-active-plugins-direct',
...
function addActivationHookSideEffectMutation(plan, { withDriverProof }) {
  plan.mutations.push({
    id: `mutation-rpp-0835-activation-hook-side-effect-${plan.mutations.length + 1}`,
    resourceKey: activationHookSideEffectResourceKey,
    resource: {
      type: 'row',
      table: 'wp_options',
    ...
    pluginOwnedResource: {
      pluginOwner: productionPluginDriverBoundary.owner,
      driver: 'wp-option',
      policySource: 'rpp-0835-activation-hook-side-effect-proof',
      supportsDelete: false,
