...
const evidencePath = path.join(repoRoot, 'docs/evidence/rpp-0979-post-release-monitoring-plan-v4.md');
const patternEvidencePath = path.join(repoRoot, 'docs/evidence/rpp-0959-post-release-monitoring-plan-v3.md');
const planPath = path.join(repoRoot, 'docs/operations/post-release-monitoring-plan.md');

const auditedLaneHead = 'ef4ca2a80106bb92cb8a99490d1a60507233c0a1';
...
const expectedValidation = Object.freeze([
  'node --check test/rpp-0979-post-release-monitoring-plan-v4.test.js',
  'node --test --test-name-pattern RPP-0979 test/rpp-0979-post-release-monitoring-plan-v4.test.js',
  'node scripts/release/artifact-redaction-scan.mjs docs/evidence/rpp-0979-post-release-monitoring-plan-v4.md',
  'git diff --check',
]);

test('RPP-0979 evidence records a held support-only monitoring v4 contract', () => {
  const { report, text } = loadMarkdownJson(evidencePath);

  assert.match(text, /^# RPP-0979 Post-Release Monitoring Plan Evidence Variant 4$/m);
  assert.match(text, /Audited local branch: `session\/rpp-979`/);
  assert.match(text, /Audited lane head before this evidence file: `ef4ca2a80106bb92cb8a99490d1a60507233c0a1`/);
  assert.match(text, /final release\s+remains `NO-GO`/);
  assert.match(text, /no release-gate status\s+movement is allowed/);

  assert.equal(report.schemaVersion, 1);
  assert.equal(report.rppId, 'RPP-0979');
  assert.equal(report.proofId, 'rpp-0979-post-release-monitoring-plan-v4');
  ...
  assert.equal(report.auditedBranch, 'session/rpp-979');
...
test('RPP-0979 names operator recovery prerequisites and safe evidence', () => {
  ...
  assert.equal(report.documents.patternEvidence, 'docs/evidence/rpp-0959-post-release-monitoring-plan-v3.md');
  ...
  assert.deepEqual(report.monitoringContract, pattern.monitoringContract);
  ...
  assert.deepEqual(report.safeRecoveryEvidence, pattern.safeRecoveryEvidence);
...
test('RPP-0979 names stop conditions and rollback escalation blockers', () => {
  const pattern = loadMarkdownJson(patternEvidencePath).report;
  const { report, text } = loadMarkdownJson(evidencePath);
  const plan = readText(planPath);
  const normalizedPlan = normalize(plan);
  ...
  assert.deepEqual(report.rollbackEscalationBlockers, pattern.rollbackEscalationBlockers);
...
test('RPP-0979 blocks hidden assumptions before activation finalization or release movement', () => {
  const pattern = loadMarkdownJson(patternEvidencePath).report;
  const { report, text } = loadMarkdownJson(evidencePath);
  const plan = readText(planPath);
  const normalizedPlan = normalize(plan);
  ...
  assert.deepEqual(report.hiddenAssumptionGuards, pattern.hiddenAssumptionGuards);
  assert.deepEqual(report.lifecycleGuards, pattern.lifecycleGuards);
...
test('RPP-0979 evidence remains redacted and causes no release-gate status movement', async () => {
  const pattern = loadMarkdownJson(patternEvidencePath).report;
  const { report, text } = loadMarkdownJson(evidencePath);
  ...
  assert.doesNotThrow(() =>
    assertEvidenceHasNoRawValues(report, { label: 'RPP-0979 post-release monitoring v4 evidence' }));
  ...
  assert.deepEqual(report.proofGaps, pattern.proofGaps);
  ...
  assert.deepEqual(report.redactionPosture, pattern.redactionPosture);
