...
const evidencePath = path.join(repoRoot, 'docs/evidence/rpp-0928-privacy-redaction-review-v2.md');
const fixedNow = '2026-05-28T08:30:00.000Z';
const auditedLaneHead = '8ebfd2b38fb8c10d31cc78640a43d9ce2a816b5e';
...
function loadEvidenceReport() {
  ...
  assert.ok(match?.groups?.json, 'RPP-0928 evidence must contain one JSON report block');
  return {
    text,
...
function makeArtifactRoot(t, fixture) {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'rpp-928-redaction-artifact-'));
  t.after(() => {
...
function writeRequiredCheckFixture(t, fixture) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'rpp-928-required-checks-'));
  t.after(() => {
...
test('RPP-0928 review v2 records support-only held redaction proof posture', () => {
  ...
  assert.equal(report.rppId, 'RPP-0928');
  assert.equal(report.sliceId, 'RPP-0928');
  assert.equal(report.workerId, 'rpp-928');
  assert.equal(report.variant, 2);
  assert.equal(report.auditedBranch, 'session/rpp-928');
...
test('RPP-0928 scanner failures become blocking required redaction proof failures', async (t) => {
...
test('RPP-0928 required release checks CLI exits nonzero for each redaction failure reason', (t) => {
  for (const [index, fixture] of redactionFailureFixtures.entries()) {
    const requiredFixture = failedRequiredCheckFixture(fixture, index);
    const fixtureFile = writeRequiredCheckFixture(t, requiredFixture);
...
test('RPP-0928 evidence artifact remains redacted and support-only', async () => {
  const scanReport = await scanArtifacts([
    'docs/evidence/rpp-0928-privacy-redaction-review-v2.md',
  ], { cwd: repoRoot });
  const { text, report } = loadEvidenceReport();
