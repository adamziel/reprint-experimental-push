...
const proofId = 'rpp-0751-remote-hash-pagination-v3';
const successResourceCount = 181;
const successBatchSize = 29;
const successMaxDurationMs = 5000;
const successMaxHeapUsedBytes = 256 * 1024 * 1024;
const failResourceCount = 31;
const failBatchSize = 8;
...
test('RPP-0751 variant 3 benchmark command reports runtime, resources, and pass/fail gates', () => {
  ...
  assert.equal(proof.rppId, 'RPP-0751');
  assert.equal(proof.proofId, proofId);
  assert.equal(proof.variant, 3);
  assert.equal(proof.status, 'passed');
  assert.equal(proof.builtOn.rppId, 'RPP-0731');
  ...
  assert.equal(proof.storagePerformance.pageCount, 7);
  assert.equal(proof.storagePerformance.cursorCount, 6);
...
test('RPP-0751 variant 3 benchmark command exposes fail gates with runtime and resources', () => {
  const result = runBenchmarkCommand([
