...
const proofId = 'rpp-0753-apply-batch-sizing-v3';
const fixedNow = new Date('2026-05-31T00:00:00.000Z');
const mutationCount = 13;
const applyBatchSize = 4;
...
test('RPP-0753 variant 3 generated chunk transfer resume cases gate apply batch output', () => {
  const cases = generatedChunkTransferCases();
  const decisions = cases.map((chunkCase) => {
    ...
    if (chunkCase.expected.updated) {
      ...
      assert.match(decision.outputHash, /^sha256:[a-f0-9]{64}$/, chunkCase.id);
    } else {
    ...
    assert.match(decision.decisionHash, /^[a-f0-9]{64}$/, chunkCase.id);
  ...
  assert.equal(summary.blockedCaseCount, 8);
  assert.equal(summary.unsafeOutputs, 0);
  assert.equal(summary.blockerCounts['resume-skips-durable-chunks'], 2);
  assert.equal(summary.blockerCounts['no-duplicate-mutation-work'], 1);
...
test('RPP-0753 variant 3 proves chunk transfer resumes without duplicate mutation work', {
  concurrency: false,
}, () => {
  const proof = buildVariant3Proof();

  assert.equal(proof.rppId, 'RPP-0753');
  assert.equal(proof.proofId, proofId);
  assert.equal(proof.variant, 3);
  assert.equal(proof.status, 'passed');
  assert.equal(proof.generatedCoverage.source, 'local-support-generated-apply-batch-chunk-resume-cases');
  assert.equal(proof.generatedCoverage.caseCount, 9);
  assert.equal(proof.generatedCoverage.outputEmitted, 1);
  assert.equal(proof.generatedCoverage.blockedCaseCount, 8);
  ...
  assert.deepEqual(proof.chunkTransfer.chunkStarts, [0, 4, 8, 12]);
  assert.deepEqual(proof.chunkTransfer.chunkEnds, [3, 7, 11, 12]);
  assert.deepEqual(proof.chunkTransfer.chunkSizes, [4, 4, 4, 1]);
  ...
  assert.match(proof.chunkTransfer.chunkCollectionHash, /^sha256:[a-f0-9]{64}$/);
  assert.ok(proof.chunkTransfer.chunkWindows.every((window) => window.mutationCount <= applyBatchSize));
  assert.ok(proof.chunkTransfer.chunkWindows.every((window) => window.chunkHash.match(/^sha256:[a-f0-9]{64}$/)));
  ...
  assert.equal(proof.firstAttempt.appliedMutationCount, 8);
  ...
  assert.equal(proof.resume.skippedMutations, 8);
  assert.deepEqual(proof.resume.appliedChunkIndexes, [2, 3]);
  assert.equal(proof.resume.appliedMutationsAfterResume, 5);
...
function buildVariant3Proof() {
  ...
  const generatedDecisions = generatedChunkTransferCases().map((chunkCase) => ({
    caseId: chunkCase.id,
  ...
  const generatedCoverage = {
    source: 'local-support-generated-apply-batch-chunk-resume-cases',
  ...
  const proofGates = [
    ...
    proofGate('generated-unsafe-chunk-transfer-cases-fail-closed',
      generatedCoverage.caseCount === 9
        && generatedCoverage.outputEmitted === 1
        && generatedCoverage.blockedCaseCount === 8
  ...
  const publicProof = {
    ...
    redaction: {
      mode: 'hash-and-count-only-apply-batch-chunk-transfer-resume',
...
function generatedChunkTransferCases() {
  const { evidence, repeatedEvidence } = buildRecordedEvidencePair();
  const safeEvidence = withPassedStatus(clone(evidence));
  const staleChunkReceipt = withPassedStatus(clone(evidence));
  staleChunkReceipt.resume.chunkReceiptsBeforeResume[0].chunkHash = sha256('stale-chunk-receipt');
  ...
  driftedResumeStorage.resume.storageBoundaryChecks[0].actualBeforeHash = sha256('drifted-resume-storage');
  ...
  outOfOrderChunk.chunkTransfer.chunkWindows[1].firstSequence = 9;
...
function runChunkTransferScenario() {
  ...
  const finalReplay = replayCompletedTransfer({
    chunkTransfer,
    storage,
    receipts: resume.receiptsAfterResume,
  });
...
function collectChunkTransferEvidence({ mutations, batchSize }) {
  ...
  const collectionCore = {
    ...
    applyBoundary: {
      ...
      resumePolicy: 'exact-chunk-receipt-skips-committed-prefix',
    },
    chunkWindows,
  };
...
function chunkWindowSummary({ entries, chunkIndex, mutationOffset, chunkCount }) {
  ...
  const core = {
    chunkIndex,
    chunkIdHash: sha256(`apply-transfer-chunk-${chunkIndex + 1}`),
    chunkCount,
    mutationOffset,
...
function duplicateWorkMetrics(evidence) {
  ...
  const safe = counts.length === evidence.chunkTransfer?.mutationCount
    && totalWork === evidence.chunkTransfer?.mutationCount
    && duplicateEntries === 0
    && evidence.resume?.duplicateMutationWork === 0
...
function buildFixtureMutations() {
  return Array.from({ length: mutationCount }, (_, sequence) => ({
    ...
    estimatedBytes: 704 + (sequence % 4) * 31,
  }));
}
...
function rawChunkTransferEvidencePattern() {
  return /"resourceKey"\s*:|fixture-resource|rpp-0753-fixture|private option value|post_content|option_value|meta_value|bearer\s+[a-z0-9._-]+|https?:\/\//i;
}
