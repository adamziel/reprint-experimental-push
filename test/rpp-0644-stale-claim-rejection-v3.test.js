...
function tempJournalPath() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'reprint-rpp-0644-stale-claim-'));
...
function baseSite() {
  ...
  for (let index = 1; index <= 5; index++) {
    files[`rpp-0644-file-${index}.txt`] = `rpp-0644-base-raw-site-value-${index}`;
  }
...
function localSite(base = baseSite()) {
  ...
  for (let index = 1; index <= 5; index++) {
    site.files[`rpp-0644-file-${index}.txt`] = `rpp-0644-local-raw-site-value-${index}`;
  }
  return site;
}
...
function preservedRemoteSite(base = baseSite()) {
  const site = cloneJson(base);
  site.files['rpp-0644-file-2.txt'] = 'rpp-0644-preserved-remote-raw-site-value-2';
  site.files['rpp-0644-file-5.txt'] = 'rpp-0644-preserved-remote-raw-site-value-5';
  return site;
}
...
function rawSiteValuesFor(...sites) {
  const values = new Set([
    'rpp-0644-base-raw-site-value',
    'rpp-0644-local-raw-site-value',
    'rpp-0644-preserved-remote-raw-site-value',
  ]);
...
function assertNoRawSiteValues(value, rawSiteValues) {
  ...
  for (const rawValue of rawSiteValues) {
    assert.equal(
      ...
      false,
      `Unexpected raw site value in RPP-0644 evidence: ${rawValue}`,
    );
  }
}
...
test('RPP-0644 stale claim rejection is restart-readable and exposes lease owner identity', () => {
  const filePath = tempJournalPath();
  const { plan, remote, preservedRemote, rawSiteValues } = buildScenario();
  const artifactRefs = {
    releaseProof: 'artifact://rpp-0644-stale-claim-rejection-v3',
    recoverySupport: 'artifact://rpp-0644-local-recovery-support',
  };
  const activeClaimId = 'rpp-0644-active-lease-owner';
  const staleClaimId = 'rpp-0644-rejected-stale-writer';
