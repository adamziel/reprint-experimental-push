...
import {
  ...
  externalWordPressTopologyCommand,
...
const proofId = 'rpp-0893-maintenance-mode-interaction-v5';
const maintenanceProofScope = 'maintenance-mode-interaction-v5';
...
const goodEnv = Object.freeze({
  REPRINT_PUSH_SOURCE_URL: 'https://source.example.test/maintenance-mode-v5',
  REPRINT_PUSH_REMOTE_URL: 'https://source.example.test/maintenance-mode-v5/',
  REPRINT_PUSH_LOCAL_URL: 'http://127.0.0.1:8080/maintenance-mode-v5-local',
  REPRINT_PUSH_REMOTE_CHANGED_URL: 'https://changed.example.test/maintenance-mode-v5',
  REPRINT_PUSH_PREFLIGHT_SOURCE_URL: 'https://source.example.test/maintenance-mode-v5/',
  REPRINT_PUSH_DRY_RUN_ROUTE_SOURCE_URL: 'https://source.example.test:443/maintenance-mode-v5',
  REPRINT_PUSH_APPLY_SOURCE_URL: 'https://source.example.test/maintenance-mode-v5',
  REPRINT_PUSH_JOURNAL_SOURCE_URL: 'https://source.example.test/maintenance-mode-v5',
  REPRINT_PUSH_RECOVERY_INSPECT_SOURCE_URL: 'https://source.example.test/maintenance-mode-v5',
  REPRINT_PUSH_USERNAME: 'maintenance-v5-admin',
  REPRINT_PUSH_APPLICATION_PASSWORD: 'rpp-0893-application-password-must-not-leak',
});
...
const forbiddenNeedles = Object.freeze([
  ...
  '127.0.0.1',
  'localhost',
  'admin:rpp0893-secret',
  'rpp0893-secret',
  'token=rpp0893-token',
  'rpp0893-token',
  'rpp0893-fragment',
  'rpp-0893-application-password-must-not-leak',
]);

test('RPP-0893 captures source/local/changed URL identities and records maintenance-mode interaction variant 5 scope', () => {
  const proof = buildMaintenanceModeInteractionVariant5Proof({ env: goodEnv });

  assert.equal(proof.schemaVersion, 1);
  assert.equal(proof.rppId, 'RPP-0893');
  ...
  assert.equal(proof.builtOn.variant4Pattern, 'RPP-0873');
  ...
  assert.equal(
    proof.builtOn.contract,
    'release-verifier carry-through for source/local/changed URL capture and maintenance-mode identity binding regression proof',
  );
  ...
  assert.equal(proof.maintenanceModeV5.proofScope, maintenanceProofScope);
  assert.equal(proof.maintenanceModeV5.variant, 5);
  assert.equal(proof.maintenanceModeV5.successCriterion, successCriterion);
  assert.equal(proof.maintenanceModeV5.scopeAcceptedForReleaseTopology, true);
  assert.equal(proof.maintenanceModeV5.roleUrlsAccepted, true);
  assert.equal(proof.maintenanceModeV5.maintenanceInteractionAccepted, true);
  assert.deepEqual(proof.maintenanceModeV5.capturedIdentityRoles, ['source', 'local', 'changed']);
  assert.equal(proof.maintenanceModeV5.sourceLocalChangedIdentitiesCaptured, true);
  assert.equal(proof.maintenanceModeV5.sourceLocalChangedIdentitiesIdentityChecked, true);
  assert.equal(proof.maintenanceModeV5.routeSourceBindingsIdentityChecked, true);
  assert.equal(proof.maintenanceModeV5.sourceLocalChangedIdentityCount, 3);
  assert.equal(proof.maintenanceModeV5.roleIdentityCount, 3);
  assert.equal(proof.maintenanceModeV5.distinctRoleIdentityCount, 3);
  assert.equal(proof.maintenanceModeV5.routeSourceCount, 5);
  assert.equal(proof.maintenanceModeV5.configuredRouteSourceCount, 5);
  assert.equal(proof.maintenanceModeV5.maintenanceRoleStateCount, 3);
  assert.equal(proof.maintenanceModeV5.maintenanceSurfaceCount, 3);
  assert.equal(proof.maintenanceModeV5.identitySurfaceCount, 12);
  assert.equal(proof.maintenanceModeV5.rejectedSurfaceCount, 0);
  assert.deepEqual(proof.maintenanceModeV5.surfaceNames, [
  ...
  assert.deepEqual(proof.maintenanceModeV5.maintenanceSurfaceNames, [
    'core-maintenance-file',
    'maintenance-plugin-option',
    'route-health-status',
  ]);
  assert.deepEqual(
    proof.maintenanceModeV5.roleStateBindings.map((entry) => [entry.role, entry.maintenanceState, entry.identityBound]),
    [
  ...
  assert.ok(proof.maintenanceModeV5.maintenanceSurfaces.every((entry) =>
    entry.payloadCaptured === false && sha256EvidencePattern.test(entry.resourceKeyHash)));
  assert.ok(proof.maintenanceModeV5.roleStateBindings.every((entry) =>
    sha256Pattern.test(entry.urlIdentityHash)
      && sha256Pattern.test(entry.urlOriginHash)
      && sha256EvidencePattern.test(entry.stateHash)));
  assert.match(proof.maintenanceModeV5.roleIdentityDigest, sha256EvidencePattern);
  assert.match(proof.maintenanceModeV5.routeIdentityDigest, sha256EvidencePattern);
  assert.match(proof.maintenanceModeV5.maintenanceSurfaceDigest, sha256EvidencePattern);
  assert.match(proof.maintenanceModeV5.stateBindingDigest, sha256EvidencePattern);
  assert.match(proof.maintenanceModeV5.surfaceDigest, sha256EvidencePattern);
  assert.match(proof.maintenanceModeV5.policyDigest, sha256EvidencePattern);
  assert.match(proof.maintenanceModeV5.scopeHash, sha256EvidencePattern);
  assert.equal(proof.maintenanceModeV5.payloadsStored, false);
  assert.equal(proof.maintenanceModeV5.rawUrlValuesStored, false);
  assert.equal(proof.maintenanceModeV5.hostnameValuesStored, false);
  assert.equal(proof.maintenanceModeV5.rejectedRawInputsStored, false);
  assert.equal(proof.maintenanceModeV5.releasePolicy, 'support-only-no-release-movement');

  assert.equal(proof.releaseVerifier.command, releaseVerifierCommand);
  assert.equal(proof.releaseVerifier.topologyProofCommand, externalWordPressTopologyCommand);
  assert.equal(proof.releaseVerifier.coverageMode, coverageMode);
  assert.equal(proof.releaseVerifier.carriedThroughByTopologyProof, true);
  assert.equal(proof.releaseVerifier.status, 'held-no-go');
  assert.equal(proof.releaseVerifier.failClosed, true);
  assert.equal(proof.releaseVerifier.acceptedForFinalRelease, false);
  assert.equal(proof.releaseVerifier.releaseGateMovementAllowed, false);
  assert.equal(proof.releaseVerifier.finalReleaseStatus, 'NO-GO');
  assert.equal(proof.releaseVerifier.integrationRecommendation, 'NO-GO');
  assert.equal(proof.releaseVerifier.sourceLocalChangedSuccessCriterion, successCriterion);
  assert.equal(proof.releaseVerifier.sourceLocalChangedSuccessCriterionSatisfied, true);
  assert.equal(proof.releaseVerifier.routeSourceBindingsIdentityChecked, true);
  assert.equal(proof.releaseVerifier.maintenanceInteractionAccepted, true);
  ...
  assert.equal(proof.releaseVerifier.noTunnelPolicyEnforced, true);
  assert.equal(proof.releaseVerifier.onlySandbox8080Ingress, true);
  assert.equal(proof.releaseVerifier.evidenceMode, 'hash-count-surface-only');
  assert.equal(proof.releaseVerifier.roleIdentityCount, 3);
  assert.equal(proof.releaseVerifier.distinctRoleIdentityCount, 3);
  assert.equal(proof.releaseVerifier.routeSourceCount, 5);
  assert.equal(proof.releaseVerifier.maintenanceSurfaceCount, 3);
  assert.equal(proof.releaseVerifier.maintenanceRoleStateCount, 3);
  assert.equal(proof.releaseVerifier.identitySurfaceCount, 12);
  assert.equal(proof.releaseVerifier.rejectedSurfaceCount, 0);
  assert.deepEqual(proof.releaseVerifier.rejectedSurfaceNames, []);
  assert.equal(proof.releaseVerifier.payloadsStored, false);
  ...
  assert.match(proof.releaseVerifier.carryThroughDigest, sha256EvidencePattern);
  ...
  assert.equal(proof.invariants.maintenanceModeVariant5ScopeRecorded, true);
  ...
  assert.equal(proof.invariants.releaseVerifierHoldsFinalReleaseNoGo, true);
  ...
  assert.doesNotThrow(() => assertEvidenceHasNoRawValues(proof, { label: 'RPP-0893 maintenance mode variant 5 proof' }));
});

test('RPP-0893 rejects tunnel, secret-shaped, duplicate, and packaged fallback inputs before accepting variant 5 scope', () => {
  const proof = buildMaintenanceModeInteractionVariant5Proof({
    env: {
      REPRINT_PUSH_SOURCE_URL: 'https://admin:rpp0893-secret@source.example.test/maintenance-mode-v5',
      REPRINT_PUSH_LOCAL_URL: 'https://source.example.test/maintenance-mode-v5?token=rpp0893-token',
      REPRINT_PUSH_REMOTE_CHANGED_URL: 'https://changed.ngrok-free.app/maintenance-mode-v5#rpp0893-fragment',
  ...
  assert.equal(proof.topology.rawUrlValuesStored, false);
  assert.equal(proof.maintenanceModeV5.scopeAcceptedForReleaseTopology, false);
  assert.equal(proof.maintenanceModeV5.roleUrlsAccepted, false);
  assert.equal(proof.maintenanceModeV5.maintenanceInteractionAccepted, false);
  assert.equal(proof.maintenanceModeV5.rejectedSurfaceCount, 5);
  assert.equal(proof.maintenanceModeV5.rawUrlValuesStored, false);
  assert.equal(proof.maintenanceModeV5.hostnameValuesStored, false);
  assert.equal(proof.maintenanceModeV5.rejectedRawInputsStored, false);
  ...
  assert.equal(proof.releaseVerifier.failClosed, true);
  assert.equal(proof.releaseVerifier.carriedThroughByTopologyProof, true);
  assert.equal(proof.releaseVerifier.sourceLocalChangedSuccessCriterionSatisfied, false);
  assert.equal(proof.releaseVerifier.routeSourceBindingsIdentityChecked, true);
  assert.equal(proof.releaseVerifier.maintenanceInteractionAccepted, false);
  ...
  assert.equal(proof.releaseVerifier.noTunnelPolicyEnforced, false);
  assert.equal(proof.releaseVerifier.acceptedForFinalRelease, false);
  assert.equal(proof.releaseVerifier.releaseGateMovementAllowed, false);
  assert.equal(proof.releaseVerifier.rejectedSurfaceCount, 5);
  assert.deepEqual(
    proof.releaseVerifier.rejectedSurfaceNames,
    [
      'source-local-changed-url-identities-distinct',
      'no-forbidden-tunnel-hosts',
      'no-url-userinfo-query-or-fragment',
  ...
  assert.equal(proof.releaseVerifier.rejectedRawInputsStored, false);
  assert.match(proof.releaseVerifier.carryThroughDigest, sha256EvidencePattern);
  assert.ok(proof.maintenanceModeV5.roleStateBindings.every((entry) => entry.identityBound === false));
  assert.deepEqual(
    proof.maintenanceModeV5.rejectedSurfaceNames,
    [
      'source-local-changed-url-identities-distinct',
      'no-forbidden-tunnel-hosts',
      'no-url-userinfo-query-or-fragment',
  ...
  assertRedactedMaintenanceProof(proof);
  assertNoNeedles(proof, forbiddenNeedles);
  assert.doesNotThrow(() => assertEvidenceHasNoRawValues(proof, { label: 'RPP-0893 rejected maintenance mode proof' }));
});

test('RPP-0893 maintenance-mode variant 5 proof is deterministic and hash/count/surface-only', () => {
  const firstProof = buildMaintenanceModeInteractionVariant5Proof({ env: goodEnv });
  const secondProof = buildMaintenanceModeInteractionVariant5Proof({ env: { ...goodEnv } });

  assert.equal(firstProof.outputHash, secondProof.outputHash);
  assert.equal(firstProof.maintenanceModeV5.scopeHash, secondProof.maintenanceModeV5.scopeHash);
  assert.equal(firstProof.releaseVerifier.carryThroughDigest, secondProof.releaseVerifier.carryThroughDigest);
  assert.equal(firstProof.maintenanceModeV5.roleIdentityDigest, secondProof.maintenanceModeV5.roleIdentityDigest);
  assert.equal(firstProof.maintenanceModeV5.routeIdentityDigest, secondProof.maintenanceModeV5.routeIdentityDigest);
  assert.equal(firstProof.maintenanceModeV5.maintenanceSurfaceDigest, secondProof.maintenanceModeV5.maintenanceSurfaceDigest);
  assert.equal(firstProof.maintenanceModeV5.stateBindingDigest, secondProof.maintenanceModeV5.stateBindingDigest);
  assert.equal(firstProof.maintenanceModeV5.surfaceDigest, secondProof.maintenanceModeV5.surfaceDigest);
  ...
  assert.deepEqual(firstProof.maintenanceModeV5.maintenanceSurfaces, secondProof.maintenanceModeV5.maintenanceSurfaces);
  assert.deepEqual(firstProof.maintenanceModeV5.roleStateBindings, secondProof.maintenanceModeV5.roleStateBindings);
  assert.deepEqual(firstProof.releaseVerifier, secondProof.releaseVerifier);
  assert.equal(firstProof.maintenanceModeV5.payloadsStored, false);
  assert.equal(firstProof.maintenanceModeV5.rawUrlValuesStored, false);
  assert.equal(firstProof.maintenanceModeV5.hostnameValuesStored, false);
  assert.equal(firstProof.maintenanceModeV5.rejectedRawInputsStored, false);
  assert.equal(firstProof.maintenanceModeV5.identitySurfaceCount, 12);
  assert.equal(firstProof.maintenanceModeV5.maintenanceSurfaceCount, 3);
  assert.equal(firstProof.maintenanceModeV5.maintenanceRoleStateCount, 3);
  ...
  assert.doesNotThrow(() => assertEvidenceHasNoRawValues(firstProof, { label: 'RPP-0893 first maintenance mode proof' }));
  assert.doesNotThrow(() => assertEvidenceHasNoRawValues(secondProof, { label: 'RPP-0893 second maintenance mode proof' }));
});

function buildMaintenanceModeInteractionVariant5Proof({ env, now = fixedNow } = {}) {
  ...
  const maintenanceModeV5ScopeCore = {
  ...
  const maintenanceModeV5 = {
    ...
    scopeHash: `sha256:${digest(maintenanceModeV5ScopeCore)}`,
    payloadsStored: false,
    rawUrlValuesStored: false,
    hostnameValuesStored: false,
    rejectedRawInputsStored: false,
    networkProbePerformed: topologyProof.constraints.networkProbePerformed,
    snapshotBoundary: maintenanceModeV5ScopeCore.snapshotBoundary,
    releasePolicy: 'support-only-no-release-movement',
  };
  const releaseVerifier = buildReleaseVerifierCarryThrough({
    ...
    routeSourceBindingsIdentityChecked,
    maintenanceStatesIdentityBound,
    maintenanceMode: maintenanceModeV5,
    localOnlyPolicy,
    rejectedSurfaceNames,
  });
  const roleIdentityChecks = {
    requiredRoleCount: roleDefinitions.length,
    capturedRoleCount: maintenanceModeV5.capturedRoleIdentityCount,
    validRoleCount: maintenanceModeV5.validRoleIdentityCount,
    acceptedRoleCount: maintenanceModeV5.acceptedRoleIdentityCount,
    distinctRoleIdentityCount,
    sourceLocalChangedUrlIdentitiesCaptured,
    sourceLocalChangedUrlIdentitiesIdentityChecked,
  ...
  const invariants = {
    ...
    maintenanceModeVariant5ScopeRecorded: maintenanceModeV5.proofScope === maintenanceProofScope
      && maintenanceModeV5.variant === 5
      && maintenanceModeV5.successCriterion === successCriterion
      && maintenanceModeV5.roleIdentityCount === 3
      && maintenanceModeV5.maintenanceRoleStateCount === 3
      && maintenanceModeV5.maintenanceSurfaceCount === 3,
    ...
    releaseVerifierCarryThroughRecorded: releaseVerifier.carriedThroughByTopologyProof === true
      && releaseVerifier.command === releaseVerifierCommand
      && releaseVerifier.topologyProofCommand === externalWordPressTopologyCommand
      && releaseVerifier.coverageMode === coverageMode
      && sha256EvidencePattern.test(releaseVerifier.carryThroughDigest),
    releaseVerifierHoldsFinalReleaseNoGo: releaseVerifier.acceptedForFinalRelease === false
      && releaseVerifier.releaseGateMovementAllowed === false
      && releaseVerifier.finalReleaseStatus === 'NO-GO'
      && releaseVerifier.integrationRecommendation === 'NO-GO'
      && releaseVerifier.failClosed === true,
    noMaintenancePayloadStored: maintenanceModeV5.payloadsStored === false
      && maintenanceSurfaces.every((entry) => entry.payloadCaptured === false),
    hashCountSurfaceOnly: maintenanceModeV5.payloadsStored === false
      && maintenanceModeV5.rawUrlValuesStored === false
      && maintenanceModeV5.hostnameValuesStored === false
      && maintenanceModeV5.rejectedRawInputsStored === false
      && maintenanceModeV5.identitySurfaceCount === 12
      && maintenanceModeV5.maintenanceSurfaceCount === 3
      && maintenanceModeV5.maintenanceRoleStateCount === 3
      && releaseVerifier.evidenceMode === 'hash-count-surface-only'
      ...
      && releaseVerifier.payloadsStored === false
      && sha256EvidencePattern.test(maintenanceModeV5.scopeHash)
      && sha256EvidencePattern.test(releaseVerifier.carryThroughDigest)
  ...
  const proofCore = {
    ...
    builtOn: {
      ...
      contract: 'release-verifier carry-through for source/local/changed URL capture and maintenance-mode identity binding regression proof',
    },
    topology: {
...
function buildReleaseVerifierCarryThrough({
  ...
  const sourceLocalChangedSuccessCriterionSatisfied = sourceLocalChangedUrlIdentitiesIdentityChecked === true;
  const releaseVerifierStatus = topologyOk
    && sourceLocalChangedSuccessCriterionSatisfied
    && routeSourceBindingsIdentityChecked === true
    && maintenanceStatesIdentityBound === true
    ? 'held-no-go'
    : 'blocked';
  const releaseVerifierCore = {
    command: releaseVerifierCommand,
    topologyProofCommand: topologyProof.command || externalWordPressTopologyCommand,
    coverageMode,
    proofScope: maintenanceProofScope,
    variant: 5,
    carriedThroughByTopologyProof: true,
    status: releaseVerifierStatus,
    failClosed: true,
    sourceLocalChangedSuccessCriterion: successCriterion,
    sourceLocalChangedSuccessCriterionSatisfied,
    routeSourceBindingsIdentityChecked,
    maintenanceInteractionAccepted: maintenanceStatesIdentityBound === true,
    roleIdentityCount: maintenanceMode.roleIdentityCount,
    distinctRoleIdentityCount: maintenanceMode.distinctRoleIdentityCount,
    routeSourceCount: maintenanceMode.routeSourceCount,
    maintenanceSurfaceCount: maintenanceMode.maintenanceSurfaceCount,
    maintenanceRoleStateCount: maintenanceMode.maintenanceRoleStateCount,
    identitySurfaceCount: maintenanceMode.identitySurfaceCount,
    rejectedSurfaceCount: maintenanceMode.rejectedSurfaceCount,
    rejectedSurfaceNames,
    scopeHash: maintenanceMode.scopeHash,
    roleIdentityDigest: maintenanceMode.roleIdentityDigest,
    routeIdentityDigest: maintenanceMode.routeIdentityDigest,
    stateBindingDigest: maintenanceMode.stateBindingDigest,
    surfaceDigest: maintenanceMode.surfaceDigest,
    noPackagedFallback: topologyProof.identityChecks.packagedFallbackDisabled.ok === true,
    packagedFallbackAllowed: false,
    packagedFallbackObserved: topologyProof.identityChecks.packagedFallbackDisabled.ok !== true,
    noTunnelPolicyEnforced: topologyProof.rppEvidence.noTunnelPolicyEnforced === true,
    onlySandbox8080Ingress: localOnlyPolicy.onlySandbox8080Ingress === true,
  ...
  return {
    ...releaseVerifierCore,
    carryThroughDigest: `sha256:${digest(releaseVerifierCore)}`,
  };
}
