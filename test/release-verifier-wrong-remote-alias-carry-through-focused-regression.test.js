...
const secretValue = 'RPP_0085_SHOULD_NOT_LEAK';
...
const expectedAcceptedAliasEvidence = {
...
function runVerifyReleaseWrongRemoteAlias() {
...
function writeEvidence(evidence, remoteAliasUrl) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'release-verifier-wrong-remote-alias-carry-through-'));
...
function runReleaseGateCheck(evidence, remoteAliasUrl) {
  return spawnSync(process.execPath, [
    scriptPath,
    '--evidence-file',
    writeEvidence(evidence, remoteAliasUrl),
    '--scope',
    'final-release',
    '--now',
    fixedNow,
...
test('release verifier carries wrong REPRINT_PUSH_REMOTE_URL alias through before mutation for RPP-0085', () => {
  const proof = runVerifyReleaseWrongRemoteAlias();
  const verifyReport = extractJsonObjects(proof.stdout)
    .find((entry) => entry?.releaseProof?.code === 'REPRINT_PUSH_SOURCE_URL_MISMATCH');
  ...
  assert.ok(
    ...
    'verify:release stdout should end with the wrong-remote-alias held marker',
  );
  assert.doesNotMatch(
    `${proof.stdout}\n${proof.stderr}`,
    /Starting Playground server/,
    'wrong remote alias must fail before starting live verifier servers',
  );
  ...
  assert.deepEqual(
    ...
    },
    {
      ...
      topologyEvidence: {
        ...
        blocker: 'REPRINT_PUSH_SOURCE_URL_MISMATCH',
      },
      releaseMovement: {
        allowed: false,
        gates: '0/4',
  ...
  }), verifyReport.boundary.liveSource.observed);
  const releaseGateReport = parseReport(releaseGateResult);
  const aliasGate = gateById(releaseGateReport, 'remote-alias');
  ...
  assert.equal(releaseGateReport.primaryFailureCode, 'REPRINT_PUSH_SOURCE_URL_MISMATCH');
  ...
  assert.deepEqual(aliasGate.evidence, expectedMismatchEvidence);
...
test('release verifier wrong remote alias carry-through keeps matching-alias positive path for RPP-0085', () => {
  const releaseGateResult = runReleaseGateCheck(completeFinalEvidence(), sourceUrl);
  const releaseGateReport = parseReport(releaseGateResult);
  const aliasGate = gateById(releaseGateReport, 'remote-alias');
...
test('release verifier wrong remote alias carry-through is recorded in RPP-0085 reports', () => {
  const evidenceReport = fs.readFileSync(evidenceReportPath, 'utf8');
  const progressReport = fs.readFileSync(progressReportPath, 'utf8');

  assert.ok(evidenceReport.includes('| RPP-0085 | Evidence toward release verifier wrong `REPRINT_PUSH_REMOTE_URL` alias carry-through'));
  assert.ok(evidenceReport.includes(`- Command: \`${focusedCommand}\``));
  assert.ok(evidenceReport.includes(`- Observed status: \`pass\`; verifier marker: \`${statusMarker}\`; remote-alias gate: \`REPRINT_PUSH_SOURCE_URL_MISMATCH\`; release marker: \`${releaseGateHeldMarker}\`.`));
  assert.ok(progressReport.includes('Release verifier wrong remote alias carry-through now checks `RPP-0085`'));
  assert.ok(progressReport.includes(`- Command: \`${focusedCommand}\``));
  assert.ok(progressReport.includes(`- Observed status: \`pass\`; verifier marker: \`${statusMarker}\`; remote-alias gate: \`REPRINT_PUSH_SOURCE_URL_MISMATCH\`; release marker: \`${releaseGateHeldMarker}\`.`));
});
