...
const sourceUrl = 'https://rpp-0588-source.invalid';
...
const sessionToken = 'psh_rpp_0588_generated_session_000000';
const authScope = 'reprint-push-lab:authenticated-http-push';
const hashPattern = /^[a-f0-9]{64}$/;

const credential = {
  username: 'rpp_0588_admin',
  password: 'rpp-0588-private-application-password',
};
...
const fixturePath = 'wp-content/uploads/reprint-push/rpp-0588-short-lived-v5.txt';
...
function fixtureHash(label) {
  return sha256Hex(`rpp-0588:${label}`);
}
...
function fixtureSnapshots() {
  return {
    base: {
      files: {
        [fixturePath]: 'rpp-0588 base short-lived push session content',
      },
      plugins: {},
      db: {},
    },
    local: {
      files: {
        [fixturePath]: 'rpp-0588 local short-lived push session content',
      },
      plugins: {},
      db: {},
    },
...
const negativeReceiptCases = Object.freeze([
  'session-drift',
  'identity-drift',
  'scope-drift',
  'auth-session-drift',
  ...
  'malformed-subject-binding',
  'malformed-push-session',
  'stale-auth-session',
  'stale-session-issue',
  'stale-idempotency-binding',
  'expired-receipt',
]);

const negativeReceiptCategories = Object.freeze({
  'session-drift': 'drifted',
  'identity-drift': 'drifted',
  'scope-drift': 'drifted',
  'auth-session-drift': 'drifted',
  'plan-drift': 'drifted',
  'missing-auth-binding': 'missing',
  'missing-subject-binding': 'missing',
  'missing-issue-binding': 'missing',
  'missing-receipt-hash': 'missing',
  'malformed-receipt-hash': 'malformed',
  'malformed-subject-binding': 'malformed',
  'malformed-push-session': 'malformed',
  'stale-auth-session': 'stale',
  'stale-session-issue': 'stale',
  'stale-idempotency-binding': 'stale',
  'expired-receipt': 'expired',
});

function negativeCaseCategory(kind) {
  const category = negativeReceiptCategories[kind];
  assert.ok(category, `missing negative case category for ${kind}`);
  return category;
}
...
function mutateReceiptForNegativeCase(receipt, kind) {
  ...
  switch (kind) {
    ...
    case 'plan-drift':
      ...
      refreshSubjectBinding(mutated);
      break;
    case 'missing-auth-binding':
      delete mutated.authBinding;
      break;
    ...
    case 'missing-issue-binding':
      ...
      break;
    case 'missing-receipt-hash':
      delete mutated.receiptHash;
      rebuild = false;
      break;
    case 'malformed-receipt-hash':
      mutated.receiptHash = fixtureHash('malformed-receipt-hash');
      rebuild = false;
      break;
    case 'malformed-subject-binding':
      mutated.authBinding.binding = 'malformed-subject-binding';
      break;
    case 'malformed-push-session':
      mutated.authBinding.pushSession = 'malformed-push-session';
      break;
    case 'stale-auth-session':
      mutated.authBinding.session.expiresAt = expiredReceiptExpiry;
      break;
    case 'stale-session-issue':
      mutated.authBinding.pushSession.issue.issuedAt = '2026-05-30T23:50:00Z';
      mutated.authBinding.pushSession.issue.expiresAt = expiredReceiptExpiry;
      break;
    case 'stale-idempotency-binding':
      mutated.authBinding.pushSession.dryRunIdempotencyKeyHash = fixtureHash('stale-idempotency-key');
      break;
    default:
  ...
  return rebuild ? rebuildReceiptHash(mutated) : mutated;
}
...
function bindingProofEnvelope({ receipt, plan, dryRunRequest, afterSnapshotHash }) {
  ...
  const envelope = {
    ...
    releaseGate: 'NO-GO',
    verifier: {
      verifierHash: fixtureHash('short-lived-push-session-issue-verifier-v5'),
...
function refusalEvidence({ kind, receipt, code, reason, requestHash, applyAttempted }) {
  const envelope = {
    ...
    negativeCategoryHash: sha256Hex(negativeCaseCategory(kind)),
    code,
...
function negativeCaseMessage(kind) {
  ...
  if (kind === 'missing-auth-binding') {
    return 'Authenticated apply requires an auth-bound dry-run receipt.';
  }
  if (kind === 'missing-receipt-hash' || kind === 'malformed-receipt-hash') {
    return 'Receipt hash does not match receipt body.';
  }
  if (kind === 'missing-issue-binding') {
    return 'Receipt short-lived push session issue binding does not match the current request.';
  }
  if (kind === 'malformed-push-session') {
    return 'Receipt signed session binding does not match the current request.';
  }
  if (kind === 'stale-auth-session') {
    return 'Receipt auth identity or session does not match the current request.';
  }
  if (kind === 'stale-session-issue') {
    return 'Receipt short-lived push session issue binding does not match the current request.';
  }
  if (kind === 'stale-idempotency-binding') {
    return 'Production-shaped apply must reuse the dry-run receipt idempotency binding.';
  }
...
async function runBoundDryRunProof() {
  ...
  const idempotencyKey = 'idem-rpp-0588-bound-dry-run-v5';
  ...
  const { seen, restore } = installFetch(({ pathname, body, contentHash }) => {
    ...
    throw new Error(`unexpected mutation path for RPP-0588 dry-run proof: ${pathname}`);
  });
  ...
  try {
    ...
    assert.equal(proofEnvelope.verifier.productionBacked, false);
    assert.equal(proofEnvelope.verifier.releaseEligible, false);
    assert.equal(proofEnvelope.verifier.acceptedForReleaseGate, false);
...
async function runNegativeReceiptCase(kind) {
  const { base, local } = fixtureSnapshots();
  const idempotencyKey = `idem-rpp-0588-${kind}-v5`;
  ...
  try {
    ...
    assert.equal(refusal.negativeCategoryHash, sha256Hex(negativeCaseCategory(kind)));
    assert.equal(refusal.verifier.productionBacked, false);
    assert.equal(refusal.verifier.releaseEligible, false);
    assert.equal(refusal.verifier.acceptedForReleaseGate, false);
...
test('RPP-0588 v5 route validates bound dry-run receipts before mutation-capable work', () => {
...
test('RPP-0588 v5 dry-run receipt binds session identity scope auth session and plan hash', async () => {
  ...
  const aggregate = {
    ...
    supportOnly: true,
    releaseGate: 'NO-GO',
    verifierHash: proofEnvelope.verifier.verifierHash,
    releaseEligible: false,
  ...
  assert.equal(aggregate.releaseEligible, false);
...
test('RPP-0588 v5 rejects expired missing malformed drifted and stale receipt evidence before mutation work', async () => {
  const refusalEnvelopes = [];
  for (const kind of negativeReceiptCases) {
  ...
  const aggregate = {
    ...
    negativeCategoryHashes: refusalEnvelopes.map((envelope) => envelope.negativeCategoryHash),
    ...
    coveredCategoryHashes: [...new Set(negativeReceiptCases.map((kind) => (
      sha256Hex(negativeCaseCategory(kind))
    )))].sort(),
  ...
  assert.equal(aggregate.caseCount, negativeReceiptCases.length);
  assert.equal(aggregate.releaseEligible, false);
  assert.deepEqual(
    aggregate.coveredCategoryHashes,
    [
      sha256Hex('drifted'),
      sha256Hex('expired'),
      sha256Hex('malformed'),
  ...
  assertHashFields(aggregate, ['verifierHash', 'aggregateHash']);
  for (const hash of [
    ...aggregate.refusalHashes,
    ...aggregate.negativeCaseHashes,
    ...aggregate.negativeCategoryHashes,
    ...aggregate.receiptHashes,
    ...aggregate.planHashes,
    ...aggregate.coveredCategoryHashes,
  ]) {
    assertHashOrNull(hash, 'aggregate negative evidence hash');
  }
  assertNoRawValues(aggregate, [
    sourceUrl,
    sessionToken,
