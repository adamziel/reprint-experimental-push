...
const auditPath = path.join(repoRoot, 'docs/evidence/rpp-0901-release-gate-1-final-audit.md');

const auditedHead = '13a0d0a293b5ba2c65045931ac404e332acf470a';
const staleHead = '93c391cd9913af4019fcf962b60e871982b347ef';
...
const exactCommands = [
  ...
  "git show -s --format='%H %s' 3ff789513 bb40db8c1 2849d0398 6e3ab6f3c e837c3b90 adc70a4fc 847a4281c bb01b0552 d5e8bb491 45da34eca c3f7a3703 4b459c956 2891399d3 dd9c889c3 e0643400e 37898f069 bf37c4e2f 838638074 43a818b14 4393d034d ff533e449 ca82bf5d7 4d6600f2a",
  ...
  'node --check test/rpp-0901-release-gate-1-final-audit.test.js',
  'node --test --test-name-pattern RPP-0901 test/rpp-0901-release-gate-1-final-audit.test.js',
  'node scripts/release/artifact-redaction-scan.mjs docs/evidence/rpp-0901-release-gate-1-final-audit.md',
  'git diff --check',
];
...
test('RPP-0901 audit is refreshed to the current lane head without moving gates', () => {
  const text = auditText();

  assert.match(text, /^Date: 2026-06-01$/m);
  assert.match(text, /^Audited local branch: `session\/rpp-901`$/m);
  assert.match(text, new RegExp(`^Audited lane head before this evidence file: \`${auditedHead}\`$`, 'm'));
  assert.doesNotMatch(text, /session\/rpp-63/);
  assert.doesNotMatch(text, new RegExp(staleHead));
...
test('RPP-0901 audit links exact commands used for evidence and validation', () => {
  const text = auditText();
...
test('RPP-0901 audit links exact support commits and current lane head commits', () => {
  const text = auditText();
...
test('RPP-0901 audit records production-backed evidence gap and integration recommendation', () => {
  const text = auditText();
  ...
  assert.match(text, /Integrate this file as release-ops evidence for RPP-0901 only/);
