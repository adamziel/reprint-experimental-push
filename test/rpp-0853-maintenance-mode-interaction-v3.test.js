...
const proofId = 'rpp-0853-maintenance-mode-interaction-v3';
const maintenanceProofScope = 'maintenance-mode-interaction-v3';
...
const goodEnv = Object.freeze({
  REPRINT_PUSH_SOURCE_URL: 'https://source.example.test/maintenance-mode-v3',
  REPRINT_PUSH_REMOTE_URL: 'https://source.example.test/maintenance-mode-v3/',
  REPRINT_PUSH_LOCAL_URL: 'https://local.example.test/maintenance-mode-v3',
  REPRINT_PUSH_REMOTE_CHANGED_URL: 'https://changed.example.test/maintenance-mode-v3',
  REPRINT_PUSH_PREFLIGHT_SOURCE_URL: 'https://source.example.test/maintenance-mode-v3/',
  REPRINT_PUSH_DRY_RUN_ROUTE_SOURCE_URL: 'https://source.example.test:443/maintenance-mode-v3',
  REPRINT_PUSH_APPLY_SOURCE_URL: 'https://source.example.test/maintenance-mode-v3',
  REPRINT_PUSH_JOURNAL_SOURCE_URL: 'https://source.example.test/maintenance-mode-v3',
  REPRINT_PUSH_RECOVERY_INSPECT_SOURCE_URL: 'https://source.example.test/maintenance-mode-v3',
  REPRINT_PUSH_USERNAME: 'maintenance-v3-admin',
  REPRINT_PUSH_APPLICATION_PASSWORD: 'rpp-0853-application-password-must-not-leak',
});

const forbiddenNeedles = Object.freeze([
  'rpp-0853-application-password-must-not-leak',
  'admin:rpp0853-secret',
  'rpp0853-secret',
  'token=rpp0853-token',
  'rpp0853-token',
...
test('RPP-0853 captures source/local/changed URLs and records maintenance-mode interaction variant 3 scope', () => {
  const proof = buildMaintenanceModeInteractionVariant3Proof({ env: goodEnv });

  assert.equal(proof.schemaVersion, 1);
  assert.equal(proof.rppId, 'RPP-0853');
  ...
  assert.equal(proof.topology.sourceUrl, 'https://source.example.test/maintenance-mode-v3');
  assert.equal(proof.topology.localUrl, 'https://local.example.test/maintenance-mode-v3');
  assert.equal(proof.topology.remoteChangedUrl, 'https://changed.example.test/maintenance-mode-v3');
  ...
  assert.deepEqual(
    ...
    [
      ['source', 'source', 'https://source.example.test/maintenance-mode-v3'],
      ['local', 'localEdited', 'https://local.example.test/maintenance-mode-v3'],
      ['changed', 'remoteChanged', 'https://changed.example.test/maintenance-mode-v3'],
    ],
  );
  ...
  assert.equal(proof.maintenanceModeV3.proofScope, maintenanceProofScope);
  assert.equal(proof.maintenanceModeV3.variant, 3);
  assert.equal(proof.maintenanceModeV3.scopeAcceptedForReleaseTopology, true);
  assert.equal(proof.maintenanceModeV3.maintenanceInteractionAccepted, true);
  assert.equal(proof.maintenanceModeV3.roleUrlCount, 3);
  assert.equal(proof.maintenanceModeV3.capturedRoleUrlCount, 3);
  assert.equal(proof.maintenanceModeV3.roleStateCount, 3);
  assert.equal(proof.maintenanceModeV3.surfaceCount, 3);
  assert.equal(proof.maintenanceModeV3.routeSourceCount, 5);
  assert.equal(proof.maintenanceModeV3.identitySurfaceCount, 9);
  assert.equal(proof.maintenanceModeV3.rejectedSurfaceCount, 0);
  assert.deepEqual(proof.maintenanceModeV3.surfaceNames, [
    'core-maintenance-file',
    'maintenance-plugin-option',
    'route-health-status',
  ]);
  assert.deepEqual(
    proof.maintenanceModeV3.roleStateBindings.map((entry) => [entry.role, entry.maintenanceState, entry.identityBound]),
    [
  ...
  assert.ok(proof.maintenanceModeV3.surfaces.every((entry) =>
    entry.payloadCaptured === false && sha256EvidencePattern.test(entry.resourceKeyHash)));
  assert.ok(proof.maintenanceModeV3.roleStateBindings.every((entry) =>
    sha256Pattern.test(entry.urlIdentityHash)
      && sha256Pattern.test(entry.urlOriginHash)
      && sha256EvidencePattern.test(entry.stateHash)));
  assert.match(proof.maintenanceModeV3.roleIdentityDigest, sha256EvidencePattern);
  assert.match(proof.maintenanceModeV3.stateBindingDigest, sha256EvidencePattern);
  assert.match(proof.maintenanceModeV3.scopeHash, sha256EvidencePattern);
  assert.equal(proof.maintenanceModeV3.payloadsStored, false);
  assert.equal(proof.maintenanceModeV3.releasePolicy, 'support-only-no-release-movement');
  ...
  assert.equal(proof.invariants.maintenanceModeVariant3ScopeRecorded, true);
  ...
  assert.doesNotThrow(() => assertEvidenceHasNoRawValues(proof, { label: 'RPP-0853 maintenance mode variant 3 proof' }));
});

test('RPP-0853 rejects tunnel and secret-shaped URLs before accepting maintenance-mode variant 3 scope', () => {
  const proof = buildMaintenanceModeInteractionVariant3Proof({
    env: {
      REPRINT_PUSH_SOURCE_URL: 'https://admin:rpp0853-secret@source.example.test/maintenance-mode-v3',
      REPRINT_PUSH_LOCAL_URL: 'https://source.example.test/maintenance-mode-v3?token=rpp0853-token',
      REPRINT_PUSH_REMOTE_CHANGED_URL: 'https://changed.ngrok-free.app/maintenance-mode-v3',
  ...
  assert.equal(proof.maintenanceModeV3.scopeAcceptedForReleaseTopology, false);
  assert.equal(proof.maintenanceModeV3.maintenanceInteractionAccepted, false);
  assert.equal(proof.maintenanceModeV3.rejectedSurfaceCount, 4);
  assert.ok(proof.maintenanceModeV3.roleStateBindings.every((entry) => entry.identityBound === false));
  ...
  assertNoNeedles(proof, forbiddenNeedles);
  assert.doesNotThrow(() => assertEvidenceHasNoRawValues(proof, { label: 'RPP-0853 rejected maintenance mode proof' }));
});

test('RPP-0853 maintenance-mode variant 3 proof is deterministic and hash/count/surface-only', () => {
  const firstProof = buildMaintenanceModeInteractionVariant3Proof({ env: goodEnv });
  const secondProof = buildMaintenanceModeInteractionVariant3Proof({ env: { ...goodEnv } });

  assert.equal(firstProof.outputHash, secondProof.outputHash);
  assert.equal(firstProof.maintenanceModeV3.scopeHash, secondProof.maintenanceModeV3.scopeHash);
  assert.equal(firstProof.maintenanceModeV3.roleIdentityDigest, secondProof.maintenanceModeV3.roleIdentityDigest);
  assert.equal(firstProof.maintenanceModeV3.stateBindingDigest, secondProof.maintenanceModeV3.stateBindingDigest);
  assert.deepEqual(firstProof.maintenanceModeV3.surfaceNames, secondProof.maintenanceModeV3.surfaceNames);
  assert.deepEqual(firstProof.maintenanceModeV3.roleStateBindings, secondProof.maintenanceModeV3.roleStateBindings);
  assert.deepEqual(firstProof.roleIdentityChecks.identitySurfaces, secondProof.roleIdentityChecks.identitySurfaces);
  assert.equal(firstProof.maintenanceModeV3.payloadsStored, false);
  assert.equal(firstProof.maintenanceModeV3.surfaceCount, 3);
  assert.equal(firstProof.maintenanceModeV3.roleStateCount, 3);
  ...
  assert.doesNotThrow(() => assertEvidenceHasNoRawValues(firstProof, { label: 'RPP-0853 first maintenance mode proof' }));
  assert.doesNotThrow(() => assertEvidenceHasNoRawValues(secondProof, { label: 'RPP-0853 second maintenance mode proof' }));
});

function buildMaintenanceModeInteractionVariant3Proof({ env, now = fixedNow } = {}) {
  ...
  const maintenanceModeV3ScopeCore = {
  ...
  const maintenanceModeV3 = {
    ...
    surfaces,
    roleStateBindings,
    roleIdentityDigest: `sha256:${digest(maintenanceModeV3ScopeCore.roleUrlIdentities)}`,
    stateBindingDigest: `sha256:${digest(maintenanceModeV3ScopeCore.roleStateBindings)}`,
    scopeHash: `sha256:${digest(maintenanceModeV3ScopeCore)}`,
    payloadsStored: false,
    networkProbePerformed: topologyProof.constraints.networkProbePerformed,
    snapshotBoundary: maintenanceModeV3ScopeCore.snapshotBoundary,
    releasePolicy: 'support-only-no-release-movement',
  };
  ...
  const invariants = {
    ...
    maintenanceModeVariant3ScopeRecorded: maintenanceModeV3.proofScope === maintenanceProofScope
      && maintenanceModeV3.variant === 3
      && maintenanceModeV3.roleUrlCount === 3
      && maintenanceModeV3.roleStateCount === 3
      && maintenanceModeV3.surfaceCount === 3,
    ...
    noMaintenancePayloadStored: maintenanceModeV3.payloadsStored === false
      && surfaces.every((entry) => entry.payloadCaptured === false),
    hashCountSurfaceOnly: maintenanceModeV3.payloadsStored === false
      && maintenanceModeV3.identitySurfaceCount === 9
      && maintenanceModeV3.surfaceCount === 3
      && maintenanceModeV3.roleStateCount === 3
      && sha256EvidencePattern.test(maintenanceModeV3.roleIdentityDigest)
      && sha256EvidencePattern.test(maintenanceModeV3.stateBindingDigest)
      && sha256EvidencePattern.test(maintenanceModeV3.scopeHash),
