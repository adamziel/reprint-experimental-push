...
const generatedUnknownDriftCases = Object.freeze([
  {
    id: 'rpp-0653-unknown-drift-five-v3',
    mutationCount: 5,
    driftIndex: 3,
  },
]);
...
function tempJournalPath() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'rpp-0653-unknown-drift-v3-'));
...
function generatedSites(generatedCase) {
  ...
  const base = {
    files: {
      [preservedKey]: `base-raw-rpp-0653-${generatedCase.id}-remote-preserved`,
    },
    plugins: {},
    db: {},
  };
  ...
  for (let index = 1; index <= generatedCase.mutationCount; index++) {
    base.files[`${generatedCase.id}-target-${index}.txt`] =
      `base-raw-rpp-0653-${generatedCase.id}-target-${index}`;
  }
  ...
  for (let index = 1; index <= generatedCase.mutationCount; index++) {
    local.files[`${generatedCase.id}-target-${index}.txt`] =
      `local-raw-rpp-0653-${generatedCase.id}-target-${index}`;
  }
  remote.files[preservedKey] =
    `remote-raw-rpp-0653-${generatedCase.id}-preserved-before-plan`;
  ...
  retryRemote.files[driftTargetKey] =
    `remote-raw-rpp-0653-${generatedCase.id}-target-${generatedCase.driftIndex}-unknown-drift`;
  retryRemote.files[preservedKey] =
    `remote-raw-rpp-0653-${generatedCase.id}-preserved-after-retry-readback`;
...
function artifactRefsFor(generatedCase) {
  return {
    releaseProof: `artifact://rpp-0653/${generatedCase.id}/release-proof`,
    recoverySupport: `artifact://rpp-0653/${generatedCase.id}/unknown-drift-classification-v3`,
    durabilityScope: `artifact://rpp-0653/${generatedCase.id}/sandbox-jsonl-only`,
  };
}
...
function openRetryReadbackUnknownDriftJournal({
  ...
  assertNoRawValues(productionInspection, rawValues, 'RPP-0653 production inspection');
...
function assertUnknownDriftInspection({
  ...
  assertNoRawValues(inspection, rawValues, 'RPP-0653 unknown drift inspection');
  return driftTarget;
}
...
function assertUnknownDriftRepairBlocksRetry({
  ...
  assert.throws(
    ...
    (error) => {
      ...
      assertNoRawValues(error.details, rawValues, 'RPP-0653 repair refusal details');
      return true;
    },
  );
  ...
  assertNoRawValues(repairInspection, rawValues, 'RPP-0653 repair inspection');
...
function oldRemoteRecoveryFromInspection({ inspection, plan, checkedPath }) {
  return {
    source: 'RPP-0653 release-bound old remote recovery classification',
    status: 200,
...
function unknownDriftRecoveryFromInspection({ inspection, plan, checkedPath }) {
  return {
    source: 'RPP-0653 release-bound unknown drift recovery classification',
    status: 200,
    state: inspection.status,
    observedState: inspection.remoteClassification.state,
  ...
  return {
    ...
    releaseProof: {
      ...
      idempotencyConflict: {
        ...
        recoveryState: {
          source: 'RPP-0653 generated different-body conflict recovery state',
          storage: 'sqlite',
          state: 'fully-updated-remote',
...
function assertReleaseProofRejectsUnknownDriftEvidence({
  ...
  assertNoRawValues(releaseProof, rawValues, 'RPP-0653 rejected release proof');
}
...
function assertHashOnlyJournal(journal, rawValues) {
  ...
  assertNoRawValues(journal, rawValues, 'RPP-0653 journal rows');
}
...
test('RPP-0653 generated unknown drift classification variant 3 preserves remote changes on retry', () => {
  ...
  assertNoRawValues(
    ...
    rawValues,
    'RPP-0653 generated unknown drift proof',
  );
});

test('RPP-0653 unknown drift release proof rejects missing malformed stale or drifted evidence', () => {
  const generatedCase = generatedUnknownDriftCases[0];
  const filePath = tempJournalPath();
  const artifactRefs = artifactRefsFor(generatedCase);
  const {
