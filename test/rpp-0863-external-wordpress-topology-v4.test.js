...
const proofId = 'rpp-0863-external-wordpress-topology-v4';
const topologyProofScope = 'external-wordpress-topology-v4';
...
const goodEnv = Object.freeze({
  REPRINT_PUSH_SOURCE_URL: 'https://source.example.test/external-topology-v4',
  REPRINT_PUSH_REMOTE_URL: 'https://source.example.test/external-topology-v4/',
  REPRINT_PUSH_LOCAL_URL: 'http://127.0.0.1:8080/external-topology-v4-local',
  REPRINT_PUSH_REMOTE_CHANGED_URL: 'https://changed.example.test/external-topology-v4',
  REPRINT_PUSH_PREFLIGHT_SOURCE_URL: 'https://source.example.test/external-topology-v4/',
  REPRINT_PUSH_DRY_RUN_ROUTE_SOURCE_URL: 'https://source.example.test:443/external-topology-v4',
  REPRINT_PUSH_APPLY_SOURCE_URL: 'https://source.example.test/external-topology-v4',
  REPRINT_PUSH_JOURNAL_SOURCE_URL: 'https://source.example.test/external-topology-v4',
  REPRINT_PUSH_RECOVERY_INSPECT_SOURCE_URL: 'https://source.example.test/external-topology-v4',
  REPRINT_PUSH_USERNAME: 'topology-v4-admin',
  REPRINT_PUSH_APPLICATION_PASSWORD: 'rpp-0863-application-password-must-not-leak',
});
...
const forbiddenNeedles = Object.freeze([
  ...
  '127.0.0.1',
  'localhost',
  'admin:rpp0863-secret',
  'rpp0863-secret',
  'token=rpp0863-token',
  'rpp0863-token',
  'rpp-0863-application-password-must-not-leak',
  '8081',
  '8082',
]);

test('RPP-0863 captures source/local/changed URL identities and route bindings for external WordPress topology variant 4', () => {
  const proof = buildExternalTopologyVariant4Proof({ env: goodEnv });

  assert.equal(proof.schemaVersion, 1);
  assert.equal(proof.rppId, 'RPP-0863');
  ...
  assert.equal(proof.builtOn.contract, 'focused source/local/changed URL identity and route binding regression proof');
  ...
  assert.equal(proof.externalTopologyV4.proofScope, topologyProofScope);
  assert.equal(proof.externalTopologyV4.variant, 4);
  assert.equal(proof.externalTopologyV4.successCriterion, successCriterion);
  assert.equal(proof.externalTopologyV4.scopeAcceptedForReleaseTopology, true);
  assert.equal(proof.externalTopologyV4.roleUrlsAccepted, true);
  assert.deepEqual(proof.externalTopologyV4.capturedIdentityRoles, ['source', 'localEdited', 'remoteChanged']);
  assert.equal(proof.externalTopologyV4.sourceLocalChangedIdentitiesCaptured, true);
  assert.equal(proof.externalTopologyV4.sourceLocalChangedIdentitiesIdentityChecked, true);
  assert.equal(proof.externalTopologyV4.routeSourceBindingsIdentityChecked, true);
  assert.equal(proof.externalTopologyV4.sourceLocalChangedIdentityCount, 3);
  assert.equal(proof.externalTopologyV4.roleIdentityCount, 3);
  assert.equal(proof.externalTopologyV4.distinctRoleIdentityCount, 3);
  assert.equal(proof.externalTopologyV4.routeSourceCount, 5);
  assert.equal(proof.externalTopologyV4.configuredRouteSourceCount, 5);
  assert.equal(proof.externalTopologyV4.identitySurfaceCount, 11);
  assert.equal(proof.externalTopologyV4.rejectedSurfaceCount, 0);
  assert.deepEqual(proof.externalTopologyV4.surfaceNames, [
  ...
  assert.match(proof.externalTopologyV4.roleIdentityDigest, sha256EvidencePattern);
  assert.match(proof.externalTopologyV4.routeIdentityDigest, sha256EvidencePattern);
  assert.match(proof.externalTopologyV4.surfaceDigest, sha256EvidencePattern);
  assert.match(proof.externalTopologyV4.policyDigest, sha256EvidencePattern);
  assert.match(proof.externalTopologyV4.scopeHash, sha256EvidencePattern);
  assert.equal(proof.externalTopologyV4.payloadsStored, false);
  assert.equal(proof.externalTopologyV4.rawUrlValuesStored, false);
  assert.equal(proof.externalTopologyV4.hostnameValuesStored, false);
  assert.equal(proof.externalTopologyV4.rejectedRawInputsStored, false);
  assert.equal(proof.externalTopologyV4.releasePolicy, 'support-only-no-release-movement');
  ...
  assert.equal(proof.invariants.variant4FocusedRegressionRecorded, true);
  ...
  assert.doesNotThrow(() => assertEvidenceHasNoRawValues(proof, { label: 'RPP-0863 external topology variant 4 proof' }));
});

test('RPP-0863 rejects tunnel, secret-shaped, loopback, and packaged fallback inputs without storing raw values', () => {
  const proof = buildExternalTopologyVariant4Proof({
    env: {
      REPRINT_PUSH_SOURCE_URL: 'https://admin:rpp0863-secret@source.example.test/external-topology-v4',
      REPRINT_PUSH_LOCAL_URL: 'http://127.0.0.1:8081/external-topology-v4?token=rpp0863-token',
      REPRINT_PUSH_REMOTE_CHANGED_URL: 'https://changed.ngrok-free.app/external-topology-v4#rpp0863-fragment',
      REPRINT_PUSH_APPLY_SOURCE_URL: 'http://localhost:8082/external-topology-v4',
  ...
  assert.equal(proof.topology.rawUrlValuesStored, false);
  assert.equal(proof.externalTopologyV4.scopeAcceptedForReleaseTopology, false);
  assert.equal(proof.externalTopologyV4.roleUrlsAccepted, false);
  assert.equal(proof.externalTopologyV4.rejectedSurfaceCount, 5);
  assert.equal(proof.externalTopologyV4.rawUrlValuesStored, false);
  assert.equal(proof.externalTopologyV4.hostnameValuesStored, false);
  assert.equal(proof.externalTopologyV4.rejectedRawInputsStored, false);
  ...
  assert.deepEqual(
    proof.externalTopologyV4.rejectedSurfaceNames,
    [
      'route-source-identities-match-source',
      'no-forbidden-tunnel-hosts',
      'no-url-userinfo-query-or-fragment',
  ...
  assertRedactedTopologyProof(proof);
  assertNoNeedles(proof, forbiddenNeedles);
  assert.doesNotThrow(() => assertEvidenceHasNoRawValues(proof, { label: 'RPP-0863 rejected topology variant 4 proof' }));
});

test('RPP-0863 external topology variant 4 proof is deterministic and hash/count/surface-only', () => {
  const firstProof = buildExternalTopologyVariant4Proof({ env: goodEnv });
  const secondProof = buildExternalTopologyVariant4Proof({ env: { ...goodEnv } });

  assert.equal(firstProof.outputHash, secondProof.outputHash);
  assert.equal(firstProof.externalTopologyV4.scopeHash, secondProof.externalTopologyV4.scopeHash);
  assert.equal(firstProof.externalTopologyV4.roleIdentityDigest, secondProof.externalTopologyV4.roleIdentityDigest);
  assert.equal(firstProof.externalTopologyV4.routeIdentityDigest, secondProof.externalTopologyV4.routeIdentityDigest);
  assert.equal(firstProof.externalTopologyV4.surfaceDigest, secondProof.externalTopologyV4.surfaceDigest);
  ...
  assert.equal(firstProof.externalTopologyV4.payloadsStored, false);
  assert.equal(firstProof.externalTopologyV4.rawUrlValuesStored, false);
  assert.equal(firstProof.externalTopologyV4.hostnameValuesStored, false);
  assert.equal(firstProof.externalTopologyV4.rejectedRawInputsStored, false);
  ...
  assert.doesNotThrow(() => assertEvidenceHasNoRawValues(firstProof, { label: 'RPP-0863 first external topology proof' }));
  assert.doesNotThrow(() => assertEvidenceHasNoRawValues(secondProof, { label: 'RPP-0863 second external topology proof' }));
});

function buildExternalTopologyVariant4Proof({ env, now = fixedNow } = {}) {
  ...
  const externalTopologyV4ScopeCore = {
  ...
  const externalTopologyV4 = {
    ...
    scopeHash: `sha256:${digest(externalTopologyV4ScopeCore)}`,
    payloadsStored: false,
    rawUrlValuesStored: false,
    hostnameValuesStored: false,
    rejectedRawInputsStored: false,
  ...
  const roleIdentityChecks = {
    ...
    capturedRoleCount: externalTopologyV4.capturedRoleIdentityCount,
    validRoleCount: externalTopologyV4.validRoleIdentityCount,
    acceptedRoleCount: externalTopologyV4.acceptedRoleIdentityCount,
    distinctRoleIdentityCount,
    sourceLocalChangedUrlIdentitiesCaptured,
  ...
  const invariants = {
    ...
    hashCountSurfaceOnly: externalTopologyV4.payloadsStored === false
      && externalTopologyV4.rawUrlValuesStored === false
      && externalTopologyV4.hostnameValuesStored === false
      && externalTopologyV4.rejectedRawInputsStored === false
      && externalTopologyV4.identitySurfaceCount === 11
      && sha256EvidencePattern.test(externalTopologyV4.scopeHash)
      && roleIdentities.every((entry) => sha256Pattern.test(entry.identityHash) && sha256Pattern.test(entry.originHash)),
    variant4FocusedRegressionRecorded: externalTopologyV4.proofScope === topologyProofScope
      && externalTopologyV4.variant === 4
      && externalTopologyV4.successCriterion === successCriterion
      && externalTopologyV4.sourceLocalChangedIdentitiesCaptured === true
      && externalTopologyV4.routeSourceBindingsIdentityChecked === true,
  ...
  const proofCore = {
    ...
    builtOn: {
      ...
      contract: 'focused source/local/changed URL identity and route binding regression proof',
    },
    topology: {
