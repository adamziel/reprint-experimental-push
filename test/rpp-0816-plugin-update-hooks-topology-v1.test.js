...
const evidencePath = path.join(
  repoRoot,
  'docs/evidence/rpp-0816-plugin-update-hooks-topology-v1.md',
);
const hexSha256Pattern = /^[a-f0-9]{64}$/;
const prefixedSha256Pattern = /^sha256:[a-f0-9]{64}$/;

test('RPP-0816 records plugin update hook topology candidate requirements', () => {
  const { report, text } = loadProgressReport();

  assert.equal(report.schemaVersion, 1);
  assert.equal(report.rppId, 'RPP-0816');
...
test('RPP-0816 topology command records exact unavailable capability fail-closed', () => {
...
test('RPP-0816 evidence remains hash/count/surface only', () => {
  const { report, text } = loadProgressReport();
  ...
  assert.doesNotMatch(
    text,
    /ngrok|cloudflared|localtunnel|serveo|localhost\.run|tailsail|tailscale funnel/i,
  );
});
...
function loadProgressReport() {
  ...
  assert.ok(match?.groups?.json, 'RPP-0816 evidence must contain one JSON progress report block');
  return {
    text,
...
function buildMissingDockerCapabilityArtifact() {
  const plan = buildDockerTopologyPlan({
    ...
    workDir: '/tmp/rpp-0816-docker-work',
    evidenceDir: '/tmp/rpp-0816-docker-evidence',
    env: {},
  });
