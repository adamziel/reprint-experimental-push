...
const evidencePath = path.join(repoRoot, 'docs/evidence/rpp-0982-release-gate-2-final-audit-release-verifier-v5.md');
const legacyEvidencePath = path.join(repoRoot, 'docs/evidence/rpp-0962-release-gate-2-final-audit-v4.md');
const releaseGatesPath = path.join(repoRoot, '.agents/RELEASE_GATES.md');
const fixedNowIso = '2026-06-01T04:00:00.000Z';
const auditedHead = 'e919309327a591cb0fa0842463f9d8df3854ebb2';
const supportBoundaryVerdict = 'PACKAGED_RELEASE_BOUNDARY_OK';
const liveBoundaryVerdict = 'LIVE_RELEASE_BOUNDARY_OK';

test('RPP-0982 audit report records GATE-2 support_only and final release NO-GO', () => {
  const { report, text } = loadAuditReport(evidencePath, 'RPP-0982');

  assert.deepEqual(report, expectedAuditReport());
  assert.equal(report.rppId, 'RPP-0982');
...
test('RPP-0982 current release gate row keeps durable recovery journal support_only', () => {
...
test('RPP-0982 carries forward the RPP-0962 v4 GATE-2 audit contract', () => {
  const { report } = loadAuditReport(evidencePath, 'RPP-0982');
  const { report: legacyReport } = loadAuditReport(legacyEvidencePath, 'RPP-0962');

  assert.equal(report.rpp0962ContractCarryForward.sourceAudit, 'RPP-0962 v4');
  ...
  assert.deepEqual(report.productionBackedTopologyRule, legacyReport.productionBackedTopologyRule);
...
test('RPP-0982 support evidence alone can reach candidate review but cannot move release', () => {
...
test('RPP-0982 final release evaluator remains held without production-backed source local changed proof', () => {
...
test('RPP-0982 failed or missing production-backed source local changed proof keeps final release NO-GO', () => {
  const cases = [
    {
      name: 'missing source',
...
test('RPP-0982 durable recovery proof only reports proven on a production-backed live boundary', () => {
...
test('RPP-0982 evidence does not claim or perform release-gate status movement', () => {
  const { report, text } = loadAuditReport(evidencePath, 'RPP-0982');
...
function expectedAuditReport() {
  return {
    ...
    rppId: 'RPP-0982',
    variant: 5,
    title: 'Release gate 2 final audit release verifier v5',
    checkedAt: fixedNowIso,
    auditedBranch: 'session/rpp-982',
    ...
    supportEvidence: {
      ...
      contractCarriedForwardFrom: 'RPP-0962 v4',
    ...
    rpp0962ContractCarryForward: {
      sourceAudit: 'RPP-0962 v4',
      carriedFields: [
        'GATE-2 support_only',
        'productionMovementRule',
        'productionBackedTopologyRule',
      ...
      extension: 'release-verifier carry-through keeps failed or missing production-backed source/local/remote-changed proof as final-release NO-GO',
    },
    evidenceLimits: {
...
function buildReleaseSummary(boundaryVerdict) {
  const activeClaimId = 'claim-active-rpp-0982';
  const previousClaimId = 'claim-previous-rpp-0982';
