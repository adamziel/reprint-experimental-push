...
const proofId = 'rpp-0770-parallel-snapshot-hashing-v4';
...
test('RPP-0770 variant 4 updates fast-path lane only after correctness gates hold', () => {
  const cases = variant4SnapshotHashCases();
  const decisions = cases.map((snapshotCase) => {
    const decision = resolveVariant4FastPathLane(snapshotCase.evidence, {
  ...
  const summary = variant4DecisionSummary(decisions);

  assert.equal(summary.caseCount, 10);
  assert.equal(summary.laneUpdates, 1);
  assert.equal(summary.blockedLaneAttempts, 9);
  ...
  assert.equal(summary.blockerCounts['correctness-gates-not-passed'], 8);
});

test('RPP-0770 variant 4 projects deterministic support-only lane gating evidence', {
  concurrency: false,
}, () => {
  const first = runSmallBenchmark();
  const second = runSmallBenchmark();
  const proof = buildVariant4Proof(first, second);

  assert.equal(proof.rppId, 'RPP-0770');
  assert.equal(proof.proofId, proofId);
  assert.equal(proof.variant, 4);
  assert.equal(proof.status, 'passed');
  assert.equal(proof.generatedCoverage.source, 'local-support-variant4-snapshot-hash-regression-cases');
  assert.equal(proof.generatedCoverage.caseCount, 10);
  assert.equal(proof.generatedCoverage.laneUpdates, 1);
  assert.equal(proof.generatedCoverage.blockedLaneAttempts, 9);
  ...
  const failingBudgetProof = buildVariant4Proof(
...
function runSmallBenchmark(overrides = {}) {
  return runGuardedExecutorBenchmark({
    ...
    now: fixedNow,
    tempDir: fs.mkdtempSync(path.join(os.tmpdir(), 'reprint-rpp-0770-')),
    ...overrides,
  });
}
...
function buildVariant4Proof(report, repeatedReport) {
  ...
  const safeDecision = resolveVariant4FastPathLane(snapshotEvidence, {
    attemptedLaneUpdate: snapshotEvidence.fastPathLane.updated,
  });
  const generatedDecisions = variant4SnapshotHashCases().map((snapshotCase) => ({
    caseId: snapshotCase.id,
    ...resolveVariant4FastPathLane(snapshotCase.evidence, {
      attemptedLaneUpdate: true,
    }),
  }));
  const generatedCoverage = variant4DecisionSummary(generatedDecisions);
  ...
  const gates = [
    ...
    proofGate('variant4-regression-cases-block-pre-gate-lane-updates',
      generatedCoverage.caseCount === 10
        && generatedCoverage.laneUpdates === 1
        && generatedCoverage.blockedLaneAttempts === 9
  ...
  const publicEvidence = {
    ...
    generatedCoverage: {
      source: 'local-support-variant4-snapshot-hash-regression-cases',
      caseIds: generatedCoverage.caseIds,
...
function resolveVariant4FastPathLane(evidence, { attemptedLaneUpdate }) {
...
function variant4SnapshotHashCases() {
  ...
  return [
    ...
    },
    {
      ...
      evidence: syntheticSnapshotEvidence({
        hashSet: {
          parallelDigest: sha256Label('rpp-0770-stale-parallel-digest'),
          secondRunDigest: sha256Label('rpp-0770-stale-parallel-digest'),
    ...
    },
    {
      ...
      evidence: syntheticSnapshotEvidence({
        hashSet: {
          secondRunDigest: sha256Label('rpp-0770-nondeterministic-second-run'),
          ...
          hashSamples: [
            {
              ...
              resourceKeyHash: sha256Label('rpp-0770-redaction-key'),
              resourceValueHash: sha256Label('rpp-0770-redaction-value'),
              workerSlot: 0,
              waveIndex: 0,
            },
          ],
    ...
    },
    {
      ...
      evidence: syntheticSnapshotEvidence({
        correctnessGates: expectedSnapshotGateIds.map((id, index) => ({
          ...
          evidence: {
            gateStatusHash: sha256Label(`rpp-0770-recorded-gate-${id}-${index}`),
          },
        })),
      }),
      expected: {
...
function variant4DecisionSummary(decisions) {
  const blockerCounts = {};
