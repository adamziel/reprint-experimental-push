...
const rawFixtureValues = [
  'rpp-0675-raw-idempotency-key',
  'rpp-0675-private-apply-body',
  'rpp-0675-private-replay-body',
  'rpp-0675-private-remote-before',
  'rpp-0675-private-remote-after',
];
...
function fixtureHash(label) {
  return sha256Hex(`rpp-0675:${label}`);
}
...
function assertNoRawValues(value, rawValues = rawFixtureValues) {
  ...
  for (const rawValue of rawValues) {
    assert.equal(
      ...
      false,
      `RPP-0675 support evidence leaked raw fixture value: ${rawValue}`,
    );
  }
}

function buildDurableJournal() {
  const activeClaimId = 'psh_rpp0675_active_claim';
  const activeClaimKeyHash = fixtureHash('active-claim-key');
  const previousClaimId = 'psh_rpp0675_previous_claim';
  ...
  return {
    ...
    releaseProof: {
      ...
      idempotencyConflict: {
        ...
        recoveryState: {
          source: 'RPP-0675 generated different-body conflict recovery state',
          storage: 'sqlite',
          state: 'fully-updated-remote',
          restartReadable: true,
          counts: {
      ...
      staleClaimRetry: {
        oldRemoteRecovery: {
          source: 'RPP-0675 generated stale-owner retry before mutation',
          status: 200,
          state: 'old-remote',
      ...
      replayAndRetry: {
        required: 'artifact://rpp-0675-checked-recovery-path',
        observed: 'artifact://rpp-0675-checked-recovery-path',
        retryAttempts: 2,
...
function baseRejectedReplayEnvelope() {
  ...
  const envelope = {
    ...
    issue: 'RPP-0675',
    variant: 4,
    proofClass: 'same-key-replay-after-rejection-v4',
...
function validateRejectedReplayEvidence(evidence) {
  ...
  if (
    ...
    || evidence.issue !== 'RPP-0675'
    || evidence.variant !== 4
    || evidence.proofClass !== 'same-key-replay-after-rejection-v4'
...
test('RPP-0675 generated same-key replay after rejection v4 proves GATE-2 on the same path', () => {
...
test('RPP-0675 generated replay evidence rejects missing malformed stale duplicated and drifted fixtures', () => {
