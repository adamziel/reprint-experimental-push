...
const priorEvidencePath = path.join(repoRoot, 'docs/evidence/rpp-0973-github-pages-progress-publish-v4.md');
...
const matchingProgressHash = 'sha256:9999999999999999999999999999999999999999999999999999999999999999';
const mismatchedPublicHash = 'sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa';
...
function writeFixture(t, fixture) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'rpp-993-progress-publish-v5-'));
  t.after(() => {
...
function comparableRequiredArtifacts(report) {
  return report.requiredProof.requiredArtifacts.map((artifact) => (
    artifact.replace(/docs\/evidence\/rpp-\d+-github-pages-progress-publish(?:-release-verifier)?-v\d+\.md/, '<variant-evidence>')
  ));
}

test('RPP-0993 evidence carries forward RPP-0973 v4 progress publish contract', () => {
  const { text, report } = loadEvidenceReport();
  const { report: priorReport } = loadEvidenceReport(priorEvidencePath);

  assert.equal(report.schemaVersion, 1);
  assert.equal(report.rppId, 'RPP-0993');
  assert.equal(report.workerId, 'rpp-993');
  assert.equal(report.variant, 5);
  assert.equal(report.auditedBranch, 'session/rpp-993');
  assert.equal(report.auditedLaneHeadBeforeEvidence, 'b1dde27b0250b966ac14d29caefc37d155a9b7c3');
  assert.equal(report.evidenceMode, 'support-only-release-verifier-required-check-fixture');
  assert.equal(report.supportOnly, true);
  assert.equal(report.productionBacked, false);
  assert.equal(report.releaseEligible, false);
  assert.equal(report.finalReleaseStatus, 'NO-GO');
  assert.equal(report.releaseGateStatusMovement, 'none');
  assert.equal(report.carriedForwardFrom.rppId, 'RPP-0973');
  assert.equal(report.carriedForwardFrom.variant, 4);
  ...
  assert.equal(report.contractCarryForward.releaseVerifierFailClosedBoundaryUnchanged, true);
  assert.equal(report.publicProgressPublishVerification.missingRequiredProofBlocksRelease, true);
  assert.equal(report.publicProgressPublishVerification.failedRequiredProofBlocksRelease, true);
...
test('RPP-0993 public progress publish verification fails closed when proof is missing or stale', () => {
...
test('RPP-0993 public progress publish verification fails closed on mismatched or missing public artifact', () => {
...
test('RPP-0993 required release checks exit nonzero and remain held on failed publish proof', (t) => {
  const failureCases = [
    {
...
test('RPP-0993 required release checks exit nonzero on missing, stale, or incomplete publish proof', (t) => {
...
test('RPP-0993 progress.html publication remains support-only and cannot move final release', () => {
  const { report } = loadEvidenceReport();
...
test('RPP-0993 unresolved production proof gaps stay open and fail closed', () => {
  const { report, text } = loadEvidenceReport();

  assert.equal(report.releaseVerifierCarryThrough.unresolvedProductionBackedProofGapsFailClosed, true);
  assert.equal(report.releaseVerifierCarryThrough.supportOnlyObservationCanMoveFinalRelease, false);
  assert.equal(report.unresolvedProductionBackedProofGaps.length, 3);
  for (const gap of report.unresolvedProductionBackedProofGaps) {
    assert.equal(gap.status, 'open');
    assert.equal(gap.releaseImpact, 'blocks-final-release');
  }
  assert.match(text, /Unresolved production-backed proof gaps remain open and fail closed/);
  assert.match(text, /Final release remains `NO-GO`/);
  assert.doesNotMatch(text, /releaseEligible": true/);
});

test('RPP-0993 support evidence remains redacted and release-gate neutral', async () => {
  ...
  assert.match(text, /does not move\s+release-gate status/);
