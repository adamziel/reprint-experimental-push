...
function fixtureHash(label) {
  return sha256Hex(`rpp-0572:${label}`);
}
...
test('RPP-0572 generated canonical signed requests normalize equivalent request shapes', () => {
  ...
  const results = canonicalCases.map((canonicalCase, index) => {
    const nonce = `rpp-0572-canon-${String(index).padStart(2, '0')}`;
...
test('RPP-0572 positive support path reaches parsing receipt and mutation counters only after signature proof', () => {
  const positiveCases = [
    {
      ...
      request: buildSignedRequest({
        ...
        nonce: 'rpp-0572-positive-dry-run',
...
test('RPP-0572 generated malformed and tampered signed requests fail before parsing receipt or mutation work', () => {
  const negativeCases = [
    {
  ...
  const results = negativeCases.map((negativeCase, index) => {
    ...
    const request = buildSignedRequest({
      ...
      nonce: `rpp-0572-negative-${String(index).padStart(2, '0')}`,
    });
...
test('RPP-0572 authenticated route source orders canonical signature verification before JSON receipt and mutation work', () => {
