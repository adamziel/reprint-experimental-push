...
const evidenceDocPath = 'docs/evidence/rpp-0978-telemetry-free-audit-mode-v4.md';
const fixedNow = '2026-06-01T04:35:00.000Z';
const observedAt = '2026-06-01T04:05:00.000Z';
...
function writeObservations(t, observations, prefix = 'rpp-0978-audit-') {
...
function loadEvidenceRecord() {
  ...
  assert.ok(match?.groups?.json, 'RPP-0978 evidence must contain one JSON record block');
  return {
    text,
...
test('RPP-0978 failed required proof blocks release without telemetry fields', (t) => {
...
test('RPP-0978 report command fails closed when required proof is missing', (t) => {
...
test('RPP-0978 release movement has no telemetry dependency and no status movement', () => {
...
test('RPP-0978 support-only evidence keeps final release NO-GO', () => {
  const { record } = loadEvidenceRecord();
  const controlSummary = summarizeRequiredReleaseChecks({
    observations: passingObservations(),
    now: fixedNow,
...
test('RPP-0978 support evidence is redaction-safe and records no release-gate movement', async () => {
  const { text, record } = loadEvidenceRecord();

  assert.equal(record.schemaVersion, 1);
  assert.equal(record.rppId, 'RPP-0978');
  assert.equal(record.workerId, 'rpp-978');
  ...
  assert.match(text, /Audited lane head: RPP-0978 telemetry-free audit mode v4/);
