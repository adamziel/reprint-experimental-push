...
const evidencePath = path.join(repoRoot, 'docs/evidence/rpp-0997-support-escalation-guide-release-verifier-v5.md');
const patternEvidencePath = path.join(repoRoot, 'docs/evidence/rpp-0977-support-escalation-guide-v4.md');
...
const expectedValidationCommands = Object.freeze([
  ...
  'node --check test/rpp-0997-support-escalation-guide-release-verifier-v5.test.js',
  'node --test --test-name-pattern RPP-0997 test/rpp-0997-support-escalation-guide-release-verifier-v5.test.js',
  'node scripts/release/artifact-redaction-scan.mjs docs/evidence/rpp-0997-support-escalation-guide-release-verifier-v5.md',
  'git diff --check',
]);

test('RPP-0997 evidence records support-only escalation guide release verifier v5 with held release posture', () => {
  const { text, report } = loadEvidenceReport();

  assert.match(text, /^# RPP-0997 support escalation guide release verifier v5 evidence$/m);
  assert.match(text, /Audited local branch: `session\/rpp-997`/);
  ...
  assert.match(text, /keeps the final release at\s+\*\*NO-GO\*\*/);

  assert.equal(report.schemaVersion, 1);
  assert.equal(report.rppId, 'RPP-0997');
  assert.equal(report.sliceId, 'RPP-0997');
  assert.equal(report.proofId, 'rpp-0997-support-escalation-guide-release-verifier-v5');
  assert.equal(report.variant, 5);
  assert.equal(report.auditedBranch, 'session/rpp-997');
  assert.equal(report.auditedLaneHeadBeforeEvidence, 'ca4584ef297b20519a5171a1a4da5ef9a5cec1b1');
  assert.equal(report.status, 'support-only-escalation-guide-release-verifier-v5-recorded');
...
test('RPP-0997 carries forward the RPP-0977 v4 support escalation guide contract', () => {
  const { report } = loadEvidenceReport();
  const { report: patternReport } = loadEvidenceReport(patternEvidencePath, 'RPP-0977');

  assert.equal(report.documents.patternEvidence, 'docs/evidence/rpp-0977-support-escalation-guide-v4.md');
  ...
  assert.equal(patternReport.documents.patternEvidence, report.documents.previousPatternEvidence);
...
test('RPP-0997 owners, prerequisites, and stop conditions are named but cannot move release', () => {
  const { report } = loadEvidenceReport();
...
test('RPP-0997 escalation triggers require production proof and bind owner prerequisites stop conditions', () => {
  const { report } = loadEvidenceReport();
...
test('RPP-0997 final release gate remains NO-GO and movement remains production-backed-evidence gated', () => {
  const { report } = loadEvidenceReport();
...
test('RPP-0997 artifact is redacted and records exact validation commands', async () => {
  const { text, report } = loadEvidenceReport();
  const scan = await scanArtifacts(['docs/evidence/rpp-0997-support-escalation-guide-release-verifier-v5.md'], {
    cwd: repoRoot,
  });

  assert.doesNotThrow(() =>
    assertEvidenceHasNoRawValues(report, { label: 'RPP-0997 support escalation guide release verifier v5 evidence' }));
...
function loadEvidenceReport(filePath = evidencePath, label = 'RPP-0997') {
