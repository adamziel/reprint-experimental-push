...
const sessionId = 'psh_01j00000000000000000599';
const rotatedSessionId = 'psh_01j0000000000000000599r';
const staleSessionId = 'psh_01j0000000000000000599s';
const driftedSessionId = 'psh_01j0000000000000000599d';
const idempotencyKey = 'idem-rpp-0599-credential-rotation-v5';
const resourcePath = 'wp-content/uploads/reprint-push/rpp-0599-credential-rotation.txt';
...
const credentials = {
  bound: {
    ...
    password: 'rpp-0599-bound-application-password',
  },
  rotated: {
    username: 'reprint_push_admin',
    password: 'rpp-0599-rotated-application-password',
  },
  invalidated: {
    username: 'reprint_push_admin',
    password: 'rpp-0599-invalidated-application-password',
  },
};
...
function fixtureHash(label) {
  return sha256Hex(`rpp-0599:${label}`);
}
...
function credentialKind(headers) {
  ...
  if (!/^[A-Za-z0-9+/]+={0,2}$/.test(token) || token.length % 4 === 1) {
    return 'malformed';
  }
...
function authEnvelope(kind = 'bound') {
  ...
  return {
    ...
    session: {
      ...
      applicationPasswordUuid: `app-pass-rpp-0599-${kind}`,
...
function buildReadyPlan() {
  ...
  const mutationId = 'mut_rpp_0599_apply_revalidates_live_source';
  const baseHash = fixtureHash('resource-base');
  const localHash = fixtureHash('resource-local');

  return {
    id: 'plan-rpp-0599-credential-rotation-behavior-v5',
    status: 'ready',
    summary: {
      mutations: 1,
      decisions: 0,
...
function rejectionPhase(kind) {
  if (kind === 'rotated') {
    return 'apply-signed-session-binding';
  }
  if (kind === 'stale-session' || kind === 'drifted-session') {
    return 'apply-session-identity';
  }
  return 'apply-authentication';
}

function observedCredentialHash(kind) {
  if (credentials[kind]) {
    return credentialHash(credentials[kind]);
  }
  if (kind === 'stale-session' || kind === 'drifted-session') {
    return credentialHash(credentials.bound);
  }
  return fixtureHash(`observed-${kind}-credential-evidence`);
}

function rejectionEnvelope({ kind, code, contentHash, state, session }) {
  return {
    ok: false,
    mode: 'apply',
    code,
    credentialRotation: {
      schemaVersion: 1,
      phase: rejectionPhase(kind),
      evidenceKind: kind,
      observedCredentialHash: observedCredentialHash(kind),
      ...
      applyPayloadParsesAtReject: state.applyPayloadParses,
      receiptValidationAttemptsAtReject: state.receiptValidationAttempts,
      releaseMovementAttemptsAtReject: state.releaseMovementAttempts,
    },
    idempotency: {
      replayed: false,
      conflict: false,
...
function createGeneratedCredentialRotationRoute() {
  const currentSnapshot = {
    files: { [resourcePath]: 'rpp-0599-live-source-before-accepted-apply' },
    plugins: {},
    db: {},
  };
  const state = {
  ...
  function rejectApplyCredentialEvidence({ kind, code, contentHash, session, status = 401 }) {
    state.preParseApplyRejections += 1;
  ...
  async function fetchHandler(url, options = {}) {
    ...
    if (pathname === `${routePrefix}/preflight`) {
      if (['invalidated', 'missing', 'malformed', 'unknown'].includes(kind)) {
        return jsonResponse({
          ok: false,
          mode: 'preflight',
    ...
    if (pathname === `${routePrefix}/apply`) {
      const requestSession = headers['x-reprint-push-session'] || '';

      if (kind === 'missing') {
        return rejectApplyCredentialEvidence({
          kind,
          code: 'reprint_push_lab_auth_required',
          contentHash,
      ...
      if (kind === 'malformed' || kind === 'unknown') {
        return rejectApplyCredentialEvidence({
          kind: 'malformed',
          code: 'MALFORMED_CREDENTIAL_EVIDENCE',
          contentHash,
          session: requestSession,
        });
      }

      if (requestSession === staleSessionId) {
        return rejectApplyCredentialEvidence({
          kind: 'stale-session',
          code: 'STALE_SESSION_BINDING',
          contentHash,
          session: requestSession,
        });
      }

      if (requestSession === driftedSessionId || requestSession === rotatedSessionId) {
        return rejectApplyCredentialEvidence({
          kind: 'drifted-session',
      ...
      assert.equal(kind, 'bound');
      assert.equal(requestSession, sessionId);
      assert.equal(headers['x-reprint-push-idempotency-key'], idempotencyKey);
      assert.equal(contentHash, sha256Hex(rawBody));
    ...
    throw new Error(`unexpected RPP-0599 route request: ${method} ${pathname}`);
  }
...
async function directApplyProbe(fetchHandler, {
  authorization,
  session = sessionId,
  key = idempotencyKey,
  body = '{not-json-for-preparse-proof',
}) {
  const headers = {
    'content-type': 'application/json',
    'x-auth-content-hash': sha256Hex(body),
    'x-auth-nonce': fixtureHash(`nonce:${session}`),
    'x-auth-timestamp': fixedNow,
    'x-auth-signature': fixtureHash(`signature:${session}`),
    'x-reprint-push-session': session,
    'x-reprint-push-idempotency-key': key,
  };
  if (authorization) {
    headers.authorization = authorization;
  }

  const response = await fetchHandler(`${sourceUrl}${routePrefix}/apply`, {
    method: 'POST',
    headers,
    body,
  });
...
function supportCode({
  ...
  rotatedRejected,
  invalidatedRejected,
  preParseCredentialEvidenceRejected,
}) {
  if (!preParseCredentialEvidenceRejected) {
    return 'PRE_PARSE_CREDENTIAL_EVIDENCE_REJECTION_REQUIRED';
  }
...
function buildCredentialRotationSupportEvidence({
  ...
  const invalidatedRejected = invalidatedApply.status === 401
    ...
    && invalidatedApply.body.credentialRotation?.applyPayloadParsesAtReject === 0
    && invalidatedApply.body.credentialRotation?.receiptValidationAttemptsAtReject === 0
    ...
    && invalidatedApply.body.credentialRotation?.releaseMovementAttemptsAtReject === 0;
  const rotatedRejected = rotatedApply.status === 401
    ...
    && rotatedApply.body.credentialRotation?.applyPayloadParsesAtReject === 0
    && rotatedApply.body.credentialRotation?.receiptValidationAttemptsAtReject === 0
    ...
    && rotatedApply.body.credentialRotation?.releaseMovementAttemptsAtReject === 0;
  const preParseCredentialEvidenceRejected = preParseRejections.length === 4
    && preParseRejections.every((entry) => entry.status === 401
      && entry.body.idempotency?.freshMutationWork === false
      && entry.body.credentialRotation?.applyPayloadParsesAtReject === 0
      && entry.body.credentialRotation?.receiptValidationAttemptsAtReject === 0
      && entry.body.credentialRotation?.mutationExecutorEntriesAtReject === 0
      && entry.body.credentialRotation?.mutationApplicationsAtReject === 0
      && entry.body.credentialRotation?.releaseMovementAttemptsAtReject === 0);
  const accepted = acceptedApply.status === 200
    ...
    && beforeFirstMutation;
  const ok = preParseCredentialEvidenceRejected && invalidatedRejected && rotatedRejected && accepted;
  ...
  return {
    ...
    rejectedCredentialBinding: {
      preParse: Object.fromEntries(preParseRejections.map((entry) => [
        entry.body.credentialRotation.evidenceKind,
        {
          status: entry.status,
          code: entry.body.code,
          phase: entry.body.credentialRotation.phase,
          observedCredentialHash: entry.body.credentialRotation.observedCredentialHash,
          expectedCredentialHash: entry.body.credentialRotation.expectedCredentialHash,
          requestHash: entry.body.credentialRotation.requestHash,
          sessionHash: entry.body.credentialRotation.sessionHash,
          freshMutationWork: entry.body.idempotency.freshMutationWork,
          applyPayloadParsesAtReject: entry.body.credentialRotation.applyPayloadParsesAtReject,
          receiptValidationAttemptsAtReject: entry.body.credentialRotation.receiptValidationAttemptsAtReject,
          mutationExecutorEntriesAtReject: entry.body.credentialRotation.mutationExecutorEntriesAtReject,
          mutationApplicationsAtReject: entry.body.credentialRotation.mutationApplicationsAtReject,
          releaseMovementAttemptsAtReject: entry.body.credentialRotation.releaseMovementAttemptsAtReject,
        },
      ])),
      invalidated: {
        ...
        sessionHash: invalidatedApply.body.credentialRotation.sessionHash,
        freshMutationWork: invalidatedApply.body.idempotency.freshMutationWork,
        applyPayloadParsesAtReject: invalidatedApply.body.credentialRotation.applyPayloadParsesAtReject,
        receiptValidationAttemptsAtReject: invalidatedApply.body.credentialRotation.receiptValidationAttemptsAtReject,
        ...
        releaseMovementAttemptsAtReject: invalidatedApply.body.credentialRotation.releaseMovementAttemptsAtReject,
      },
      rotated: {
        ...
        sessionHash: rotatedApply.body.credentialRotation.sessionHash,
        freshMutationWork: rotatedApply.body.idempotency.freshMutationWork,
        applyPayloadParsesAtReject: rotatedApply.body.credentialRotation.applyPayloadParsesAtReject,
        receiptValidationAttemptsAtReject: rotatedApply.body.credentialRotation.receiptValidationAttemptsAtReject,
        ...
        releaseMovementAttemptsAtReject: rotatedApply.body.credentialRotation.releaseMovementAttemptsAtReject,
      },
    },
    acceptedApply: {
...
test('RPP-0599 v5 keeps credential checks and live-source revalidation before mutation', () => {
...
test('RPP-0599 v5 rejects credential/session evidence before parsing and mutation work', async () => {
  ...
  try {
    ...
    const missingApply = await directApplyProbe(route.fetchHandler, {
      authorization: '',
    });
    const malformedApply = await directApplyProbe(route.fetchHandler, {
      authorization: 'Basic not-valid-base64!',
    });
    const staleSessionApply = await directApplyProbe(route.fetchHandler, {
      authorization: authorizationHeader(credentials.bound),
      session: staleSessionId,
    });
    const driftedSessionApply = await directApplyProbe(route.fetchHandler, {
      authorization: authorizationHeader(credentials.bound),
      session: driftedSessionId,
    });
    const preParseRejections = [
      missingApply,
      malformedApply,
      staleSessionApply,
      driftedSessionApply,
    ...
    assert.equal(missingApply.status, 401);
    assert.equal(missingApply.body.code, 'reprint_push_lab_auth_required');
    assert.equal(malformedApply.status, 401);
    assert.equal(malformedApply.body.code, 'MALFORMED_CREDENTIAL_EVIDENCE');
    assert.equal(staleSessionApply.status, 401);
    assert.equal(staleSessionApply.body.code, 'STALE_SESSION_BINDING');
    assert.equal(driftedSessionApply.status, 401);
    assert.equal(driftedSessionApply.body.code, 'SESSION_IDENTITY_DRIFT');
    for (const rejectedApply of preParseRejections) {
      assert.equal(rejectedApply.body.idempotency.freshMutationWork, false);
      assert.equal(rejectedApply.body.credentialRotation.applyPayloadParsesAtReject, 0);
      assert.equal(rejectedApply.body.credentialRotation.receiptValidationAttemptsAtReject, 0);
      assert.equal(rejectedApply.body.credentialRotation.mutationExecutorEntriesAtReject, 0);
      assert.equal(rejectedApply.body.credentialRotation.mutationApplicationsAtReject, 0);
      assert.equal(rejectedApply.body.credentialRotation.releaseMovementAttemptsAtReject, 0);
    }
    assert.equal(route.state.applyPayloadParses, 0);
    assert.equal(route.state.receiptValidationAttempts, 0);
    assert.equal(route.state.mutationExecutorEntries, 0);
    assert.equal(route.state.mutationApplications, 0);
    assert.equal(route.state.releaseMovementAttempts, 0);
    assert.equal(route.state.preParseApplyRejections, 4);
    ...
    assert.equal(invalidatedApply.body.credentialRotation.applyPayloadParsesAtReject, 0);
    assert.equal(invalidatedApply.body.credentialRotation.receiptValidationAttemptsAtReject, 0);
    ...
    assert.equal(invalidatedApply.body.credentialRotation.releaseMovementAttemptsAtReject, 0);
    assert.equal(route.state.mutationExecutorEntries, 0);
    assert.equal(route.state.mutationApplications, 0);
    ...
    assert.equal(rotatedApply.body.credentialRotation.applyPayloadParsesAtReject, 0);
    assert.equal(rotatedApply.body.credentialRotation.receiptValidationAttemptsAtReject, 0);
    ...
    assert.equal(rotatedApply.body.credentialRotation.releaseMovementAttemptsAtReject, 0);
    assert.equal(route.state.mutationExecutorEntries, 0);
    assert.equal(route.state.mutationApplications, 0);
    assert.equal(route.state.preParseApplyRejections, 6);
    ...
    assert.equal(route.state.receiptValidationAttempts, 1);
    assert.equal(route.state.releaseMovementAttempts, 0);
    ...
    assert.deepEqual(Object.keys(evidence.rejectedCredentialBinding.preParse), [
      'missing',
      'malformed',
      'stale-session',
      'drifted-session',
    ...
    for (const rejection of Object.values(evidence.rejectedCredentialBinding.preParse)) {
      assert.equal(rejection.freshMutationWork, false);
      assert.equal(rejection.applyPayloadParsesAtReject, 0);
      assert.equal(rejection.receiptValidationAttemptsAtReject, 0);
      assert.equal(rejection.mutationExecutorEntriesAtReject, 0);
      assert.equal(rejection.mutationApplicationsAtReject, 0);
      assert.equal(rejection.releaseMovementAttemptsAtReject, 0);
    }
    ...
    for (const rejection of [
      ...Object.values(evidence.rejectedCredentialBinding.preParse),
      evidence.rejectedCredentialBinding.invalidated,
      evidence.rejectedCredentialBinding.rotated,
    ]) {
