...
const signedNonce = 'rpp0557acceptednonce';
const sessionId = 'psh_01j00000000000000000557';
const invalidSessionId = 'psh_01j00000000000000090557';
const idempotencyKey = 'idem-rpp-0557-same-key-different-body-v3';
...
const wrongCredential = {
  ...
  password: 'rpp-0557-wrong-application-password',
};
const auth = {
  identity: {
    userId: 557,
...
function fixtureHash(label) {
  return sha256Hex(`rpp-0557:${label}`);
}
...
function summarizeNegativeAuthEvidence(response) {
  ...
  return {
    schemaVersion: 1,
    slice: 'RPP-0557',
    proofClass: 'same-key-different-body-conflict-v3',
...
test('RPP-0557 v3 route order rejects bad auth before JSON and conflict before mutation setup', () => {
...
test('RPP-0557 v3 proves same-key different-body conflict is hash-only before fresh mutation work', async () => {
  const originalFetch = global.fetch;
  const idempotencyKeyHash = sha256Hex(idempotencyKey);
  const receiptHash = fixtureHash('dry-run-receipt');
  const base = {
    files: {
      'wp-content/uploads/reprint-push/rpp-0557.txt': 'base-body',
    },
    plugins: {},
    db: {},
  };
  const local = {
    files: {
      'wp-content/uploads/reprint-push/rpp-0557.txt': 'local-body',
    },
    plugins: {},
    db: {},
  };
  ...
  let applyCount = 0;
  let freshMutationSetupCount = 0;
  let mutationWorkCount = 0;
  let conflictMutationSetupCount = 0;
  ...
  global.fetch = async (url, options = {}) => {
    ...
    if (pathname === `${routePrefix}/apply`) {
      ...
      if (body.durableJournalBoundaryProbe) {
        ...
        assert.equal(freshMutationSetupCount, 1, 'conflict probe must not start fresh mutation setup');
  ...
  try {
    ...
    assert.equal(freshMutationSetupCount, 1);
    assert.equal(mutationWorkCount, 1);
    assert.equal(conflictMutationSetupCount, 0);
    ...
    assertNoRawValues(conflictResponseBody.idempotency, [
      ...
      credential.password,
      sessionId,
      base.files['wp-content/uploads/reprint-push/rpp-0557.txt'],
      local.files['wp-content/uploads/reprint-push/rpp-0557.txt'],
    ]);
    assertNoRawValues({
...
test('RPP-0557 v3 negative auth cases fail before JSON parsing or mutation setup', async () => {
  ...
  for (const negativeCase of negativeCases) {
    ...
    assert.equal(summary.negativeAuth.mutationSetupStarted, false);
  ...
  assert.equal(route.state.applySetupAttempts, 0);
