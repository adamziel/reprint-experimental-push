...
const evidencePath = path.join(
  repoRoot,
  'docs/evidence/rpp-0870-multisite-subdomain-topology-v4.md',
);
const hexSha256Pattern = /^[a-f0-9]{64}$/;

test('RPP-0870 progress report records candidate versus release-ready multisite subdomain scope', () => {
  const { report, text } = loadProgressReport();

  assert.equal(report.schemaVersion, 1);
  assert.equal(report.rppId, 'RPP-0870');
  ...
  assert.equal(report.builtOn.candidateScopePattern.rppId, 'RPP-0850');
  ...
  assert.equal(report.builtOn.previousCandidateScopePattern.rppId, 'RPP-0830');
  assert.equal(report.builtOn.previousCandidateScopePattern.variant, 2);
  ...
  assert.equal(report.candidateScope.status, 'multisite-subdomain-topology-candidate-v4');
  ...
  assert.equal(report.candidateScope.sourcePattern, 'local-static-external-wordpress-topology-derived-v4');
...
test('RPP-0870 records exact production-topology release gaps and final no-go', () => {
...
test('RPP-0870 evidence remains hash/count/surface only', () => {
  const { report, text } = loadProgressReport();

  assert.deepEqual(findEvidenceRedactionIssues(report), []);
  assert.doesNotThrow(() =>
    assertEvidenceHasNoRawValues(report, { label: 'RPP-0870 multisite subdomain topology progress report' }));
...
test('RPP-0870 evidence documents exact validation commands and results', () => {
  const { text } = loadProgressReport();

  assert.match(text, /node --check test\/rpp-0870-multisite-subdomain-topology-v4\.test\.js/);
  assert.match(
    text,
...
function loadProgressReport() {
  ...
  assert.ok(match?.groups?.json, 'RPP-0870 evidence must contain one JSON progress report block');
  return {
    text,
