...
const generatedProcessKillCases = Object.freeze([
  {
    id: 'rpp-0678-mid-set-five-v4',
    mutationCount: 5,
    killAfterMutations: 2,
  },
  {
    id: 'rpp-0678-mid-set-seven-v4',
    mutationCount: 7,
    killAfterMutations: 4,
  },
]);

function tempJournalPath() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'rpp-0678-mid-mutation-set-v4-'));
...
function generatedSites(generatedCase) {
  ...
  const base = {
    files: {
      [beforePlanPreservedKey]:
        `base-raw-rpp-0678-${generatedCase.id}-preserved-before-plan`,
      [afterKillPreservedKey]:
        `base-raw-rpp-0678-${generatedCase.id}-preserved-after-kill`,
    },
    plugins: {},
    db: {},
  };
  ...
  for (let index = 1; index <= generatedCase.mutationCount; index++) {
    base.files[targetFileName(generatedCase, index)] =
      `base-raw-rpp-0678-${generatedCase.id}-target-${index}`;
  }
  ...
  for (let index = 1; index <= generatedCase.mutationCount; index++) {
    local.files[targetFileName(generatedCase, index)] =
      `local-raw-rpp-0678-${generatedCase.id}-target-${index}`;
  }
  remote.files[beforePlanPreservedKey] =
    `remote-raw-rpp-0678-${generatedCase.id}-preserved-before-plan`;

  const preservedAfterKillValue =
    `remote-raw-rpp-0678-${generatedCase.id}-preserved-after-kill`;
...
function artifactRefsFor(generatedCase) {
  return {
    releaseProof: `artifact://rpp-0678/${generatedCase.id}/local-release-proof-not-production-durable`,
    recoverySupport: `artifact://rpp-0678/${generatedCase.id}/process-kill-mid-mutation-set-v4`,
    durabilityScope: `artifact://rpp-0678/${generatedCase.id}/sandbox-jsonl-only`,
  };
}
...
function spawnProcessKilledWriter({
  ...
  const childScript = `
    ...
    const plan = JSON.parse(process.env.RPP0678_PLAN);
    const remote = JSON.parse(process.env.RPP0678_REMOTE_SITE);
    const artifactRefs = JSON.parse(process.env.RPP0678_ARTIFACT_REFS);
    const killAfterMutations = Number(process.env.RPP0678_KILL_AFTER_MUTATIONS);
    const filePath = process.env.RPP0678_JOURNAL_PATH;
    const remoteAfterKillPath = process.env.RPP0678_REMOTE_AFTER_KILL_PATH;
    ...
    const writer = openProductionRecoveryJournal({
      ...
      current: remote,
      artifactRefs,
      now: new Date(process.env.RPP0678_NOW),
      truncate: true,
      claimId: process.env.RPP0678_CLAIM_ID,
    });
    ...
    applyPlan(remote, plan, {
      durableJournal,
      journal: JSON.parse(process.env.RPP0678_PREVIOUS_JOURNAL),
      mutateRemote: true,
      artifactRefs,
    });
  ...
  return spawnSync(process.execPath, ['--input-type=module', '-e', childScript], {
    env: {
      ...process.env,
      RPP0678_ARTIFACT_REFS: JSON.stringify(artifactRefs),
      RPP0678_CLAIM_ID: claimId,
      RPP0678_JOURNAL_PATH: filePath,
      RPP0678_KILL_AFTER_MUTATIONS: String(killAfterMutations),
      RPP0678_NOW: fixedNow.toISOString(),
      RPP0678_PLAN: JSON.stringify(plan),
      RPP0678_PREVIOUS_JOURNAL: JSON.stringify(oldRemoteJournalForPlan(remote, plan)),
      RPP0678_REMOTE_AFTER_KILL_PATH: remoteAfterKillPath,
      RPP0678_REMOTE_SITE: JSON.stringify(remote),
    },
    encoding: 'utf8',
  });
}
...
function assertPartialInspection({
  ...
  assertNoRawValues(inspection, rawValues, 'RPP-0678 partial inspection');
}
...
function assertPartialRepairInspection({
  ...
  assertNoRawValues(repairInspection.journal.records, rawValues, 'RPP-0678 repair inspection rows');
}
...
function openProductionRetry({
  ...
  assertNoRawValues(productionInspection, rawValues, 'RPP-0678 production retry inspection');
...
function assertReplayPreservedRemoteChanges({
  ...
  assertNoRawValues(replay.before.journal.records, rawValues, 'RPP-0678 repair-before journal rows');
  assertNoRawValues(replay.after.journal.records, rawValues, 'RPP-0678 repair-after journal rows');
}
...
function assertHashOnlyJournalRows(records, rawValues) {
  ...
  assertNoRawValues(records, rawValues, 'RPP-0678 journal rows');
}
...
test('RPP-0678 generated process kill mid mutation set variant 4 retry preserves remote changes', () => {
  for (const generatedCase of generatedProcessKillCases) {
    ...
    assert.equal(evidence.retryPreservation.retryDidNotOverwritePreservedRemoteChanges, true);
    assert.match(evidence.processKillBoundary.journalRowsHash, hashPattern);
    assertNoRawValues(evidence, rawValues, 'RPP-0678 evidence summary');
    assertNoRawValues(fs.readFileSync(filePath, 'utf8'), rawValues, 'RPP-0678 journal file');
  }
});
