...
const auditPath = path.join(repoRoot, 'docs/evidence/rpp-0941-release-gate-1-final-audit-v3.md');

const auditedHead = 'bf27d9e3a0523d3d6e69eba9116b832644610456';
...
const exactCommands = [
  ...
  "git show -s --format='%H %s' 3ff789513 bb40db8c1 2849d0398 6e3ab6f3c e837c3b90 adc70a4fc 847a4281c bb01b0552 d5e8bb491 45da34eca c3f7a3703 4b459c956 2891399d3 dd9c889c3 e0643400e 37898f069 bf37c4e2f 838638074 43a818b14 4393d034d ff533e449 ca82bf5d7 4d6600f2a 3fd76ca37 bf27d9e3a0523d3d6e69eba9116b832644610456 6a8b87ae01ad5ae1d4b7d61cd340695f71e41f53 6f6e634e61e283c0a91a8fda6b7958ed647a66a5",
  'node scripts/release/agents-release-gates-status-row.mjs .agents/RELEASE_GATES.md',
  'node scripts/release/check-release-gates.mjs --scope final-release --now 2026-06-01T02:48:00.000Z',
  'timeout 300s npm run verify:release',
  'node --check test/rpp-0941-release-gate-1-final-audit-v3.test.js',
  'node --test --test-name-pattern RPP-0941 test/rpp-0941-release-gate-1-final-audit-v3.test.js',
  'node scripts/release/artifact-redaction-scan.mjs docs/evidence/rpp-0941-release-gate-1-final-audit-v3.md',
...
test('RPP-0941 audit v3 is anchored to the current lane head without moving gates', () => {
  const text = auditText();

  assert.match(text, /^Date: 2026-06-01$/m);
  assert.match(text, /^Audited local branch: `session\/rpp-941`$/m);
  assert.match(text, new RegExp(`^Audited lane head before this evidence file: \`${auditedHead}\`$`, 'm'));
  assert.match(text, /Checklist item: RPP-0941 - Release gate 1 final audit, variant 3\./);
  assert.doesNotMatch(text, /session\/rpp-921/);
...
test('RPP-0941 audit v3 links exact commands used for evidence and validation', () => {
  const text = auditText();
...
test('RPP-0941 audit v3 links exact support commits and current lane head commits', () => {
  const text = auditText();
...
test('RPP-0941 audit v3 records production-backed status requirement and missing final proof', () => {
  const text = auditText();

  assert.match(text, /The required gate standard remains production-backed/);
  assert.match(text, /support-only evidence\s+does not satisfy final release movement/);
  assert.match(text, /Missing live topology, production\s+auth\/session, and durable journal proof keeps/);
  ...
  assert.match(text, /final live durability proof is absent/);
  assert.match(text, /No remote tunnel was used for this audit/);
  assert.match(text, /Integrate this file as release-ops evidence for RPP-0941 only/);
