...
const proofId = 'rpp-0754-large-post-table-benchmark-v3';
...
test('RPP-0754 variant 3 proves generated large post table coverage stays inside budgets', {
  concurrency: false,
}, () => {
  const proof = buildVariant3Proof();

  assert.equal(proof.rppId, 'RPP-0754');
  assert.equal(proof.proofId, proofId);
  assert.equal(proof.variant, 3);
  assert.equal(proof.status, 'passed');
  assert.equal(proof.builtOn.rppId, 'RPP-0714');
  assert.equal(proof.builtOn.previousVariantRppId, 'RPP-0734');
...
test('RPP-0754 variant 3 fails closed for unsafe generated large post table evidence', () => {
  ...
  const unsafeDecisions = unsafeEvidenceDecisions(evidence);
...
function unsafeEvidenceDecisions(evidence) {
  ...
  staleWindowHash.batchCollection.batchWindows[0].rowHashDigest = digest('rpp-0754-stale-row-window');
  ...
  return {
    ...
    rawValueLeak: resolveLargePostTableStoragePerformanceProof(rawValueLeak),
