...
const proofId = 'rpp-0775-large-media-library-benchmark-v4';
...
test('RPP-0775 variant 4 proves generated large media library coverage gates before fast-path output', {
  concurrency: false,
}, () => {
  const proof = buildVariant4Proof();

  assert.equal(proof.rppId, 'RPP-0775');
  ...
  assert.equal(proof.generatedCoverage.variant, 'large-media-library-benchmark-v4');
  ...
  assert.equal(proof.resources.storage.appliedMediaWrites, 9);
  ...
  assert.equal(proof.resources.storage.staleAtWriteMediaWrites, 2);
  assert.equal(proof.resources.storage.tempFsyncFailedBeforeRenameMediaWrites, 2);
  ...
  assert.equal(proof.resources.database.metadataRowsPreconditioned, 65);
  ...
  assert.equal(proof.resources.database.batchCount, 12);
  ...
  assert.deepEqual(proof.fastPathLaneEvidence.sampleOutcomeCounts, {
    applied: 7,
    'applied-fsync-incomplete': 1,
    'fsync-failed-before-rename': 2,
    'stale-at-write': 2,
  });
  ...
  assert.equal(proof.unsafe.failingRecordedGateStatus.updated, false);
  assert.equal(proof.unsafe.failingRecordedGateStatus.attemptedPassBlocked, true);
  assert.ok(proof.unsafe.failingRecordedGateStatus.blockedBy.includes('correctness-gates-not-passed'));
...
test('RPP-0775 variant 4 fails closed before fast-path lane output on unsafe generated evidence', () => {
...
function buildVariant4Proof() {
  ...
  const proofGates = [
    ...
    proofGate('fast-path-lane-output-requires-correctness-gates-hold',
      safeDecision.updated === true
        && safeDecision.correctnessGatesHold === true
        && safeDecision.outputEmitted === true
        && unsafe.failingRecordedGateStatus.updated === false
        && unsafe.failingRecordedGateStatus.blockedBy.includes('correctness-gates-not-passed'), {
      correctnessGatesHold: safeDecision.correctnessGatesHold,
      failingRecordedGateBlockedBy: unsafe.failingRecordedGateStatus.blockedBy,
    }),
  ...
  const publicProof = {
    ...
    correctness: {
      ...
      fastPathLaneOutputRequiresCorrectnessGatesHold: safeDecision.updated === safeDecision.correctnessGatesHold
...
function generatedCoverageSummary(benchmark, fastPathLaneEvidence) {
  return {
    ...
    variant: 'large-media-library-benchmark-v4',
    profile: benchmark.profile,
    deterministic: true,
  ...
  return [
    ...
    proofGate('generated-large-media-library-coverage-recorded',
      generatedCoverage.source === 'local-support-generated-large-media-library-cases'
        && generatedCoverage.variant === 'large-media-library-benchmark-v4'
...
function unsafeLargeMediaEvidenceDecisions(evidence, repeatedEvidence) {
  ...
  mismatchedLaneEvidenceHash.fastPathLaneEvidence.laneEvidenceHash = sha256(
    'rpp-0775-mismatched-lane-evidence-hash',
  );
  ...
  return {
    ...
    failingRecordedGateStatus: resolveLargeMediaLibraryProof(failingRecordedGateStatus, { repeatedEvidence }),
  };
}
