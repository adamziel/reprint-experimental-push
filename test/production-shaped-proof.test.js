...
test('packaged release verifier readiness helper fails closed when signed preflight keeps a broken top-level session envelope after the snapshot probe times out', async () => {
  const timeoutError = new Error('Timed out fetching http://127.0.0.1:65535/wp-json/reprint/v1/push/snapshot');
  const brokenSessionEnvelopes = [
    {
...
test('packaged release verifier readiness helper fails closed when signed preflight keeps a broken top-level auth envelope after the snapshot probe times out', async () => {
  const timeoutError = new Error('Timed out fetching http://127.0.0.1:65535/wp-json/reprint/v1/push/snapshot');
...
test('packaged release verifier readiness helper fails closed when signed preflight keeps a broken auth identity envelope after the snapshot probe times out', async () => {
  const timeoutError = new Error('Timed out fetching http://127.0.0.1:65535/wp-json/reprint/v1/push/snapshot');
  const brokenAuthIdentityEnvelopes = [
    {
...
test('packaged release verifier readiness helper fails closed when signed preflight session identities diverge after the snapshot probe times out', async () => {
  const timeoutError = new Error('Timed out fetching http://127.0.0.1:65535/wp-json/reprint/v1/push/snapshot');
  const identityCases = [
    {
      label: 'mismatched ids',
      authSessionId: 'session_456',
      topLevelSessionId: 'session_123',
    },
    {
      label: 'missing auth session id',
    ...
    },
    {
      ...
      topLevelSessionId: 123,
    },
  ];

  for (const { label, authSessionId, topLevelSessionId } of identityCases) {
    const parsedBody = {
      ok: true,
      routeProfile: {
        profile: 'production-shaped',
      ...
      auth: {
        session: {
          ...(authSessionId === undefined ? {} : { id: authSessionId }),
          status: 'active',
          type: 'production-auth-session',
          expiresAt: '2099-01-01T00:00:00Z',
        },
...
test('packaged release verifier readiness helper fails closed when signed preflight keeps a broken top-level session envelope after snapshot readiness succeeds', async () => {
  ...
  for (const { label, session } of brokenSessionEnvelopes) {
...
test('packaged release verifier readiness helper fails closed when signed preflight top-level session lifecycle is broken after snapshot readiness succeeds', async () => {
  const readySnapshotBody = JSON.stringify({
    ok: true,
    snapshot: {},
  });
  const brokenLifecycleSessions = [
    {
  ...
  for (const { label, session } of brokenLifecycleSessions) {
    const brokenSessionLifecycleBody = JSON.stringify({
      ok: true,
      routeProfile: {
        profile: 'production-shaped',
        restNamespace: 'reprint/v1',
      ...
      fetchTextWithTimeout: async (url) => {
        ...
        if (url.endsWith('/wp-json/reprint/v1/push/preflight')) {
          return {
            ...
            bodyText: brokenSessionLifecycleBody,
          };
        }
        throw new Error(`unexpected readiness fetch ${url}`);
      },
...
test('packaged release verifier readiness helper fails closed when signed preflight keeps a broken top-level auth envelope after snapshot readiness succeeds', async () => {
  const readySnapshotBody = JSON.stringify({
    ok: true,
    snapshot: {},
  });
...
test('packaged release verifier readiness helper fails closed when signed preflight keeps a broken auth identity envelope after snapshot readiness succeeds', async () => {
  const readySnapshotBody = JSON.stringify({
    ok: true,
    snapshot: {},
  });
  ...
  for (const { label, identity } of brokenAuthIdentityEnvelopes) {
    const brokenAuthIdentityBody = JSON.stringify({
      ok: true,
      routeProfile: {
        profile: 'production-shaped',
        restNamespace: 'reprint/v1',
      ...
      fetchTextWithTimeout: async (url) => {
        ...
        if (url.endsWith('/wp-json/reprint/v1/push/preflight')) {
          return {
            ...
            bodyText: brokenAuthIdentityBody,
          };
        }
        throw new Error(`unexpected readiness fetch ${url}`);
      },
...
test('packaged release verifier readiness helper fails closed when signed preflight auth session id is missing after snapshot readiness succeeds', async () => {
  const readySnapshotBody = JSON.stringify({
    ok: true,
    snapshot: {},
  });
  const missingAuthSessionIdBody = JSON.stringify({
    ok: true,
    routeProfile: {
      profile: 'production-shaped',
      restNamespace: 'reprint/v1',
    ...
    fetchTextWithTimeout: async (url) => {
      ...
      if (url.endsWith('/wp-json/reprint/v1/push/preflight')) {
        return {
          ...
          bodyText: missingAuthSessionIdBody,
        };
      }
      throw new Error(`unexpected readiness fetch ${url}`);
    },
...
test('packaged release verifier readiness helper fails closed when signed preflight auth session has rotated status after snapshot readiness succeeds', async () => {
  const readySnapshotBody = JSON.stringify({
    ok: true,
    snapshot: {},
  });
...
test('packaged release verifier readiness helper fails closed when signed preflight auth session carries cleanup status aliases after snapshot readiness succeeds', async () => {
  const readySnapshotBody = JSON.stringify({
    ok: true,
    snapshot: {},
  });
  const cleanupStatusBodies = [
    {
  ...
  for (const { label, bodyText } of cleanupStatusBodies) {
    ...
    const helper = buildPackagedReleaseVerifierWaitHelper({
      ...
      fetchPackagedPreflightProbe: async () => {
        throw new Error(`unexpected snapshot-startup fallback preflight probe for ${label}`);
      },
      throwPlaygroundReadinessFailure: async (child, prefix) => {
        const error = new Error(prefix);
        error.isPlaygroundReadinessFailure = true;
...
test('packaged release verifier readiness helper fails closed when signed preflight auth session has expired status after snapshot readiness succeeds', async () => {
  const readySnapshotBody = JSON.stringify({
    ok: true,
    snapshot: {},
  });
  const expiredStatusAuthSessionBody = JSON.stringify({
    ok: true,
    routeProfile: {
      profile: 'production-shaped',
      restNamespace: 'reprint/v1',
    ...
    fetchTextWithTimeout: async (url) => {
      ...
      if (url.endsWith('/wp-json/reprint/v1/push/preflight')) {
        return {
          ...
          bodyText: expiredStatusAuthSessionBody,
        };
      }
      throw new Error(`unexpected readiness fetch ${url}`);
    },
...
test('packaged release verifier readiness helper fails closed when signed preflight auth session is explicitly expired after snapshot readiness succeeds', async () => {
  const readySnapshotBody = JSON.stringify({
    ok: true,
    snapshot: {},
  });
  const explicitlyExpiredAuthSessionBody = JSON.stringify({
    ok: true,
    routeProfile: {
      profile: 'production-shaped',
      restNamespace: 'reprint/v1',
    ...
    fetchTextWithTimeout: async (url) => {
      ...
      if (url.endsWith('/wp-json/reprint/v1/push/preflight')) {
        return {
          ...
          bodyText: explicitlyExpiredAuthSessionBody,
        };
      }
      throw new Error(`unexpected readiness fetch ${url}`);
    },
...
test('packaged release verifier readiness helper fails closed when signed preflight becomes terminal after snapshot startup fallback loses the /wp-json/ probe', async () => {
  const snapshotStartupBody = JSON.stringify({
    code: 'rest_no_route',
    message: 'No route was found matching the URL and request method.',
  });
  const fetchCalls = [];
  let indexProbeCalls = 0;
  let capturedContext = null;
  const helper = buildPackagedReleaseVerifierWaitHelper({
    packagedProductionPluginRouteStartupClassificationReady: () => true,
    fetchPackagedWordPressIndexProbe: async () => {
      indexProbeCalls += 1;
      throw new Error('Timed out fetching /wp-json/');
    },
    buildPackagedTimeoutFallbackProbe: (route, error) => {
      assert.equal(route, '/wp-json/');
      assert.match(error.message, /Timed out fetching \/wp-json\//);
      return {
        route,
        timedOut: true,
      };
    ...
    sleepUnlessChildExit: async () => {
      throw new Error('unexpected readiness sleep during snapshot-startup terminal fallback runtime proof');
    },
    fetchPackagedTimeoutFallbackProbes: async () => {
      throw new Error('unexpected timeout fallback probes during snapshot-startup terminal fallback runtime proof');
    },
    fetchTextWithTimeout: async (url) => {
      fetchCalls.push(url);
      if (url.endsWith('/wp-json/reprint/v1/push/snapshot')) {
  ...
  assert.equal(indexProbeCalls, 1);
  ...
  assert.equal(capturedContext?.invalidReadinessBody, undefined);
});
...
test('packaged release verifier readiness helper fails closed when signed preflight keeps a broken route profile while snapshot startup is still in progress', async () => {
  const snapshotStartupBody = JSON.stringify({
    code: 'rest_no_route',
    message: 'No route was found matching the URL and request method.',
  });
  ...
  for (const { label, body } of brokenRouteProfiles) {
    const fetchCalls = [];
    let capturedContext = null;
    const helper = buildPackagedReleaseVerifierWaitHelper({
      packagedProductionPluginRouteStartupClassificationReady: () => {
    ...
    assert.equal(capturedContext?.snapshotStartupFallback, true, `${label} should preserve snapshot startup fallback context`);
    assert.equal(capturedContext?.preflightTerminal, true, `${label} should remain terminal`);
    assert.equal(capturedContext?.snapshotNotReadyProbeCount, 1, `${label} should keep the startup probe count`);
    assert.equal(capturedContext?.invalidReadinessBody, true, `${label} should fail closed as an invalid readiness body`);
  }
});

test('packaged release verifier readiness helper fails closed when signed preflight keeps a broken top-level session envelope while snapshot startup is still in progress', async () => {
  const snapshotStartupBody = JSON.stringify({
    code: 'rest_no_route',
    message: 'No route was found matching the URL and request method.',
  });
...
test('packaged release verifier readiness helper fails closed when signed preflight keeps a broken top-level session lifecycle while snapshot startup is still in progress', async () => {
  const snapshotStartupBody = JSON.stringify({
    code: 'rest_no_route',
    message: 'No route was found matching the URL and request method.',
  });
  ...
  for (const { label, session } of brokenLifecycleSessions) {
    ...
    const helper = buildPackagedReleaseVerifierWaitHelper({
      packagedProductionPluginRouteStartupClassificationReady: () => {
        throw new Error(`unexpected route startup classification during ${label} snapshot-startup lifecycle runtime proof`);
      },
      fetchPackagedWordPressIndexProbe: async () => {
        throw new Error(`unexpected /wp-json/ probe during ${label} snapshot-startup lifecycle runtime proof`);
      },
      sleepUnlessChildExit: async () => {
        throw new Error(`unexpected readiness sleep during ${label} snapshot-startup lifecycle runtime proof`);
      },
      fetchPackagedTimeoutFallbackProbes: async () => {
        throw new Error(`unexpected timeout fallback probes during ${label} snapshot-startup lifecycle runtime proof`);
      },
      fetchTextWithTimeout: async (url) => {
        fetchCalls.push(url);
        if (url.endsWith('/wp-json/reprint/v1/push/snapshot')) {
...
test('packaged release verifier readiness helper fails closed when signed preflight keeps a broken auth session envelope while snapshot startup is still in progress', async () => {
  ...
  for (const { label, body } of brokenAuthSessionEnvelopes) {
    const fetchCalls = [];
    let capturedContext = null;
    const helper = buildPackagedReleaseVerifierWaitHelper({
      packagedProductionPluginRouteStartupClassificationReady: () => {
...
test('packaged release verifier readiness helper fails closed when signed preflight keeps a broken auth identity envelope while snapshot startup is still in progress', async () => {
  const snapshotStartupBody = JSON.stringify({
    code: 'rest_no_route',
    message: 'No route was found matching the URL and request method.',
  });
  ...
  for (const { label, body } of brokenAuthIdentityEnvelopes) {
    const fetchCalls = [];
    let capturedContext = null;
    const helper = buildPackagedReleaseVerifierWaitHelper({
      packagedProductionPluginRouteStartupClassificationReady: () => {
...
test('packaged release verifier readiness helper fails closed when signed preflight session identities diverge while snapshot startup is still in progress', async () => {
  const snapshotStartupBody = JSON.stringify({
    code: 'rest_no_route',
    message: 'No route was found matching the URL and request method.',
  });
  ...
  const helper = buildPackagedReleaseVerifierWaitHelper({
    packagedProductionPluginRouteStartupClassificationReady: () => {
      throw new Error('unexpected route startup classification during mismatched session snapshot-startup runtime proof');
    },
    fetchPackagedWordPressIndexProbe: async () => {
      throw new Error('unexpected /wp-json/ probe during mismatched session snapshot-startup runtime proof');
    },
    sleepUnlessChildExit: async () => {
      throw new Error('unexpected readiness sleep during mismatched session snapshot-startup runtime proof');
    },
    fetchPackagedTimeoutFallbackProbes: async () => {
      throw new Error('unexpected timeout fallback probes during mismatched session snapshot-startup runtime proof');
    },
    fetchTextWithTimeout: async (url) => {
      fetchCalls.push(url);
      if (url.endsWith('/wp-json/reprint/v1/push/snapshot')) {
...
test('packaged production plugin smoke readiness helper fails closed when signed preflight keeps a broken top-level session envelope after the snapshot probe times out', async () => {
  const timeoutError = new Error('Timed out fetching http://127.0.0.1:65535/wp-json/reprint/v1/push/snapshot');
  const brokenSessionEnvelopes = [
    {
      label: 'missing top-level session',
...
test('packaged production plugin smoke readiness helper fails closed when signed preflight keeps a broken top-level auth envelope after the snapshot probe times out', async () => {
  const timeoutError = new Error('Timed out fetching http://127.0.0.1:65535/wp-json/reprint/v1/push/snapshot');
  const brokenAuthEnvelopes = [
    {
      label: 'missing top-level auth',
...
test('packaged production plugin smoke readiness helper fails closed when signed preflight keeps a broken auth identity envelope after the snapshot probe times out', async () => {
  const timeoutError = new Error('Timed out fetching http://127.0.0.1:65535/wp-json/reprint/v1/push/snapshot');
  const brokenAuthIdentityEnvelopes = [
    {
      label: 'missing auth identity',
...
test('packaged production plugin smoke readiness helper fails closed when signed preflight session identities diverge after the snapshot probe times out', async () => {
  const timeoutError = new Error('Timed out fetching http://127.0.0.1:65535/wp-json/reprint/v1/push/snapshot');
  const identityCases = [
    {
      label: 'mismatched ids',
...
test('packaged production plugin smoke readiness helper fails closed when signed preflight top-level session lifecycle is broken after snapshot readiness succeeds', async () => {
  const readySnapshotBody = JSON.stringify({
    ok: true,
    snapshot: {},
  });
...
test('packaged production plugin smoke readiness helper fails closed when signed preflight keeps a broken top-level auth envelope after snapshot readiness succeeds', async () => {
  const readySnapshotBody = JSON.stringify({
    ok: true,
    snapshot: {},
  });
...
test('packaged production plugin smoke readiness helper fails closed when signed preflight keeps a broken auth identity envelope after snapshot readiness succeeds', async () => {
  const readySnapshotBody = JSON.stringify({
    ok: true,
    snapshot: {},
  });
...
test('packaged production plugin smoke readiness helper fails closed when signed preflight auth session id is missing after snapshot readiness succeeds', async () => {
  const readySnapshotBody = JSON.stringify({
    ok: true,
    snapshot: {},
  });
  ...
  const helper = buildPackagedSmokeWaitHelper({
    packagedProductionPluginRouteStartupClassificationReady: () => {
      throw new Error('unexpected route startup classification during missing auth session id runtime proof');
    },
    fetchPackagedWordPressIndexProbe: async () => {
      throw new Error('unexpected /wp-json/ probe during missing auth session id runtime proof');
    },
    sleepUnlessChildExit: async () => {
      throw new Error('unexpected readiness sleep during missing auth session id runtime proof');
    },
    fetchPackagedTimeoutFallbackProbes: async () => {
      throw new Error('unexpected timeout fallback probes during missing auth session id runtime proof');
    },
    fetchTextWithTimeout: async (url) => {
      fetchCalls.push(url);
      if (url.endsWith('/wp-json/reprint/v1/push/snapshot')) {
...
test('packaged production plugin smoke readiness helper fails closed when signed preflight auth session has expired status after snapshot readiness succeeds', async () => {
  const readySnapshotBody = JSON.stringify({
    ok: true,
    snapshot: {},
  });
  ...
  const helper = buildPackagedSmokeWaitHelper({
    packagedProductionPluginRouteStartupClassificationReady: () => {
      throw new Error('unexpected route startup classification during expired-status auth session runtime proof');
    },
    fetchPackagedWordPressIndexProbe: async () => {
      throw new Error('unexpected /wp-json/ probe during expired-status auth session runtime proof');
    },
    sleepUnlessChildExit: async () => {
      throw new Error('unexpected readiness sleep during expired-status auth session runtime proof');
    },
    fetchPackagedTimeoutFallbackProbes: async () => {
      throw new Error('unexpected timeout fallback probes during expired-status auth session runtime proof');
    },
    fetchTextWithTimeout: async (url) => {
      fetchCalls.push(url);
      if (url.endsWith('/wp-json/reprint/v1/push/snapshot')) {
...
test('packaged production plugin smoke readiness helper fails closed when signed preflight auth session is explicitly expired after snapshot readiness succeeds', async () => {
  const readySnapshotBody = JSON.stringify({
    ok: true,
    snapshot: {},
  });
  ...
  const helper = buildPackagedSmokeWaitHelper({
    packagedProductionPluginRouteStartupClassificationReady: () => {
      throw new Error('unexpected route startup classification during explicit expired auth session runtime proof');
    },
    fetchPackagedWordPressIndexProbe: async () => {
      throw new Error('unexpected /wp-json/ probe during explicit expired auth session runtime proof');
    },
    sleepUnlessChildExit: async () => {
      throw new Error('unexpected readiness sleep during explicit expired auth session runtime proof');
    },
    fetchPackagedTimeoutFallbackProbes: async () => {
      throw new Error('unexpected timeout fallback probes during explicit expired auth session runtime proof');
    },
    fetchTextWithTimeout: async (url) => {
      fetchCalls.push(url);
      if (url.endsWith('/wp-json/reprint/v1/push/snapshot')) {
...
test('packaged production plugin smoke readiness helper fails closed when signed preflight keeps a broken route profile while snapshot startup is still in progress', async () => {
  const snapshotStartupBody = JSON.stringify({
    code: 'rest_no_route',
    message: 'No route was found matching the URL and request method.',
  });
...
test('packaged production plugin smoke readiness helper fails closed when signed preflight keeps a broken top-level session envelope while snapshot startup is still in progress', async () => {
  const snapshotStartupBody = JSON.stringify({
    code: 'rest_no_route',
    message: 'No route was found matching the URL and request method.',
  });
...
test('packaged production plugin smoke readiness helper fails closed when signed preflight keeps a broken top-level session lifecycle while snapshot startup is still in progress', async () => {
  const snapshotStartupBody = JSON.stringify({
    code: 'rest_no_route',
    message: 'No route was found matching the URL and request method.',
  });
...
test('packaged production plugin smoke readiness helper fails closed when signed preflight keeps a broken auth session envelope while snapshot startup is still in progress', async () => {
  const snapshotStartupBody = JSON.stringify({
    code: 'rest_no_route',
    message: 'No route was found matching the URL and request method.',
  });
...
test('packaged production plugin smoke readiness helper fails closed when signed preflight keeps a broken auth identity envelope while snapshot startup is still in progress', async () => {
  const snapshotStartupBody = JSON.stringify({
    code: 'rest_no_route',
    message: 'No route was found matching the URL and request method.',
  });
...
test('packaged production plugin smoke readiness helper fails closed when signed preflight session identities diverge while snapshot startup is still in progress', async () => {
  const snapshotStartupBody = JSON.stringify({
    code: 'rest_no_route',
    message: 'No route was found matching the URL and request method.',
  });
...
test('packaged production plugin smoke readiness helper fails closed when signed preflight becomes terminal after snapshot startup fallback loses the /wp-json/ probe', async () => {
  const snapshotStartupBody = JSON.stringify({
    code: 'rest_no_route',
    message: 'No route was found matching the URL and request method.',
  });
...
test('packaged production plugin smoke readiness helper fails closed when global WordPress startup exceeds the packaged snapshot budget', async () => {
  ...
  const helper = buildPackagedSmokeWaitHelper({
    ...
    fetchPackagedTimeoutFallbackProbes: async () => {
      throw new Error('unexpected timeout fallback fetch in packaged smoke global-startup limit runtime proof');
    },
    sleepUnlessChildExit: async (ms, child) => {
      sleepCalls.push({ ms, child });
    },
  ...
  await assert.rejects(
    ...
    (error) => {
      ...
      assert.match(error.message, /"globalWordPressStartup": true/);
      assert.match(error.message, /"snapshotNotReadyProbeCount": 4/);
      return true;
    },
  );
...
test('production auth/session lifecycle helper treats invalid or past expiry as expired', () => {
  ...
  assert.equal(
    isExpiredAuthSession(
      { expiresAt: '2099-01-01T00:00:00Z', expired: true },
      new Date('2000-01-01T00:00:01Z'),
    ),
    true,
  );
});
