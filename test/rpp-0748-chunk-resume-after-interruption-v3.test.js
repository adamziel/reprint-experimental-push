...
const proofId = 'rpp-0748-chunk-resume-after-interruption-v3';
...
test('RPP-0748 variant 3 generated interruption coverage resumes without duplicate mutation work', {
  concurrency: false,
}, () => {
  const proof = buildVariant3Proof();

  assert.equal(proof.rppId, 'RPP-0748');
  ...
  assert.equal(proof.generatedCoverage.resumeVariant, 'chunk-resume-after-interruption-v3');
  assert.equal(proof.generatedCoverage.caseCount, 4);
  assert.equal(proof.generatedCoverage.deterministicCaseVector, true);
  assert.deepEqual(proof.generatedCoverage.interruptionPoints, [1, 2, 3, 4]);
  ...
  assert.equal(proof.interruption.caseCount, 4);
  assert.equal(proof.interruption.allCasesPassed, true);
  assert.equal(proof.interruption.totalChunksSkippedByReceipt, 10);
  assert.equal(proof.interruption.totalChunksUploadedAfterResume, 8);
  ...
  assert.equal(proof.interruption.cases.length, 4);
...
test('RPP-0748 variant 3 fails closed for missing receipts, duplicate mutation work, and stale gates', () => {
...
function buildVariant3Proof() {
  ...
  const publicProof = {
    ...
    redaction: {
      mode: 'hash-count-only-chunk-resume-interruption-storage-performance',
...
function runUnitBenchmark() {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'reprint-rpp-0748-interruption-v3-'));
  try {
...
function buildChunkResumeInterruptionEvidence({ report, generatedCases, repeatedCases }) {
  ...
  return {
    ...
    generatedCoverage: {
      ...
      resumeVariant: 'chunk-resume-after-interruption-v3',
    ...
    interruption: {
      caseCount: generatedCases.length,
      allCasesPassed: generatedCases.every((interruptionCase) =>
      ...
      maxMutationWorkBeforeInterruption: Math.max(
        ...generatedCases.map((interruptionCase) =>
          interruptionCase.mutationWorkBeforeInterruption),
      ),
...
function recomputeInterruptionProofGates(evidence) {
  ...
  const generatedCasesCovered = generatedCoverage.source === 'local-support-generated-interruption-cases'
    && generatedCoverage.resumeVariant === 'chunk-resume-after-interruption-v3'
    ...
    && sameArray(generatedCoverage.interruptionPoints || [], [1, 2, 3, 4])
...
function unsafeInterruptionDecisions(evidence) {
  ...
  earlyApplyBoundary.interruption.cases[2].firstApplyBoundarySequence =
    earlyApplyBoundary.interruption.cases[2].transferFinalizeSequence - 1;
  earlyApplyBoundary.interruption.cases[2].applyOpenedAfterTransferFinalize = false;
...
function generatedInterruptionSpecs() {
  return [3, 4, 5, 6].map((chunkCount, index) => ({
    caseId: `interruption-${index + 1}`,
    fileBytes: chunkCount * benchmarkOptions.chunkSizeBytes,
    chunkSizeBytes: benchmarkOptions.chunkSizeBytes,
    chunkCount,
    interruptionAfterChunks: index + 1,
  }));
}

function buildGeneratedInterruptionCase(spec) {
