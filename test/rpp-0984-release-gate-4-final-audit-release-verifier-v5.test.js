...
const auditPath = path.join(repoRoot, 'docs/evidence/rpp-0984-release-gate-4-final-audit-release-verifier-v5.md');
const previousAuditPath = path.join(repoRoot, 'docs/evidence/rpp-0964-release-gate-4-final-audit-v4.md');
...
const auditedHead = '5487953e6f1680151ec7f1b86bbb146212241d05';

const validationCommands = [
  'git rev-parse HEAD',
  'node --check test/rpp-0984-release-gate-4-final-audit-release-verifier-v5.test.js',
  'node --test --test-name-pattern RPP-0984 test/rpp-0984-release-gate-4-final-audit-release-verifier-v5.test.js',
  'node scripts/release/artifact-redaction-scan.mjs docs/evidence/rpp-0984-release-gate-4-final-audit-release-verifier-v5.md',
  'git diff --check',
];

const forbiddenNetworkExposureTools = [
  'ngrok',
  'cloudflared',
  'localtunnel',
  'serveo',
...
test('RPP-0984 evidence audits the current GATE-4 lane head without release movement', () => {
  const { text, audit } = loadAudit();

  assert.match(text, /^# RPP-0984 release gate 4 final audit release verifier v5$/m);
  assert.match(text, /^Date: 2026-06-01$/m);
  assert.match(text, /^Audited local branch: `session\/rpp-984`$/m);
  assert.match(text, new RegExp(`^Audited lane head before this evidence file: \`${auditedHead}\`$`, 'm'));
  assert.equal(audit.rppId, 'RPP-0984');
  assert.equal(audit.variant, 5);
  assert.equal(audit.auditedBranch, 'session/rpp-984');
  ...
  assertIncludesNormalized(text, 'Unresolved production-backed proof gaps remain open');
});

test('RPP-0984 evidence carries forward the RPP-0964 v4 audit contract', () => {
  ...
  assert.equal(audit.carryForward.sourceEvidence, 'docs/evidence/rpp-0964-release-gate-4-final-audit-v4.md');
  ...
  assert.deepEqual(audit.carryForward.carriedForwardContract, previousAudit.carryForward.carriedForwardContract);

  assert.deepEqual(audit.safeRecoveryDocumentation.prerequisites, previousAudit.safeRecoveryDocumentation.prerequisites);
  assert.deepEqual(audit.safeRecoveryDocumentation.operatorEvidence, previousAudit.safeRecoveryDocumentation.operatorEvidence);
...
test('RPP-0984 evidence proves operator recovery prerequisites and explicit evidence are named', () => {
...
test('RPP-0984 evidence proves stop conditions block unsafe recovery assumptions', () => {
  const operatorDoc = readText(operatorDocPath);
  const { audit } = loadAudit();
...
test('RPP-0984 evidence proves rollback and escalation blockers are named before release movement', () => {
  const operatorDoc = readText(operatorDocPath);
  const { audit } = loadAudit();
...
test('RPP-0984 evidence proves hidden-assumption guards are named before retry or finalization', () => {
  const operatorDoc = readText(operatorDocPath);
  const { audit } = loadAudit();
...
test('RPP-0984 evidence keeps GATE-4 proof production-backed-evidence gated and fail closed', () => {
  const { text, audit } = loadAudit();

  assert.equal(audit.releaseVerifier.mode, 'support-only carry-through');
  assert.equal(audit.releaseVerifier.gate, 'GATE-4');
  assert.equal(audit.releaseVerifier.proofGate, 'production-backed-evidence');
  assert.equal(audit.releaseVerifier.productionBackedEvidenceRequired, true);
  assert.equal(audit.releaseVerifier.failClosedWithoutProductionBackedEvidence, true);
  assert.equal(audit.releaseVerifier.acceptedForReleaseMovement, false);
  assert.equal(audit.releaseVerifier.mutationAttemptedByThisAudit, false);
  assert.equal(audit.releaseVerifier.requiresProductionOwnedLiveSourceBoundary, true);
  assert.equal(audit.releaseVerifier.requiresPluginOwnedDriverMutationProof, true);
  assert.equal(audit.releaseVerifier.requiresAllowlistedSemantics, true);
  assert.equal(audit.releaseVerifier.requiresPreconditionEvidence, true);
  assert.equal(audit.releaseVerifier.requiresRejectedRemotePreservation, true);
  assert.equal(audit.releaseVerifier.requiresApplyTimeRevalidation, true);
  assert.equal(audit.releaseVerifier.requiresAuditEvidenceOnReleaseBoundary, true);
  assert.equal(audit.releaseVerifier.releaseGateMovementAllowed, false);
  assert.equal(audit.releaseHold.gate4ProofProductionBackedEvidenceGated, true);
  assert.equal(audit.releaseHold.failClosedWithoutProductionBackedEvidence, true);
  assert.equal(audit.releaseHold.productionBackedProofGapsOpen, true);
  assertIncludesNormalized(text, 'GATE-4 proof remains production-backed-evidence gated');
  assertIncludesNormalized(text, 'the release verifier posture must fail closed');
});

test('RPP-0984 evidence keeps unresolved production-backed proof gaps open', () => {
  const { text, audit } = loadAudit();
  const gaps = audit.releaseVerifier.unresolvedProductionBackedProofGaps;

  assert.ok(gaps.length >= 4);
  assert.ok(gaps.every((gap) => /production|GATE-4|release gates/.test(gap)), 'proof gaps must stay production scoped');
  assert.ok(
    audit.releaseVerifier.blockedWhenEvidenceMissing.includes('plugin-owned driver mutation proof on the checked release boundary'),
  );
  assert.ok(
    audit.releaseVerifier.blockedWhenEvidenceMissing.includes('audit evidence on the checked release boundary'),
  );

  for (const gap of gaps) {
    assertIncludesNormalized(text, gap, `evidence must keep proof gap explicit: ${gap}`);
  }
});

test('RPP-0984 evidence keeps release-gate status and redaction posture held', () => {
  const { text, audit } = loadAudit();
  ...
  assert.doesNotMatch(releaseGates, /RPP-0984/);
  ...
  assert.equal(audit.redactionPosture.networkExposureInstructionsIncluded, false);

  for (const command of validationCommands) {
    assert.ok(text.includes(`\`${command}\``), `missing validation command: ${command}`);
  }

  for (const forbidden of forbiddenNetworkExposureTools) {
    assert.doesNotMatch(text.toLowerCase(), new RegExp(escapeRegExp(forbidden)));
  }
});
