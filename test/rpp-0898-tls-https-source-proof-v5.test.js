...
const evidencePath = path.join(
  repoRoot,
  'docs/evidence/rpp-0898-tls-https-source-proof-v5.md',
);

const fixedNow = new Date('2026-06-01T00:00:00.000Z');
const proofId = 'rpp-0898-tls-https-source-proof-v5';
...
const goodEnv = Object.freeze({
  REPRINT_PUSH_SOURCE_URL: 'https://source-v5.example.test/variant-five-source',
  REPRINT_PUSH_REMOTE_URL: 'https://source-v5.example.test:443/variant-five-source/',
  REPRINT_PUSH_LOCAL_URL: 'https://local-v5.example.test/variant-five-local',
  REPRINT_PUSH_REMOTE_CHANGED_URL: 'https://changed-v5.example.test/variant-five-changed',
  REPRINT_PUSH_PREFLIGHT_SOURCE_URL: 'https://source-v5.example.test/variant-five-source/',
  REPRINT_PUSH_DRY_RUN_ROUTE_SOURCE_URL: 'https://source-v5.example.test:443/variant-five-source',
  REPRINT_PUSH_APPLY_SOURCE_URL: 'https://source-v5.example.test/variant-five-source',
  REPRINT_PUSH_JOURNAL_ROUTE_SOURCE_URL: 'https://source-v5.example.test/variant-five-source',
  REPRINT_PUSH_RECOVERY_INSPECT_SOURCE_URL: 'https://source-v5.example.test/variant-five-source',
  REPRINT_PUSH_USERNAME: 'rpp-0898-admin',
  REPRINT_PUSH_APPLICATION_PASSWORD: 'rpp-0898-application-password-must-not-leak',
});

const forbiddenProofNeedles = Object.freeze([
  'https://source-v5.example.test',
  'https://local-v5.example.test',
  'https://changed-v5.example.test',
  'source-v5.example.test',
  'local-v5.example.test',
  'changed-v5.example.test',
  'variant-five-source',
  'variant-five-local',
  'variant-five-changed',
  'rpp-0898-admin',
  'rpp-0898-application-password-must-not-leak',
]);

test('RPP-0898 captures source/local/changed HTTPS URLs and identity-checks variant 5 roles', () => {
  const proof = buildTlsHttpsSourceProof({ env: goodEnv });

  assert.equal(proof.schemaVersion, 1);
  assert.equal(proof.rppId, 'RPP-0898');
  ...
  assert.equal(proof.patternLineage.patternOnlyReference.rppId, 'RPP-0878');
  assert.equal(proof.patternLineage.patternOnlyReference.variant, 4);
  assert.equal(proof.patternLineage.patternOnlyReference.reusesProofArtifact, false);

  assert.equal(proof.tlsHttpsSource.scope, 'tls-https-source-proof-v5');
  ...
  assert.doesNotThrow(() => assertEvidenceHasNoRawValues(proof, { label: 'RPP-0898 TLS HTTPS source proof v5' }));
});

test('RPP-0898 rejects tunnel URLs and secret-shaped URL parts before accepting TLS source scope', () => {
  const proof = buildTlsHttpsSourceProof({
    env: {
      REPRINT_PUSH_SOURCE_URL: 'https://admin:rpp0898-secret@source-v5.example.test/variant-five-source',
      REPRINT_PUSH_LOCAL_URL: 'https://local-v5.example.test/secret/rpp0898-token',
      REPRINT_PUSH_REMOTE_CHANGED_URL: 'https://changed-v5.ngrok-free.app/variant-five-changed',
  ...
  assert.ok(proof.failures.some((failure) => failure.code === 'RPP_0898_SECRET_SHAPED_URL_PART_REJECTED'
    && failure.role === 'source'
  ...
  assertNoNeedles(proof, [
    'admin:rpp0898-secret',
    'rpp0898-secret',
    'secret/rpp0898-token',
    'rpp0898-token',
    'changed-v5.ngrok-free.app',
  ]);
  assertNoRawUrls(proof);
});

test('RPP-0898 rejects non-HTTPS role URLs before TLS scope acceptance', () => {
  const proof = buildTlsHttpsSourceProof({
    env: {
      ...goodEnv,
      REPRINT_PUSH_SOURCE_URL: 'http://source-v5.example.test/variant-five-source',
      REPRINT_PUSH_REMOTE_URL: 'http://source-v5.example.test/variant-five-source',
      REPRINT_PUSH_PREFLIGHT_SOURCE_URL: 'http://source-v5.example.test/variant-five-source',
      REPRINT_PUSH_DRY_RUN_ROUTE_SOURCE_URL: 'http://source-v5.example.test/variant-five-source',
      REPRINT_PUSH_APPLY_SOURCE_URL: 'http://source-v5.example.test/variant-five-source',
      REPRINT_PUSH_JOURNAL_ROUTE_SOURCE_URL: 'http://source-v5.example.test/variant-five-source',
      REPRINT_PUSH_RECOVERY_INSPECT_SOURCE_URL: 'http://source-v5.example.test/variant-five-source',
    },
  });
  ...
  assert.ok(proof.failures.some((failure) =>
    failure.code === 'RPP_0898_REQUIRED_ROLE_URL_MUST_BE_HTTPS'
      && failure.role === 'source'));
  assertNoRawUrls(proof);
});

test('RPP-0898 rejects duplicate source/local/changed URL identities before TLS scope acceptance', () => {
  const proof = buildTlsHttpsSourceProof({
    env: {
      ...goodEnv,
      REPRINT_PUSH_LOCAL_URL: 'https://source-v5.example.test:443/variant-five-source/',
    },
  });
...
test('RPP-0898 TLS HTTPS source proof v5 is deterministic and hash/count/surface-only', () => {
...
test('RPP-0898 evidence document records the same NO-GO hash/count/surface-only scope', () => {
  const { report, text } = loadEvidenceReport();
  const proof = buildTlsHttpsSourceProof({ env: goodEnv });

  assert.equal(report.schemaVersion, 1);
  assert.equal(report.rppId, 'RPP-0898');
  ...
  assert.deepEqual(report.tlsHttpsSourceScope.roleIdentityHashes, proof.tlsHttpsSource.roleIdentityHashes);
  ...
  assert.doesNotMatch(text, /source-v5\.example|local-v5\.example|changed-v5\.example|ngrok|token=|admin:/i);
});
...
function buildTlsHttpsSourceProof({ env, now = fixedNow } = {}) {
  ...
  const tlsFailures = roleUrlEvidence
    ...
    .map((entry) => ({
      code: 'RPP_0898_REQUIRED_ROLE_URL_MUST_BE_HTTPS',
      role: entry.role,
      envKey: entry.envKey,
    }));
  ...
  const proofCore = {
    ...
    tlsHttpsSource: {
      scope: 'tls-https-source-proof-v5',
...
function findSecretShapedUrlPartFailures(env = {}) {
  ...
  for (const definition of urlDefinitionsForSecretPartScan(env)) {
    ...
    for (const partSurface of partSurfaces) {
      failures.push({
        code: 'RPP_0898_SECRET_SHAPED_URL_PART_REJECTED',
...
function loadEvidenceReport() {
  ...
  assert.ok(match?.groups?.json, 'RPP-0898 evidence must contain one JSON report block');
  return {
    text,
