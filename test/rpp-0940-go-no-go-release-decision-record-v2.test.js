...
const evidencePath = path.join(repoRoot, 'docs/evidence/rpp-0940-go-no-go-release-decision-record-v2.md');
const fixedNowIso = '2026-06-01T02:45:00.000Z';
const auditedHeadBeforeEvidence = 'bed8124f1c5ecf63db86741968e832bdfb388ec3';
...
test('RPP-0940 v2 evidence proves support-only NO-GO and names every remaining risk', () => {
  ...
  assert.match(evidenceText, /^# RPP-0940 go\/no-go release decision record v2 evidence$/m);
  ...
  assert.equal(evidence.rppId, 'RPP-0940');
  assert.equal(evidence.proofId, 'rpp-0940-go-no-go-release-decision-record-v2');
  assert.equal(evidence.variant, 2);
  assert.equal(evidence.status, 'passed-support-only');
  ...
  assert.equal(evidence.auditedBranch, 'session/rpp-940');
  ...
  assert.equal(evidence.evidencePath, 'docs/evidence/rpp-0940-go-no-go-release-decision-record-v2.md');
...
test('RPP-0940 v2 risk register matches the current final release gate evaluator', () => {
  ...
  const namedOrClosedRiskIds = new Set([
    ...decision.remainingRisks.map((risk) => risk.id),
    ...decision.closedRisks.map((risk) => risk.id),
  ]);
  ...
  for (const [index, gate] of missing.entries()) {
    const risk = decision.remainingRisks[index];
...
test('RPP-0940 v2 closes no risk without production-backed proof and moves no release gate status', () => {
  const decision = loadMarkdownJson(decisionPath).report;
  const evidence = loadMarkdownJson(evidencePath).report;
...
test('RPP-0940 v2 artifacts remain redacted and do not authorize release movement', async () => {
  ...
  const scan = await scanArtifacts([
    ...
    'docs/evidence/rpp-0940-go-no-go-release-decision-record-v2.md',
  ], { cwd: repoRoot });

  assert.doesNotThrow(() =>
    assertEvidenceHasNoRawValues(decision.report, { label: 'RPP-0940 go/no-go decision record' }));
  assert.doesNotThrow(() =>
    assertEvidenceHasNoRawValues(evidence.report, { label: 'RPP-0940 go/no-go evidence' }));
