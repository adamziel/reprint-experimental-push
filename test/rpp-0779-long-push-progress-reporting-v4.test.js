...
const proofId = 'rpp-0779-long-push-progress-reporting-v4';
...
test('RPP-0779 variant 4 generated progress cases gate long-push output', () => {
...
test('RPP-0779 variant 4 proves long-push progress budgets and support-only NO-GO', {
  concurrency: false,
}, () => {
  const proof = buildVariant4Proof();

  assert.equal(proof.rppId, 'RPP-0779');
  ...
  assert.equal(proof.progressCollection.policy.proofVariant, 4);
...
test('RPP-0779 variant 4 fails closed for over-budget, missing, stale, and premature evidence', () => {
...
function unsafeProgressEvidenceDecisions(evidence) {
  ...
  staleCursorHash.progressCollection.events[12].durableCursor.actionRefHash = sha256('rpp-0779-stale-cursor');
...
function generatedLongPushProgressCases() {
  return [
    ...
    generatedProgressCase('stale-cursor-hash', mutateGeneratedEvidence((evidence) => {
      evidence.progressCollection.events[12].durableCursor.actionRefHash = sha256('rpp-0779-generated-stale-cursor');
    }), {
      updated: false,
