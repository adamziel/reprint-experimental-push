...
const evidencePath = path.join(repoRoot, 'docs/evidence/rpp-0942-release-gate-2-final-audit-v3.md');
const releaseGatesPath = path.join(repoRoot, '.agents/RELEASE_GATES.md');
const fixedNowIso = '2026-06-01T02:49:00.000Z';
const auditedHead = '000a1f2f5b5f65bbc65496667a0b9b163fc3090f';
const supportBoundaryVerdict = 'PACKAGED_RELEASE_BOUNDARY_OK';
const liveBoundaryVerdict = 'LIVE_RELEASE_BOUNDARY_OK';

test('RPP-0942 audit report records GATE-2 support_only and final release NO-GO', () => {
  const { report, text } = loadAuditReport();

  assert.deepEqual(report, expectedAuditReport());
  assert.equal(report.rppId, 'RPP-0942');
...
test('RPP-0942 current release gate row keeps durable recovery journal support_only', () => {
...
test('RPP-0942 support evidence alone can reach candidate review but cannot move release', () => {
...
test('RPP-0942 final release evaluator remains held without live topology auth recovery proof', () => {
...
test('RPP-0942 durable recovery proof only reports proven on a production-backed live boundary', () => {
...
test('RPP-0942 evidence does not claim or perform release-gate status movement', () => {
  const { report, text } = loadAuditReport();
...
function loadAuditReport() {
  ...
  assert.ok(match?.groups?.json, 'RPP-0942 evidence must contain one JSON audit report block');
  return {
    text,
...
function buildReleaseSummary(boundaryVerdict) {
  const activeClaimId = 'claim-active-rpp-0942';
  const previousClaimId = 'claim-previous-rpp-0942';
