...
const evidencePath = path.join(repoRoot, 'docs/evidence/rpp-0955-versioned-protocol-docs-v3.md');
const rpp0935EvidencePath = path.join(repoRoot, 'docs/evidence/rpp-0935-versioned-protocol-docs-v2.md');
const protocolDocPath = path.join(repoRoot, 'docs/protocol/versioned-release-protocol.md');

const auditedHead = '30579db75230cd3734020a5a298d7393046caf90';
...
const expectedRiskIdsByDomain = {
  protocol: [
    'RPP-0955-PROTOCOL-RISK-01',
    'RPP-0955-PROTOCOL-RISK-02',
    'RPP-0955-PROTOCOL-RISK-03',
    'RPP-0955-PROTOCOL-RISK-04',
  ],
  compatibility: [
    'RPP-0955-COMPATIBILITY-RISK-01',
    'RPP-0955-COMPATIBILITY-RISK-02',
    'RPP-0955-COMPATIBILITY-RISK-03',
    'RPP-0955-COMPATIBILITY-RISK-04',
  ],
  migration: [
    'RPP-0955-MIGRATION-RISK-01',
    'RPP-0955-MIGRATION-RISK-02',
    'RPP-0955-MIGRATION-RISK-03',
    'RPP-0955-MIGRATION-RISK-04',
  ],
  'final-release': [
    'RPP-0955-FINAL-RELEASE-RISK-01',
    'RPP-0955-FINAL-RELEASE-RISK-02',
    'RPP-0955-FINAL-RELEASE-RISK-03',
    'RPP-0955-FINAL-RELEASE-RISK-04',
  ],
};

const validationCommands = [
  'node --check test/rpp-0955-versioned-protocol-docs-v3.test.js',
  'node --test --test-name-pattern RPP-0955 test/rpp-0955-versioned-protocol-docs-v3.test.js',
  'node scripts/release/artifact-redaction-scan.mjs docs/evidence/rpp-0955-versioned-protocol-docs-v3.md',
  'git diff --check',
];

test('RPP-0955 evidence records the audited lane head and keeps final release NO-GO', () => {
  const { text, record } = loadEvidence(evidencePath);
  const { record: rpp0935 } = loadEvidence(rpp0935EvidencePath);

  assert.match(text, /^# RPP-0955 versioned protocol docs v3 evidence$/m);
  assert.match(text, /^Date: 2026-06-01$/m);
  assert.match(text, /^Audited local branch: `session\/rpp-955`$/m);
  assert.match(text, new RegExp(`^Audited lane head before this evidence file: \`${auditedHead}\`$`, 'm'));
  assert.equal(record.schemaVersion, 1);
  assert.equal(record.rppId, 'RPP-0955');
  assert.equal(record.sliceId, 'RPP-0955');
  assert.equal(record.proofId, 'rpp-0955-versioned-protocol-docs-v3');
  assert.equal(record.variant, 3);
  assert.equal(record.auditedBranch, 'session/rpp-955');
  ...
  assert.equal(record.patternEvidence, 'docs/evidence/rpp-0935-versioned-protocol-docs-v2.md');
  assert.deepEqual(record.versionedProtocolRecord, rpp0935.versionedProtocolRecord);
});

test('RPP-0955 protocol document package keeps the RPP-0935 version contract', () => {
...
test('RPP-0955 names every protocol compatibility migration and final-release risk', () => {
  const { record } = loadEvidence(evidencePath);
...
test('RPP-0955 keeps every named risk open without production-backed closure proof', () => {
  const { record } = loadEvidence(evidencePath);
  ...
  for (const risk of record.remainingRisks) {
    ...
    assert.match(risk.sourceRiskId, /^RPP-0935-/);
    assert.match(risk.closureRequired, /^Production|^Independent|^Release gate|^Passing/);
  }
});

test('RPP-0955 carries every RPP-0935 risk contract entry forward without closing it', () => {
  const { record } = loadEvidence(evidencePath);
  const { record: rpp0935 } = loadEvidence(rpp0935EvidencePath);
  const sourceRisksById = new Map(rpp0935.remainingRisks.map((risk) => [risk.id, risk]));
  const targetRisksBySourceId = new Map(record.remainingRisks.map((risk) => [risk.sourceRiskId, risk]));

  assert.deepEqual(record.rpp0935RiskContractCarryForward, {
    sourceEvidencePath: 'docs/evidence/rpp-0935-versioned-protocol-docs-v2.md',
    sourceProofId: 'rpp-0935-versioned-protocol-docs-v2',
    sourceRiskCount: 16,
    targetRiskCount: 16,
  ...
  assert.deepEqual(record.rpp0935RiskCrosswalk.map((entry) => entry.sourceRiskId), rpp0935.remainingRisks.map((risk) => risk.id));

  for (const entry of record.rpp0935RiskCrosswalk) {
    const sourceRisk = sourceRisksById.get(entry.sourceRiskId);
    const targetRisk = targetRisksBySourceId.get(entry.sourceRiskId);

    assert.ok(sourceRisk, `${entry.sourceRiskId} must exist in RPP-0935`);
    assert.ok(targetRisk, `${entry.sourceRiskId} must be carried by an RPP-0955 risk`);
    ...
    assert.equal(targetRisk.closureRequired.replace('RPP-0955', 'RPP-0935'), sourceRisk.closureRequired);
    assert.equal(targetRisk.disposition, 'open');
    assert.equal(targetRisk.productionBackedClosureObserved, false);
  }
});

test('RPP-0955 evidence is redacted and records no release-gate movement', async () => {
  const { text, record } = loadEvidence(evidencePath);
  const scan = await scanArtifacts([
    'docs/evidence/rpp-0955-versioned-protocol-docs-v3.md',
  ], { cwd: repoRoot });

  assert.doesNotThrow(() =>
    assertEvidenceHasNoRawValues(record, { label: 'RPP-0955 versioned protocol docs v3 evidence' }));
...
test('RPP-0955 final-release evaluator remains held without production closure proof', () => {
  const { record } = loadEvidence(evidencePath);
