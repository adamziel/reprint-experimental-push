...
const secretValue = 'RPP_0082_SHOULD_NOT_LEAK';
const focusedCommand = 'node --test test/release-verifier-missing-local-url-carry-through-focused-regression.test.js test/release-gate-missing-local-url-regression.test.js test/release-gate-local-url-generated.test.js test/release-verifier-missing-source-url-carry-through-focused-regression.test.js test/release-gates.test.js test/release-gate-cli.test.js';
const statusMarker = '[verify-release:held exit=1 reason=REPRINT_PUSH_LOCAL_URL_REQUIRED mutationAttempted=false]';
const releaseGateStatusMarker = '[release-gates-ci:held final=19/20 candidate=19/20 reason=REPRINT_PUSH_LOCAL_URL_REQUIRED]';
const localReason = 'REPRINT_PUSH_LOCAL_URL is required to prove the local edited site boundary.';
const expectedMissingLocalEvidence = {
...
function runVerifyReleaseMissingOnlyLocal() {
...
function writeEvidence(evidence) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'release-verifier-missing-local-carry-through-'));
...
test('release verifier carries missing REPRINT_PUSH_LOCAL_URL through before mutation for RPP-0082', () => {
  const proof = runVerifyReleaseMissingOnlyLocal();
  const verifyReport = extractJsonObjects(proof.stdout)
    .find((entry) => entry?.releaseProof?.code === 'REPRINT_PUSH_LOCAL_URL_REQUIRED');
  ...
  assert.ok(
    ...
    'verify:release stdout should end with the missing-local-url held marker',
  );
  assert.doesNotMatch(
    `${proof.stdout}\n${proof.stderr}`,
    /Starting Playground server/,
    'missing local URL must fail before starting live verifier servers',
  );
  ...
  assert.deepEqual(
    ...
    },
    {
      ...
      topologyEvidence: {
        ...
        blocker: 'REPRINT_PUSH_LOCAL_URL_REQUIRED',
      },
      releaseMovement: {
        allowed: false,
        gates: '0/4',
  ...
  const localGate = gateById(releaseGateReport, 'local-url');
  ...
  assert.equal(releaseGateReport.primaryFailureCode, 'REPRINT_PUSH_LOCAL_URL_REQUIRED');
  ...
  assert.deepEqual(localGate.evidence, expectedMissingLocalEvidence);
  assert.equal(gateById(releaseGateReport, 'source-url').status, 'passed');
  assert.equal(gateById(releaseGateReport, 'remote-changed-url').status, 'passed');
    ...
    missingEvidence: [
      {
        ...
        code: 'REPRINT_PUSH_LOCAL_URL_REQUIRED',
        reason: localReason,
        evidence: expectedMissingLocalEvidence,
      },
    ],
  });
...
test('release verifier missing local carry-through is recorded in RPP-0082 reports', () => {
  const evidenceReport = fs.readFileSync(evidenceReportPath, 'utf8');
  const progressReport = fs.readFileSync(progressReportPath, 'utf8');

  assert.ok(evidenceReport.includes('| RPP-0082 | Evidence toward release verifier missing `REPRINT_PUSH_LOCAL_URL` carry-through'));
  assert.ok(evidenceReport.includes(`- Command: \`${focusedCommand}\``));
  assert.ok(evidenceReport.includes(`- Observed status: \`pass\`; verifier marker: \`${statusMarker}\`; local gate: \`REPRINT_PUSH_LOCAL_URL_REQUIRED\`; release status: \`NO-GO\`.`));
  assert.ok(progressReport.includes('Release verifier missing local URL carry-through now checks `RPP-0082`'));
  assert.ok(progressReport.includes(`- Command: \`${focusedCommand}\``));
  assert.ok(progressReport.includes(`- Observed status: \`pass\`; verifier marker: \`${statusMarker}\`; local gate: \`REPRINT_PUSH_LOCAL_URL_REQUIRED\`; release status: \`NO-GO\`.`));
});
