...
function tempJournalPath() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'rpp-0670-old-remote-v4-'));
...
function baseSite() {
  ...
  for (let index = 1; index <= 4; index++) {
    files[`rpp-0670-file-${index}.txt`] = `rpp-0670-base-private-value-${index}`;
  }
...
function localSite(base = baseSite()) {
  ...
  for (let index = 1; index <= 4; index++) {
    site.files[`rpp-0670-file-${index}.txt`] = `rpp-0670-local-private-value-${index}`;
  }
  return site;
}
...
function rawSiteValuesFor(...sites) {
  const values = new Set([
    'rpp-0670-base-private-value',
    'rpp-0670-local-private-value',
  ]);
...
function assertNoRawSiteValues(value, rawSiteValues) {
  ...
  for (const rawValue of rawSiteValues) {
    assert.equal(
      ...
      false,
      `RPP-0670 hash-only evidence leaked raw value: ${rawValue}`,
    );
  }
}
...
function openRestartedOldRemoteJournal({ filePath, plan, remote, rawSiteValues }) {
  const artifactRefs = {
    releaseProof: 'artifact://rpp-0670-old-remote-v4-release-proof',
    recoverySupport: 'artifact://rpp-0670-old-remote-v4-support',
  };
  const claimStaleThresholdMs = 2_000;
  const activeClaimId = 'rpp-0670-old-remote-active-claim';
  const retryClaimId = 'rpp-0670-old-remote-retry-claim';
...
function oldRemoteRecoveryFromInspection({ inspection, plan, checkedPath }) {
  return {
    source: 'RPP-0670 focused old remote recovery classification',
    status: 200,
  ...
  return {
    ...
    releaseProof: {
      ...
      idempotencyConflict: {
        ...
        recoveryState: {
          source: 'RPP-0670 different-body conflict recovery state',
          storage: 'sqlite',
          state: 'fully-updated-remote',
...
test('RPP-0670 old remote recovery classification variant 4 proves GATE-2 on the same checked path', () => {
...
test('RPP-0670 old remote release proof rejects missing malformed stale or drifted classification evidence', () => {
  const filePath = tempJournalPath();
  const { plan, remote, rawSiteValues } = buildScenario();
  const {
    productionInspection,
