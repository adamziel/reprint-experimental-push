...
test('RPP-0821 records the three-site topology contract and exact unavailable capability', () => {
  ...
  assert.equal(proof.rppId, 'RPP-0821');
  ...
  assert.doesNotThrow(() => assertEvidenceHasNoRawValues(proof, {
    label: 'RPP-0821 three-site topology proof',
  }));
});

test('RPP-0821 validation rejects a non-started topology without an exact capability code', () => {
  const { artifact, plan, probe } = buildMissingDockerCapabilityArtifact();
  const proof = buildThreeSiteTopologyProof({ artifact, plan, probe });
  const ambiguous = {
    ...proof,
...
test('RPP-0821 validation rejects counting support-only roles as primary sites', () => {
  const { artifact, plan, probe } = buildMissingDockerCapabilityArtifact();
  const proof = buildThreeSiteTopologyProof({ artifact, plan, probe });
  const widened = {
    ...proof,
...
function buildMissingDockerCapabilityArtifact() {
  const plan = buildDockerTopologyPlan({
    ...
    workDir: '/tmp/rpp-0821-docker-work',
    evidenceDir: '/tmp/rpp-0821-docker-evidence',
    env: {},
  });
...
function buildThreeSiteTopologyProof({ artifact, plan, probe }) {
  ...
  const proofCore = {
    ...
    rppId: 'RPP-0821',
    variant: 2,
    proofId: 'rpp-0821-three-site-local-production-topology-v2',
    checkedAt: fixedNow,
    ...
    patternReferences: [
      ...
      'RPP-0811 support-only fail-closed topology proof',
    ],
    builtOn: {
      dockerTopologyVariant,
    ...
    threeSiteContract: {
      variant: 'three-site-local-production-topology-v2',
