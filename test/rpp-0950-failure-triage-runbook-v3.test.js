...
const evidencePath = path.join(repoRoot, 'docs/evidence/rpp-0950-failure-triage-runbook-v3.md');
const rpp0930EvidencePath = path.join(repoRoot, 'docs/evidence/rpp-0930-failure-triage-runbook-v2.md');
...
const auditedHead = '0099a1920c0b9b61a4835867b04422e3abcdce69';
...
test('RPP-0950 evidence updates the audited lane head and keeps final release NO-GO', () => {
  const { text, record } = loadEvidence(evidencePath);
  const releaseGates = readText(releaseGatesPath);

  assert.match(text, /^# RPP-0950 failure triage runbook v3 evidence$/m);
  assert.match(text, /^Date: 2026-06-01$/m);
  assert.match(text, /^Audited local branch: `session\/rpp-950`$/m);
  assert.match(text, new RegExp(`^Audited lane head before this evidence file: \`${auditedHead}\`$`, 'm'));
  assert.equal(record.schemaVersion, 1);
  assert.equal(record.rppId, 'RPP-0950');
  assert.equal(record.sliceId, 'RPP-0950');
  assert.equal(record.proofId, 'rpp-0950-failure-triage-runbook-v3');
  assert.equal(record.variant, 3);
  assert.equal(record.auditedBranch, 'session/rpp-950');
  assert.equal(record.auditedLaneHeadBeforeEvidence, auditedHead);
  assert.equal(record.evidencePath, 'docs/evidence/rpp-0950-failure-triage-runbook-v3.md');
...
test('RPP-0950 names every remaining failure-triage risk and keeps it open', () => {
  const { record } = loadEvidence(evidencePath);
  const { record: rpp0930 } = loadEvidence(rpp0930EvidencePath);

  assert.equal(record.patternEvidence, 'docs/evidence/rpp-0930-failure-triage-runbook-v2.md');
  assert.equal(record.baseFailureTriageRiskEvidence, 'docs/evidence/rpp-0910-failure-triage-runbook.md');
  ...
  assert.deepEqual(
    ...
    rpp0930.failureTriageRiskRegister.map(projectFailureTriageRisk),
  );
...
test('RPP-0950 names every remaining final-release risk from the go/no-go record and keeps it open', () => {
  const { record } = loadEvidence(evidencePath);
...
test('RPP-0950 go/no-go record closes no risk without production-backed closure proof', () => {
  const { record } = loadEvidence(evidencePath);
  const allRisks = [
...
test('RPP-0950 maps failing proof classes to safe operator actions', () => {
  const { record } = loadEvidence(evidencePath);
...
test('RPP-0950 evidence is redacted and records no release-gate movement', async () => {
  const { text, record } = loadEvidence(evidencePath);
...
test('RPP-0950 final-release evaluator remains held without production closure proof', () => {
  const { record } = loadEvidence(evidencePath);
