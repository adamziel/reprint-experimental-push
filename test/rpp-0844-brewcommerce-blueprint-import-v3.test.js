...
const proofId = 'rpp-0844-brewcommerce-blueprint-import-v3';
const variant = 'RPP-0844-variant-3';
...
test('RPP-0844 support report records support-only BrewCommerce import scope with exact unavailable capability', () => {
  const { report, text } = loadSupportReport();

  assert.equal(report.schemaVersion, 1);
  assert.equal(report.rppId, 'RPP-0844');
...
test('RPP-0844 records missing real WordPress import/export capability as fail-closed NO-GO', () => {
  const proof = evaluateBrewcommerceBlueprintImportV3({
...
test('RPP-0844 accepts a real WordPress import/export artifact only when plugin and graph evidence survive', () => {
  const proof = evaluateBrewcommerceBlueprintImportV3({
  ...
  const partial = evaluateBrewcommerceBlueprintImportV3({
    blueprintAssets: syntheticBlueprintAssets(),
    commandInventory: {
      docker: { present: true, usable: true },
    },
...
test('RPP-0844 evidence remains redacted, support-only, and production-backed claims stay blocked', () => {
  const { report, text } = loadSupportReport();

  assert.deepEqual(findEvidenceRedactionIssues(report), []);
  assert.doesNotThrow(() =>
    assertEvidenceHasNoRawValues(report, { label: 'RPP-0844 BrewCommerce support report' }));
...
function loadSupportReport() {
  ...
  assert.ok(match?.groups?.json, 'RPP-0844 evidence must contain one JSON support report block');
  return {
    text,
...
function evaluateBrewcommerceBlueprintImportV3({
  blueprintAssets,
  commandInventory = {},
...
function validateObservedImportExportSurvival(observed) {
  ...
  if (observed.runtime !== 'real-wordpress-import-export' || importExport.realWordPress !== true) {
    failures.push({
      ...
      reason: 'RPP-0844 requires real WordPress import/export, not local Playground or synthetic substitute evidence.',
    });
  }
