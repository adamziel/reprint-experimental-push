...
const evidencePath = path.join(
  repoRoot,
  'docs/evidence/rpp-0869-multisite-subdirectory-topology-v4.md',
);
const variant = 'RPP-0869-variant-4';
...
test('RPP-0869 support report records fail-closed multisite subdirectory variant 4 scope', () => {
  const { report, text } = loadSupportReport();

  assert.equal(report.schemaVersion, 1);
  assert.equal(report.rppId, 'RPP-0869');
  ...
  assert.equal(report.builtOn.previousSupportScope.rppId, 'RPP-0849');
  assert.equal(report.builtOn.previousSupportScope.variant, 3);
  ...
  assert.equal(report.supportReport.variantFocus, 'ordered-real-wordpress-import-export-topology-survival-v4');
  ...
  assert.equal(report.candidateScope.status, 'multisite-subdirectory-import-export-survival-support-candidate-v4');
  assert.equal(report.candidateScope.coverageMode, 'focused-regression-support-contract-candidate-vs-release-ready');
...
test('RPP-0869 records exact unavailable capabilities and production-topology release gaps', () => {
...
test('RPP-0869 accepts only production-backed real subdirectory import/export survival proof', () => {
  const proof = evaluateMultisiteSubdirectoryImportExportV4(
  ...
  assert.match(proof.pluginEvidence.afterImportHash, hexSha256Pattern);
  assert.match(proof.pluginEvidence.afterExportHash, hexSha256Pattern);
...
test('RPP-0869 rejects missing, playground, subdomain, and partial survival evidence', () => {
  const missingProof = evaluateMultisiteSubdirectoryImportExportV4(null);
  ...
  observed.importExport.sequence = ['export-source-multisite-subdirectory'];
  ...
  const proof = evaluateMultisiteSubdirectoryImportExportV4(observed);
  ...
  assert.ok(proof.failures.some((failure) =>
    failure.code === 'IMPORT_EXPORT_SEQUENCE_MISMATCH'));
  assert.ok(proof.failures.some((failure) =>
    failure.code === 'PLUGIN_EVIDENCE_EXPORT_SURVIVAL_MISSING'));
  assert.ok(proof.failures.some((failure) =>
    failure.code === 'PLUGIN_EVIDENCE_AFTER_EXPORT_HASH_MISSING'));
  assert.ok(proof.failures.some((failure) =>
...
test('RPP-0869 evidence remains hash/count/surface only and scoped to support', () => {
  const { report, text } = loadSupportReport();

  assert.deepEqual(findEvidenceRedactionIssues(report), []);
  assert.doesNotThrow(() =>
    assertEvidenceHasNoRawValues(report, { label: 'RPP-0869 multisite subdirectory support report' }));
...
test('RPP-0869 evidence documents scoped validation commands', () => {
  const { text } = loadSupportReport();

  assert.match(text, /node --check test\/rpp-0869-multisite-subdirectory-topology-v4\.test\.js/);
  assert.match(
    text,
...
function evaluateMultisiteSubdirectoryImportExportV4(observed) {
  ...
  return {
    event: 'rpp-0869-multisite-subdirectory-topology-v4',
    variant,
...
function validateObservedImportExportSurvival(observed) {
  ...
  if (observed.runtime !== 'real-wordpress-import-export' || importExport.realWordPress !== true) {
    failures.push({
      ...
      reason: 'RPP-0869 requires real WordPress import/export, not local Playground or synthetic substitute evidence.',
    });
  }
  if (importExport.productionBacked !== true) {
    failures.push({
      code: 'PRODUCTION_BACKED_ARTIFACT_REQUIRED',
      reason: 'RPP-0869 variant 4 remains support-only until production-backed proof is supplied.',
    });
  }
  if (
...
function loadSupportReport() {
  ...
  assert.ok(match?.groups?.json, 'RPP-0869 evidence must contain one JSON support report block');
  return {
    text,
...
function sampleHash(label) {
  return digest({ rpp: 'RPP-0869', label });
}
