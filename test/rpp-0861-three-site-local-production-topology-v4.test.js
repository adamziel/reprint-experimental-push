...
test('RPP-0861 records focused three-site topology v4 support evidence', () => {
  ...
  assert.equal(proof.rppId, 'RPP-0861');
  ...
  assert.deepEqual(proof.topologyCommand.exactUnavailableCapability.requiredFor, [
    ...
    'local-production-topology-v4-startup-proof',
  ]);
  ...
  assert.doesNotThrow(() => assertEvidenceHasNoRawValues(proof, {
    label: 'RPP-0861 three-site topology proof',
  }));
});

test('RPP-0861 validation rejects a non-started topology without an exact capability object', () => {
  const { artifact, plan, probe } = buildMissingDockerCapabilityArtifact();
  const proof = buildThreeSiteTopologyProof({ artifact, plan, probe });
  const ambiguous = {
    ...proof,
...
test('RPP-0861 validation rejects incomplete primary-site topology reports', () => {
  const { artifact, plan, probe } = buildMissingDockerCapabilityArtifact();
  const proof = buildThreeSiteTopologyProof({ artifact, plan, probe });
  const incomplete = {
    ...proof,
...
test('RPP-0861 validation rejects counting support-only roles as primary sites', () => {
  const { artifact, plan, probe } = buildMissingDockerCapabilityArtifact();
  const proof = buildThreeSiteTopologyProof({ artifact, plan, probe });
  const widened = {
    ...proof,
...
test('RPP-0861 validation rejects packaged fallback or release movement claims', () => {
  const { artifact, plan, probe } = buildMissingDockerCapabilityArtifact();
  const proof = buildThreeSiteTopologyProof({ artifact, plan, probe });
  const widened = {
    ...proof,
...
function buildMissingDockerCapabilityArtifact() {
  const plan = buildDockerTopologyPlan({
    ...
    workDir: '/tmp/rpp-0861-docker-work',
    evidenceDir: '/tmp/rpp-0861-docker-evidence',
    env: {},
  });
...
function buildThreeSiteTopologyProof({ artifact, plan, probe }) {
  ...
  const proofCore = {
    ...
    proofId: 'rpp-0861-three-site-local-production-topology-v4',
    checkedAt: fixedNow,
    ...
    threeSiteContract: {
      variant: 'three-site-local-production-topology-v4',
