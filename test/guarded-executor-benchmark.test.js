...
test('guarded benchmark carries hidden memory-ceiling-match blockers into release-bundle pause summaries under visible production capability evidence', () => {
  const report = smallBenchmark();
  const mutated = clone(report);
...
test('guarded benchmark carries hidden memory-ceiling-match blockers into rollout summaries under visible production capability evidence', () => {
  const report = smallBenchmark();
  const mutated = clone(report);
