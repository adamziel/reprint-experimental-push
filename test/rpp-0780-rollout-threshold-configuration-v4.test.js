...
const proofId = 'rpp-0780-rollout-threshold-configuration-v4';
...
test('RPP-0780 variant 4 proves rollout thresholds gate fast-path lane updates', {
  concurrency: false,
}, () => {
  const proof = buildVariant4Proof();

  assert.equal(proof.rppId, 'RPP-0780');
  ...
  assert.equal(proof.fastPathLaneEvidence.performance.p95DecisionMs, 5.0);
...
test('RPP-0780 variant 4 fails closed for unsafe rollout threshold evidence', () => {
  ...
  performanceThresholdExceeded.fastPathLaneEvidence.performance.p95DecisionMs = 5.6;
  ...
  mismatchedConfigurationHash.thresholdConfiguration.configHash = sha256(
    'rpp-0780-mismatched-threshold-configuration',
  );
...
function buildStoragePerformanceSamples(config) {
  return decisionSamplesMs.map((decisionMs, index) => {
    ...
    const core = {
      sampleIndex: index,
      thresholdBps,
      resourceKeyHash: sha256(`rpp-0780-resource-${index}`),
      expectedStorageHash: sha256(`rpp-0780-storage-before-${index}`),
      actualStorageHash: sha256(`rpp-0780-storage-before-${index}`),
...
function recomputeRolloutThresholdGates(evidence, repeatedEvidence) {
  ...
  return [
    proofGate('deterministic-threshold-configuration', config.schemaVersion === 4
      && config.variant === 4
