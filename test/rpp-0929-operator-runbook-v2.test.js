...
const evidencePath = path.join(repoRoot, 'docs/evidence/rpp-0929-operator-runbook-v2.md');
const runbookPath = path.join(repoRoot, 'docs/operations/operator-runbook.md');

const auditedHead = 'ace2cf55c6457b8b54da5b2b98408a9fd704c909';
...
const validationCommands = Object.freeze([
  'git rev-parse HEAD',
  'node --check test/rpp-0929-operator-runbook-v2.test.js',
  'node --test --test-name-pattern RPP-0929 test/rpp-0929-operator-runbook-v2.test.js',
  'node scripts/release/artifact-redaction-scan.mjs docs/evidence/rpp-0929-operator-runbook-v2.md',
  'git diff --check',
]);

test('RPP-0929 evidence audits the operator runbook lane head with verdict held', () => {
  const { text, report } = loadEvidenceReport();

  assert.match(text, /^# RPP-0929 Operator Runbook Evidence Variant 2$/m);
  assert.match(text, /^Audited local branch: `session\/rpp-929`$/m);
  assert.match(text, new RegExp(`^Audited lane head before this evidence file: \`${auditedHead}\`$`, 'm'));
  assert.equal(report.schemaVersion, 1);
  assert.equal(report.rppId, 'RPP-0929');
  assert.equal(report.proofId, 'rpp-0929-operator-runbook-v2');
  ...
  assert.equal(report.auditedBranch, 'session/rpp-929');
  ...
  assert.equal(report.documents.patternEvidence, 'docs/evidence/rpp-0909-operator-runbook.md');
...
test('RPP-0929 operator runbook names production prerequisites before mutation', () => {
...
test('RPP-0929 operator runbook names recovery evidence before retry or finalization', () => {
  const runbook = readText(runbookPath);
  const { report } = loadEvidenceReport();
...
test('RPP-0929 operator runbook names stop conditions before unsafe action', () => {
  const runbook = readText(runbookPath);
  const { report } = loadEvidenceReport();
...
test('RPP-0929 operator runbook blocks hidden assumptions before release movement', () => {
  const runbook = readText(runbookPath);
  const { report } = loadEvidenceReport();
...
test('RPP-0929 evidence remains redacted and lists focused validation commands', () => {
  const { text, report } = loadEvidenceReport();
  const runbook = readText(runbookPath);

  assert.doesNotThrow(() =>
    assertEvidenceHasNoRawValues(report, { label: 'RPP-0929 operator runbook v2 evidence' }));
...
function loadEvidenceReport() {
  ...
  assert.ok(match?.groups?.json, 'RPP-0929 evidence must contain one JSON record block');
  return {
    text,
