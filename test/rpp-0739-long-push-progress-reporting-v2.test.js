...
const proofId = 'rpp-0739-long-push-progress-reporting-v2';
...
const expectedGateIds = Object.freeze([
  ...
  'progress-policy-v2-support-contract',
...
test('RPP-0739 variant 2 proves long-push progress budgets and support-only NO-GO', {
  concurrency: false,
}, () => {
  const proof = buildVariant2Proof();

  assert.equal(proof.rppId, 'RPP-0739');
  ...
  assert.equal(proof.progressCollection.policy.proofVariant, 2);
...
test('RPP-0739 variant 2 fails closed for over-budget, missing, stale, and premature evidence', () => {
...
function recomputeLongPushProgressProofGates(evidence) {
  ...
  return [
    ...
    proofGate('progress-policy-v2-support-contract', policy.valid, policy),
...
function unsafeProgressEvidenceDecisions(evidence) {
  ...
  staleCursorHash.progressCollection.events[12].durableCursor.actionRefHash = sha256('rpp-0739-stale-cursor');
