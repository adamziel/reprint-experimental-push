...
const evidencePath = path.join(repoRoot, 'docs/evidence/rpp-0968-privacy-redaction-review-v4.md');
const priorEvidencePath = path.join(repoRoot, 'docs/evidence/rpp-0948-privacy-redaction-review-v3.md');
const fixedNow = '2026-05-28T08:30:00.000Z';
const freshObservedAt = '2026-05-28T08:00:00.000Z';
const auditedLaneHead = 'b847007af075ad768c0ab6a268503186d260ed89';
...
function loadEvidenceReport(filePath = evidencePath, label = 'RPP-0968') {
...
function makeArtifactRoot(t, fixture) {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'rpp-968-redaction-artifact-'));
  t.after(() => {
...
function writeRequiredCheckFixture(t, fixture) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'rpp-968-required-checks-'));
  t.after(() => {
...
test('RPP-0968 review v4 records support-only held redaction proof posture', () => {
  ...
  assert.equal(report.rppId, 'RPP-0968');
  assert.equal(report.sliceId, 'RPP-0968');
  assert.equal(report.workerId, 'rpp-968');
  assert.equal(report.variant, 4);
  assert.equal(report.auditedBranch, 'session/rpp-968');
...
test('RPP-0968 carries forward RPP-0948 v3 redaction-scanner contract', () => {
  const { report } = loadEvidenceReport();
  const { report: priorReport } = loadEvidenceReport(priorEvidencePath, 'RPP-0948');

  assert.equal(report.carriedForwardContract.sourceRppId, 'RPP-0948');
  assert.equal(report.carriedForwardContract.sourceVariant, 3);
...
test('RPP-0968 scanner failures become blocking required redaction proof failures', async (t) => {
...
test('RPP-0968 required release checks CLI exits nonzero for each redaction failure reason', (t) => {
  for (const [index, fixture] of redactionFailureFixtures.entries()) {
    const requiredFixture = failedRequiredCheckFixture(fixture, index);
...
test('RPP-0968 support-only redaction observation cannot move final release readiness', (t) => {
...
test('RPP-0968 evidence artifact remains redacted and records no gate movement', async () => {
  const scanReport = await scanArtifacts([
    'docs/evidence/rpp-0968-privacy-redaction-review-v4.md',
  ], { cwd: repoRoot });
  const { text, report } = loadEvidenceReport();
