...
const evidencePath = path.join(
  repoRoot,
  'docs/evidence/rpp-0878-tls-https-source-proof-v4.md',
);

const fixedNow = new Date('2026-06-01T00:00:00.000Z');
const proofId = 'rpp-0878-tls-https-source-proof-v4';
...
const goodEnv = Object.freeze({
  REPRINT_PUSH_SOURCE_URL: 'https://source-v4.example.test/variant-four-source',
  REPRINT_PUSH_REMOTE_URL: 'https://source-v4.example.test:443/variant-four-source/',
  REPRINT_PUSH_LOCAL_URL: 'https://local-v4.example.test/variant-four-local',
  REPRINT_PUSH_REMOTE_CHANGED_URL: 'https://changed-v4.example.test/variant-four-changed',
  REPRINT_PUSH_PREFLIGHT_SOURCE_URL: 'https://source-v4.example.test/variant-four-source/',
  REPRINT_PUSH_DRY_RUN_ROUTE_SOURCE_URL: 'https://source-v4.example.test:443/variant-four-source',
  REPRINT_PUSH_APPLY_SOURCE_URL: 'https://source-v4.example.test/variant-four-source',
  REPRINT_PUSH_JOURNAL_ROUTE_SOURCE_URL: 'https://source-v4.example.test/variant-four-source',
  REPRINT_PUSH_RECOVERY_INSPECT_SOURCE_URL: 'https://source-v4.example.test/variant-four-source',
  REPRINT_PUSH_USERNAME: 'rpp-0878-admin',
  REPRINT_PUSH_APPLICATION_PASSWORD: 'rpp-0878-application-password-must-not-leak',
});

const forbiddenProofNeedles = Object.freeze([
  'https://source-v4.example.test',
  'https://local-v4.example.test',
  'https://changed-v4.example.test',
  'source-v4.example.test',
  'local-v4.example.test',
  'changed-v4.example.test',
  'variant-four-source',
  'variant-four-local',
  'variant-four-changed',
  'rpp-0878-admin',
  'rpp-0878-application-password-must-not-leak',
]);

test('RPP-0878 captures source/local/changed HTTPS URLs and identity-checks variant 4 roles', () => {
  const proof = buildTlsHttpsSourceProof({ env: goodEnv });

  assert.equal(proof.schemaVersion, 1);
  assert.equal(proof.rppId, 'RPP-0878');
  ...
  assert.equal(proof.patternLineage.patternOnlyReference.rppId, 'RPP-0858');
  assert.equal(proof.patternLineage.patternOnlyReference.variant, 3);
  assert.equal(proof.patternLineage.patternOnlyReference.reusesProofArtifact, false);

  assert.equal(proof.tlsHttpsSource.scope, 'tls-https-source-proof-v4');
  ...
  assert.doesNotThrow(() => assertEvidenceHasNoRawValues(proof, { label: 'RPP-0878 TLS HTTPS source proof v4' }));
});

test('RPP-0878 rejects tunnel URLs and secret-shaped URL parts before accepting TLS source scope', () => {
  const proof = buildTlsHttpsSourceProof({
    env: {
      REPRINT_PUSH_SOURCE_URL: 'https://admin:rpp0878-secret@source-v4.example.test/variant-four-source',
      REPRINT_PUSH_LOCAL_URL: 'https://local-v4.example.test/secret/rpp0878-token',
      REPRINT_PUSH_REMOTE_CHANGED_URL: 'https://changed-v4.ngrok-free.app/variant-four-changed',
  ...
  assert.ok(proof.failures.some((failure) => failure.code === 'RPP_0878_SECRET_SHAPED_URL_PART_REJECTED'
    && failure.role === 'source'
  ...
  assertNoNeedles(proof, [
    'admin:rpp0878-secret',
    'rpp0878-secret',
    'secret/rpp0878-token',
    'rpp0878-token',
    'changed-v4.ngrok-free.app',
  ]);
  assertNoRawUrls(proof);
});

test('RPP-0878 rejects non-HTTPS role URLs before TLS scope acceptance', () => {
  const proof = buildTlsHttpsSourceProof({
    env: {
      ...goodEnv,
      REPRINT_PUSH_SOURCE_URL: 'http://source-v4.example.test/variant-four-source',
      REPRINT_PUSH_REMOTE_URL: 'http://source-v4.example.test/variant-four-source',
      REPRINT_PUSH_PREFLIGHT_SOURCE_URL: 'http://source-v4.example.test/variant-four-source',
      REPRINT_PUSH_DRY_RUN_ROUTE_SOURCE_URL: 'http://source-v4.example.test/variant-four-source',
      REPRINT_PUSH_APPLY_SOURCE_URL: 'http://source-v4.example.test/variant-four-source',
      REPRINT_PUSH_JOURNAL_ROUTE_SOURCE_URL: 'http://source-v4.example.test/variant-four-source',
      REPRINT_PUSH_RECOVERY_INSPECT_SOURCE_URL: 'http://source-v4.example.test/variant-four-source',
    },
  });
  ...
  assert.ok(proof.failures.some((failure) =>
    failure.code === 'RPP_0878_REQUIRED_ROLE_URL_MUST_BE_HTTPS'
      && failure.role === 'source'));
  assertNoRawUrls(proof);
});

test('RPP-0878 rejects duplicate source/local/changed URL identities before TLS scope acceptance', () => {
  const proof = buildTlsHttpsSourceProof({
    env: {
      ...goodEnv,
      REPRINT_PUSH_LOCAL_URL: 'https://source-v4.example.test:443/variant-four-source/',
    },
  });
  ...
  assert.equal(proof.tlsHttpsSource.sourceLocalChangedUrlsCaptured, true);
  assert.equal(proof.tlsHttpsSource.httpsRoleUrlCount, 3);
  assert.equal(proof.tlsHttpsSource.roleIdentitiesDistinct, false);
  assert.equal(proof.tlsHttpsSource.identityChecked, false);
  assert.equal(proof.tlsHttpsSource.scopeAcceptedForReleaseTopology, false);
  assert.ok(proof.failures.some((failure) =>
    failure.code === 'EXTERNAL_WORDPRESS_SOURCE_LOCAL_CHANGED_URLS_NOT_DISTINCT'));
  assertNoNeedles(proof, forbiddenProofNeedles);
  assertNoRawUrls(proof);
});

test('RPP-0878 TLS HTTPS source proof v4 is deterministic and hash/count/surface-only', () => {
...
test('RPP-0878 evidence document records the same NO-GO hash/count/surface-only scope', () => {
  const { report, text } = loadEvidenceReport();

  assert.equal(report.schemaVersion, 1);
  assert.equal(report.rppId, 'RPP-0878');
  ...
  assert.equal(report.negativeControls.duplicateRoleUrlRejected, true);
  ...
  assert.doesNotMatch(text, /source-v4\.example|local-v4\.example|changed-v4\.example|ngrok|token=|admin:/i);
});
...
function buildTlsHttpsSourceProof({ env, now = fixedNow } = {}) {
  ...
  const tlsFailures = roleUrlEvidence
    ...
    .map((entry) => ({
      code: 'RPP_0878_REQUIRED_ROLE_URL_MUST_BE_HTTPS',
      role: entry.role,
      envKey: entry.envKey,
    }));
  ...
  const proofCore = {
    ...
    tlsHttpsSource: {
      scope: 'tls-https-source-proof-v4',
...
function findSecretShapedUrlPartFailures(env = {}) {
  ...
  for (const definition of urlDefinitionsForSecretPartScan(env)) {
    ...
    for (const partSurface of partSurfaces) {
      failures.push({
        code: 'RPP_0878_SECRET_SHAPED_URL_PART_REJECTED',
...
function loadEvidenceReport() {
  ...
  assert.ok(match?.groups?.json, 'RPP-0878 evidence must contain one JSON report block');
  return {
    text,
