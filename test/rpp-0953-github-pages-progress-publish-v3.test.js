...
const evidencePath = path.join(repoRoot, 'docs/evidence/rpp-0953-github-pages-progress-publish-v3.md');
...
const matchingProgressHash = 'sha256:5555555555555555555555555555555555555555555555555555555555555555';
const mismatchedPublicHash = 'sha256:6666666666666666666666666666666666666666666666666666666666666666';
...
function loadEvidenceReport() {
  ...
  assert.ok(match?.groups?.json, 'RPP-0953 evidence must contain one JSON report block');
  return {
    text,
...
function writeFixture(t, fixture) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'rpp-953-progress-publish-v3-'));
  t.after(() => {
...
test('RPP-0953 evidence keeps GitHub Pages publish v3 proof held and support-only', () => {
  const { text, report } = loadEvidenceReport();

  assert.equal(report.schemaVersion, 1);
  assert.equal(report.rppId, 'RPP-0953');
  assert.equal(report.workerId, 'rpp-953');
  assert.equal(report.variant, 3);
  assert.equal(report.auditedBranch, 'session/rpp-953');
  assert.equal(report.auditedLaneHeadBeforeEvidence, '9e56557585f94a9a34853eaf22d289f10d8342dd');
...
test('RPP-0953 public progress publish verification fails closed when proof is missing or stale', () => {
...
test('RPP-0953 progress.html publication remains support-only and cannot move final release readiness', () => {
  const { report } = loadEvidenceReport();
...
test('RPP-0953 public progress artifact proof fails closed on mismatch or missing public artifact', () => {
...
test('RPP-0953 required release checks CLI exits nonzero and held on failed publish proof', (t) => {
  const { result, report } = runRequiredChecksFixture(t, publishObservationFromArtifactProof({
    publishedPublicArtifact: {
      path: 'progress.html',
      sha256: mismatchedPublicHash,
    },
...
test('RPP-0953 required release checks CLI exits nonzero on missing or stale publish proof', (t) => {
...
test('RPP-0953 support evidence remains redacted and release-gate neutral', async () => {
