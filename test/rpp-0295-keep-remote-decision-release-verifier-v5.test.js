...
function assertGeneratedEvidence(evidence) {
  ...
  assert.equal(evidence.casesWithKeepRemote, 544);
  assert.equal(evidence.readyCasesWithKeepRemote, 295);
  assert.equal(evidence.nonReadyCasesWithKeepRemote, 249);
  assert.equal(evidence.keepRemoteDecisionCount, 1845);
  ...
  assert.deepEqual(evidence.keepRemoteLocalChanges, { unchanged: 1845 });
