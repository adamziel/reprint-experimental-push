...
const proofId = 'rpp-0839-sandbox-8080-ingress-rule-proof-v2';
const variant = 'RPP-0839-variant-2';
...
const rpp0839Env = Object.freeze({
...
test('RPP-0839 support report records sandbox 8080 ingress variant 2 scope', () => {
  const { report, text } = loadSupportReport();
  const expected = buildSupportReport();

  assert.deepEqual(report, expected);
  assert.equal(report.schemaVersion, 1);
  assert.equal(report.rppId, 'RPP-0839');
  ...
  assert.equal(report.builtOn.rpp0819Pattern.proofId, 'rpp-0819-sandbox-8080-ingress-rule-proof-v1');
  assert.equal(report.builtOn.rpp0819Pattern.contractInherited, true);
  ...
  assert.equal(report.candidateScope.status, 'sandbox-8080-ingress-rule-candidate-v2');
  ...
  assert.equal(report.productionArtifactAudit.repositoryRpp0839ProductionArtifactPresent, false);
...
test('RPP-0839 topology command fails closed while preserving 8080-only ingress', () => {
...
test('RPP-0839 accepts survival only with production-backed import/export under sandbox 8080', () => {
...
test('RPP-0839 ingress validation rejects public, non-8080, and multiple HTTP surfaces', () => {
...
test('RPP-0839 evidence remains deterministic hash/count/surface-only', () => {
  const { report, text } = loadSupportReport();

  assert.deepEqual(report, buildSupportReport());
  assert.deepEqual(findEvidenceRedactionIssues(report), []);
  assert.doesNotThrow(() =>
    assertEvidenceHasNoRawValues(report, { label: 'RPP-0839 sandbox 8080 ingress rule proof v2' }));
...
function loadSupportReport() {
  ...
  assert.ok(match?.groups?.json, 'RPP-0839 evidence must contain one JSON support report block');
  return {
    text,
...
function buildBasePlan(options = {}) {
  return buildDockerTopologyPlan({
    ...
    workDir: '/tmp/rpp-0839-docker-work',
    evidenceDir: '/tmp/rpp-0839-docker-evidence',
    env: rpp0839Env,
    ...options,
  });
}
...
function validateObservedImportExportSurvival(observed) {
  ...
  if (observed.runtime !== 'real-wordpress-import-export' || importExport.realWordPress !== true) {
    failures.push({
      ...
      reason: 'RPP-0839 requires real WordPress import/export, not local Playground or synthetic substitute evidence.',
    });
  }
  if (importExport.productionBacked !== true) {
    failures.push({
      code: 'PRODUCTION_BACKED_IMPORT_EXPORT_REQUIRED',
      reason: 'RPP-0839 release movement requires a production-backed import/export artifact.',
    });
  }
