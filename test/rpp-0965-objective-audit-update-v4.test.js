...
const evidencePath = path.join(repoRoot, 'docs/evidence/rpp-0965-objective-audit-update-v4.md');
const contractEvidencePath = path.join(repoRoot, 'docs/evidence/rpp-0945-objective-audit-update-v3.md');
const fixedNowIso = '2026-06-01T03:45:00.000Z';
const auditedLaneHead = 'ad7a2e47980d3a3ae5ff2f0b44f3c28fda719921';
...
test('RPP-0965 evidence is support-only and keeps the final release verdict held', () => {
  const { report, text } = loadEvidenceReport();

  assert.match(text, /^# RPP-0965 objective audit update v4$/m);
  ...
  assert.equal(report.rppId, 'RPP-0965');
  assert.equal(report.proofId, 'rpp-0965-objective-audit-update-v4');
  ...
  assert.equal(report.auditedBranch, 'session/rpp-965');
...
test('RPP-0965 final go/no-go record names every current final-release risk and closes none', () => {
...
test('RPP-0965 carries forward the RPP-0945 v3 objective-audit risk contract', () => {
  const { report } = loadEvidenceReport();
  ...
  assert.equal(contract.rppId, 'RPP-0945');
  assert.equal(contract.proofId, 'rpp-0945-objective-audit-update-v3');
  assert.equal(contract.variant, 3);
...
test('RPP-0965 carries forward R1-R16 objective risks without production-backed closure proof', () => {
  const { report } = loadEvidenceReport();
...
test('RPP-0965 release-gate snapshot matches the current held final-release evaluator', () => {
  const { report } = loadEvidenceReport();
...
test('RPP-0965 evidence remains redacted and records no release-gate movement', async () => {
  const { report, text } = loadEvidenceReport();
  const scan = await scanArtifacts([
    'docs/evidence/rpp-0965-objective-audit-update-v4.md',
  ], { cwd: repoRoot });

  assert.doesNotThrow(() =>
    assertEvidenceHasNoRawValues(report, { label: 'RPP-0965 objective audit update v4' }));
