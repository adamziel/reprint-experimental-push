...
const evidencePath = path.join(repoRoot, 'docs/evidence/rpp-0976-migration-docs-v4.md');
...
const expectedCommitSubjects = Object.freeze({
  '513e51027': 'Merge published progress page state',
  '270102309': 'docs: publish progress page',
  '695ddbb5b': 'docs: refresh progress for RPP-0971 integration',
  df5639281: "Merge branch 'session/rpp-971' into lane/evidence-integration-20260527",
  '86825b6f7': 'Merge published progress page state',
  ea84588ad: 'docs: publish progress page',
  f9fa042ca: 'docs: refresh progress for RPP-0970 integration',
  '1c3845c17': "Merge branch 'session/rpp-970' into lane/evidence-integration-20260527",
  '4aa80fc42': 'Merge published progress page state',
  '94910d6f6': 'docs: publish progress page',
  '63946dbed': 'Add RPP-0971 rollback repair runbook v4 evidence',
  '4f23898f8': 'docs: refresh progress for RPP-0969 integration',
  be1d6bec6: 'docs: refresh progress for RPP-0956 integration',
  ca3440069: 'Add RPP-0956 migration docs v3 evidence',
  ...
  d9001fe57: 'docs: refresh progress for RPP-0681 integration',
  '5df68c6cc': 'Add RPP-0681 journal schema migration release proof',
  cf4316459: 'docs: refresh progress for RPP-0661 integration',
  cbc259b3b: 'Add RPP-0661 journal table schema migration proof',
  '31dc9f4f5': 'docs: refresh progress for RPP-0641 integration',
  e5145c196: 'Add RPP-0641 journal schema migration coverage',
  '6f0fe1e93': 'docs: refresh progress for RPP-0621 integration',
  ...
  cad3afe6b: 'chore: merge public progress update after RPP-0601 evidence',
  da2b34027: 'docs: record RPP-0601 integration',
  '608302051': 'chore: merge public progress update after RPP-0601 auxiliary evidence',
  d5e02663e: 'docs: record RPP-0601 auxiliary journal evidence',
...
const expectedRequiredProductionProof = Object.freeze([
  'source-target-identity-hashes',
  'current-release-envelope-id',
...
test('RPP-0976 evidence records support-only migration docs v4 posture', () => {
  const { report } = loadEvidenceReport();

  assert.equal(report.schemaVersion, 1);
  assert.equal(report.rppId, 'RPP-0976');
  assert.equal(report.proofId, 'rpp-0976-migration-docs-v4');
  ...
  assert.equal(report.documents.evidence, 'docs/evidence/rpp-0976-migration-docs-v4.md');
  assert.equal(report.documents.patternEvidence, 'docs/evidence/rpp-0956-migration-docs-v3.md');
  assert.equal(report.documents.previousPatternEvidence, 'docs/evidence/rpp-0936-migration-docs-v2.md');
  ...
  assert.equal(report.migrationDocContract.carriedForwardFrom, 'rpp-0956-migration-docs-v3');
...
test('RPP-0976 audit file keeps migration proof gaps open', () => {
  const { report, text } = loadEvidenceReport();

  assert.deepEqual(report.migrationPrerequisites, expectedMigrationPrerequisites);
  assert.deepEqual(report.requiredProductionBackedMigrationProof, expectedRequiredProductionProof);
  assert.deepEqual(report.openProductionProofGaps, expectedRequiredProductionProof);
  ...
  for (const gap of expectedRequiredProductionProof) {
    assert.ok(text.includes(gap), `${gap} must remain open in the audit file`);
  }
...
test('RPP-0976 audit file links exact commands to existing commit anchors', () => {
  const { report, text } = loadEvidenceReport();
  ...
  assert.equal(report.relevantCurrentCommits[0].name, 'current-branch-head-before-rpp-0976');
  assert.equal(report.relevantCurrentCommits[0].sha, '513e51027');
...
test('RPP-0976 final release blocks movement without production-backed migration proof', () => {
  const { report, text } = loadEvidenceReport();
  ...
  assert.doesNotThrow(() =>
    assertEvidenceHasNoRawValues(report, { label: 'RPP-0976 migration docs v4 evidence' }));
...
function loadEvidenceReport() {
  ...
  assert.ok(match?.groups?.json, 'RPP-0976 evidence must contain one JSON record block');
  return {
    text,
