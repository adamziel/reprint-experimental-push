...
test('benchmark model covers large uploads and plugin installs', () => {
  ...
  assert.ok(
    model.rejectedFastPaths.some(
      (rejection) => rejection.id === 'index-and-compressed-row-receipts-completes-plugin-update',
    ),
  ...
  assert.ok(
    model.rejectedFastPaths.some(
      (rejection) => rejection.id === 'index-and-compressed-row-batch-skips-backpressure',
    ),
    'model rejects treating a compressed row batch plus a fresh remote index as backpressure bypass for plugin updates',
  );
  assert.ok(
    model.rejectedFastPaths.some(
...
test('rejected fast paths cover precondition bypasses and atomic group splits', () => {
  ...
  assert.ok(
    rejectedById.get('compressed-receipts-replace-durable-progress').violates.includes('chunk-receipts'),
  );
  assert.ok(
    rejectedById.get('compressed-receipts-replace-durable-progress').violates.includes('durable-progress'),
  );
  assert.equal(
    rejectedById.get('compressed-receipts-replace-durable-progress').rejectedGate,
    'recovery',
  );
  ...
  assert.ok(
    rejectedById
      .get('index-and-compressed-package-cache-completes-plugin-update')
      .violates.includes('remote-index-planning-only'),
  );
  assert.ok(
    rejectedById
  ...
  assert.ok(
    rejectedById
      .get('index-and-compressed-row-batch-completes-plugin-activation')
      .violates.includes('plugin-preconditions'),
  );
  assert.ok(
    rejectedById
  ...
  assert.equal(
    rejectedById.get('index-and-compressed-row-batch-completes-plugin-activation').rejectedGate,
    'recovery',
  );
  assert.ok(
    rejectedById
  ...
  assert.equal(
    rejectedById.get('index-and-compressed-file-hash-completes-plugin-install').rejectedGate,
    'recovery',
  );
  assert.ok(
    rejectedById
  ...
  assert.equal(
    rejectedById.get('index-and-compressed-file-hash-completes-plugin-update').rejectedGate,
    'recovery',
  );
  assert.ok(
  ...
  assert.ok(
    rejectedById
      .get('compressed-file-hash-skips-chunk-receipts')
      .violates.includes('compression'),
  );
  assert.ok(
    rejectedById
  ...
  assert.equal(
    rejectedById.get('compressed-chunk-receipts-complete-large-upload').rejectedGate,
    'recovery',
  );
  assert.ok(
    rejectedById
      .get('compressed-receipts-plus-cached-hash-complete-large-upload')
      .violates.includes('file-hashing'),
  );
  assert.ok(
    rejectedById
  ...
  assert.equal(
    rejectedById.get('compressed-receipts-plus-cached-hash-complete-large-upload').rejectedGate,
    'recovery',
  );
  assert.ok(
  ...
  assert.ok(
    rejectedById
      .get('index-and-compressed-row-summary-completes-plugin-activation')
      .violates.includes('row-preconditions'),
  );
  assert.ok(
    rejectedById
  ...
  assert.ok(
    rejectedById
      .get('index-and-compressed-row-summary-completes-plugin-install')
      .violates.includes('plugin-preconditions'),
  );
  assert.ok(
    rejectedById
  ...
  assert.equal(
    rejectedById.get('index-and-compressed-row-summary-completes-plugin-activation').rejectedGate,
    'recovery',
  );
  assert.equal(
    rejectedById.get('index-and-compressed-row-summary-completes-plugin-install').rejectedGate,
    'recovery',
  );
  assert.ok(
    rejectedById
      .get('index-and-compressed-row-summary-completes-plugin-update')
      .violates.includes('remote-index-planning-only'),
  );
  assert.ok(
    rejectedById
  ...
  assert.equal(
    rejectedById.get('index-and-compressed-row-summary-completes-plugin-update').rejectedGate,
    'recovery',
  );
  assert.equal(
  ...
  for (const id of [
    ...
    'index-and-compressed-row-summary-completes-plugin-activation',
