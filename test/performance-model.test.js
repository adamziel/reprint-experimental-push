...
test('benchmark model covers large uploads and plugin installs', () => {
  ...
  assert.ok(
    model.safeFastPaths.some(
      (fastPath) =>
        fastPath.area === 'database-row-batching' &&
        fastPath.allowedShortcut === 'reuse-canonical-row-batch-manifests-to-size-bounded-plugin-update-batches' &&
        fastPath.guardrails.includes('canonical-manifests-stay-planning-evidence-only') &&
        fastPath.gateProofs.skip.includes('each row still keeps its own precondition'),
    ),
    'canonical row-batch manifests can size plugin-update batches without weakening row preconditions',
  );
  assert.ok(
    model.safeFastPaths.some(
      (fastPath) =>
        fastPath.area === 'database-row-batching' &&
        fastPath.allowedShortcut === 'compress-planning-row-batch-manifests-and-reuse-dependency-graph-to-size-bounded-plugin-install-batches' &&
        fastPath.guardrails.includes('compressed-manifest-remains-planning-evidence-only') &&
        fastPath.gateProofs.skip.includes('each row still keeps its own precondition'),
    ),
    'compressed row-batch manifests can size plugin-install batches without weakening row preconditions',
  );
  assert.ok(
    model.safeFastPaths.some(
      (fastPath) =>
  ...
  assert.ok(
    ...
    'independent owner scans can size release-bundle fanout without weakening recovery evidence',
  );
  assert.ok(
    model.safeFastPaths.some(
      (fastPath) =>
  ...
  assert.ok(
    model.safeFastPaths.some(
      (fastPath) =>
        fastPath.area === 'parallelism-limits' &&
        fastPath.allowedShortcut === 'reuse-memory-headroom-and-cached-release-manifest-cursor-to-size-bounded-release-bundle-retry-windows' &&
        ...
        fastPath.gateProofs.recovery.includes('the measured headroom and cached cursor are advisory'),
    ),
    'memory headroom and cached release-manifest cursors can size release-bundle retry windows without weakening recovery evidence',
  );
  assert.ok(
    model.safeFastPaths.some(
      (fastPath) =>
        fastPath.area === 'parallelism-limits' &&
        fastPath.allowedShortcut === 'reuse-queue-headroom-and-cached-release-manifest-cursor-to-size-bounded-release-bundle-resume' &&
        fastPath.guardrails.includes('queue-headroom-stays-planning-evidence-only') &&
        fastPath.guardrails.includes('cached-release-manifest-cursor-remains-planning-evidence-only') &&
        fastPath.gateProofs.skip.includes('sizing the next bounded release-bundle resume window') &&
        fastPath.gateProofs.recovery.includes('durable receipts and the guarded release record still classify pause, retry, or crash'),
    ),
    'queue headroom and cached release-manifest cursors can size release-bundle resume windows without weakening recovery evidence',
  );
  assert.ok(
    model.safeFastPaths.some(
      (fastPath) =>
  ...
  assert.ok(
    model.safeFastPaths.some(
      (fastPath) =>
        fastPath.allowedShortcut === 'reuse-cached-release-manifest-cursor-to-size-bounded-release-bundle-fanout' &&
        fastPath.visibilityBoundary === 'planning-only-for-release-bundle-fanout' &&
        fastPath.guardrails.includes('cached-release-manifest-cursor-remains-planning-evidence-only') &&
        fastPath.gateProofs.recovery.includes('guarded release record'),
    ),
    'cached release-manifest cursors can size release-bundle fanout without weakening recovery evidence',
  );
  assert.ok(
    model.safeFastPaths.some(
      (fastPath) =>
  ...
  assert.ok(
    ...
    'compressed budget summaries and cached release manifests can size retry windows without weakening release recovery evidence',
  );
  assert.ok(
    model.safeFastPaths.some(
      (fastPath) =>
...
test('fast-path proofs and rejections carry the expected gate metadata', () => {
  ...
  assert.ok(
    model.safeFastPaths.some(
      (fastPath) =>
        fastPath.area === 'backpressure' &&
        fastPath.allowedShortcut === 'compress-kind-scoped-receipt-ledgers-for-bounded-replay-without-changing-raw-order' &&
        fastPath.visibilityBoundary === 'kind-scoped-memory-and-journal-planning-only' &&
        fastPath.failureEvidence.includes('ordered raw durable receipt keys'),
    ),
    'compressed kind-scoped receipt ledgers stay planning-only and preserve raw replay order',
  );
  assert.ok(
    model.safeFastPaths.some(
      (fastPath) =>
  ...
  assert.ok(
    model.safeFastPaths.some(
      (fastPath) =>
        fastPath.area === 'backpressure' &&
        fastPath.allowedShortcut === 'reuse-measured-queue-headroom-to-size-bounded-large-upload-retry-windows' &&
        fastPath.guardrails.includes('queue-headroom-stays-planning-evidence-only') &&
        fastPath.gateProofs.skip.includes('measured queue headroom can size the next bounded large-upload retry window') &&
        fastPath.gateProofs.recovery.includes('durable chunk receipts and the guarded publish record still classify pause, retry, or crash'),
    ),
    'measured queue headroom can size large-upload retry windows without weakening publish or recovery gates',
  );
  assert.ok(
    model.safeFastPaths.some(
      (fastPath) =>
  ...
  assert.ok(
    ...
    'compressed budget summaries can size plugin-update retry windows without widening the visibility boundary',
  );
  assert.ok(
    model.safeFastPaths.some(
      (fastPath) =>
  ...
  assert.ok(
    model.safeFastPaths.some(
      (fastPath) =>
        fastPath.area === 'compression' &&
        fastPath.allowedShortcut === 'compress-canonical-per-kind-budget-summaries-and-reuse-planned-dependency-graph-to-size-bounded-plugin-install-fanout' &&
        fastPath.guardrails.includes('planned-dependency-graph-stays-planning-evidence-only') &&
        fastPath.guardrails.includes('plugin-install-fanout-revalidates-before-write') &&
        fastPath.gateProofs.skip.includes('planned dependency graph can shorten plugin-install fanout planning') &&
        fastPath.gateProofs.recovery.includes('plugin-install staging record'),
    ),
    'compressed budget summaries can reuse the planned dependency graph for plugin-install fanout without widening the visibility boundary',
  );
  assert.ok(
    model.safeFastPaths.some(
      (fastPath) =>
        fastPath.area === 'compression' &&
        fastPath.allowedShortcut === 'compress-canonical-per-kind-budget-summaries-and-reuse-planned-dependency-graph-to-size-bounded-plugin-update-fanout' &&
        fastPath.guardrails.includes('planned-dependency-graph-stays-planning-evidence-only') &&
        fastPath.guardrails.includes('plugin-update-fanout-revalidates-before-write') &&
        fastPath.gateProofs.skip.includes('planned dependency graph can shorten plugin-update fanout planning') &&
        fastPath.gateProofs.recovery.includes('plugin-update staging record'),
    ),
    'compressed budget summaries can reuse the planned dependency graph for plugin-update fanout without widening the visibility boundary',
  );
  assert.ok(
    model.safeFastPaths.some(
      (fastPath) =>
  ...
  assert.ok(
    model.rejectedFastPaths.find((fastPath) => fastPath.id === 'compressed-receipt-log-skips-raw-order-replay')?.violates.includes('durable-progress'),
  );
  assert.equal(
  ...
  assert.equal(
    model.rejectedFastPaths.find((fastPath) => fastPath.id === 'compressed-remote-index-and-cached-source-identity-skips-live-revalidation-after-pause')?.rejectedGate,
    'recovery',
  );
  assert.equal(
  ...
  assert.equal(
    model.rejectedFastPaths.find((fastPath) => fastPath.id === 'cached-receipt-cursor-and-queue-budget-match-skips-backpressure-replay-after-retry')?.rejectedGate,
    'recovery',
  );
  assert.ok(
    model.rejectedFastPaths.find((fastPath) => fastPath.id === 'cached-receipt-cursor-and-queue-budget-match-skips-backpressure-replay-after-retry')?.violates.includes('backpressure'),
    'queue-budget matches still cannot bypass the backpressure replay boundary after a retry',
  );
  assert.ok(
    model.rejectedFastPaths.find((fastPath) => fastPath.id === 'cached-receipt-cursor-and-queue-budget-match-skips-backpressure-replay-after-retry')?.violates.includes('durable-progress'),
    'queue-budget matches still cannot bypass the durable replay trail after a retry',
  );
  assert.equal(
    model.rejectedFastPaths.find((fastPath) => fastPath.id === 'cached-receipt-cursor-queue-budget-memory-ceiling-and-queue-headroom-skips-backpressure-pause-after-retry')?.rejectedGate,
    'recovery',
  );
  assert.ok(
    model.rejectedFastPaths.find((fastPath) => fastPath.id === 'cached-receipt-cursor-queue-budget-memory-ceiling-and-queue-headroom-skips-backpressure-pause-after-retry')?.violates.includes('backpressure'),
    'queue budget, memory ceiling, and queue headroom still cannot bypass the backpressure pause boundary',
  );
  assert.ok(
    model.rejectedFastPaths.find((fastPath) => fastPath.id === 'cached-receipt-cursor-queue-budget-memory-ceiling-and-queue-headroom-skips-backpressure-pause-after-retry')?.violates.includes('chunk-receipts'),
    'queue budget, memory ceiling, and queue headroom still cannot replace durable chunk-receipt ordering after a retry',
  );
  assert.ok(
    model.rejectedFastPaths.find((fastPath) => fastPath.id === 'cached-receipt-cursor-queue-budget-memory-ceiling-and-queue-headroom-skips-backpressure-pause-after-retry')?.violates.includes('durable-progress'),
  ...
  assert.ok(
    model.rejectedFastPaths.find((fastPath) => fastPath.id === 'cached-receipt-cursor-queue-headroom-and-journal-lag-skips-backpressure-pause-after-retry')?.violates.includes('backpressure'),
    'queue headroom and journal lag still cannot bypass the backpressure pause boundary',
  );
  assert.equal(
    model.rejectedFastPaths.find((fastPath) => fastPath.id === 'cached-receipt-cursor-queue-slack-and-journal-lag-skips-backpressure-pause-after-retry')?.rejectedGate,
    'recovery',
  );
  assert.ok(
  ...
  assert.equal(
    model.rejectedFastPaths.find((fastPath) => fastPath.id === 'compressed-remote-index-and-cached-file-hash-skips-large-upload-window-sizing-after-pause')?.rejectedGate,
    'recovery',
  );
  assert.ok(
    model.rejectedFastPaths.find((fastPath) => fastPath.id === 'compressed-remote-index-and-cached-file-hash-skips-large-upload-window-sizing-after-pause')?.violates.includes('file-hashing'),
    'cached file hashes still cannot bypass large-upload window sizing after a pause',
  );
  assert.ok(
    model.rejectedFastPaths.find((fastPath) => fastPath.id === 'compressed-remote-index-and-cached-file-hash-skips-large-upload-window-sizing-after-pause')?.violates.includes('backpressure'),
    'cached file hashes still cannot bypass backpressure-aware window sizing after a pause',
  );
  assert.equal(
  ...
  assert.ok(
    model.rejectedFastPaths.find((fastPath) => fastPath.id === 'compressed-remote-index-and-cached-row-receipts-skips-plugin-update-batch-parallelism-after-pause')?.violates.includes('parallelism-limits'),
    'cached row receipts still cannot bypass bounded row-batch parallelism',
  );
  assert.ok(
    model.rejectedFastPaths.find((fastPath) => fastPath.id === 'compressed-remote-index-and-cached-row-receipts-skips-plugin-update-batch-parallelism-after-pause')?.violates.includes('atomic-groups'),
  ...
  assert.equal(
    model.rejectedFastPaths.find((fastPath) => fastPath.id === 'compressed-remote-index-and-cached-row-receipts-skips-release-bundle-commit-after-pause')?.rejectedGate,
    'group',
  );
  assert.ok(
    model.rejectedFastPaths.find((fastPath) => fastPath.id === 'compressed-remote-index-and-cached-row-receipts-skips-release-bundle-commit-after-pause')?.violates.includes('atomic-groups'),
  ...
  assert.ok(
    model.rejectedFastPaths.find((fastPath) => fastPath.id === 'compressed-remote-index-and-cached-source-identity-skips-live-revalidation-after-pause')?.violates.includes('remote-index-planning-only'),
  );
  assert.ok(
    model.rejectedFastPaths.find((fastPath) => fastPath.id === 'compressed-remote-index-and-cached-source-identity-skips-live-revalidation-after-pause')?.violates.includes('live-preconditions'),
  );
  assert.ok(
    model.rejectedFastPaths.find((fastPath) => fastPath.id === 'compressed-remote-index-and-cached-source-identity-skips-live-revalidation-after-pause')?.violates.includes('atomic-file-publish'),
  );
  assert.equal(
  ...
  assert.equal(
    model.rejectedFastPaths.find((fastPath) => fastPath.id === 'compressed-remote-index-and-cached-file-hash-skips-plugin-install-and-large-upload-finalize-after-pause')?.rejectedGate,
    'recovery',
  );
  assert.ok(
    model.rejectedFastPaths.find((fastPath) => fastPath.id === 'compressed-remote-index-and-cached-file-hash-skips-plugin-install-and-large-upload-finalize-after-pause')?.violates.includes('remote-index-planning-only'),
  );
  assert.ok(
    model.rejectedFastPaths.find((fastPath) => fastPath.id === 'compressed-remote-index-and-cached-file-hash-skips-plugin-install-and-large-upload-finalize-after-pause')?.violates.includes('compression'),
  );
  assert.ok(
    model.rejectedFastPaths.find((fastPath) => fastPath.id === 'compressed-remote-index-and-cached-file-hash-skips-plugin-install-and-large-upload-finalize-after-pause')?.violates.includes('file-hashing'),
  );
  assert.ok(
    model.rejectedFastPaths.find((fastPath) => fastPath.id === 'compressed-remote-index-and-cached-file-hash-skips-plugin-install-and-large-upload-finalize-after-pause')?.violates.includes('chunk-receipts'),
  );
  assert.ok(
    model.rejectedFastPaths.find((fastPath) => fastPath.id === 'compressed-remote-index-and-cached-file-hash-skips-plugin-install-and-large-upload-finalize-after-pause')?.violates.includes('row-preconditions'),
  );
  assert.ok(
    model.rejectedFastPaths.find((fastPath) => fastPath.id === 'compressed-remote-index-and-cached-file-hash-skips-plugin-install-and-large-upload-finalize-after-pause')?.violates.includes('plugin-preconditions'),
  );
  assert.ok(
    model.rejectedFastPaths.find((fastPath) => fastPath.id === 'compressed-remote-index-and-cached-file-hash-skips-plugin-install-and-large-upload-finalize-after-pause')?.violates.includes('atomic-groups'),
  );
  assert.ok(
    model.rejectedFastPaths.find((fastPath) => fastPath.id === 'compressed-remote-index-and-cached-file-hash-skips-plugin-install-and-large-upload-finalize-after-pause')?.violates.includes('atomic-file-publish'),
  );
  assert.ok(
    model.rejectedFastPaths.find((fastPath) => fastPath.id === 'compressed-remote-index-and-cached-file-hash-skips-plugin-install-and-large-upload-finalize-after-pause')?.violates.includes('durable-progress'),
  );
  assert.ok(
  ...
  assert.equal(
    model.rejectedFastPaths.find((fastPath) => fastPath.id === 'compressed-remote-index-and-cached-file-hash-skips-large-upload-publish-after-pause-and-backpressure')?.rejectedGate,
    'recovery',
  );
  assert.ok(
    model.rejectedFastPaths.find((fastPath) => fastPath.id === 'compressed-remote-index-and-cached-file-hash-skips-large-upload-publish-after-pause-and-backpressure')?.violates.includes('file-hashing'),
  );
  assert.ok(
    model.rejectedFastPaths.find((fastPath) => fastPath.id === 'compressed-remote-index-and-cached-file-hash-skips-large-upload-publish-after-pause-and-backpressure')?.violates.includes('backpressure'),
  );
  assert.ok(
  ...
  assert.equal(
    model.rejectedFastPaths.find((fastPath) => fastPath.id === 'compressed-remote-index-and-cached-file-hash-skips-large-upload-chunk-hash-backpressure-after-pause')?.rejectedGate,
    'recovery',
  );
  assert.equal(
    model.rejectedFastPaths.find((fastPath) => fastPath.id === 'compressed-remote-index-and-cached-file-fingerprint-skips-large-upload-chunk-hash-backpressure-after-pause')?.rejectedGate,
    'recovery',
  );
  assert.ok(
    model.rejectedFastPaths.find((fastPath) => fastPath.id === 'compressed-remote-index-and-cached-file-fingerprint-skips-large-upload-chunk-hash-backpressure-after-pause')?.violates.includes('file-hashing'),
    'cached file fingerprints still cannot bypass chunk-hash backpressure after a pause',
  );
  assert.ok(
    model.rejectedFastPaths.find((fastPath) => fastPath.id === 'compressed-remote-index-and-cached-file-fingerprint-skips-large-upload-chunk-hash-backpressure-after-pause')?.violates.includes('backpressure'),
    'cached file fingerprints still cannot bypass backpressure after a pause',
  );
  assert.equal(
    model.rejectedFastPaths.find((fastPath) => fastPath.id === 'compressed-remote-index-and-cached-file-hash-skips-plugin-update-writeback-after-pause')?.rejectedGate,
    'group',
  );
  assert.ok(
    model.rejectedFastPaths.find((fastPath) => fastPath.id === 'compressed-remote-index-and-cached-file-hash-skips-plugin-update-writeback-after-pause')?.violates.includes('plugin-preconditions'),
    'cached file hashes still cannot bypass plugin update preconditions',
  );
  assert.ok(
    model.rejectedFastPaths.find((fastPath) => fastPath.id === 'compressed-remote-index-and-cached-file-hash-skips-plugin-update-writeback-after-pause')?.violates.includes('atomic-groups'),
    'cached file hashes still cannot bypass the plugin update atomic-group barrier',
  );
  assert.ok(
    model.rejectedFastPaths.find((fastPath) => fastPath.id === 'compressed-remote-index-and-cached-file-hash-skips-plugin-update-writeback-after-pause')?.violates.includes('backpressure'),
    'cached file hashes still cannot bypass backpressure recovery evidence',
  );
  assert.ok(
  ...
  assert.ok(
    model.rejectedFastPaths.find((fastPath) => fastPath.id === 'compressed-remote-index-and-cached-file-hash-skips-large-upload-chunk-hash-backpressure-after-pause')?.violates.includes('backpressure'),
  );
  assert.ok(
    model.rejectedFastPaths.find((fastPath) => fastPath.id === 'compressed-remote-index-and-cached-file-hash-skips-large-upload-chunk-hash-backpressure-after-pause')?.violates.includes('chunk-receipts'),
  );
  assert.ok(
  ...
  assert.ok(
    model.safeFastPaths.find((fastPath) => fastPath.allowedShortcut === 'reuse-remote-index-cursor-and-dependency-graph-to-presize-bounded-plugin-install-batches')?.gateProofs.skip.includes('remote index cursor and dependency graph'),
    'plugin-install batch planning may reuse the cursor and dependency graph only as planning evidence',
  );
  assert.ok(
    model.safeFastPaths.find((fastPath) => fastPath.allowedShortcut === 'reuse-remote-index-cursor-and-dependency-graph-to-presize-bounded-plugin-install-batches')?.gateProofs.live.includes('live compare'),
    'plugin-install batch planning keeps the live row compare in place',
  );
  assert.equal(
...
test('unsafe shortcuts stay rejected when they would bypass live preconditions or atomic groups', () => {
  ...
  const skipBudgetMemoryAndHeadroomPause = model.rejectedFastPaths.find(
    (fastPath) =>
  ...
  assert.ok(skipBudgetMemoryAndHeadroomPause, 'queue budget, memory ceiling, and queue headroom cannot skip the backpressure pause after a retry');
  assert.equal(skipBudgetMemoryAndHeadroomPause.rejectedGate, 'recovery');
...
test('rejected fast paths cover precondition bypasses and atomic group splits', () => {
  ...
  assert.equal(
    rejectedById.get('compressed-remote-index-and-cached-row-batch-receipts-skips-plugin-update-commit-after-pause').rejectedGate,
    'group',
  );
  assert.ok(
  ...
  assert.equal(
    rejectedById.get('compressed-remote-index-and-cached-row-receipts-skips-plugin-update-row-preconditions-after-pause').rejectedGate,
    'group',
  );
  assert.ok(
    rejectedById.get('compressed-remote-index-and-cached-row-receipts-skips-plugin-update-row-preconditions-after-pause').violates.includes('row-preconditions'),
  );
  assert.ok(
    rejectedById.get('compressed-remote-index-and-cached-row-receipts-skips-plugin-update-row-preconditions-after-pause').violates.includes('plugin-preconditions'),
  );
  assert.ok(
    rejectedById.get('compressed-remote-index-and-cached-row-receipts-skips-plugin-update-row-preconditions-after-pause').violates.includes('atomic-groups'),
  );
  assert.equal(
  ...
  assert.equal(
    rejectedById.get('compressed-remote-index-and-cached-row-batch-receipts-skips-plugin-install-final-activation-after-pause').rejectedGate,
    'group',
  );
  assert.ok(rejectedById.get('compressed-remote-index-and-cached-row-batch-receipts-skips-plugin-install-final-activation-after-pause').violates.includes('backpressure'));
  assert.ok(rejectedById.get('compressed-remote-index-and-cached-row-batch-receipts-skips-plugin-install-final-activation-after-pause').violates.includes('row-preconditions'));
  assert.ok(rejectedById.get('compressed-remote-index-and-cached-row-batch-receipts-skips-plugin-install-final-activation-after-pause').violates.includes('plugin-preconditions'));
  assert.ok(rejectedById.get('compressed-remote-index-and-cached-row-batch-receipts-skips-plugin-install-final-activation-after-pause').violates.includes('atomic-groups'));
  ...
  assert.equal(
    rejectedById.get('compressed-remote-index-and-cached-termmeta-summary-skips-plugin-update-finalize-after-pause').rejectedGate,
    'group',
  );
  assert.ok(
    rejectedById.get('compressed-remote-index-and-cached-termmeta-summary-skips-plugin-update-finalize-after-pause').violates.includes('row-preconditions'),
  );
  assert.ok(
    rejectedById.get('compressed-remote-index-and-cached-termmeta-summary-skips-plugin-update-finalize-after-pause').violates.includes('plugin-preconditions'),
  );
  assert.ok(
    rejectedById.get('compressed-remote-index-and-cached-termmeta-summary-skips-plugin-update-finalize-after-pause').violates.includes('atomic-groups'),
  );
  ...
  assert.equal(
    rejectedById.get('compressed-remote-index-and-batched-receipt-flush-skips-plugin-install-finalize-after-pause').rejectedGate,
    'group',
  );
  assert.ok(
    rejectedById.get('compressed-remote-index-and-batched-receipt-flush-skips-plugin-install-finalize-after-pause').violates.includes('backpressure'),
  );
  assert.ok(
    rejectedById.get('compressed-remote-index-and-batched-receipt-flush-skips-plugin-install-finalize-after-pause').violates.includes('atomic-groups'),
  );
  assert.ok(
  ...
  assert.equal(
    rejectedById.get('compressed-remote-index-and-cached-dependency-graph-skips-plugin-install-activation-after-pause-and-backpressure').rejectedGate,
    'group',
  );
  assert.ok(
    rejectedById.get('compressed-remote-index-and-cached-dependency-graph-skips-plugin-install-activation-after-pause-and-backpressure').violates.includes('backpressure'),
  );
  assert.ok(
    rejectedById.get('compressed-remote-index-and-cached-dependency-graph-skips-plugin-install-activation-after-pause-and-backpressure').violates.includes('atomic-groups'),
  );
  ...
  assert.equal(
    rejectedById.get('compressed-remote-index-and-cached-row-batch-receipts-skips-plugin-install-finalize-after-pause').rejectedGate,
    'group',
  );
  assert.ok(
    rejectedById.get('compressed-remote-index-and-cached-row-batch-receipts-skips-plugin-install-finalize-after-pause').violates.includes('backpressure'),
  );
  assert.ok(
    rejectedById.get('compressed-remote-index-and-cached-row-batch-receipts-skips-plugin-install-finalize-after-pause').violates.includes('row-preconditions'),
  );
  assert.ok(
    rejectedById.get('compressed-remote-index-and-cached-row-batch-receipts-skips-plugin-install-finalize-after-pause').violates.includes('plugin-preconditions'),
  );
  assert.ok(
    rejectedById.get('compressed-remote-index-and-cached-row-batch-receipts-skips-plugin-install-finalize-after-pause').violates.includes('atomic-groups'),
  );
  assert.ok(
    rejectedById.get('compressed-remote-index-and-cached-row-batch-receipts-skips-plugin-install-finalize-after-pause').violates.includes('durable-progress'),
  );
  ...
  assert.equal(
    rejectedById.get('compressed-remote-index-and-cached-upload-buffer-skips-large-upload-windowing-after-pause').rejectedGate,
    'recovery',
  );
  assert.ok(
  ...
  assert.ok(
    rejectedById.get('compressed-remote-index-and-cached-upload-buffer-skips-large-upload-windowing-after-pause').violates.includes('chunk-receipts'),
  );
  assert.ok(
  ...
  assert.equal(
    rejectedById.get('compressed-remote-index-and-cached-chunk-digests-skips-large-upload-backpressure-after-pause').rejectedGate,
    'recovery',
  );
  assert.ok(
    rejectedById.get('compressed-remote-index-and-cached-chunk-digests-skips-large-upload-backpressure-after-pause').violates.includes('chunk-receipts'),
  );
  assert.ok(
    rejectedById.get('compressed-remote-index-and-cached-chunk-digests-skips-large-upload-backpressure-after-pause').violates.includes('backpressure'),
  );
  assert.equal(
  ...
  assert.ok(
    rejectedById.get('compressed-remote-index-and-parallel-chunk-sends-skips-backpressure').violates.includes('remote-index-planning-only'),
  );
  assert.ok(
    rejectedById.get('compressed-remote-index-and-parallel-chunk-sends-skips-backpressure').violates.includes('compression'),
  );
  assert.ok(
    rejectedById.get('compressed-remote-index-and-parallel-chunk-sends-skips-backpressure').violates.includes('atomic-file-publish'),
  );
  assert.equal(
    rejectedById.get('compressed-remote-index-and-parallel-chunk-sends-skips-large-upload-backpressure-after-pause').rejectedGate,
    'recovery',
  );
  assert.ok(
    rejectedById.get('compressed-remote-index-and-parallel-chunk-sends-skips-large-upload-backpressure-after-pause').violates.includes('parallelism-limits'),
  );
  assert.ok(
  ...
  assert.equal(
    rejectedById.get('compressed-remote-index-and-parallel-chunk-and-row-fanout-skips-large-upload-and-plugin-update-recovery-after-pause').rejectedGate,
    'recovery',
  );
  assert.ok(
    rejectedById.get('compressed-remote-index-and-parallel-chunk-and-row-fanout-skips-large-upload-and-plugin-update-recovery-after-pause').violates.includes('chunk-receipts'),
  );
  assert.ok(
    rejectedById.get('compressed-remote-index-and-parallel-chunk-and-row-fanout-skips-large-upload-and-plugin-update-recovery-after-pause').violates.includes('row-preconditions'),
  );
  assert.ok(
  ...
  assert.ok(
    rejectedById.get('cached-dependency-graph-and-remote-index-cursor-skips-plugin-update-row-batch-revalidation-after-pause').violates.includes('database-row-batching'),
  );
  assert.ok(
    rejectedById.get('cached-dependency-graph-and-remote-index-cursor-skips-plugin-update-row-batch-revalidation-after-pause').violates.includes('atomic-groups'),
  );
  assert.equal(
  ...
  assert.equal(compressedLargeUploadWindowing.area, 'chunk-upload');
  ...
  assert.equal(compressedLargeUploadWindowing.publishesStagedDataEarly, false);
  assert.ok(compressedLargeUploadWindowing.guardrails.includes('compressed-index-remains-planning-evidence-only'));
  ...
  const compressedLargeUploadRetryWindowing = model.safeFastPaths.find(
    (fastPath) => fastPath.allowedShortcut === 'compress-remote-index-listings-and-reuse-chunk-receipts-to-size-bounded-large-upload-retry-windows',
  );
  assert.ok(compressedLargeUploadRetryWindowing, 'compressed large-upload retry windowing fast path exists');
  assert.equal(compressedLargeUploadRetryWindowing.area, 'chunk-upload');
  assert.equal(compressedLargeUploadRetryWindowing.bypassesLivePreconditions, false);
  assert.equal(compressedLargeUploadRetryWindowing.splitsAtomicGroup, false);
  assert.equal(compressedLargeUploadRetryWindowing.publishesStagedDataEarly, false);
  assert.ok(compressedLargeUploadRetryWindowing.guardrails.includes('chunk-receipts-stay-durable-and-plan-scoped'));
  assert.ok(compressedLargeUploadRetryWindowing.gateProofs.skip.includes('cached chunk receipts size the next bounded retry window'));
  assert.ok(compressedLargeUploadRetryWindowing.gateProofs.live.includes('live remote resource hash'));
  assert.ok(compressedLargeUploadRetryWindowing.gateProofs.recovery.includes('durable chunk receipts'));
  ...
  assert.equal(
    rejectedById.get('compressed-remote-index-and-drained-queue-skips-plugin-install-finalize-after-pause').rejectedGate,
    'recovery',
  );
  assert.ok(
    rejectedById.get('compressed-remote-index-and-drained-queue-skips-plugin-install-finalize-after-pause').violates.includes('backpressure'),
  );
  assert.ok(
    rejectedById.get('compressed-remote-index-and-drained-queue-skips-plugin-install-finalize-after-pause').violates.includes('plugin-preconditions'),
  );
  assert.ok(
    rejectedById.get('compressed-remote-index-and-drained-queue-skips-plugin-install-finalize-after-pause').violates.includes('atomic-groups'),
  );
  assert.ok(
...
test('production throughput details fail closed when receipt-cursor memory-headroom visibility disappears from the pause boundary', () => {
  ...
  report.evidence.backpressure.receiptCursorMemoryHeadroomVisible = false;
  const details = productionThroughputDetails(report);
  const blockers = productionThroughputBlockers(report);
...
test('production throughput details fail closed when receipt-cursor queue-slack visibility disappears from the pause boundary', () => {
  const report = runGuardedExecutorBenchmark({ profile: 'ci' });

  report.evidence.backpressure.receiptCursorQueueSlackVisible = false;
  const details = productionThroughputDetails(report);
  const blockers = productionThroughputBlockers(report);
...
test('production throughput details fail closed when aligned queue-slack proof disappears from the pause boundary', () => {
  const report = runGuardedExecutorBenchmark({ profile: 'ci' });

  report.evidence.backpressure.queuePauseHasMeasuredAndAlignedReceiptCursorQueueSlack = false;
  const details = productionThroughputDetails(report);
  const blockers = productionThroughputBlockers(report);
