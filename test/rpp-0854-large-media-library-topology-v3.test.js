...
const evidencePath = path.join(
  repoRoot,
  'docs/evidence/rpp-0854-large-media-library-topology-v3.md',
);
const fixedNow = '2026-06-01T00:00:00.000Z';
const variant = 'RPP-0854-variant-3';
const proofId = 'rpp-0854-large-media-library-topology-v3';
...
test('RPP-0854 support report records fail-closed large media topology variant 3 scope', () => {
  const { report, text } = loadSupportReport();
  const externalTopology = buildExternalTopologyVariant3SupportSummary();

  assert.equal(report.schemaVersion, 1);
  assert.equal(report.rppId, 'RPP-0854');
  assert.equal(report.variant, 3);
  assert.equal(report.title, 'Large media library topology variant 3 support scope');
  ...
  assert.equal(report.builtOn.largeMediaTopologyVariant2.rppId, 'RPP-0834');
  assert.equal(report.builtOn.largeMediaTopologyVariant2.variant, 2);
  ...
  assert.equal(report.builtOn.externalTopologyVariant3.rppId, 'RPP-0843');
  assert.equal(report.builtOn.externalTopologyVariant3.variant, 3);
  assert.equal(report.builtOn.externalTopologyVariant3.sourceLocalChangedUrlCapture, true);
  assert.equal(report.builtOn.externalTopologyVariant3.roleIdentityHashOnly, true);
  assert.deepEqual(report.builtOn.externalTopologyVariant3.routeSourceIdentityChecks, [...routeOrder]);
  assert.equal(report.builtOn.externalTopologyVariant3.identitySurfaceCount, 10);
  assert.equal(report.builtOn.externalTopologyVariant3.redactedHashCountSurfaceOnly, true);
  assert.equal(report.builtOn.externalTopologyVariant3.networkProbePerformed, false);
  ...
  assert.equal(report.supportReport.variantFocus, 'large-media-real-wordpress-import-export-survival-contract-v3');
  ...
  assert.equal(report.candidateScope.status, 'large-media-library-topology-v3-support-candidate');
  ...
  assert.equal(shape.topologyVariant, 'external-wordpress-topology-v3');
  ...
  assert.equal(shape.identitySurfaceCount, 10);
  assert.deepEqual(shape.identitySurfaceNames, externalTopology.identitySurfaceNames);
  assert.equal(shape.externalTopologyV3ScopeHash, externalTopology.scopeHash);
  assert.equal(shape.externalTopologyV3RoleIdentityDigest, externalTopology.roleIdentityDigest);
  assert.equal(shape.externalTopologyV3RouteIdentityDigest, externalTopology.routeIdentityDigest);
  assert.equal(shape.externalTopologyV3SurfaceDigest, externalTopology.surfaceDigest);
  ...
  assert.equal(survival.runtimeRequired, 'real-wordpress-import-export');
  assert.equal(survival.productionBackedRequired, true);
  assert.equal(survival.largeMediaRequired, true);
  assert.equal(survival.externalTopologyVariantRequired, 'external-wordpress-topology-v3');
  assert.equal(survival.pluginDriver, pluginDriver);
  assert.equal(survival.pluginOwner, pluginOwner);
  assert.equal(survival.pluginResourceKeyHash, pluginResourceKeyHash);
  assert.deepEqual(survival.requiredGraphTypes, [...requiredGraphTypes]);
  assert.ok(report.candidateScope.acceptanceRules.includes('external-topology-v3-role-identities-hash-only'));
  ...
  assert.ok(report.candidateScope.excludedFromCandidate.includes('production-bound-large-media-import-export'));
  ...
  assert.ok(report.releaseReadyScope.blockers.includes('external-wordpress-live-topology-v3-not-configured'));
...
test('RPP-0854 records exact unavailable topology capabilities and stays NO-GO', () => {
  ...
  assert.equal(
    ...
    'complete-external-wordpress-topology-v3-with-auth',
  );
...
test('RPP-0854 accepts only production-backed real WordPress large media survival proof', () => {
  const proof = evaluateLargeMediaLibraryTopologyV3(successfulRealLargeMediaImportExportArtifact());
  ...
  assert.equal(proof.topology.topologyVariant, 'external-wordpress-topology-v3');
  ...
  assert.doesNotThrow(() => assertEvidenceHasNoRawValues(proof, {
    label: 'RPP-0854 accepted contract fixture',
  }));
});

test('RPP-0854 rejects missing, playground, non-production, and partial survival evidence', () => {
  const missingProof = evaluateLargeMediaLibraryTopologyV3(null);
  ...
  observed.topology.topologyVariant = 'external-wordpress-topology-v2';
  ...
  const proof = evaluateLargeMediaLibraryTopologyV3(observed);
  ...
  assert.ok(proof.failures.some((failure) =>
    failure.code === 'EXTERNAL_TOPOLOGY_V3_LIVE_READBACK_REQUIRED'));
  assert.ok(proof.failures.some((failure) =>
...
test('RPP-0854 evidence remains hash/count/surface only and scoped to support', () => {
  const { report, text } = loadSupportReport();

  assert.deepEqual(findEvidenceRedactionIssues(report), []);
  assert.doesNotThrow(() =>
    assertEvidenceHasNoRawValues(report, { label: 'RPP-0854 large media topology support report' }));
...
function loadSupportReport() {
  ...
  assert.ok(match?.groups?.json, 'RPP-0854 evidence must contain one JSON support report block');
  return {
    text,
...
function buildExternalTopologyVariant3SupportSummary() {
  ...
  const core = {
    proofScope: 'external-wordpress-topology-v3-static-identity-support',
    roleIdentityHashes,
  ...
  return {
    ...
    roleIdentityDigest: `sha256:${digest(roleIdentityHashes)}`,
...
function buildMissingDockerCapabilityArtifact() {
  const plan = buildDockerTopologyPlan({
    ...
    workDir: '/tmp/rpp-0854-docker-work',
    evidenceDir: '/tmp/rpp-0854-docker-evidence',
    env: {},
  });
...
function evaluateLargeMediaLibraryTopologyV3(observed) {
...
function validateObservedImportExportSurvival(observed) {
  ...
  if (observed.runtime !== 'real-wordpress-import-export' || importExport.realWordPress !== true) {
    failures.push({
      ...
      reason: 'RPP-0854 requires real WordPress import/export, not local Playground or synthetic substitute evidence.',
    });
  }
  if (importExport.productionBacked !== true) {
    failures.push({
      code: 'PRODUCTION_BACKED_ARTIFACT_REQUIRED',
      reason: 'RPP-0854 variant 3 remains support-only until production-backed proof is supplied.',
    });
  }
  if (
    topology.largeMediaLibrary !== true
    || topology.topologyVariant !== 'external-wordpress-topology-v3'
    ...
    failures.push({
      code: 'EXTERNAL_TOPOLOGY_V3_LIVE_READBACK_REQUIRED',
      reason: 'The artifact must prove external topology v3 role identities and live route receipts by hash.',
    });
  }
  if (
...
function topologyArtifactHash(artifact, probe) {
  return `sha256:${digest({
    ...
    variant: 'external-wordpress-topology-v3',
...
function sampleHash(label) {
  return digest({ rpp: 'RPP-0854', label });
}
