...
const checkedRecoveryPath = 'artifact://rpp-0660-manual-recovery-audit-export-v3/checked-recovery-path';
...
function fixtureHash(label) {
  return sha256Hex(`rpp-0660:${label}`);
}
...
function baseSite() {
  return {
    files: {
      'rpp-0660-alpha.txt': 'rpp-0660-base-private-alpha',
      'rpp-0660-bravo.txt': 'rpp-0660-base-private-bravo',
      'rpp-0660-charlie.txt': 'rpp-0660-base-private-charlie',
      'rpp-0660-delta.txt': 'rpp-0660-base-private-delta',
    },
    plugins: {},
    db: {},
  };
...
function localSite(base = baseSite()) {
  ...
  site.files['rpp-0660-alpha.txt'] = 'rpp-0660-local-private-alpha';
  site.files['rpp-0660-bravo.txt'] = 'rpp-0660-local-private-bravo';
  site.files['rpp-0660-charlie.txt'] = 'rpp-0660-local-private-charlie';
  site.files['rpp-0660-delta.txt'] = 'rpp-0660-local-private-delta';
  return site;
}

function rawSiteValuesFor(...sites) {
  const values = new Set([
    'rpp-0660-base-private-alpha',
    'rpp-0660-base-private-bravo',
    'rpp-0660-base-private-charlie',
    'rpp-0660-base-private-delta',
    'rpp-0660-local-private-alpha',
    'rpp-0660-local-private-bravo',
    'rpp-0660-local-private-charlie',
    'rpp-0660-local-private-delta',
  ]);
...
function assertNoRawValues(value, rawValues) {
  ...
  for (const rawValue of rawValues) {
    assert.equal(
      ...
      false,
      `RPP-0660 hash-only evidence leaked raw fixture value: ${rawValue}`,
    );
  }
}
...
function fullyUpdatedRecoveryFromPlan(plan) {
  ...
  return {
    source: 'RPP-0660 generated release recovery inspect',
...
function buildGeneratedManualAudit({ plan, recovery }) {
  return buildManualRecoveryAuditExport({
    ...
    source: {
      kind: 'rpp-0660-generated-release-recovery-inspect',
    ...
    artifactRefs: {
      recoveryAudit: 'artifact://rpp-0660-manual-recovery-audit-export-v3',
      checkedRecoveryPath,
    },
  });
}
...
function buildDurableJournal() {
  const activeClaimId = 'rpp-0660-active-claim';
  const activeClaimKeyHash = fixtureHash('active-claim-key');
  const previousClaimId = 'rpp-0660-previous-claim';
...
function buildRecoveryReleaseSummary({
  ...
  const oldRemoteRecovery = {
    source: 'RPP-0660 generated stale-owner retry before mutation',
    status: 200,
    state: 'old-remote',
  ...
  return {
    ...
    releaseProof: {
      ...
      idempotencyConflict: {
        ...
        recoveryState: {
          source: 'RPP-0660 generated different-body conflict recovery state',
          storage: 'sqlite',
          state: 'fully-updated-remote',
          restartReadable: true,
          counts: {
...
test('RPP-0660 manual recovery audit export variant 3 proves GATE-2 on the same release verifier path', () => {
  const {
    plan,
    recovery,
    audit,
...
test('RPP-0660 generated manual recovery audit export rejects malformed variant 3 evidence', () => {
  const {
    plan,
    recovery,
    audit,
