...
test('production recovery journal descriptor fails closed on non-enumerable nested artifact refs', () => {
  const writer = {
    kind: 'production-recovery-journal',
    productionAdapter: true,
    supportedSurface: 'production-recovery-journal-adapter',
  ...
  Object.defineProperty(writer.artifactRefs, 'remote', {
    value: '/var/lib/reprint/recovery-remote.jsonl',
    enumerable: false,
  });
...
test('production recovery journal descriptor fails closed on unsupported nested artifact refs keys', () => {
  const writer = {
    kind: 'production-recovery-journal',
    productionAdapter: true,
    supportedSurface: 'production-recovery-journal-adapter',
    ...
    artifactRefs: {
      journal: '/var/lib/reprint/recovery.jsonl',
      remote: '/var/lib/reprint/recovery-remote.jsonl',
      extra: '/var/lib/reprint/recovery-extra.jsonl',
    },
    schemaVersion: 1,
  };
...
test('production recovery journal adapter fails closed when stale claim advancement omits its lease epoch on reopen', () => {
  const filePath = tempJournalPath();
  const remote = baseSite();
  const plan = planFor(baseSite(), localSite(), remote);
  const remoteArtifactPath = `${filePath}.remote`;
...
test('production recovery journal consumption fails closed when claimId hides behind a non-enumerable compatibility-overload key', () => {
  const filePath = tempJournalPath();
  const remote = baseSite();
  const plan = planFor(baseSite(), localSite(), remote);
  const claimId = 'claim-hidden-consume-claim';
  const artifactRefs = {
    journal: filePath,
  };
  const journal = openProductionRecoveryJournal({
  ...
  Object.defineProperty(consumeOptions, 'claimId', {
    value: claimId,
    enumerable: false,
  });
...
test('production recovery journal consumption fails closed when writerLease hides behind a non-enumerable compatibility-overload key', () => {
  const filePath = tempJournalPath();
  const remote = baseSite();
  const plan = planFor(baseSite(), localSite(), remote);
  const claimId = 'claim-hidden-consume-writer';
  const artifactRefs = {
    journal: filePath,
  };
  const writerLease = { id: claimId };
  const journal = openProductionRecoveryJournal({
    filePath,
    plan,
    current: remote,
  ...
  Object.defineProperty(consumeOptions, 'writerLease', {
    value: writerLease,
    enumerable: false,
  });
...
test('production recovery journal consumption fails closed when compatibility-overload options use a null prototype', () => {
  const filePath = tempJournalPath();
  const remote = baseSite();
  const plan = planFor(baseSite(), localSite(), remote);
  const claimId = 'claim-consume-null-prototype-options';
  const artifactRefs = {
    journal: filePath,
  };
  const journal = openProductionRecoveryJournal({
  ...
  assert.throws(() => {
    consumeProductionRecoveryJournal(Object.assign(Object.create(null), {
      filePath,
      plan,
      current: remote,
      artifactRefs,
    ...
    name: 'UnsupportedProductionRecoveryJournalError',
    code: 'UNSUPPORTED_PRODUCTION_RECOVERY_JOURNAL',
    message: 'Production recovery journal compatibility overload requires a strict plain options object.',
  });
});

test('production recovery journal consumption fails closed when compatibility-overload options inherit a custom prototype', () => {
  const filePath = tempJournalPath();
  const remote = baseSite();
  const plan = planFor(baseSite(), localSite(), remote);
  const claimId = 'claim-consume-prototype-options';
  const artifactRefs = {
    journal: filePath,
  };
  const journal = openProductionRecoveryJournal({
  ...
  const consumeOptions = Object.create({ inherited: true });
  consumeOptions.filePath = filePath;
  consumeOptions.plan = plan;
  consumeOptions.current = remote;
  consumeOptions.artifactRefs = artifactRefs;
  consumeOptions.claimId = claimId;
...
test('production recovery journal consumption derives claim identity from the fenced writer lease when claimId is omitted', () => {
  ...
  const consumedRecord = persisted.records.at(-1);
  assert.equal(consumedRecord.type, 'recovery-journal-consumed');
  assert.equal(consumedRecord.claimId, claimId);
...
test('production recovery journal reopen fails closed when the persisted consumed claim omits its claim id', () => {
  const filePath = tempJournalPath();
  const remote = baseSite();
  const plan = planFor(baseSite(), localSite(), remote);
  const claimId = 'claim-consumed-missing-id';
  const writerLease = { id: claimId, epoch: 4 };
  const remoteArtifactPath = `${filePath}.remote`;
  const artifactRefs = {
    journal: filePath,
...
test('production recovery journal reopen fails closed when the persisted consumed claim id diverges from its fenced lease identity', () => {
  const filePath = tempJournalPath();
  const remote = baseSite();
  const plan = planFor(baseSite(), localSite(), remote);
  const claimId = 'claim-consumed-id-drift';
  const writerLease = { id: claimId, epoch: 4 };
  const remoteArtifactPath = `${filePath}.remote`;
  const artifactRefs = {
    journal: filePath,
  ...
  const persisted = readRecoveryJournal(filePath);
  const consumedRecord = persisted.records.at(-1);
  consumedRecord.claimId = `${claimId}-drifted`;
  fs.writeFileSync(filePath, `${persisted.records.map((record) => JSON.stringify(record)).join('\n')}\n`);
...
test('production recovery journal reopen fails closed when the persisted consumed claim lease omits its optional claim key hash', () => {
  const filePath = tempJournalPath();
  const remote = baseSite();
  const plan = planFor(baseSite(), localSite(), remote);
  const claimId = 'claim-consumed-lease-claim-key-hash-omitted';
  const writerLease = { id: claimId, epoch: 4, claimKeyHash: 'a'.repeat(64) };
  const remoteArtifactPath = `${filePath}.remote`;
  const artifactRefs = {
    journal: filePath,
    remote: remoteArtifactPath,
  ...
  const persisted = readRecoveryJournal(filePath);
  const consumedRecord = persisted.records.at(-1);
  consumedRecord.claimLease = { id: claimId, epoch: 4 };
  fs.writeFileSync(filePath, `${persisted.records.map((record) => JSON.stringify(record)).join('\n')}\n`);
...
test('production recovery journal reopen fails closed when the persisted consumed claim lease nulls its optional claim key hash', () => {
  const filePath = tempJournalPath();
  const remote = baseSite();
  const plan = planFor(baseSite(), localSite(), remote);
  const claimId = 'claim-consumed-lease-claim-key-hash-null';
  const writerLease = { id: claimId, epoch: 4, claimKeyHash: 'a'.repeat(64) };
  const remoteArtifactPath = `${filePath}.remote`;
  const artifactRefs = {
    journal: filePath,
  ...
  const persisted = readRecoveryJournal(filePath);
  const consumedRecord = persisted.records.at(-1);
  consumedRecord.claimLease = { id: claimId, epoch: 4, claimKeyHash: null };
  fs.writeFileSync(filePath, `${persisted.records.map((record) => JSON.stringify(record)).join('\n')}\n`);
...
test('production recovery journal compatibility overload fails closed when artifact refs are explicitly undefined', () => {
  const filePath = tempJournalPath();
  const remote = baseSite();
  const plan = planFor(baseSite(), localSite(), remote);
  const claimId = 'claim-undefined-compatibility-artifact-ref';
  const remoteArtifactPath = `${filePath}.remote`;
  const artifactRefs = {
    journal: filePath,
    remote: remoteArtifactPath,
  ...
  assert.throws(() => {
    openProductionRecoveryJournal({
      ...
      plan,
      current: remote,
      claimId: 'claim-open-undefined-remote-ref',
      artifactRefs: {
        journal: filePath,
        remote: undefined,
      },
  ...
  assert.throws(() => {
    ...
    name: 'UnsupportedProductionRecoveryJournalError',
    code: 'UNSUPPORTED_PRODUCTION_RECOVERY_JOURNAL',
    message: 'Production recovery journal compatibility overload requires artifactRefs.journal to match the owned journal path.',
  });
...
test('production recovery journal reopen fails closed when a later persisted record sets the owned remote artifact ref to undefined', () => {
  const filePath = tempJournalPath();
  const remote = baseSite();
  const plan = planFor(baseSite(), localSite(), remote);
  const remoteArtifactPath = `${filePath}.remote`;
  const claimId = 'claim-late-undefined-remote-ref';
  const artifactRefs = {
    journal: filePath,
    remote: remoteArtifactPath,
  };
...
test('production recovery journal consumption fails closed when a later persisted record sets the owned remote artifact ref to undefined', () => {
  const filePath = tempJournalPath();
  const remote = baseSite();
  const plan = planFor(baseSite(), localSite(), remote);
  const remoteArtifactPath = `${filePath}.remote`;
  const claimId = 'claim-consume-late-undefined-remote-ref';
  const artifactRefs = {
    journal: filePath,
    remote: remoteArtifactPath,
  };
...
test('production recovery journal reopen fails closed when a later persisted record sets the owned journal artifact ref to an empty string', () => {
  const filePath = tempJournalPath();
  const remote = baseSite();
  const plan = planFor(baseSite(), localSite(), remote);
  const claimId = 'claim-late-undefined-journal-ref';
  const artifactRefs = {
    journal: filePath,
  };
  const journal = openProductionRecoveryJournal({
...
test('production recovery journal consumption fails closed when a later persisted record sets the owned journal artifact ref to an empty string', () => {
  const filePath = tempJournalPath();
  const remote = baseSite();
  const plan = planFor(baseSite(), localSite(), remote);
  const claimId = 'claim-consume-late-undefined-journal-ref';
  const artifactRefs = {
    journal: filePath,
  };
  const journal = openProductionRecoveryJournal({
...
test('production recovery journal consumption fails closed when a later persisted record rewrites the owned journal artifact path', () => {
  const filePath = tempJournalPath();
  const remote = baseSite();
  const plan = planFor(baseSite(), localSite(), remote);
  const claimId = 'claim-consume-late-rewritten-journal-ref';
  const artifactRefs = {
    journal: filePath,
  };
  const journal = openProductionRecoveryJournal({
