...
const evidencePath = path.join(repoRoot, 'docs/evidence/rpp-0980-go-no-go-release-decision-record-v4.md');
const fixedNowIso = '2026-06-01T04:50:00.000Z';
const auditedHeadBeforeEvidence = 'a53973083198269d2e4f653b423a0681a0c471ee';
...
test('RPP-0980 v4 evidence proves support-only NO-GO and names every remaining risk', () => {
  ...
  assert.match(evidenceText, /^# RPP-0980 go\/no-go release decision record v4 evidence$/m);
  ...
  assert.equal(evidence.rppId, 'RPP-0980');
  assert.equal(evidence.proofId, 'rpp-0980-go-no-go-release-decision-record-v4');
  ...
  assert.equal(evidence.auditedBranch, 'session/rpp-980');
  ...
  assert.equal(evidence.evidencePath, 'docs/evidence/rpp-0980-go-no-go-release-decision-record-v4.md');
...
test('RPP-0980 v4 risk register matches the current final release gate evaluator', () => {
...
test('RPP-0980 v4 closes no risk without production-backed proof and moves no release gate status', () => {
  const decision = loadMarkdownJson(decisionPath).report;
  const evidence = loadMarkdownJson(evidencePath).report;
...
test('RPP-0980 v4 artifacts remain redacted and do not authorize release movement', async () => {
  ...
  const scan = await scanArtifacts([
    ...
    'docs/evidence/rpp-0980-go-no-go-release-decision-record-v4.md',
  ], { cwd: repoRoot });

  assert.doesNotThrow(() =>
    assertEvidenceHasNoRawValues(decision.report, { label: 'RPP-0980 go/no-go decision record' }));
  assert.doesNotThrow(() =>
    assertEvidenceHasNoRawValues(evidence.report, { label: 'RPP-0980 go/no-go evidence' }));
