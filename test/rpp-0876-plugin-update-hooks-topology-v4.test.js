...
const evidencePath = path.join(
  repoRoot,
  'docs/evidence/rpp-0876-plugin-update-hooks-topology-v4.md',
);
...
const updateHookRuntimeRequirements = Object.freeze([
...
test('RPP-0876 records focused plugin update hook topology variant 4 support evidence', () => {
  const { report, text } = loadProgressReport();
  const validation = validatePluginUpdateTopologyVariant4Report(report);

  assert.equal(validation.ok, true, JSON.stringify(validation.failures));
  assert.equal(report.schemaVersion, 1);
  assert.equal(report.rppId, 'RPP-0876');
  ...
  assert.equal(report.candidateScope.status, 'plugin-update-hooks-topology-candidate-v4');
  ...
  assert.ok(report.candidateScope.candidateClaims.includes('variant-4-startup-or-exact-capability-contract-recorded'));
...
test('RPP-0876 topology command starts sites or records exact unavailable capability', () => {
  ...
  assert.deepEqual(report.topologyCommand.exactUnavailableCapability, {
    ...
    requiredFor: updateHookRuntimeRequirements,
  });
  assert.equal(
...
test('RPP-0876 release movement stays NO-GO until live update hook proof passes', () => {
  const { report } = loadProgressReport();
...
test('RPP-0876 validation rejects ambiguous non-started topology reports', () => {
  const { report } = loadProgressReport();
  const ambiguous = {
    ...report,
    topologyCommand: {
  ...
  const validation = validatePluginUpdateTopologyVariant4Report(ambiguous);
...
test('RPP-0876 validation rejects packaged fallback or release movement claims', () => {
  const { report } = loadProgressReport();
  const widened = {
    ...report,
    ...
    topologyCommand: {
      ...
      policy: {
        ...report.topologyCommand.policy,
        packagedFallbackAllowed: true,
        packagedFallbackObserved: true,
      },
    },
  };
  const validation = validatePluginUpdateTopologyVariant4Report(widened);

  assert.equal(validation.ok, false);
  assert.ok(validation.failures.some((failure) =>
    failure.code === 'PLUGIN_UPDATE_TOPOLOGY_PACKAGED_FALLBACK_NOT_DISABLED'));
  assert.ok(validation.failures.some((failure) =>
    failure.code === 'PLUGIN_UPDATE_TOPOLOGY_RELEASE_POSTURE_NOT_NO_GO'));
});

test('RPP-0876 evidence remains hash/count/surface only', () => {
  const { report, text } = loadProgressReport();

  assert.deepEqual(findEvidenceRedactionIssues(report), []);
  assert.doesNotThrow(() =>
    assertEvidenceHasNoRawValues(report, { label: 'RPP-0876 plugin update topology progress report' }));
...
function loadProgressReport() {
  ...
  assert.ok(match?.groups?.json, 'RPP-0876 evidence must contain one JSON progress report block');
  return {
    text,
...
function buildMissingDockerCapabilityArtifact() {
  const plan = buildDockerTopologyPlan({
    ...
    workDir: '/tmp/rpp-0876-docker-work',
    evidenceDir: '/tmp/rpp-0876-docker-evidence',
    env: {},
  });
...
function validatePluginUpdateTopologyVariant4Report(report) {
  const failures = [];
  const topologyCommand = report?.topologyCommand || {};
  const capability = topologyCommand.exactUnavailableCapability || {};
  const hookRequirements = report?.candidateScope?.hookTopologyRequirements || {};
  ...
  if (topologyCommand.sitesStarted !== true
    && (topologyCommand.failClosed !== true || topologyCommand.acceptedForReleaseGate !== false)) {
    failures.push({
      code: 'PLUGIN_UPDATE_TOPOLOGY_NON_STARTED_REPORT_NOT_FAIL_CLOSED',
    });
  }

  if (topologyCommand.sitesStarted !== true && capability.code !== 'DOCKER_CLI_MISSING') {
    failures.push({
      code: 'PLUGIN_UPDATE_TOPOLOGY_UNEXPECTED_CAPABILITY_CODE',
    });
  }

  if (topologyCommand.policy?.packagedFallbackAllowed !== false
    || topologyCommand.policy?.packagedFallbackObserved !== false) {
    failures.push({
  ...
  if (topologyCommand.policy?.onlySandbox8080Ingress !== true
    || topologyCommand.policy?.tunnelCommandCount !== 0) {
    failures.push({
      code: 'PLUGIN_UPDATE_TOPOLOGY_8080_INGRESS_NOT_ENFORCED',
    });
  }

  if (hookRequirements.candidateHookCount !== pluginUpdateHookSurface.length
    || !Array.isArray(hookRequirements.hookSurface)
    || hookRequirements.hookSurface.length !== pluginUpdateHookSurface.length) {
    failures.push({
      code: 'PLUGIN_UPDATE_TOPOLOGY_HOOK_SURFACE_INCOMPLETE',
    });
  }

  if (report?.finalReleaseStatus !== 'NO-GO'
    || report?.integrationRecommendation !== 'NO-GO'
    || topologyCommand.releaseMovementAllowed !== false) {
    failures.push({
      code: 'PLUGIN_UPDATE_TOPOLOGY_RELEASE_POSTURE_NOT_NO_GO',
    });
  }

  if (report?.redaction?.format !== 'hash-count-surface-only'
    || hookRequirements.rawHookPayloadsIncluded !== false) {
    failures.push({
      code: 'PLUGIN_UPDATE_TOPOLOGY_EVIDENCE_NOT_SURFACE_ONLY',
    });
  }
