...
const proofId = 'rpp-0735-large-media-library-benchmark-v2';
...
const expectedFastPathLaneUpdates = 9;
const expectedFastPathLaneBlocked = 4;
const expectedRowPreconditions = 52;
const expectedLaneRowPreconditions = 36;
...
test('RPP-0735 variant 2 proves large media storage performance gates and support-only NO-GO', {
  concurrency: false,
}, () => {
  const proof = buildVariant2Proof();

  assert.equal(proof.rppId, 'RPP-0735');
  ...
  assert.equal(proof.resources.storage.appliedFsyncIncompleteMediaWrites, 1);
  ...
  assert.equal(proof.resources.database.metadataRowsPreconditioned, 39);
  ...
  assert.equal(proof.resources.database.batchCount, 11);
  ...
  assert.equal(proof.fastPathLaneEvidence.sampleOutcomeCounts.applied, 9);
  assert.equal(proof.fastPathLaneEvidence.sampleOutcomeCounts['stale-at-write'], 2);
  assert.equal(proof.fastPathLaneEvidence.sampleOutcomeCounts['fsync-failed-before-rename'], 1);
...
test('RPP-0735 variant 2 fails closed before fast-path lane output on unsafe evidence', () => {
  ...
  mismatchedLaneEvidenceHash.fastPathLaneEvidence.laneEvidenceHash = sha256(
    'rpp-0735-mismatched-lane-evidence-hash',
  );
...
function buildVariant2Proof() {
  ...
  const proofGates = [
    ...
    proofGate('unsafe-large-media-evidence-fails-closed', Object.values(unsafe).every((decision) => (
  ...
  const publicProof = {
    ...
    redaction: {
      mode: 'hash-only-storage-and-row-evidence',
...
function buildLargeMediaLibraryEvidence({ benchmark }) {
  return {
    ...
    correctnessGates: [],
    fastPathLaneEvidence: collectFastPathLaneEvidence(benchmark),
    runtime: {
      generatedAt: benchmark.runtime.generatedAt,
      durationMs: benchmark.runtime.durationMs,
      budgets: benchmark.runtime.budgets,
...
function collectFastPathLaneEvidence(benchmark) {
  ...
  const core = {
    evidenceMode: 'support-only-hash-only-storage-performance',
