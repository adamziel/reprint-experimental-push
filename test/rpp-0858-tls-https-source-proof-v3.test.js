...
const evidencePath = path.join(
  repoRoot,
  'docs/evidence/rpp-0858-tls-https-source-proof-v3.md',
);

const fixedNow = new Date('2026-06-01T00:00:00.000Z');
const proofId = 'rpp-0858-tls-https-source-proof-v3';
...
const goodEnv = Object.freeze({
  REPRINT_PUSH_SOURCE_URL: 'https://source-v3.example.test/variant-three-source',
  REPRINT_PUSH_REMOTE_URL: 'https://source-v3.example.test:443/variant-three-source/',
  REPRINT_PUSH_LOCAL_URL: 'https://local-v3.example.test/variant-three-local',
  REPRINT_PUSH_REMOTE_CHANGED_URL: 'https://changed-v3.example.test/variant-three-changed',
  REPRINT_PUSH_PREFLIGHT_SOURCE_URL: 'https://source-v3.example.test/variant-three-source/',
  REPRINT_PUSH_DRY_RUN_ROUTE_SOURCE_URL: 'https://source-v3.example.test:443/variant-three-source',
  REPRINT_PUSH_APPLY_SOURCE_URL: 'https://source-v3.example.test/variant-three-source',
  REPRINT_PUSH_JOURNAL_ROUTE_SOURCE_URL: 'https://source-v3.example.test/variant-three-source',
  REPRINT_PUSH_RECOVERY_INSPECT_SOURCE_URL: 'https://source-v3.example.test/variant-three-source',
  REPRINT_PUSH_USERNAME: 'rpp-0858-admin',
  REPRINT_PUSH_APPLICATION_PASSWORD: 'rpp-0858-application-password-must-not-leak',
});

const forbiddenProofNeedles = Object.freeze([
  'https://source-v3.example.test',
  'https://local-v3.example.test',
  'https://changed-v3.example.test',
  'source-v3.example.test',
  'local-v3.example.test',
  'changed-v3.example.test',
  'variant-three-source',
  'variant-three-local',
  'variant-three-changed',
  'rpp-0858-admin',
  'rpp-0858-application-password-must-not-leak',
]);

test('RPP-0858 captures source/local/changed HTTPS URLs and identity-checks variant 3 roles', () => {
  const proof = buildTlsHttpsSourceProof({ env: goodEnv });

  assert.equal(proof.schemaVersion, 1);
  assert.equal(proof.rppId, 'RPP-0858');
  ...
  assert.equal(proof.patternLineage.patternOnlyReference.rppId, 'RPP-0838');
  assert.equal(proof.patternLineage.patternOnlyReference.variant, 2);
  assert.equal(proof.patternLineage.patternOnlyReference.reusesProofArtifact, false);

  assert.equal(proof.tlsHttpsSource.scope, 'tls-https-source-proof-v3');
  ...
  assert.doesNotThrow(() => assertEvidenceHasNoRawValues(proof, { label: 'RPP-0858 TLS HTTPS source proof v3' }));
});

test('RPP-0858 rejects tunnel URLs and secret-shaped URL parts before accepting TLS source scope', () => {
  const proof = buildTlsHttpsSourceProof({
    env: {
      REPRINT_PUSH_SOURCE_URL: 'https://admin:rpp0858-secret@source-v3.example.test/variant-three-source',
      REPRINT_PUSH_LOCAL_URL: 'https://local-v3.example.test/secret/rpp0858-token',
      REPRINT_PUSH_REMOTE_CHANGED_URL: 'https://changed-v3.ngrok-free.app/variant-three-changed',
  ...
  assert.ok(proof.failures.some((failure) => failure.code === 'RPP_0858_SECRET_SHAPED_URL_PART_REJECTED'
    && failure.role === 'source'
  ...
  assertNoNeedles(proof, [
    'admin:rpp0858-secret',
    'rpp0858-secret',
    'secret/rpp0858-token',
    'rpp0858-token',
    'changed-v3.ngrok-free.app',
  ]);
  assertNoRawUrls(proof);
});

test('RPP-0858 rejects non-HTTPS role URLs before TLS scope acceptance', () => {
  const proof = buildTlsHttpsSourceProof({
    env: {
      ...goodEnv,
      REPRINT_PUSH_SOURCE_URL: 'http://source-v3.example.test/variant-three-source',
      REPRINT_PUSH_REMOTE_URL: 'http://source-v3.example.test/variant-three-source',
      REPRINT_PUSH_PREFLIGHT_SOURCE_URL: 'http://source-v3.example.test/variant-three-source',
      REPRINT_PUSH_DRY_RUN_ROUTE_SOURCE_URL: 'http://source-v3.example.test/variant-three-source',
      REPRINT_PUSH_APPLY_SOURCE_URL: 'http://source-v3.example.test/variant-three-source',
      REPRINT_PUSH_JOURNAL_ROUTE_SOURCE_URL: 'http://source-v3.example.test/variant-three-source',
      REPRINT_PUSH_RECOVERY_INSPECT_SOURCE_URL: 'http://source-v3.example.test/variant-three-source',
    },
  });
  ...
  assert.ok(proof.failures.some((failure) =>
    failure.code === 'RPP_0858_REQUIRED_ROLE_URL_MUST_BE_HTTPS'
      && failure.role === 'source'));
  assertNoRawUrls(proof);
});

test('RPP-0858 TLS HTTPS source proof v3 is deterministic and hash/count/surface-only', () => {
...
test('RPP-0858 evidence document records the same NO-GO hash/count/surface-only scope', () => {
  const { report, text } = loadEvidenceReport();

  assert.equal(report.schemaVersion, 1);
  assert.equal(report.rppId, 'RPP-0858');
  ...
  assert.doesNotMatch(text, /source-v3\.example|local-v3\.example|changed-v3\.example|ngrok|token=|admin:/i);
});
...
function buildTlsHttpsSourceProof({ env, now = fixedNow } = {}) {
  ...
  const tlsFailures = roleUrlEvidence
    ...
    .map((entry) => ({
      code: 'RPP_0858_REQUIRED_ROLE_URL_MUST_BE_HTTPS',
      role: entry.role,
      envKey: entry.envKey,
    }));
  ...
  const proofCore = {
    ...
    tlsHttpsSource: {
      scope: 'tls-https-source-proof-v3',
...
function findSecretShapedUrlPartFailures(env = {}) {
  ...
  for (const definition of urlDefinitionsForSecretPartScan(env)) {
    ...
    for (const partSurface of partSurfaces) {
      failures.push({
        code: 'RPP_0858_SECRET_SHAPED_URL_PART_REJECTED',
...
function loadEvidenceReport() {
  ...
  assert.ok(match?.groups?.json, 'RPP-0858 evidence must contain one JSON report block');
  return {
    text,
