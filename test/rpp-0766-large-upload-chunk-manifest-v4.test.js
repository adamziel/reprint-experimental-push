...
const proofId = 'rpp-0766-large-upload-chunk-manifest-v4';
...
test('RPP-0766 variant 4 proves large upload chunk manifest benchmark gates', {
  concurrency: false,
}, () => {
  const proof = buildVariant4Proof();

  assert.equal(proof.rppId, 'RPP-0766');
  assert.equal(proof.proofId, proofId);
  assert.equal(proof.variant, 4);
  assert.equal(proof.status, 'passed');
  assert.equal(proof.builtOn.rppId, 'RPP-0746');
  assert.equal(proof.builtOn.proofId, 'rpp-0746-large-upload-chunk-manifest-v3');
...
test('RPP-0766 variant 4 fails closed for incomplete, mismatched, stale, and premature manifest evidence', {
  concurrency: false,
}, () => {
...
function buildVariant4Proof() {
  ...
  const publicProof = {
    ...
    builtOn: {
      rppId: 'RPP-0746',
      proofId: 'rpp-0746-large-upload-chunk-manifest-v3',
      variant: 3,
      status: 'passed',
...
function buildRecordedEvidencePair() {
  ...
  const benchmark = runGuardedExecutorBenchmark({
    ...benchmarkOptions,
    now: fixedNow,
    tempDir: tempBenchmarkDir('reprint-rpp-0766-manifest-v4-'),
  });
  const repeatedBenchmark = runGuardedExecutorBenchmark({
    ...benchmarkOptions,
    now: fixedNow,
    tempDir: tempBenchmarkDir('reprint-rpp-0766-manifest-v4-repeat-'),
  });
...
function runBenchmarkCommandReport() {
  const stdout = execFileSync(process.execPath, [
    ...
    `--temp-dir=${tempBenchmarkDir('reprint-rpp-0766-manifest-v4-cli-')}`,
  ], {
    cwd: repoRoot,
    encoding: 'utf8',
    maxBuffer: 16 * MIB,
