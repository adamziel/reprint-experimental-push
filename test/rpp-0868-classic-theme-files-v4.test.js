...
const evidencePath = path.join(
  repoRoot,
  'docs/evidence/rpp-0868-classic-theme-files-v4.md',
);

const fixedNow = new Date('2026-06-01T00:00:00.000Z');
const proofId = 'rpp-0868-classic-theme-files-v4';
const themeSlug = 'rpp-0868-classic-v4';
...
const classicThemeSurfaceFiles = Object.freeze([
  ...
  Object.freeze({ path: `${themeRoot}/assets/classic-v4.css`, surface: 'asset', kind: 'classic-theme-static-asset', required: false }),
...
const goodEnv = Object.freeze({
  REPRINT_PUSH_SOURCE_URL: 'https://source.example.test/classic-theme-v4',
  REPRINT_PUSH_REMOTE_URL: 'https://source.example.test:443/classic-theme-v4/',
  REPRINT_PUSH_LOCAL_URL: 'https://local.example.test/classic-theme-v4',
  REPRINT_PUSH_REMOTE_CHANGED_URL: 'https://changed.example.test/classic-theme-v4',
  REPRINT_PUSH_PREFLIGHT_SOURCE_URL: 'https://source.example.test/classic-theme-v4/',
  REPRINT_PUSH_DRY_RUN_ROUTE_SOURCE_URL: 'https://source.example.test:443/classic-theme-v4',
  REPRINT_PUSH_APPLY_SOURCE_URL: 'https://source.example.test/classic-theme-v4',
  REPRINT_PUSH_JOURNAL_ROUTE_SOURCE_URL: 'https://source.example.test/classic-theme-v4',
  REPRINT_PUSH_RECOVERY_INSPECT_SOURCE_URL: 'https://source.example.test/classic-theme-v4',
  REPRINT_PUSH_USERNAME: 'rpp-0868-admin',
  REPRINT_PUSH_APPLICATION_PASSWORD: 'rpp-0868-application-password-must-not-leak',
});

const badEnv = Object.freeze({
  REPRINT_PUSH_SOURCE_URL: 'https://admin:rpp0868-secret@source.example.test/classic-theme-v4',
  REPRINT_PUSH_LOCAL_URL: 'https://local.example.test/classic-theme-v4?token=rpp0868-token',
  REPRINT_PUSH_REMOTE_CHANGED_URL: 'https://changed.ngrok-free.app/classic-theme-v4',
  REPRINT_PUSH_PACKAGE_SMOKE_MODE: 'driver-guard-only',
});

const duplicateEnv = Object.freeze({
  ...goodEnv,
  REPRINT_PUSH_LOCAL_URL: 'https://source.example.test:443/classic-theme-v4/',
});
...
const forbiddenProofNeedles = Object.freeze([
  ...
  'rpp-0868-admin',
  'rpp-0868-application-password-must-not-leak',
  'rpp-0868-source-style-private',
  'rpp-0868-local-style-private',
  'rpp-0868-local-functions-private',
  'rpp-0868-local-front-page-private',
  'rpp-0868-remote-style-private',
]);

if (process.env.RPP_0868_REPORT_JSON !== '1') {
  test('RPP-0868 captures source/local/changed role URLs and identity-checks classic theme file scope', () => {
    const proof = buildClassicThemeFilesV4Proof({ env: goodEnv });

    assert.equal(proof.schemaVersion, 1);
    assert.equal(proof.rppId, 'RPP-0868');
    ...
    assert.deepEqual(proof.classicTheme.localChangedFileKeys, [
      `file:${themeRoot}/assets/classic-v4.css`,
    ...
    assert.doesNotThrow(() => assertEvidenceHasNoRawValues(proof, {
      label: 'RPP-0868 classic theme files v4 proof',
    }));
  });

  test('RPP-0868 rejects tunnel and secret-shaped role URLs before accepting classic theme scope', () => {
    const proof = buildClassicThemeFilesV4Proof({ env: badEnv });
  ...
  test('RPP-0868 rejects duplicate source/local/changed role URL identities before accepting classic theme scope', () => {
    const proof = buildClassicThemeFilesV4Proof({ env: duplicateEnv });
  ...
  test('RPP-0868 classic theme files proof is deterministic and hash/count/surface-only', () => {
    const firstProof = buildClassicThemeFilesV4Proof({ env: goodEnv });
    const secondProof = buildClassicThemeFilesV4Proof({ env: { ...goodEnv } });
  ...
  test('RPP-0868 evidence document records the same NO-GO hash/count/surface scope', () => {
    const { report, text } = loadEvidenceReport();
    const expectedReport = buildEvidenceReport();

    assert.equal(report.schemaVersion, 1);
    assert.equal(report.rppId, 'RPP-0868');
...
function buildClassicThemeFilesV4Proof({ env, now = fixedNow } = {}) {
...
function buildEvidenceReport() {
  const proof = buildClassicThemeFilesV4Proof({ env: goodEnv });
  const negativeProof = buildClassicThemeFilesV4Proof({ env: badEnv });
  const duplicateProof = buildClassicThemeFilesV4Proof({ env: duplicateEnv });
  const reportCore = {
    schemaVersion: 1,
    rppId: 'RPP-0868',
    variant: 4,
...
function classicThemeSite(role) {
  const files = {
    [`${themeRoot}/style.css`]: [
      '/*',
      'Theme Name: RPP 0868 Classic V4',
      'Template: none',
      'Version: 4.0.0',
      '*/',
      'body { --rpp-source-marker: "rpp-0868-source-style-private"; }',
    ].join('\n'),
    [`${themeRoot}/functions.php`]: '<?php function rpp_0868_classic_source() { return "source"; }',
    [`${themeRoot}/index.php`]: '<?php echo "RPP 0868 source index";',
    [`${themeRoot}/page.php`]: '<?php echo "RPP 0868 source page";',
    [`${themeRoot}/header.php`]: '<?php ?><header>RPP 0868 source header</header>',
    [`${themeRoot}/footer.php`]: '<?php ?><footer>RPP 0868 source footer</footer>',
    [`${themeRoot}/screenshot.png`]: 'rpp-0868-source-screenshot-bytes',
    [`${themeRoot}/assets/classic-v4.css`]: '.rpp-0868-source { display: block; }',
    [`${themeRoot}/assets/logo.svg`]: '<svg><title>RPP 0868 source logo</title></svg>',
  };
  ...
  if (role === 'localEdited') {
    files[`${themeRoot}/style.css`] = [
      ...
      'Version: 4.0.1-local',
      '*/',
      'body { --rpp-local-marker: "rpp-0868-local-style-private"; }',
    ].join('\n');
    files[`${themeRoot}/functions.php`] = '<?php function rpp_0868_classic_local() { return "rpp-0868-local-functions-private"; }';
    files[`${themeRoot}/front-page.php`] = '<?php echo "rpp-0868-local-front-page-private";';
    files[`${themeRoot}/page.php`] = '<?php echo "RPP 0868 local page";';
    files[`${themeRoot}/assets/classic-v4.css`] = '.rpp-0868-local { display: grid; }';
  }
  ...
  if (role === 'remoteChanged') {
    files[`${themeRoot}/style.css`] = [
      ...
      '*/',
      'body { --rpp-remote-marker: "rpp-0868-remote-style-private"; }',
    ].join('\n');
  }
...
function loadEvidenceReport() {
  ...
  assert.ok(match?.groups?.json, 'RPP-0868 evidence must contain one JSON report block');
  return {
    text,
