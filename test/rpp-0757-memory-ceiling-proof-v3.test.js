...
const proofId = 'rpp-0757-memory-ceiling-proof-v3';
...
test('RPP-0757 variant 3 rejects stale storage state after streamed memory-ceiling temp write', () => {
  const rootDir = createFilesystemMemoryCeilingTempRoot('reprint-rpp-0757-stale-');
  const logicalPath = 'wp-content/uploads/rpp-0757/stale/reject.txt';
  writeFixture(rootDir, logicalPath, 'rpp0757-base-stale-payload');
  ...
  const stale = applyFilesystemMemoryCeilingWrite({
    ...
    createChunk: deterministicChunkSource('rpp0757-planned-stale-payload'),
    operation: 'update',
    driver: 'rpp-0757-focused-proof',
    chunkSizeBytes,
    maxBufferedBytes,
    afterTempWrite: ({ absolutePath }) => {
      afterTempWriteObserved = true;
      assert.equal(readFixture(rootDir, logicalPath), 'rpp0757-base-stale-payload');
      assert.equal(tempNamesBesideTarget(absolutePath).length, 1);
      fs.writeFileSync(absolutePath, 'rpp0757-drift-stale-payload');
    },
  });

  assert.equal(afterTempWriteObserved, true);
  assert.equal(stale.applied, false);
  assert.equal(readFixture(rootDir, logicalPath), 'rpp0757-drift-stale-payload');
...
test('RPP-0757 variant 3 proves storage performance gates and support-only NO-GO', {
  concurrency: false,
}, () => {
  const proof = buildVariant3Proof();

  assert.equal(proof.rppId, 'RPP-0757');
  ...
  assert.equal(proof.resources.workload.createFiles, 2);
  assert.equal(proof.resources.workload.staleFiles, 2);
  assert.equal(proof.resources.storage.guardedWritesAttempted, 7);
  assert.equal(proof.resources.storage.appliedWrites, 5);
  assert.equal(proof.resources.storage.staleAtWriteWrites, 2);
  ...
  assert.equal(proof.resources.memoryCeiling.streamedWrites, 7);
  assert.equal(proof.resources.bytes.driftPreservedBytes, 2 * fileBytes);
  assert.equal(proof.resources.tempLeaks, 0);

  assert.equal(proof.guardedWrites.expectedWriteCount, 7);
  assert.equal(proof.guardedWrites.appliedSampleCount, 5);
  assert.equal(proof.guardedWrites.staleSampleCount, 2);
  assert.equal(proof.guardedWrites.staleWriteHashes.length, 2);
  ...
  assert.equal(staleSamples.length, 2);
...
test('RPP-0757 variant 3 fails closed for stale guard, memory, and premature evidence', () => {
...
function rawFilesystemEvidencePattern() {
  return /rpp0757-(?:base|planned|drift)-stale-payload|memory-(?:base|planned|drift)-payload|filesystem memory raw fixture|wp-content\/uploads|\/tmp\/|\.tmp/i;
}

function rawFilesystemTokens(rootDir = null) {
  return [
    'rpp0757-base-stale-payload',
    'rpp0757-planned-stale-payload',
    'rpp0757-drift-stale-payload',
    'memory-base-payload',
