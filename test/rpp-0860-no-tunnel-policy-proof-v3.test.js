...
const evidencePath = path.join(
  repoRoot,
  'docs/evidence/rpp-0860-no-tunnel-policy-proof-v3.md',
);
const fixedNow = '2026-06-01T00:00:00.000Z';
const proofId = 'rpp-0860-no-tunnel-policy-proof-v3';
const sourcePattern = 'RPP-0840-no-tunnel-policy-pattern';
...
test('RPP-0860 progress report records candidate versus release-ready no-tunnel scope', () => {
  const { report, text } = loadProgressReport();
  const expected = buildNoTunnelPolicyProof();

  assert.deepEqual(report, expected);
  assert.equal(report.schemaVersion, 1);
  assert.equal(report.rppId, 'RPP-0860');
...
test('RPP-0860 rejects known tunnel command and domain surfaces without invoking tools', () => {
...
test('RPP-0860 permits only local sandbox ingress semantics', () => {
  const matrix = buildNoTunnelPolicyMatrix();
...
test('RPP-0860 keeps packaged fallback disabled and release movement at NO-GO', () => {
  const matrix = buildNoTunnelPolicyMatrix();
...
test('RPP-0860 evidence remains deterministic hash/count/surface-only', () => {
  ...
  assert.doesNotThrow(() =>
    assertEvidenceHasNoRawValues(report, { label: 'RPP-0860 no-tunnel policy proof v3' }));
  ...
  for (const needle of rawEvidenceNeedles) {
    assert.equal(text.includes(needle), false, `RPP-0860 evidence leaked raw tunnel or URL payload ${needle}`);
  }
});
...
function loadProgressReport() {
  ...
  assert.ok(match?.groups?.json, 'RPP-0860 evidence must contain one JSON progress report block');
  return {
    text,
...
function buildBasePlan(options = {}) {
  return buildDockerTopologyPlan({
    ...
    workDir: '/tmp/rpp-0860-docker-work',
    evidenceDir: '/tmp/rpp-0860-docker-evidence',
    env: {},
    ...options,
  });
}
