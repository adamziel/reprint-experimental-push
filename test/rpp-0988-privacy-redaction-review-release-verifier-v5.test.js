...
const evidencePath = path.join(
  repoRoot,
  'docs/evidence/rpp-0988-privacy-redaction-review-release-verifier-v5.md',
);
const priorEvidencePath = path.join(repoRoot, 'docs/evidence/rpp-0968-privacy-redaction-review-v4.md');
const fixedNow = '2026-05-28T08:30:00.000Z';
const freshObservedAt = '2026-05-28T08:00:00.000Z';
const auditedLaneHead = '7e869edb4cb562796d915048ad6908fa9ea9bb70';
...
function loadEvidenceReport(filePath = evidencePath, label = 'RPP-0988') {
  const text = fs.readFileSync(filePath, 'utf8');
  const match = text.match(/```json\n(?<json>{[\s\S]*?})\n```/);

  assert.ok(match?.groups?.json, `${label} evidence must contain one JSON report block`);
  return {
    text,
...
function makeArtifactRoot(t, fixture) {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'rpp-988-redaction-artifact-'));
  t.after(() => {
...
function missingRequiredCheckFixture() {
  const payload = loadFixture();
  payload.evaluation.now = fixedNow;
  delete payload.observations[redactionCheckId];
...
function writeRequiredCheckFixture(t, fixture) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'rpp-988-required-checks-'));
  t.after(() => {
...
test('RPP-0988 release verifier v5 records support-only held redaction proof posture', () => {
  ...
  assert.equal(report.rppId, 'RPP-0988');
  assert.equal(report.sliceId, 'RPP-0988');
  assert.equal(report.workerId, 'rpp-988');
  assert.equal(report.variant, 5);
  assert.equal(report.auditedBranch, 'session/rpp-988');
  ...
  assert.equal(report.hashCountSurfaceOnly.missingProofCaseCount, 1);
...
test('RPP-0988 carries forward RPP-0968 v4 redaction review contract', () => {
  const { report } = loadEvidenceReport();
  const { report: priorReport } = loadEvidenceReport(priorEvidencePath, 'RPP-0968');

  assert.equal(report.carriedForwardContract.sourceRppId, 'RPP-0968');
  assert.equal(report.carriedForwardContract.sourceVariant, 4);
  assert.equal(report.carriedForwardContract.sourceEvidence, 'docs/evidence/rpp-0968-privacy-redaction-review-v4.md');
  assert.equal(report.carriedForwardContract.redactionScannerFailureCaseCount, 6);
  assert.equal(report.carriedForwardContract.requiredProofFailureOutcome, 'nonzero-release-check-report');
  assert.equal(report.carriedForwardContract.missingProofFailureOutcome, 'nonzero-release-check-report');
  assert.equal(report.carriedForwardContract.finalReleaseStatus, 'NO-GO');
  assert.equal(report.carriedForwardContract.releaseGateStatusMovement, 'none');
  assert.deepEqual(report.requiredProof, priorReport.requiredProof);
  assert.deepEqual(report.failureProofCases.map((entry) => ({
    case: entry.case,
    scannerCode: entry.scannerCode,
    requiredProofObservationStatus: entry.requiredProofObservationStatus,
    requiredReleaseCheckCode: entry.requiredReleaseCheckCode,
    expectedCiExit: entry.expectedCiExit,
    expectedReportStatus: entry.expectedReportStatus,
    expectedReleaseReady: entry.expectedReleaseReady,
    expectedFinalReleaseStatus: entry.expectedFinalReleaseStatus,
    releaseGateStatusMovement: entry.releaseGateStatusMovement,
  })), priorReport.failureProofCases.map((entry) => ({
    case: entry.case,
    scannerCode: entry.scannerCode,
    requiredProofObservationStatus: entry.requiredProofObservationStatus,
    requiredReleaseCheckCode: entry.requiredReleaseCheckCode,
  ...
  assert.deepEqual(report.surfaceHashes, priorReport.surfaceHashes);
  assert.equal(report.finalReleaseStatus, priorReport.finalReleaseStatus);
  assert.equal(report.releaseGateStatusMovement, priorReport.releaseGateStatusMovement);
});

test('RPP-0988 scanner failures become blocking required redaction proof failures', async (t) => {
...
test('RPP-0988 required release checks CLI exits nonzero for each redaction failure reason', (t) => {
  for (const [index, fixture] of redactionFailureFixtures.entries()) {
    const requiredFixture = failedRequiredCheckFixture(fixture, index);
...
test('RPP-0988 missing redaction proof blocks release reporting fail closed', (t) => {
  const requiredFixture = missingRequiredCheckFixture();
  const summary = summarizeFixture(requiredFixture);
  const problem = redactionProblem(summary);
  const result = runRequiredChecksCli(t, requiredFixture);
  const report = JSON.parse(result.stdout);
  ...
  assert.ok(problem);
  assert.equal(problem.code, 'REQUIRED_RELEASE_CHECK_OBSERVATION_MISSING');
  ...
  assert.deepEqual(report.summary.missingChecks.map((entry) => [entry.id, entry.code]), [
    [redactionCheckId, 'REQUIRED_RELEASE_CHECK_OBSERVATION_MISSING'],
  ]);
  assert.equal(redactionReport.observation.status, 'missing');
  assert.equal(redactionReport.observation.commandRecorded, false);
  assert.equal(redactionReport.observation.commandMatches, false);
  assert.equal(redactionReport.observation.artifactCount, 0);
  assert.equal(redactionReport.observation.requiredArtifactCount, requiredRedactionArtifacts.length);
});

test('RPP-0988 support-only redaction observation cannot move final release readiness', (t) => {
...
test('RPP-0988 evidence artifact remains redacted and records no gate movement', async () => {
  const evidenceArtifact = 'docs/evidence/rpp-0988-privacy-redaction-review-release-verifier-v5.md';
  const scanReport = await scanArtifacts([evidenceArtifact], { cwd: repoRoot });
  const { text, report } = loadEvidenceReport();

  assert.equal(scanReport.ok, true);
  assert.deepEqual(scanReport.rejectedFiles, []);
  assert.deepEqual(scanReport.scannedFiles, [evidenceArtifact]);
  assert.ok(scanReport.allowedHashEvidence >= 5);
  assert.equal(report.supportOnly, true);
  assert.equal(report.releaseGateStatusMovement, 'none');
  assert.equal(report.releaseGateStatusUpdateAttempted, false);
  assert.deepEqual(report.releaseGateStatusFilesChanged, []);
  assert.equal(report.missingProofCase.releaseGateStatusMovement, 'none');
  assert.equal(report.supportOnlyObservationCase.releaseGateStatusMovement, 'none');
  assert.equal(report.ciBlockingEvidence.releaseGateStatusMovement, 'none');
  assert.equal(report.releaseReadiness, 'held');
  assert.equal(report.finalReleaseStatus, 'NO-GO');
  ...
  assert.doesNotMatch(text, /\bhttps?:\/\//i);
  assert.doesNotMatch(text, /\b(?:Bearer|Basic|Set-Cookie|Cookie:|ghp_|github_pat_|sk-)/);
  assert.doesNotMatch(text, /releaseGateStatusMovement": "allowed/);
});
