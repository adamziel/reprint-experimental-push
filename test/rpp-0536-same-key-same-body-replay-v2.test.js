...
const sessionId = 'psh_01j00000000000000000536';
const credential = {
...
function fixtureHash(label) {
  return sha256Hex(`rpp-0536:${label}`);
}
...
function checkedJournal({ idempotencyKeyHash, requestHash, conflictRequestHash }) {
  ...
  return {
    ...
    claim: {
      ...
      previousClaimId: 'psh_01j00000000000000000535',
...
test('RPP-0536 v2 proves same-key same-body replay before separate different-body conflict', async () => {
  const originalFetch = global.fetch;
  const idempotencyKey = 'idem-rpp-0536-same-key-same-body-v2';
  const idempotencyKeyHash = sha256Hex(idempotencyKey);
  const receiptHash = fixtureHash('dry-run-receipt');
  const base = {
    files: {
      'wp-content/uploads/reprint-push/rpp-0536.txt': 'base',
    },
    plugins: {},
    db: {},
  };
  const local = {
    files: {
      'wp-content/uploads/reprint-push/rpp-0536.txt': 'local',
    },
    plugins: {},
    db: {},
  };
  ...
  try {
    ...
    assert.equal(summary.sameKeySameBodyReplay.requestBodyHash, applyRequest.headers['x-auth-content-hash']);
    assert.equal(summary.sameKeySameBodyReplay.applyContentHash, applyRequest.headers['x-auth-content-hash']);
    assert.equal(summary.sameKeySameBodyReplay.replayContentHash, replayRequest.headers['x-auth-content-hash']);
    ...
    const journalEvents = summary.dbJournal.latestEvents.map((entry) => entry.event);
    ...
    assert.equal(summary.dbJournal.latestEvents[4].appliedCount, 0);
    assert.equal(summary.dbJournal.latestEvents[5].appliedCount, 0);
    ...
    }, [idempotencyKey, credential.password, sessionId]);
  } finally {
