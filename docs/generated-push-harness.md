...
- local edits, remote-only edits, independent merge, same independent content,
  ...
  explicit variant-3, variant-4, and release-verifier variant-5 hash-only
  replay refusal evidence, and
  explicit atomic plugin install stack release-verifier variant-5 hash-only
  carry-through,
...
RPP-0198 adds `sameIndependentContentReleaseVerifierVariant5` coverage for the
same already-synchronized `wp_posts` surface as an explicit
release-verifier-v5 target. The deterministic summary exposes the same 10
ready cases, one in every tier, and the carry-through proof cross-checks the
target against the legacy, variant-3, and variant-4 same-independent-content
coverage. For every selected case, apply carries the ready plan while the shared
row stays on the remote hash with no mutation or live-remote precondition, and
all unplanned remote resources remain preserved. Evidence remains local
generated-model proof only and hash-only for resource keys, row hashes,
decision hashes, apply-proof hashes, unplanned-preservation proof hashes, and
release-verifier model proof hashes.
...
RPP-0191 adds `wpTermsTermmetaGraphReleaseVerifierVariant5` coverage for the
same `wp_terms` and `wp_termmeta` graph surface with an explicit
release-verifier-v5 target tag. The deterministic roster emits 20 variant-5
target cases: 10 ready term/termmeta graph creates and 10 stale non-ready term
drift cases, with two cases in every tier. The focused proof keeps the evidence
hash-only, verifies every ready case applies both graph rows with matching
live-remote preconditions, preserves unplanned remote data, and rejects stale
replay against both rows with `PRECONDITION_FAILED` before mutation. The stale
cases keep the remote-drifted term as non-ready support evidence, emit no
graph-row mutation or precondition, and refuse apply with `PLAN_NOT_READY`
before mutation. This is local generated-model support evidence only; release
posture remains `NO-GO` without production-backed release-gate validation.
...
The `pluginOwnedReferenceFieldVariant3` target coverage records explicit
contract-bound plugin row-driver references beyond fixture-scoped allowlists.
The deterministic roster emits 20 target cases: 10 ready generated
`wp_forms_contract_rows` updates and 10 blocked stale-target cases, with two
targets in every tier. Each row is owned by `forms`, uses the explicit
`forms-reference-row` driver contract, declares row schema evidence, and binds
`payload.post_id` to `wp_posts.ID` through normalized `referenceFields`. Ready
cases carry accepted contract, payload, and reference-target validation
evidence, apply the planned row with a matching live-remote precondition, and
refuse apply-time target drift with `INVALID_PLUGIN_DRIVER_REFERENCE_TARGET`.
Stale cases suppress the row mutation and block as
`stale-plugin-driver-reference-target` with
`PLUGIN_DRIVER_CONTRACT_BOUND_REFERENCE_TARGET_REMOTE_ABSENT`, then refuse
apply with `PLAN_NOT_READY` before mutation. Evidence remains hash-only for row
values, target posts, contract validation, payload validation, and refusal
details. This is local generated-model support evidence only; release posture
remains `NO-GO` without production-backed release-gate validation.
...
parent drift cases, with two cases in every tier. Ready cases map a local parent
page row to an equivalent remote parent page row, preserve that remote parent,
rewrite the child page `post_parent` to the proven remote ID, and reject stale
...
The `blogVersionBlogIdReferenceVariant3` target adds generated multisite
`wp_blog_versions.blog_id` references. The deterministic roster emits 20
...
blog, rewrite the dependent `wp_blog_versions` row key from `blog_id:<source>`
to `blog_id:<target>`, and bind the row `blog_id` payload to the same proven
remote blog ID. Stale cases drift the base blog remotely and require the
...
variant-3 target cases: 10 ready site identity-map rewrite cases and 10 stale
site drift cases, with two cases in every tier. Ready cases map a local
`wp_site` row to an equivalent remote site row, preserve that remote site, and
rewrite the dependent `wp_blogs` row's `site_id` to the proven remote ID without
changing the `blog_id:<id>` row key. Stale cases drift the base site remotely
and require the dependent blog row to fail closed as
`stale-wordpress-graph-identity` with hash-only target evidence. This remains
local generated support evidence only; final release remains `NO-GO`.
...
RPP-0197 adds `staleRemoteAfterDryRunReleaseVerifierVariant5` coverage for the
same ready-plan precondition surface as an explicit release-verifier-v5 target.
The deterministic summary exposes the same 344 ready replay-refusal cases
across all 10 tiers, and the focused carry-through proof independently
recounts the target, cross-checks it against the legacy, variant-3, and
variant-4 stale-replay targets, selects the highest-mutation-count ready case
in each tier, drifts a midpoint planned mutation after dry-run, and verifies
`PRECONDITION_FAILED` leaves the remote digest unchanged before mutation. The
evidence remains local generated-model proof only and hash-only for resource
keys, mutation/precondition hashes, refusal-detail hashes, tier counts, status
counts, and release-verifier model proof hashes.
...
RPP-0196 adds `atomicPluginInstallStackReleaseVerifierVariant5` coverage for
the same generated atomic plugin install stack with an explicit
release-verifier-v5 target tag. The deterministic roster emits 20 variant-5
target cases: 10 ready atomic stack installs and 10 non-ready
missing-dependency stacks, with two cases in every tier. The focused proof
keeps generated plugin file contents and private install option values
hash-only, verifies the ready stack installs the dependency plugin, dependent
plugin, plugin metadata, and plugin-owned option inside one atomic group,
rejects stale dependency replay with `PRECONDITION_FAILED` before mutation, and
