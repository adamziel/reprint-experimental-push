...
- local edits, remote-only edits, independent merge, same independent content,
  ...
  option changes with per-tier target counts, explicit variant-4 regression
  coverage, and redacted hash-only evidence for private serialized payloads, `wp_posts`
