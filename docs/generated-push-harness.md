...
- local edits, remote-only edits, independent merge, same independent content,
  ...
  deletion cases with per-tier ready/conflict target counts, file type-swap
...
directory deletes with and without remote descendants. Ready cases delete an
empty remote directory while preserving unrelated remote-only files and reject
stale replay before mutation; conflict cases include a remote child below the
deleted directory and refuse apply without exposing raw descendant contents in
evidence.
...
```json
{
  ...
  "featureFamilies": {
    ...
    "directory-descendant-target": 20,
