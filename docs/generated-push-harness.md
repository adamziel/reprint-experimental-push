...
The `wp-options-scalar` surface seeds a regular `wp_options` row whose
`option_value` is a scalar string or number, not plugin-owned metadata. Its
invariant is that a local scalar option update is ready only while the live
remote still matches the base row; a concurrent remote scalar value change must
stay `conflict`, refuse apply, and leave the remote unchanged.
