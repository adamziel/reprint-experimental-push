...
- Matching independent file deletes, row edits, and file type swaps can coexist
  ...
  preserved.
- Matching independent file deletes can also coexist with a live-preconditioned
  file delete and file type swap while remote-only plugin drift stays preserved.
...
- A live-preconditioned delete may coexist with matching independent delete,
...
- Live-preconditioned file and row deletes may coexist with a matching
  independent file type swap while remote-only plugin drift remains preserved.
...
- A plugin-owned delete may coexist with matching independent edits and file
  type swaps while remote-only plugin drift stays preserved and the delete keeps
  its own live remote precondition.
...
- A plugin-owned delete must stop when the owner plugin was removed remotely,
  even if matching independent deletes, edits, type swaps, and other remote-
  only plugin removals are already preserved.
...
- A local file type swap that would hide a live remote descendant must stop
  ...
  is present.
- A local delete that would hide a live remote descendant must stop as a
  `file-topology-conflict`, even when matching independent deletes, edits,
  file type swaps, and remote-only plugin drift are already preserved.
