...
- Matching independent row restores can also coexist with an unrelated deletion while remote-only plugin drift changes elsewhere; the restore stays `already-in-sync`, the deletion still needs its own live remote precondition, and the remote-only plugin drift stays preserved.
...
- Remote-only plugin changes do not weaken the live-remote precondition for an unrelated file delete, matching independent edit, or file type swap.
...
- The same boundary also holds when the mixed local changes include a live-preconditioned delete plus matching independent edit, matching row edit, and file type swap; remote-only plugin drift stays preserved and does not widen the overwrite scope.
...
- Remote-only plugin removals can also coexist with a live-preconditioned file delete, matching restore, and matching file type swap without widening the overwrite boundary.
- Remote-only plugin removals can also coexist with a live-preconditioned file delete and matching restore without widening the overwrite boundary.
