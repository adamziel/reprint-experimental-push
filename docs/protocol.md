...
the live remote through snapshot planning, dry-run receipt generation,
apply-time revalidation, recovery inspect, and journal readback.
...
that the auth/session type matches the production-shaped handshake, that the
remote snapshot hash listing stays planning-only, and that the journal
readback still contains durable apply-committed rows after apply and recovery
inspect.
...
- when the real push secret is missing, it fails with
  `REPRINT_PUSH_SECRET_REQUIRED` and reports that production push credentials
  are missing before preflight, dry-run, or apply can run.
