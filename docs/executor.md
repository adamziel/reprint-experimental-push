...
- it does not reserve remote state
- it does not replace the live hash listing for apply-time revalidation
- it cannot be replayed as write authority after the remote drifts
...
provenance, pull-to-push mapping, preflight, snapshot listing, dry-run,
apply, journal inspect, and inspect-first recovery all share the same
one-remote, one-local, one-runner shape.
