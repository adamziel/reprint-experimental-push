...
| Remote and local independently changed a resource to identical content | No mutation or precondition is produced for that resource; plan records `already-in-sync` with hash-only/redacted evidence. | `RPP-0234 already-in-sync decisions are hash-only and emit no mutations` / `node --test --test-name-pattern=RPP-0234 test/push-planner.test.js` |
