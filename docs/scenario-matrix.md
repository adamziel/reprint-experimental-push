...
| Local commentmeta changes | Plan is `blocked`; comment meta rows stay outside the supported release-candidate slice and do not become ready mutations. | `blocks a local commentmeta reference owned by a same-plan comment` |
| Local comment parent changes | Plan is `blocked`; comment rows stay outside the supported release-candidate slice and do not become ready mutations. | `blocks a local comment parent reference owned by a same-plan comment` |
| Same-plan attachment parent plus thumbnail graph | A post, its attachment child, and the thumbnail metadata that points at that attachment can be created in one ordered plan when every referenced object is created in the same plan, including when unrelated remote attachment noise exists. | `allows a local attachment parent and thumbnail graph to resolve in the same plan`, `allows a local attachment parent and thumbnail graph to resolve in the same plan even when an unrelated remote attachment exists` |
...
| Menu item parent reference owned by a revision | A `menu_item_parent` row owned by a revision stays blocked as a release-candidate surface even when it targets a same-plan post. | `blocks a local menu item parent reference owned by a revision even when it targets a same-plan post` |
...
| Same-plan termmeta reference with unrelated remote nav menu taxonomy | A termmeta row can resolve its same-plan term reference but stays blocked when that term already has a remote nav menu taxonomy row. | `blocks a local termmeta reference to a same-plan term when a remote nav menu taxonomy exists` |
