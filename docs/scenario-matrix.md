...
| Local deleted a directory, remote added a descendant under it | Plan is `conflict`; the remote-only descendant is preserved; conflict evidence names both paths without file contents. | `stops a local directory deletion that would remove a remote-only descendant` |
