# RPP-0676 different-body idempotency conflict, variant 4
...
`test/rpp-0676-different-body-idempotency-conflict-v4.test.js` creates a
...
```sh
node --check test/rpp-0676-different-body-idempotency-conflict-v4.test.js
...
node scripts/release/artifact-redaction-scan.mjs docs/evidence/rpp-0676-different-body-idempotency-conflict-v4.md
git diff --check
git diff --cached --check
```

Observed result: all listed commands exited 0. The focused RPP-0676 run
reported 2 passes / 0 failures. The adjacent RPP-0656 run reported
2 passes / 0 failures, the adjacent RPP-0616 run reported
1 pass / 0 failures, the adjacent RPP-0597 run reported
3 passes / 0 failures, and the adjacent RPP-0636 run reported
1 pass / 0 failures. The scoped artifact redaction scan returned
