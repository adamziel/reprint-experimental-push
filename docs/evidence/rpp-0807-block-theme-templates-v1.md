# RPP-0807 block theme templates v1 evidence

Date: 2026-06-01
Lane: RPP-0807 block theme templates, variant 1
Checklist item: RPP-0807 - Implement block theme templates, variant 1.

## Scope

This slice adds deterministic local support evidence for block theme template
handling on the production-topology track. It follows the adjacent RPP-0808
source/local/changed topology pattern, but records a block theme scope:
`theme.json`, `templates/*.html`, `parts/*.html`, a filesystem pattern, a style
variation, and the active theme option row keys.
...
```json
{
  ...
  "rppId": "RPP-0807",
  "variant": 1,
  "title": "Block theme templates topology support proof",
  ...
  "blockThemeTemplatesScope": {
    "themeType": "block",
    "themeRoot": "wp-content/themes/rpp-0807-block",
    "sourcePattern": "local-static-source-local-changed-snapshots",
    "requiredFiles": [
      "theme.json",
  ...
  "releaseVerifierTarget": {
    ...
    "successTarget": "verify-release-passes-without-packaged-fallback-on-production-topology",
  ...
  "scopeEvidenceHash": "28937ddcd38d096306e4b931040db065df7263816e1aa2b05cbb50181a155c1d"
}
```

## Proof Surface

`test/rpp-0807-block-theme-templates-v1.test.js` builds a local proof around
...
The block theme layer records a `wp-content/themes/rpp-0807-block/` scope with:
...
It also records a local-only `templates/front-page.html` addition as part of
the scoped mutation set. The proof stores paths, counts, hashes, planner
...
- header and footer template part areas are declared through theme metadata;
...
- remote drift on `templates/index.html` produces a fail-closed conflict before
...
verify:release` run on the topology without packaged fallback. That capability
...
```sh
node --check test/rpp-0807-block-theme-templates-v1.test.js
node --test --test-name-pattern RPP-0807 test/rpp-0807-block-theme-templates-v1.test.js
node scripts/release/artifact-redaction-scan.mjs docs/evidence/rpp-0807-block-theme-templates-v1.md
git diff --check
```

Observed local results after implementation:

- `node --check test/rpp-0807-block-theme-templates-v1.test.js`: exit 0
- RPP-0807 focused proof test: passed
...
This is deterministic local support evidence for the RPP-0807 block theme
