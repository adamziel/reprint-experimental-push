# RPP-0895 Plugin Activation Hooks Topology Variant 5

Date: 2026-06-01

## Scope

RPP-0895 records a fifth activation-hook topology proof using the same
candidate-scope pattern as RPP-0875. This is deterministic local support-only
...
```json
{
  ...
  "rppId": "RPP-0895",
  "variant": 5,
  "title": "Plugin activation hooks topology candidate scope variant 5",
  ...
  "candidateScope": {
    "status": "activation-hook-topology-candidate-v5",
    "sourcePattern": "rpp-0875-candidate-scope",
    "supportOnly": true,
    "productionBacked": false,
    "releaseEligible": false,
...
```sh
node --check test/rpp-0895-plugin-activation-hooks-topology-v5.test.js
node --test --test-name-pattern RPP-0895 test/rpp-0895-plugin-activation-hooks-topology-v5.test.js
node scripts/release/artifact-redaction-scan.mjs docs/evidence/rpp-0895-plugin-activation-hooks-topology-v5.md
git diff --check
```
