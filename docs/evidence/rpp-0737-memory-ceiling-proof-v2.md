# RPP-0737 memory ceiling proof variant 2 evidence

Evidence for RPP-0737. This slice is support-only and builds on the RPP-0717
filesystem memory-ceiling proof. Final release remains **NO-GO** because this
proof does not supply a live production service, production storage receipts,
...
`test/rpp-0737-memory-ceiling-proof-v2.test.js` exercises the existing
...
The variant 2 projection runs the RPP-0717 benchmark API with fixed local unit
inputs:
...
- guarded writes attempted: `4`
- expected applied writes: `3`
- expected stale-at-write rejections: `1`
...
- the benchmark-backed projection records one stale write, one preserved drift,
  zero unsafe rename-on-stale writes, and zero temp leaks.
...
- `node --check test/rpp-0737-memory-ceiling-proof-v2.test.js`
- `node --test --test-name-pattern RPP-0737 test/rpp-0737-memory-ceiling-proof-v2.test.js`
- `node --test test/filesystem-memory-ceiling-proof-benchmark.test.js`
- `node --test --test-name-pattern RPP-0732 test/rpp-0732-dry-run-batch-sizing-v2.test.js`
- `node scripts/release/artifact-redaction-scan.mjs docs/evidence/rpp-0737-memory-ceiling-proof-v2.md`
- `git diff --check`
- `git diff --cached --check`

Observed result after local validation:

- RPP-0737 proof test: 3 pass, 0 fail
- Adjacent RPP-0717 memory-ceiling benchmark test: 3 pass, 0 fail
