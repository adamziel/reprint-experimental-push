# RPP-0659 missing commit finalization, variant 3
...
- Added standalone generated coverage in
  `test/rpp-0659-missing-commit-finalization-v3.test.js`.
...
- Before finalization, restart inspection still classifies the target as
  ...
  `claimKeyHash`, sequence, event type, and a hash of the lease owner block.
...
```bash
node --check test/rpp-0659-missing-commit-finalization-v3.test.js
...
node scripts/release/artifact-redaction-scan.mjs docs/evidence/rpp-0659-missing-commit-finalization-v3.md
git diff --check
...
- Focused RPP-0659 test passed 1 subtest, 0 failures.
- Prior RPP-0639 missing-finalization proof passed 1 subtest, 0 failures.
- Adjacent RPP-0649 generated committed-state proof passed 3 subtests, 0
  failures.
- Reference RPP-0654 generated replay proof passed 2 subtests, 0 failures.
