# RPP-0758 timeout budget proof variant 3 evidence

Evidence for RPP-0758. This slice is local support-only generated coverage for
...
`test/rpp-0758-timeout-budget-proof-v3.test.js` runs the local guarded executor
unit profile and builds generated timeout-budget cases from the existing chunk
...
- generated timeout-budget cases cover transfer timeout after 1, 2, 3, and 4
  durable local chunk receipts;
...
- generated timeout case count: `4`
- generated timeout receipt counts: `1`, `2`, `3`, `4`
...
- generated mutation work before timeout: `0`
...
statuses, blocker identifiers, and decision hashes.
...
2. `generated-timeout-budget-cases-covered`
3. `timeout-budget-interrupts-transfer-before-apply`
4. `durable-local-receipts-before-timeout`
5. `receipt-only-resume-after-timeout`
...
work, no longer proves timeout before transfer completion, opens apply before
transfer finalization, exceeds the unit runtime budget, or clears the recorded
...
behavior, exact local receipt skips, zero duplicate mutation work, unit
runtime/resource gates, and fail-closed stale or incomplete evidence behavior.

## Redaction posture

Timeout budget evidence is hash-and-count-only. It stores receipt counts,
duplicate-work counters, runtime budgets, gate status vectors, blocker
...
- `node --check test/rpp-0758-timeout-budget-proof-v3.test.js`
...
- `node --test test/rpp-0718-timeout-budget-proof.test.js`
- `node scripts/release/artifact-redaction-scan.mjs docs/evidence/rpp-0758-timeout-budget-proof-v3.md`
- `git diff --check`

Observed result after local validation:

- syntax check: passed
- RPP-0758 proof test: 2 pass, 0 fail
