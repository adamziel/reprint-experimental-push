...
| `scripts/playground/production-snapshot-hashes-route-live-smoke.mjs` | Production-shaped live endpoint proof on sandbox-local loopback | Starts WordPress Playground on `127.0.0.1` with an ephemeral port, discovers `/reprint/v1/push/snapshot-hashes` in the REST index, checks unauthenticated, unsigned, and invalid-session failures with a payload that would fail route validation if parsed, then sends a signed production-shaped snapshot-hashes request through the authenticated client. |
...
focused route test reported 6 subtests ok. The adjacent auth/session/route
bundle reported 158 subtests ok. Checklist lint and the scoped artifact
redaction scan returned `ok: true`. The live smoke returned `ok: true`,
loopback-only exposure with `tunnel: none`, negative auth/session failures
before payload validation, no negative-case snapshot hash evidence,
`negativeMutated: false`, a `production-auth-session`, and 64- or 71-character
hash evidence for the signed snapshot-hashes request and receipt.
