...
- The conflict response also reports `status: conflict`, the conflicting request hash, and zero mutation event counts for the rejected body.
...
- The focused live endpoint test also checks negative auth/signature paths fail before route JSON payload parsing or mutation setup, then proves the same-key different-body conflict against a disposable local WordPress Playground URL.
