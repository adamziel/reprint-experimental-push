...
Date: 2026-05-28
Lane: release-gates
Primary checklist range: RPP-0001 through RPP-0026, plus RPP-0028, RPP-0030, RPP-0031, RPP-0032, RPP-0033, RPP-0034, RPP-0035, RPP-0036, RPP-0037, RPP-0038, RPP-0039, RPP-0040, RPP-0050, RPP-0051, RPP-0058, RPP-0062, RPP-0067, and RPP-0072
...
| RPP-0072 | Evidence toward variant-4 dry-run route eligibility regression now runs the release-gate CLI with every other final-release gate supplied, asserting exit `1`, exact `DRY_RUN_ROUTE_ELIGIBILITY_REQUIRED` reason/evidence, final held marker, redaction, and `mutationAttempted: false` while release remains `NO-GO`. |

## Focused verification

```sh
node --test test/release-gate-dry-run-route-eligibility-regression.test.js test/release-gate-missing-production-secret-regression.test.js test/release-gate-missing-local-url-regression.test.js test/release-gate-progress-release-timestamp-generated.test.js test/release-gate-preflight-route-identity-generated.test.js test/release-gate-same-source-generated.test.js test/verify-release-failure-reason.test.js test/progress-html-release-timestamp.test.js test/release-gates-status-row.test.js test/release-gates.test.js test/release-gate-cli.test.js
```
...
Dry-run route eligibility regression proof:

- Command: `node --test test/release-gate-dry-run-route-eligibility-regression.test.js test/release-gates.test.js test/release-gate-cli.test.js`
- Observed status: `pass`; the fixture supplies every other final-release gate and compares the expected dry-run route with an ineligible pre-apply route observation while release stays `NO-GO`.
- Evidence link: the checked command exits `1` with `DRY_RUN_ROUTE_ELIGIBILITY_REQUIRED`, `finalGates: "19/20"`, exact `dry-run-route-eligibility` reason/evidence, `[release-gates-ci:held final=19/20 candidate=19/20 reason=DRY_RUN_ROUTE_ELIGIBILITY_REQUIRED]`, redacted credential output, and `mutationAttempted: false`.
...
- Dry-run route eligibility regression coverage records an ineligible pre-apply route observation while all other final gates are supplied; the checked CLI exits nonzero with exact `dry-run-route-eligibility` reason/evidence, redacted credential output, `NO-GO`, and `mutationAttempted: false`.
