...
Date: 2026-05-28
Lane: release-gates
Primary checklist range: RPP-0001 through RPP-0026, plus RPP-0028, RPP-0030, RPP-0031, RPP-0032, RPP-0033, RPP-0034, RPP-0035, RPP-0036, RPP-0037, RPP-0038, RPP-0039, RPP-0040, and RPP-0047
...
| RPP-0047 | Evidence toward variant-3 missing production secret gate now generates a final-release fixture with every other gate supplied, omits production credential env and `productionSecret` evidence, and asserts exit `1`, `REPRINT_PUSH_SECRET_REQUIRED`, exact missing-secret evidence, final bracketed status marker, and `mutationAttempted: false`. |

## Focused verification

```sh
node --test test/release-gate-missing-production-secret-generated.test.js test/verify-release-failure-reason.test.js test/progress-html-release-timestamp.test.js test/release-gates-status-row.test.js test/release-gates.test.js test/release-gate-cli.test.js
```
...
Generated missing production secret proof:

- Command: `node --test test/release-gate-missing-production-secret-generated.test.js test/release-gates.test.js test/release-gate-cli.test.js`
- Observed status: `pass`; generated fixture supplies every other final-release gate while omitting production credential env and the `productionSecret` evidence key.
- Evidence link: `check-release-gates` exits `1` with `REPRINT_PUSH_SECRET_REQUIRED`, `finalGates: "19/20"`, exact `production-secret` missing-credential evidence, `[release-gates-ci:held final=19/20 candidate=19/20 reason=REPRINT_PUSH_SECRET_REQUIRED]`, and `mutationAttempted: false`.
...
- Generated missing production secret coverage now shows a final-release fixture with every other gate supplied still fails closed at `production-secret` only, with `REPRINT_PUSH_SECRET_REQUIRED`, a `[release-gates-ci:held final=19/20 candidate=19/20 reason=REPRINT_PUSH_SECRET_REQUIRED]` marker, exact missing-credential evidence, and `mutationAttempted: false`.
