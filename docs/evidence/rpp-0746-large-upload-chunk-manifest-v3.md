# RPP-0746 large upload chunk manifest variant 3 evidence

Evidence for RPP-0746. This slice is support-only local generated coverage for
...
`test/rpp-0746-large-upload-chunk-manifest-v3.test.js` exercises the guarded
...
- duration: `1053.41 ms` within the `30000 ms` budget
- heap used: `7779008 bytes` within the `268435456 bytes` budget
- RSS: `73523200 bytes`
- CPU: `410.07 ms user`, `31.50 ms system`
...
The RPP-0746 proof recomputes this gate vector from the hash-only manifest
...
The variant 3 public proof projection stores counts, byte totals, gate status
...
- `node --check test/rpp-0746-large-upload-chunk-manifest-v3.test.js`
- `node --test --test-name-pattern RPP-0746 test/rpp-0746-large-upload-chunk-manifest-v3.test.js`
- `node scripts/release/artifact-redaction-scan.mjs docs/evidence/rpp-0746-large-upload-chunk-manifest-v3.md`
- `git diff --check`
- `git diff --cached --check`

Observed focused proof result:

- RPP-0746 proof test: 2 pass, 0 fail
