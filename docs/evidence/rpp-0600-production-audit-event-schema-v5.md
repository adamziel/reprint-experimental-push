# RPP-0600 production audit event schema, variant 5
...
route-evidence summary through `dbJournal.auditEventSchema`. Missing,
malformed, duplicated, stale, or drifted schema evidence must be rejected before
release movement, and all support proof values must remain hash-only.

## Proof Surface

`test/rpp-0600-production-audit-event-schema-v5.test.js` adds seven focused
checks:

- a deterministic production-shaped executor flow reaches DB-journal readback,
...
- missing audit-schema evidence is rejected as
  `PRODUCTION_AUDIT_EVENT_SCHEMA_REQUIRED`;
- duplicated route-evidence summaries are rejected as
  `PRODUCTION_AUDIT_EVENT_SCHEMA_DUPLICATED`;
- stale schema hashes are rejected as
  `PRODUCTION_AUDIT_EVENT_SCHEMA_STALE`;
- drifted route-evidence fields are rejected as
  `PRODUCTION_AUDIT_EVENT_SCHEMA_DRIFTED`; and
...
The proof objects store route names, booleans, counts, and SHA-256 digests
only. They do not store raw auth material, endpoint origins, sessions,
...
- The verifier proof keeps release status at `NO-GO` with
  `releaseMovement.allowed: false` for malformed, missing, duplicated, stale,
  and drifted audit-schema evidence.
...
```sh
node --check test/rpp-0600-production-audit-event-schema-v5.test.js
...
node scripts/release/artifact-redaction-scan.mjs docs/evidence/rpp-0600-production-audit-event-schema-v5.md
git diff --check
git diff --cached --check
```

Observed result: all listed commands exited 0. The focused RPP-0600 test
reported 7 passes / 0 failures. The adjacent RPP-0580 proof reported 4 passes
/ 0 failures, the adjacent RPP-0560 proof reported 3 passes / 0 failures, the
adjacent RPP-0540 proof reported 3 passes / 0 failures, and the production
audit schema route test reported 3 passes / 0 failures. The scoped artifact
redaction scan returned `"ok": true`; both whitespace checks returned no
findings.
