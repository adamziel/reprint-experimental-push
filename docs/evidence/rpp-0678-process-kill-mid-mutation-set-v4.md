# RPP-0678 process kill mid mutation set variant 4 evidence
...
- Added standalone generated coverage in
  `test/rpp-0678-process-kill-mid-mutation-set-v4.test.js`.
- The test builds deterministic five-target and seven-target file plans with
  two remote-only preserved resources outside the mutation and precondition
  sets.
...
```bash
node --check test/rpp-0678-process-kill-mid-mutation-set-v4.test.js
...
node --test --test-name-pattern 'process kill mid mutation set retry preserves remote-only changes' test/recovery-journal.test.js
node scripts/release/artifact-redaction-scan.mjs docs/evidence/rpp-0678-process-kill-mid-mutation-set-v4.md
git diff --check
