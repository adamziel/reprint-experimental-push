# RPP-0896 Plugin Update Hooks Topology Variant 5

Date: 2026-06-01
Lane: RPP-0896 plugin update hooks topology release-verifier carry-through, variant 5
Checklist item: RPP-0896 - Carry through the release verifier for plugin update hooks topology, variant 5.

## Scope

RPP-0896 records local support evidence that the plugin update hooks topology
contract carries the release verifier through the Docker local-production
harness. Success for this slice is bounded to the topology command contract:
...
exact unavailable capability, carries through the release verifier failure,
keeps packaged fallback disabled, and leaves release posture at NO-GO.

This variant keeps the evidence hash/count/surface-only. It records hook names,
counts, command names, capability codes, local ingress policy, release verifier
status, and stable hashes. It does not record raw hook payloads, site payloads,
route bodies, credential material, or remote tunnel output.
...
```json
{
  ...
  "rppId": "RPP-0896",
  "variant": 5,
  "title": "Plugin update hooks topology release-verifier carry-through",
  ...
  "candidateScope": {
    "status": "plugin-update-hooks-topology-candidate-v5",
    "supportOnly": true,
    "productionBacked": false,
    "releaseEligible": false,
    ...
    "hookTopologyRequirements": {
      ...
      "requiresReleaseVerifierCarryThrough": true,
    ...
    "candidateClaims": [
      ...
      "variant-5-startup-or-exact-capability-contract-recorded",
  ...
  "topologyCommand": {
    ...
    "artifactHash": "sha256:a0ac5a67df193eeb94c7a3a6d82862cccc6ebfd2de15ce87c6af9d17394e4f84"
  },
  "releaseVerifier": {
    ...
    "requiredCarryThrough": [
      ...
      "verify-release-failure-reason-recorded-when-docker-unavailable",
      "no-packaged-fallback-observed-by-release-verifier",
      "accepted-for-release-gate-false-when-sites-not-started"
    ],
    "requiredHookProofs": [
      "pre_set_site_transient_update_plugins",
      "upgrader_pre_install",
  ...
  "releaseReadyScope": {
    ...
    "requiredEvidence": [
      ...
      "release-verifier-accepts-live-plugin-update-hook-proof",
      "no-packaged-fallback-observed-by-release-verifier",
    ...
    "blockers": [
      ...
      "release-verifier-carry-through-is-fail-closed-not-release-ready",
    ...
    "readyWhen": "topology-command-starts-sites-and-release-verifier-accepts-live-plugin-update-hook-proof"
  },
  "invariants": {
    ...
    "releaseVerifierFailureCarriedThrough": true,
    "releaseVerifierReasonMatchesUnavailableCapability": true,
    "releaseVerifierRejectedForReleaseGate": true,
  ...
  "validation": {
    "commands": [
      {
        "command": "node --check test/rpp-0896-plugin-update-hooks-topology-v5.test.js",
        "result": "exit-0"
      },
      {
        "command": "node --test --test-name-pattern RPP-0896 test/rpp-0896-plugin-update-hooks-topology-v5.test.js",
        "result": "exit-0"
      },
      {
        "command": "npm run verify:release:docker-local-production",
      ...
      {
        "command": "node scripts/release/artifact-redaction-scan.mjs docs/evidence/rpp-0896-plugin-update-hooks-topology-v5.md",
        "result": "exit-0"
      },
      {
...
The topology command is checked at the artifact level and the release verifier
failure is carried through. In this sandbox it did not start sites because the
Docker CLI is unavailable. The result is accepted only as fail-closed support
evidence because the exact unavailable capability is recorded and release
...
and the release verifier accepts live plugin update hook proof.
...
```sh
node --check test/rpp-0896-plugin-update-hooks-topology-v5.test.js
...
node scripts/release/artifact-redaction-scan.mjs docs/evidence/rpp-0896-plugin-update-hooks-topology-v5.md
git diff --check
```

Observed local results after implementation:

- `node --check test/rpp-0896-plugin-update-hooks-topology-v5.test.js`: exit 0
- `node --test --test-name-pattern RPP-0896 test/rpp-0896-plugin-update-hooks-topology-v5.test.js`: 8 subtests passed, 0 failed
