# RPP-0146 wp_options serialized option changes variant 3

Status: focused generated-harness proof added for variant 3. Release remains NO-GO.

## Scenario

Variant 3 adds an explicit `wpOptionsSerializedChangesVariant3` target coverage
surface for deterministic, regular, non-plugin-owned `wp_options` serialized
option updates. The variant-3 tag is emitted on both the ready serialized update
...
- `scripts/harness/generated-push-cases.js` now tags serialized option cases
  with `wp-options-serialized-v3` plus ready/non-ready variant-3 tags and
  exposes `summary.targetCoverage.wpOptionsSerializedChangesVariant3`.
- `test/generated-push-harness.test.js` adds `RPP-0146 wp_options serialized
  option changes variant 3 records redacted surface and invariant`.
...
```json
{
  ...
  "featureFamilies": {
    "wp-options-serialized-v3": 20,
    "wp-options-serialized-v3-ready": 10,
    "wp-options-serialized-v3-non-ready": 10
  },
...
```sh
node --test --test-name-pattern=RPP-0146 test/generated-push-harness.test.js
```
...
```sh
node --test --test-name-pattern='generated push harness covers|RPP-0146' test/generated-push-harness.test.js
```
