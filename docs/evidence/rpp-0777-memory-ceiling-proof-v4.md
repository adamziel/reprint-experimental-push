# RPP-0777 memory ceiling proof variant 4 evidence

Evidence for RPP-0777. This slice is deterministic local support-only generated
...
`test/rpp-0777-memory-ceiling-proof-v4.test.js` exercises the existing
...
- three stale generated samples are rejected under the benchmark-backed proof;
...
The variant 4 projection runs the RPP-0717 benchmark API with fixed local unit
inputs:
...
- expected applied writes: `7`
- expected stale-at-write rejections: `3`
- expected unsafe rename-on-stale writes: `0`
- expected preserved stale drift bytes: `196608`
...
- the benchmark-backed projection records three stale writes, three preserved
...
- `node --check test/rpp-0777-memory-ceiling-proof-v4.test.js`
...
- `node scripts/release/artifact-redaction-scan.mjs docs/evidence/rpp-0777-memory-ceiling-proof-v4.md`
- `git diff --check`

Observed result after local validation:

- RPP-0777 syntax check: exit `0`
- RPP-0777 proof test: 3 pass, 0 fail
