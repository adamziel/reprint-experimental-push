# RPP-0836 Plugin Update Hooks Topology Variant 2

Date: 2026-06-01

## Scope

RPP-0836 proves the plugin update hooks topology requirement shape for variant
2 and records the exact topology-command capability that is unavailable in this
sandbox. This is deterministic support evidence only. It does not run a live
plugin update, does not start Docker WordPress sites in this sandbox, and does
not move release readiness.
...
```json
{
  ...
  "candidateScope": {
    ...
    "candidateClaims": [
      ...
      "variant-2-release-block-contract-recorded",
  ...
  "scopeComparisonHash": "f36fa7da0873ae87738e76ab7843e57691f4719c1dcf276ae93a7a94cc2c51d4",
  "validation": {
    "commands": [
      {
        "command": "node --check test/rpp-0836-plugin-update-hooks-topology-v2.test.js",
        "result": "exit-0"
      },
      {
        "command": "node --test test/rpp-0836-plugin-update-hooks-topology-v2.test.js",
        "result": "exit-0"
      },
      {
        "command": "npm run verify:release:docker-local-production",
      ...
      {
        "command": "node scripts/release/artifact-redaction-scan.mjs docs/evidence/rpp-0836-plugin-update-hooks-topology-v2.md",
        "result": "exit-0"
      },
      {
...
```sh
node --check test/rpp-0836-plugin-update-hooks-topology-v2.test.js
node --test test/rpp-0836-plugin-update-hooks-topology-v2.test.js
npm run verify:release:docker-local-production
node scripts/release/artifact-redaction-scan.mjs docs/evidence/rpp-0836-plugin-update-hooks-topology-v2.md
git diff --check
```

Observed local results after implementation:

- `node --check test/rpp-0836-plugin-update-hooks-topology-v2.test.js`: exit 0
- `node --test test/rpp-0836-plugin-update-hooks-topology-v2.test.js`: 4 subtests passed, 0 failed
