# RPP-0597 same-key different-body conflict, variant 5

Date: 2026-05-31

Status: local support-only release-verifier proof. Final release remains
**NO-GO** until this behavior is covered by the checked production release
boundary.
...
start. The accepted same-key same-body replay must remain hash-only support
evidence and must not widen release readiness.

## Proof Surface

The focused v5 regression adds local support coverage for:
...
- a deterministic production-shaped support proof that drives apply, same-body
  replay, and a same-key different-body conflict;
- a deterministic release-verifier support summary proving accepted replay and
  conflict evidence are hash-only while release movement stays blocked; and
...
- The accepted replay release-verifier summary carries only key, request,
  canonical body, and receipt hashes; it reports `releaseMovement.allowed:
  false`, `gates: "0/4"`, and a blocked production boundary.
...
```sh
node --check test/rpp-0597-same-key-different-body-conflict-v5.test.js
...
node scripts/release/artifact-redaction-scan.mjs docs/evidence/rpp-0597-same-key-different-body-conflict-v5.md
git diff --check
git diff --cached --check
```

Observed result: all listed commands exited 0. The focused RPP-0597 run
reported 3 passes / 0 failures. The adjacent RPP-0577 proof reported
3 passes / 0 failures. The adjacent RPP-0557 proof reported
