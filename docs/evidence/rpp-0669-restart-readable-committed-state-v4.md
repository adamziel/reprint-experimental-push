# RPP-0669 restart-readable committed state variant 4 evidence
...
- Added standalone generated coverage in
  `test/rpp-0669-restart-readable-committed-state-v4.test.js`.
...
state: observed state, latest committed row identity, journal row identity, and
lease owner claim/key hashes are SHA-256-shaped values rather than raw site
payloads, secrets, or bearer tokens.

## Validation run

```bash
node --check test/rpp-0669-restart-readable-committed-state-v4.test.js
...
node --test --test-name-pattern 'committed state survives|RPP-0629' test/recovery-journal.test.js
node scripts/release/artifact-redaction-scan.mjs docs/evidence/rpp-0669-restart-readable-committed-state-v4.md
git diff --check
...
- Focused RPP-0669 test passed 3 subtests, 0 failures.
- Predecessor RPP-0649 committed-state test passed 3 subtests, 0 failures.
- Recovery-journal committed-state restart selection passed 2 subtests, 0
  failures.
