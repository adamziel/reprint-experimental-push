...
- `node --test test/generated-push-harness.test.js test/push-planner.test.js test/graph-mapping-inventory.test.js` — passed (95 tests), including generated ready/stale `wp_commentmeta.comment_id` graph cases and a targeted hash-only blocker proof for a diverged comment target.
...
- RPP-0308 / RPP-0328: commentmeta `comment_id` references include generated ready/stale coverage and fail closed with hash-only `stale-wordpress-graph-identity` evidence when the target comment diverged remotely; the checklist item remains unchecked.
