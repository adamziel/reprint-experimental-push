...
- Detects serialized block attributes that carry numeric graph references inside post content and fails closed with hash-only evidence until parser-aware identity rewriting exists.
...
RPP-0317 focused worker verification:

- `node --test test/local-production-complex-site-proof.test.js test/push-planner.test.js test/generated-push-harness.test.js test/graph-mapping-inventory.test.js` — passed (118 tests), including planner and local-production proof coverage for serialized block reference fail-closed blockers.
...
- `nav_menu` taxonomy, custom/plugin taxonomy rows such as `product_cat`, menu item graph metadata, serialized block references in post content, and unsupported post graph rows such as `nav_menu_item`, `revision`, and `wp_navigation` remain intentionally unmapped until an explicit owner/driver or identity-map proof exists.
...
- RPP-0317: serialized block references in post content are detected as unsupported graph references and remain blocked with `preserve-remote-wordpress-graph-and-stop`; focused proof asserts blocker evidence redacts raw block captions, labels, slugs, and post content.
