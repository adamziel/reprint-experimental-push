...
- Rewrites dependent graph references to proven remote IDs, including page/post parent references, postmeta `post_id` composite row IDs, comment `comment_post_ID` and `comment_parent` thread references, term relationship `object_id`/`term_taxonomy_id` composite row IDs, and termmeta `term_id`.
...
- `node --test test/generated-push-harness.test.js test/push-planner.test.js test/graph-mapping-inventory.test.js` — passed (95 tests), including explicit comment parent identity-map rewrite coverage and generated ready/stale comment parent thread cases.
...
- RPP-0306 / RPP-0326: comment `comment_parent` thread references are rewritten to mapped remote comment identities when proven and fail closed with hash-only blockers when the parent comment changed remotely; the checklist item remains unchecked.
