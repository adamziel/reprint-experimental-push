...
- Rewrites dependent graph references to proven remote IDs, including page/post parent references, postmeta `post_id` composite row IDs, comment `comment_post_ID`, commentmeta `comment_id`, term relationship `object_id`/`term_taxonomy_id` composite row IDs, and termmeta `term_id`.
...
Commentmeta comment variant 2 focused worker verification:

- `node --test test/local-production-complex-site-proof.test.js test/push-planner.test.js test/generated-push-harness.test.js test/graph-mapping-inventory.test.js` — passed (123 tests), including deterministic identity-map rewriting for commentmeta `comment_id` and hash-only stale commentmeta-comment blockers.
...
- RPP-0328: commentmeta `comment_id` references are rewritten from mapped local comment IDs to proven remote comment IDs; stale remote comment targets keep release movement blocked with hash-only evidence.
