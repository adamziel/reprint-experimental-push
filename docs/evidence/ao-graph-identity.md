...
- Keeps nav menu item graph surfaces fail-closed: `nav_menu_item` post rows and `_menu_item_*` metadata do not move without an explicit safe mapper and only emit hash evidence.
...
RPP-0315 focused worker verification:

- `node --test test/local-production-complex-site-proof.test.js test/push-planner.test.js test/generated-push-harness.test.js test/graph-mapping-inventory.test.js` — passed (118 tests), including planner and local-production proof coverage for nav menu item fail-closed blockers and hash-only evidence redaction.
...
- `nav_menu` taxonomy, custom/plugin taxonomy rows such as `product_cat`, menu item graph metadata (`_menu_item_object_id`, `_menu_item_menu_item_parent`, `_menu_item_type`, `_menu_item_object`, and `menu_item_parent`), and unsupported post graph rows such as `nav_menu_item`, `revision`, and `wp_navigation` remain intentionally unmapped until an explicit owner/driver or identity-map proof exists.
- Those unmapped surfaces continue to stop as `stale-wordpress-graph-identity` blockers with hash-only change evidence; the focused planner/local-production proofs assert private term names, slugs, menu labels, and menu content are not leaked in blocker JSON.
...
- RPP-0315: nav menu item posts and menu item metadata remain blocked with `preserve-remote-wordpress-graph-and-stop` and hash-only evidence until a safe mapper/driver can prove menu item semantics.
