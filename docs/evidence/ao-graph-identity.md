...
- Adds focused local-production evidence that `wp_navigation` posts remain fail-closed with hash-only graph blockers, including a stale remote drift conflict case that keeps release movement blocked instead of rewriting unsupported navigation document surfaces.
...
- `node --test test/local-production-complex-site-proof.test.js test/push-planner.test.js test/graph-mapping-inventory.test.js` — passed (117 tests), including `wp_navigation` fail-closed planner evidence, stale remote drift evidence, and hash-only blocker/conflict evidence.
...
- `nav_menu` taxonomy, custom/plugin taxonomy rows such as `product_cat`, menu item graph metadata, and unsupported post graph rows such as `nav_menu_item`, `revision`, and `wp_navigation` remain intentionally unmapped until an explicit owner/driver or identity-map proof exists. RPP-0336 records focused evidence that `wp_navigation` rows stay in this fail-closed bucket rather than claiming production release movement.
- Those unmapped surfaces continue to stop as `stale-wordpress-graph-identity` blockers or preserved-remote conflicts with hash-only change evidence; the planner and local-production proof tests assert private term/navigation names, slugs, bodies, URLs, and descriptions are not leaked in blocker JSON for unsupported graph surfaces.
...
- RPP-0336: `wp_navigation` post rows remain fail-closed with hash-only blocker/conflict evidence unless an explicit owner/driver or stable identity mapping exists.
