...
- Adds generated harness evidence for featured image attachment references: ready cases create `_thumbnail_id` postmeta plus the attachment target in one plan, while stale cases drift the attachment remotely and stop with hash-only `stale-wordpress-graph-identity` blocker evidence.
...
- `node --test test/generated-push-harness.test.js` — local focused generated harness run covered 9 tests and ready/stale featured image attachment graph coverage.
...
- RPP-0342: generated featured image attachment reference cases now cover same-plan `_thumbnail_id` + attachment creation and stale remote attachment fail-closed behavior with redacted hash-only blocker evidence.
