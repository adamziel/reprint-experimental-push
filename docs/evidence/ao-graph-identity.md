...
- Adds focused custom-taxonomy evidence that a plugin/custom taxonomy reference such as `product_cat` remains fail-closed: the term taxonomy row and dependent `wp_term_relationships.term_taxonomy_id` reference stop with hash-only graph blockers or a preserved-remote conflict when the remote drifts, rather than being rewritten without an explicit stable identity mapping.
...
- `node --test test/local-production-complex-site-proof.test.js test/push-planner.test.js test/graph-mapping-inventory.test.js` — passed (117 tests), including custom taxonomy `product_cat` fail-closed planner evidence, stale remote drift conflict evidence, and hash-only blocker evidence.
...
- `nav_menu` taxonomy, custom/plugin taxonomy rows such as `product_cat`, menu item graph metadata, and unsupported post graph rows such as `nav_menu_item`, `revision`, and `wp_navigation` remain intentionally unmapped until an explicit owner/driver or identity-map proof exists. RPP-0331 records focused evidence that `product_cat` custom taxonomy references stay in this fail-closed bucket rather than claiming production release movement.
- Those unmapped surfaces continue to stop as `stale-wordpress-graph-identity` blockers or preserved-remote conflicts with hash-only change evidence; the planner and local-production proof tests assert private term names/slugs/descriptions are not leaked in blocker JSON for unsupported taxonomy surfaces.
...
- RPP-0331: custom taxonomy references such as `product_cat` remain fail-closed with hash-only blocker evidence unless an explicit owner/driver or stable identity mapping exists.
