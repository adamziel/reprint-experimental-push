...
- Rewrites dependent graph references to proven remote IDs, including page/post parent references, postmeta `post_id` composite row IDs, comment `comment_post_ID`, comment `user_id`, term relationship `object_id`/`term_taxonomy_id` composite row IDs, and termmeta `term_id`.
...
Comment user variant 2 focused worker verification:

- `node --test test/local-production-complex-site-proof.test.js test/push-planner.test.js test/generated-push-harness.test.js test/graph-mapping-inventory.test.js` — passed (121 tests), including deterministic identity-map rewriting for comment `user_id` and hash-only stale comment-user blockers.
...
- RPP-0327: comment `user_id` references are rewritten from mapped local user IDs to proven remote user IDs; stale remote user targets keep release movement blocked with hash-only evidence.
