...
- Rewrites dependent graph references to proven remote IDs, including page/post parent references, postmeta `post_id` composite row IDs, comment `comment_post_ID`, comment `comment_parent`, term relationship `object_id`/`term_taxonomy_id` composite row IDs, and termmeta `term_id`.
...
Comment parent variant 2 focused worker verification:

- `node --test test/local-production-complex-site-proof.test.js test/push-planner.test.js test/generated-push-harness.test.js test/graph-mapping-inventory.test.js` — passed (120 tests), including deterministic identity-map rewriting for `comment_parent` and hash-only stale parent-thread blockers.
...
- RPP-0326: comment `comment_parent` thread references are rewritten from mapped local parent comment IDs to proven remote parent comment IDs; stale remote parent targets keep release movement blocked with hash-only evidence.
