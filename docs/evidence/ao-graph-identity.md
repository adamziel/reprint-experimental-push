...
- Rewrites dependent graph references to proven remote IDs, including page/post parent references, postmeta `post_id` composite row IDs, comment `comment_post_ID`, category `wp_term_taxonomy.term_id`, term relationship `object_id`/`term_taxonomy_id` composite row IDs, and termmeta `term_id`.
...
Category term taxonomy variant 2 focused worker verification:

- `node --test test/local-production-complex-site-proof.test.js test/push-planner.test.js test/graph-mapping-inventory.test.js` — passed (117 tests), including deterministic category `wp_term_taxonomy.term_id` identity-map rewriting and hash-only stale term-taxonomy blockers.
...
- RPP-0329: category `wp_term_taxonomy.term_id` references are rewritten from mapped local term IDs to proven remote term IDs; stale remote term targets keep release movement blocked with hash-only evidence.
