# RPP-0771 remote hash pagination variant 4 evidence

Evidence for RPP-0771. This slice is support-only local regression coverage
for remote hash pagination, variant 4. Final release remains **NO-GO** because
this proof does not supply production storage receipts, production row batch
execution, production atomic group commit evidence, live topology, credentials,
or release-verifier carry-through.

## Proof scope

The focused proof test
`test/rpp-0771-remote-hash-pagination-v4.test.js` runs the existing remote hash
...
- `node scripts/bench/remote-hash-pagination.js --resource-count=197 --batch-size=23 --max-duration-ms=5000 --max-heap-used-bytes=268435456`
...
- remote hash resources: `197`
- requested page size: `23`
- page count: `9`
- cursor count: `8`
- complete page count: `1`
- duplicate resource keys: `0`
- raw value evidence leaks: `0`
- duration: `34 ms` within the `5000 ms` budget
- heap used: `6457568 bytes` within the `268435456 bytes` budget
- RSS: `66641920 bytes`
- CPU: `76.4 ms user`, `21.17 ms system`
...
The RPP-0771 proof recomputes this support-only gate vector from the command
report:
...
- `node scripts/bench/remote-hash-pagination.js --resource-count=43 --batch-size=9 --max-duration-ms=5000 --max-heap-used-bytes=1`
...
- raw value evidence leaks: `0`
- duration: `19.5 ms`
- heap used: `4833240 bytes`
- RSS: `57511936 bytes`
- CPU: `36.56 ms user`, `24.84 ms system`
...
The variant 4 public proof projection stores counts, gate status vectors,
...
- `node --check test/rpp-0771-remote-hash-pagination-v4.test.js`
...
- `node scripts/release/artifact-redaction-scan.mjs docs/evidence/rpp-0771-remote-hash-pagination-v4.md`
- `git diff --check`

Observed validation results from this sandbox:

- RPP-0771 syntax check: exit `0`, no syntax errors reported.
- RPP-0771 proof test: 2 pass, 0 fail.
- Adjacent RPP-0751 proof test: 2 pass, 0 fail.
- Adjacent RPP-0731 proof test: 2 pass, 0 fail.
- Evidence redaction scan: `ok: true`, 1 scanned file, 0 rejected files.
