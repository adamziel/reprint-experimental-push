# RPP-0884 BrewCommerce blueprint import variant 5 evidence

Date: 2026-06-01
Lane: RPP-0884 BrewCommerce blueprint import release-verifier carry-through, variant 5
Checklist item: RPP-0884 - Carry through the release verifier for BrewCommerce blueprint import, variant 5.
...
BrewCommerce blueprint import gate. It carries forward the RPP-0844 variant 3
and RPP-0864 variant 4 import/export survival contracts, then adds a focused
variant 5 regression lock that the release verifier can only accept the same
production-backed artifact that proves plugin and graph survival.

This is support evidence only. It does not modify live topology code, auth
flows, release verifier code, progress surfaces, package metadata, shared
scripts, or network exposure. This is not a production-backed proof.
...
- the release verifier must carry `npm run verify:release` through against the
  same artifact binding as the import/export, plugin, graph, and topology
  evidence;
- packaged fallback evidence remains rejected; and
- local Playground, placeholder assets, partial survival artifacts, split
  survival artifacts, and support-only release-verifier output remain rejected
  as release-ready substitutes.
...
```json
{
  ...
  "validation": {
    "commands": [
      {
        "command": "node --check test/rpp-0884-brewcommerce-blueprint-import-v5.test.js",
        "result": "exit-0"
      },
      {
        "command": "node --test --test-name-pattern RPP-0884 test/rpp-0884-brewcommerce-blueprint-import-v5.test.js",
        "result": "exit-0"
      },
      {
        "command": "node scripts/release/artifact-redaction-scan.mjs docs/evidence/rpp-0884-brewcommerce-blueprint-import-v5.md",
        "result": "exit-0"
      },
      {
...
Docker is unavailable in this sandbox, and no complete external WordPress
...
`test/rpp-0884-brewcommerce-blueprint-import-v5.test.js` covers six paths:

- the JSON support report preserves support-only `NO-GO` release posture,
  scoped-file expectations, exact unavailable-capability details, RPP-0844 and
  RPP-0864 template lineage, and release-verifier carry-through requirements;
...
- the deterministic contract branch accepts only real WordPress import/export
  ...
  far production-topology route hashes are present, and the release verifier
  carries the same artifact binding;
- a split-survival artifact with valid-looking booleans but mismatched binding
  hashes remains rejected;
- packaged fallback release-verifier evidence remains rejected even when the
  survival booleans look valid; and
...
```sh
node --check test/rpp-0884-brewcommerce-blueprint-import-v5.test.js
node --test --test-name-pattern RPP-0884 test/rpp-0884-brewcommerce-blueprint-import-v5.test.js
node scripts/release/artifact-redaction-scan.mjs docs/evidence/rpp-0884-brewcommerce-blueprint-import-v5.md
git diff --check
```
...
Integrate as support-only RPP-0884 evidence. Keep final release blocked until a
...
production-topology route-hash, release-verifier carry-through, packaged
fallback rejection, and same-artifact binding checks.
