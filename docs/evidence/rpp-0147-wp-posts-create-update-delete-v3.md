# RPP-0147 wp_posts create/update/delete variant 3

Status: focused generated-harness proof added for variant 3. Release remains NO-GO.

## Scenario

Variant 3 adds an explicit `wpPostsCreateUpdateDeleteVariant3` target coverage
surface for deterministic, regular `wp_posts` create/update/delete changes. The
variant-3 tag is emitted on both the ready post create/update/delete family and
...
- `scripts/harness/generated-push-cases.js` now tags `wp_posts`
  create/update/delete cases with `wp-posts-create-update-delete-v3` plus
  ready/non-ready variant-3 tags and exposes
  `summary.targetCoverage.wpPostsCreateUpdateDeleteVariant3`.
- `test/generated-push-harness.test.js` adds `RPP-0147 wp_posts
...
```json
{
  ...
  "featureFamilies": {
    "wp-posts-create-update-delete-v3": 20,
    "wp-posts-create-update-delete-v3-ready": 10,
    "wp-posts-create-update-delete-v3-non-ready": 10
  },
...
```sh
node --test --test-name-pattern=RPP-0147 test/generated-push-harness.test.js
```
...
```sh
node --test --test-name-pattern='generated push harness covers|RPP-0147' test/generated-push-harness.test.js
```
