...
Current top blocker, rechecked on 2026-05-25: this checkout still does not contain a checked release invocation that proves the live boundary. The repo has green regression and lab evidence, plus helper and Playground scripts, but no checked run here proves production auth/session lifecycle and durable journal semantics on the real push path in one fail-closed invocation. Graph identity mapping and plugin-driver coverage are still only lab-shaped. This checkout also still exposes only helper-oriented commands in `package.json`; there is no checked-in `verify`, `verify:release`, or `release` script here, so no default command can currently own the live verdict. That command-surface gap is real, but it is not the decisive blocker. The gate stays closed until a checked invocation proves production auth/session lifecycle, durable journal semantics, graph identity, and plugin-driver evidence at apply time.
