...
- `origin/lane/reliable-executor` at [`c00eb112`](https://github.com/adamziel/reprint-experimental-push/commit/c00eb112) does expose `verify:release` and maps it to `test:playground:production-shaped-release-verify`, so command absence is no longer the right blocker to cite.
