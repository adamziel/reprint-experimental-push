...
| Required release gate | Helper and Playground scripts only in this checkout; upstream release-verifier claims exist and reportedly pass `npm run verify:release`, but this checkout does not own a checked live-boundary verdict | A checked run in this checkout or enforced default entrypoint that completes the real live boundary and proves auth/session lifecycle plus durable journal semantics | The gate shape is important, but the live-boundary verdict is still incomplete where release safety matters |
