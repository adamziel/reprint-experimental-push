...
2. **The failure test is weaker than the reliability claim.** It proves that a
   ...
   `apply-committed` row is missing. The storage-boundary file write smoke adds
   lab evidence for stale-at-write rejection around an existing fixture upload
   file update, but it is not production filesystem durability, `fsync`,
   ...
   MySQL/InnoDB or filesystem CAS behavior, or all-old stale-claim retry proof.
