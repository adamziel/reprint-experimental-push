...
| Required release gate | Helper and Playground scripts only in this checkout | A checked-in `verify:release`-style command that runs the live boundary and fails closed on missing proof | There is no enforced live-boundary verdict in this checkout, so helper success cannot be promoted to release proof |
...
| Preserved remote drift / real topology | Helper smokes and fixtures model it indirectly | A required command that proves the retained source remains live across preflight and apply in the same invocation | The current evidence can still be satisfied by lab topology, so it does not close the live-boundary risk |
...
- There is no checked-in `verify:release` entry here, and no `test:playground:production-shaped-release-proof` helper either. The existing `production-shaped` helper remains a lab-shaped route smoke rather than a release gate.
