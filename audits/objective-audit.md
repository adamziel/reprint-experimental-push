...
| Required release gate | Helper and Playground scripts only in this checkout | A checked-in `verify:release`-style command that runs the live boundary and fails closed on missing proof | There is no enforced release verdict in this checkout, so helper success cannot be promoted to release proof |
...
- This checkout exposes helper and Playground scripts, but no checked-in `verify`, `verify:release`, or `release` script. That absence matters for enforcement, but it is not the decisive blocker by itself: the live-boundary proof itself is still missing.
