...
| Production auth/session lifecycle | `ea74b2bd` improves packaged release-verify readiness with signed preflight probing and broader not-ready retries, so the checked path now gets further before failing. | Production-backed issuance, read, expiry, rotation, revocation, replay rejection, and cleanup on the live `verify:release` boundary. | Blocked |
...
2. `ea74b2bd` lowers the packaged readiness friction by moving the preflight check to signed headers and allowing more not-ready retries, but it remains a release-verifier support patch rather than production lifecycle proof.
