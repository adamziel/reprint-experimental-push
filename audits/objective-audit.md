...
- Fresh remote heads re-polled at audit time:
  - `origin/lane/reliable-executor` -> `88674b4bdd8f936f9aab4c1938a3ae3e5267b315` (`Bind packaged source to runtime server`)
...
1. `reliable-executor` moved from replay diagnostics to replay-equivalence visibility, journal-ownership proof surfacing, release-gate dependency evidence, recovery claim fencing, preserved-remote retry attempts, package-mode signaling, and now binding packaged source to the runtime server on the checked release path. Those are real product-path changes, but they still stop short of proving production-backed auth/session lifecycle or durable-journal ownership on the live `verify:release` boundary.
