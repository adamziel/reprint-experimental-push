...
- `npm run test:playground:production-shaped-push` and `npm run test:playground:production-plugin-package` prove route shape and packaging behavior. They still report `labBacked: true`, so they are explicitly not production proof and cannot be used as release evidence for speed, durability, or live-source topology.
