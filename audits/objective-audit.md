...
Current top blocker, rechecked on 2026-05-25: the live release boundary is still not proven in this checkout. The repo has green regression and lab evidence, plus helper and Playground scripts, but no checked run here proves production auth/session lifecycle and durable journal semantics on the real push path in one fail-closed invocation. Graph identity mapping and plugin-driver coverage are still only lab-shaped. The command surface here is still helper-oriented: `package.json` exposes `test`, `plan`, `apply`, and optional Playground helpers, but no checked-in `verify`, `verify:release`, or `release` script. External claims about a different branch do not change the evidence in this tree. So even a successful helper run would not by itself become an enforced release verdict. If a future `verify:release` lands here, it still needs live-boundary evidence for auth/session, journal durability, graph identity, and plugin drivers before the gate can open.
