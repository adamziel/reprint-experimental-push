...
| Required release gate | Helper and Playground scripts only in this checkout | A checked-in `verify:release`-style command that runs the live boundary and fails closed on missing proof | The checkout still lacks an enforced live-boundary verdict, so helper success cannot be promoted to release proof |
