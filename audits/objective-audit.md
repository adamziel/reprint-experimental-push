...
DB journal/idempotency slice, and a narrow allowlisted plugin-owned forms data
slice, not a production-safe WordPress push transport. That distinction matters:
...
inspection, JSON-model file-backed journal evidence, and fixture-scoped DB
journal/idempotency evidence if all public status and docs keep that scope
explicit.
...
| DB journal idempotency and process kill | `npm run test:playground:db-journal-idempotency` requires `X-Reprint-Push-Idempotency-Key`, rejects missing keys with `400 MISSING_IDEMPOTENCY_KEY`, records apply/replay/conflict events in `wp_reprint_push_lab_push_journal`, replays same key/body as `BATCH_ALREADY_COMMITTED` with no fresh mutation work or extra mutation events, rejects same key/different body with `409 IDEMPOTENCY_KEY_CONFLICT`, and proves DB-native concurrent first-apply claiming with one opened claim and one fresh mutation executor. `npm run test:playground:db-journal-process-kill` sends real `SIGKILL` during an in-flight DB-journaled REST apply, restarts a host-mounted localhost Playground site, verifies opened/started DB rows persist with no false commit, reports `blocked-recovery`, and blocks retry without overwriting partial state. | Production durable recovery or resumability. It is fixture-scoped local Playground SQLite/host-mount evidence, not production storage durability. DB-native per-mutation evidence can be short after hard kill because mutation rows append after protocol return, and missing-commit finalization/replay remains pending. Redaction checks are key-based plus fixture-value smoke checks, not a formal sanitizer for arbitrary future messages. |
...
2. **The failure test is weaker than the reliability claim.** It proves that a
   ...
   rows, no false commit, blocked recovery, and retry refusal, but it is not
   production durability and does not finalize or replay a missing commit.
...
4. **The Playground endpoints are intentionally not production endpoints.** They
   ...
   concurrent first-apply claiming, and local Playground process-kill/restart
   blocked recovery.
   They do not provide production auth, sessions, durable journals, source-site
   capability checks, receipt signing, receipt expiry, production
   process-kill safety, or missing-commit finalization/replay.

5. **The plugin-owned forms slice is fixture-scoped.** The allowlisted
   `reprint_push_forms_fixture` option and `_reprint_push_forms_schema`
   postmeta prove guarded handling for known fixture resources only.
   `wp_reprint_push_forms_lab` custom-table rows and
   `reprint-push-forms-fixture` metadata are detection-only, and unsupported
   plugin-owned rows block instead of applying.
...
3. Durable production apply journal, likely DB-table backed for WordPress
   ...
   boundaries, including missing-commit finalization/replay. The current DB
   journal/idempotency/process-kill slice is fixture-scoped local Playground
   SQLite/host-mount evidence, and the JSONL lab journal has per-append
   `fsync` evidence, but neither is enough for this production gate.
...
and fixture-scoped DB journal/idempotency/process-kill evidence; it does not
yet prove no data loss, reliability, or speed for a live WordPress push.**
