...
| Required release gate | `origin/lane/reliable-executor` at `9975dfc9` exposes `verify:release` and its release verifier reports live preflight, dry-run, apply, recovery inspect, and journal readback; this checkout does not yet carry that release gate in-tree | A checked run in this checkout or an enforced default entrypoint that completes the real live boundary and proves auth/session lifecycle plus durable journal semantics | The gate shape exists upstream, but this checkout still lacks the live-boundary verdict where release safety matters |
