...
| Bucket | What it is good for | Why it is not release proof |
| --- | --- | --- |
| Executable proof | Local invariants, planner rules, recovery classification, and refusal behavior | It still does not reach the live-source mutation boundary the release claim names, so it can only block bad claims, not approve the good ones |
| Lab/fixture proof | End-to-end route shape, journal flow, and storage guard smokes | It still runs under lab-scoped route profiles and does not clear the live production boundary, even when the route looks production-shaped |
| Docs-only proof | Naming the intended protocol, release flow, and safety matrix | It can describe the release bar without enforcing it, so it is advisory only |
| Missing proof | The single required gate and live-source evidence | It is the gap that still blocks release and the reason the green default path is not release proof |
| Release blockers | `labBacked: true`, fixture-only scope, benchmark-only evidence, missing live-source proof, missing enforced gate | Any one of them keeps the release claim false, and today several are present at once |
...
- `package.json` does not contain a release entrypoint, so the strongest evidence is still opt-in by command choice.
...
- [`test/performance-model.test.js`](/home/claude/reprint-experimental-push-lanes/cycle-20260525-keep-busy-loop-2/independent-auditor/test/performance-model.test.js) proves benchmark shape and guardrail modeling. It does not time the real push path, measure memory, or establish a release threshold.
- [`test/guarded-executor-benchmark.test.js`](/home/claude/reprint-experimental-push-lanes/cycle-20260525-keep-busy-loop-2/independent-auditor/test/guarded-executor-benchmark.test.js) proves the repo refuses unsupported throughput claims and blocks tampered benchmark evidence. It does not prove a positive speed claim, so it cannot justify a "fast" release claim.
- [`npm test`](/home/claude/reprint-experimental-push-lanes/cycle-20260525-keep-busy-loop-2/independent-auditor/package.json) is a sanity check, not a release gate, and even a passing 89-test run is still only blocker evidence because it cannot prove no data loss, reliability, or speed on the live source boundary.
