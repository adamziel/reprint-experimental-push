...
| --- | --- | --- | --- |
| Production-backed auth/session lifecycle | Support-side fail-closed auth and replay checks in `reliable-executor`, including tighter recovery drift classification and durable-journal proof rejection | Live production-backed auth/session lifecycle on the release path | Blocked |
