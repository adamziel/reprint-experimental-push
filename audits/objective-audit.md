...
| Expose one required release command | Audit prose describes the needed command | A checked-in `verify`, `verify:release`, or `release` command | `package.json` has no mandatory release entrypoint, so no real-site push preflight exists |
