...
- This checkout exposes helper and Playground scripts, but it does not carry a checked `verify:release` command surface in-tree. That is enforcement debt in this lane, but the primary release blocker remains the missing production-boundary proof for auth/session lifecycle plus durable journal semantics.
