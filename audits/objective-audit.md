...
| R3 preconditioned apply | `applyPlan()` validates mutation preconditions before applying to a staged clone. Tests reject drift on a planned file mutation. The Playground protocol smokes reject stale apply with `PRECONDITION_FAILED`; the local REST lab returns `412 PRECONDITION_FAILED` while preserving the drifted remote fixture. The storage-boundary DB smoke adds guarded SQL update evidence for narrow existing fixture rows, and `npm run test:playground:storage-guarded-file-write` adds guarded compare-and-rename evidence for an existing fixture upload file update after the JIT pre-write hash. | Preconditions are lab hash checks plus narrow local Playground fixture storage-boundary guards, not production compare-and-swap writes on arbitrary files, MySQL, SQLite, or authenticated Reprint HTTP endpoints. The file smoke covers upload-file update, not create/delete, arbitrary files, or a fixture plugin-file update, and does not prove production `fsync`, filesystem CAS/locks, rollback, or filesystem durability. No test covers remote drift during a multi-resource production apply. | Yes. This is the central no-data-loss gate and currently has no production-boundary proof. |
...
2. **The failure test is weaker than the reliability claim.** It proves that a
   ...
   adds lab evidence for stale-at-write rejection around an existing fixture
   upload file update, but it is not production filesystem durability, `fsync`,
   filesystem CAS/locking, rollback, create/delete guarding, arbitrary file
   safety, or a fixture plugin-file smoke. These are still not production
...
2. Storage-level compare-and-swap or equivalent guarded writes for every file
   ...
   narrow fixture rows and guarded compare-and-rename for an existing fixture
   upload file update only; it does not cover create/delete, arbitrary files,
   production filesystem durability/`fsync`, production locks, or production
   Reprint HTTP mutation.
