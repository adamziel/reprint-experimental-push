...
The repository currently proves an early JSON-snapshot safety model, not a
production-safe WordPress push transport. That distinction matters: most of the
strongest no-data-loss, reliability, and speed claims still depend on design
intent, not direct executable evidence.

Release status: **not releasable as a production push path**. It is acceptable
as a lab harness for planner invariants if all public status and docs keep that
scope explicit.
...
| --- | --- | --- | --- |
| R1 three-way planning | `createPushPlan()` enumerates file, plugin, and row resources from base/local/remote and compares stable hashes. Tests cover unchanged remote, remote-only changes, non-overlapping changes, and direct conflicts. | No real Reprint exporter/importer snapshots. No coverage for WordPress IDs, postmeta relationships, taxonomies, uploads, plugin tables, serialized options, or multisite resources. | Yes. A production claim needs fixture-backed WordPress snapshots and resource identity rules. |
...
| R3 preconditioned apply | `applyPlan()` validates mutation preconditions before applying to a staged clone. A test rejects drift on a planned file mutation. | Preconditions are in-memory hash checks, not compare-and-swap writes on files, MySQL, SQLite, or HTTP endpoints. No test covers remote drift during a multi-resource production apply. | Yes. This is the central no-data-loss gate and currently has no production-boundary proof. |
| R4 conflict stop and evidence | Direct same-resource conflicts become `conflict`, and `applyPlan()` refuses non-ready plans. Plugin-owned row conflicts get a separate class. | No reviewed resolution workflow, persisted conflict record, operator audit trail, or fixtures for realistic plugin-owned data. No delete/edit conflict coverage. | Yes. Conflict detection without durable evidence is insufficient for release. |
| R5 atomic coupled changes | Push intents can group resources; tests cover a plugin install missing a dependency and a plugin stack included in one plan. `applyPlan()` stages a clone before returning. | No transaction boundary exists for actual file/database/plugin writes. `requireAtomic` is metadata, not a production enforcement mechanism. Plugin activation, rollback, and mixed DB/filesystem failure are untested. | Yes. Partial plugin/application state is explicitly rejected by project docs. |
...
| R9 real WordPress execution | Progress page correctly says the real executor is pending. Scenario matrix lists real WordPress coverage as missing. | No Docker/Playground fixtures, no HTTP mutation protocol, no WP-CLI or REST executor, no database transaction tests, and no filesystem permission tests. | Yes. The current project is a model only. |
| R10 speed | The model mutates only changed resources instead of replacing the whole site. Progress page marks fast path at 8 percent and lists chunking/streaming as open. | No benchmarks, complexity budget, large-file streaming, parallel upload tests, memory ceiling, or latency targets. | Yes for any speed claim beyond "not whole-site replacement in the model." |
| R11 honest dry run | README states apply can refuse if the live remote changes after dry run. Drift test covers one file mutation. | No stale-plan expiry, no plan signing/binding to a remote snapshot, no UI/operator warning tests, and no concurrency test for remote changes between individual production writes. | Blocking for UX/reliability release, not for the current lab scope. |
...
| Atomic plugin install dependency | A missing dependency blocks in the simple absent case. | Version constraints, active/inactive state, dependency removed remotely after pull, activation hooks, mixed filesystem/database failure, or rollback. |
...
1. **Planner dependency check can admit a dependency that only exists locally.**
   The current `willHavePlugin()` logic accepts `hasPlugin(local, plugin)`. If
   the dependency existed in the pull base/local snapshot but the remote removed
   it after pull, the dependency resource becomes a `keep-remote` decision while
   the atomic group can still be marked `ready`. `applyPlan()` may later fail
   with `ATOMIC_GROUP_DEPENDENCY_MISSING`, but the dry-run status was already
   too optimistic. This is a release blocker for trustworthy dry-run output.

2. **Atomic group membership is caller-trusted.** If a push intent omits a
...
3. **The failure test is weaker than the reliability claim.** It proves that a
...
4. **Speed has almost no proof.** The only positive evidence is resource-level
...
1. Real WordPress fixture suite covering files, uploads, posts, postmeta,
...
invariants for simplified JSON snapshots; it does not yet prove no data loss,
reliability, or speed for a live WordPress push.**
