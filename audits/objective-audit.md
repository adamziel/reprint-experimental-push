...
- `origin/lane/reliable-executor` at [`d3978bed`](https://github.com/adamziel/reprint-experimental-push/commit/d3978bed) does expose `verify:release` and `test:playground:production-shaped-missing-secret`, and still maps the release verifier to `test:playground:production-shaped-release-verify`, so command absence is no longer the right blocker to cite.
