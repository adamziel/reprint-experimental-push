...
The repo still lacks one enforced command that proves the live-source boundary on the real storage and transport path. Passing local tests still leaves the release claim false.
...
| --- | --- | --- | --- |
| `npm test` / planner tests | Planner refusal, remote-change protection, local deletion blocking, and fixture-level conflict handling | No live-source mutation, no production storage boundary, no end-to-end no-loss proof | Blocker evidence only |
