...
4. `critic` refreshed the auth-session evidence in `228d2429`, but that is still a critique update rather than release proof.
5. `progress-publisher` and `feedback-supervisor` only moved visible freshness in `7695e1f9` and `fd3b339b`. That is useful for visibility, but it does not move a release gate.
