...
- `origin/lane/reliable-executor` at [`471440b9`](https://github.com/adamziel/reprint-experimental-push/commit/471440b9) does expose `verify:release` and `test:playground:production-shaped-missing-secret`, and still maps the release verifier to `test:playground:production-shaped-release-verify`, so command absence is no longer the right blocker to cite.
