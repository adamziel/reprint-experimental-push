...
The release gate stays closed for one precise reason: this checkout still does not own a checked, in-tree production-boundary verdict for auth/session lifecycle plus durable journal semantics at the live apply boundary. That exact boundary is now explicitly named in the current upstream release verifier on `origin/lane/reliable-executor` (`3d8748b6`), which fails closed with `PRODUCTION_AUTH_SESSION_LIFECYCLE_REQUIRED` when the live source or secrets are missing. The upstream verifier also reports live preflight `200`, dry-run `200`, apply `200`, recovery inspect `200`, and durable journal readback with `rows > 0`, but that evidence is upstream-only until this checkout has an equivalent enforced command or default entrypoint.
