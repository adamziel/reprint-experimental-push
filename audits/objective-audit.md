...
- `npm test` passed with 77 tests, 0 failures, and 0 skips.
- `npm run test:playground:production-shaped-push` passed against
  ...
  classified recovery as `fully-updated-remote`. The route still reported
  `labBacked: true`.
- `npm run test:playground:production-plugin-package` passed with the temporary
  ...
  disabled, auth bootstrap disabled, an unprovisioned alternate credential
  rejected with `401`, expired signed session/nonce cleanup observed
  (`deletedExpiredTotal: 2`, `sessionsDeleted: 1`, `noncesDeleted: 1`), 8
  fixture mutations applied, and the final visible fixture surface matching
  local.
...
| --- | --- | --- | --- |
| No-overwrite planner | Unit tests cover unchanged remote mutations, remote-only preservation, deletion behind preconditions, delete/update conflict, directory deletion that would hide a remote-only descendant, file type swap that would hide a remote-only descendant, matching independent edits, plugin dependency drift, stale precondition refusal, one stale WordPress graph-reference blocker, and redacted plugin-data/dependency evidence. | These are JSON-model resources plus fixture policy. The new graph blocker is useful but narrow: it covers a local `wp_postmeta.post_id` reference to a stale remote-created `wp_posts` identity, not full WordPress graph semantics. The tests cannot prove post/postmeta/attachment/taxonomy/menu/plugin relationships are complete or safe. | Add real WordPress graph fixtures where local and remote edit different related resources, then prove the planner blocks or preserves every relationship explicitly. |
...
| R17 | Prove WordPress graph identity and relationship semantics, including referenced targets, ID remapping or fail-closed behavior, and redacted relationship evidence. |
...
| R3 preserve remote changes | Unit tests keep remote-only row and plugin changes, block local deletion versus remote update, stop local directory deletion that would hide a remote-only descendant, stop file type swaps that would hide remote-only descendants, and keep a remote-created post when local postmeta references that stale identity. Playground stale apply smokes preserve fixture drift. | No proof for semantic graph preservation across posts/postmeta/attachments/terms/menus/options/plugin tables. No proof for remote changes to resources that are not directly mutated but are semantically coupled to local changes beyond one model-level post/postmeta case. | Yes |
...
| R9 auth and authorization | `test:playground:authenticated-http-push` and `test:playground:authenticated-cli-push` prove authenticated local Playground aliases, Basic-auth-shaped Application Password credentials, `manage_options`, signed lab requests, nonce replay rejection, auth-bound receipts, idempotency keys, stale refusal, and replay with zero fresh mutation work. `test:playground:production-shaped-push` proves the same route binding shape under `/wp-json/reprint/v1/push/*`, including cross-route receipt rejection before mutation. `test:playground:production-plugin-package` proves the public lab namespace is disabled when the temporary plugin package is activated, auth bootstrap is disabled by default in the package, an unprovisioned alternate credential gets `401`, and expired signed session/nonce artifacts are cleaned up. | It still uses lab-backed implementation and lab signing key derivation. The auth bootstrap hardening is package hygiene, not production credential lifecycle. No production Reprint auth integration, TLS deployment, push credential scoping, nonce/replay retention policy, durable session lifecycle, rate limiting, or real exporter credential binding. | Yes |
...
| R16 release suite | `npm test` passed 77 tests during this audit. `npm run test:playground:production-shaped-push` and `npm run test:playground:production-plugin-package` also passed when run explicitly. | No `.github/` workflow directory was found. `npm test` does not run the strongest Playground smokes. `npm run test:playground` only chains plan/apply/protocol and excludes auth, HTTP, DB journal, storage guards, process kill, stale claim, plugin atomic, forms lab, authenticated CLI, production-shaped route/package, and recovery smokes unless invoked separately. | Yes |
| R17 WordPress graph identity | `npm test` includes `blocks local postmeta references to stale remote-created post identity`. It blocks a local `wp_postmeta.post_id` write that points at a remote-created post identity, preserves the remote post, emits relationship-key/target-hash evidence, and redacts raw post/meta payloads. | This is one JavaScript model case. No production scanner identity map, ID rewrite proof, attachment/term/menu/user relationship matrix, duplicate ID collision handling, or live WordPress fixture proving graph-safe apply exists. | Yes |
...
- One narrow WordPress graph-reference blocker for a local postmeta row pointing
  at a stale remote-created post identity, with redacted relationship evidence.
- Raw plugin dependency metadata redaction in blocker evidence.
...
- Production-shaped route/package smokes prove that the fixture implementation
  ...
  work, that a temporary plugin package disables the public lab namespace and
  auth bootstrap, that an unprovisioned alternate credential rejects with
  `401`, and that expired signed session/nonce option artifacts are cleaned up.
...
3. **No-data-loss proof is resource-narrow.** The tests are strongest for
   simplified resources and named fixtures. The new stale postmeta/post blocker
   is a good fail-closed model case, but it does not prove semantic no-loss
   behavior for WordPress data graphs such as posts plus postmeta plus
...
7. **Auth is lab-auth, not production-auth.** The authenticated and
   production-shaped Playground tests are good protocol-shape evidence, and
   packaged auth bootstrap disablement plus signed-store cleanup remove obvious
   fixture shortcuts. They still do not prove production credentials, TLS,
   secret scoping, durable nonce/session retention, replay retention, or
...
5. A broader WordPress fixture suite covering posts, postmeta, attachments,
   ...
   plugin activation, schema changes, ID remapping or fail-closed relationship
   proof, and multisite if in scope.
