...
| Speed | Refusal-only benchmark assertions | Benchmark model stays `productionThroughput: 'not-claimed'` | Audit prose says `speed unclaimed` is the only safe wording | No measured live-path throughput or mandatory release verdict | Throughput cannot be promoted beyond blocker status |
