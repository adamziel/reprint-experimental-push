...
| Missing proof | No required `verify` / `release` / `verify:release` script, no checked-in CI workflow, no measured live-path throughput, no live-source mutation proof, no mandatory `speed unclaimed` verdict emitted by a required gate | The objective requires a mandatory, production-bound release decision | Blocks release |
