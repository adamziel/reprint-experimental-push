...
| Missing proof | No required `verify:release`-style command in `package.json`; this checkout also has no checked-in `.github` workflow directory at all, so there is no CI entrypoint visible here that composes auth/session, journal, storage, graph identity, plugin-data-driver, real topology, crash-boundary, recovery, and benchmark checks | Nothing by itself; this bucket marks the gap | The objective still lacks a single required release gate that fails closed when any safety proof remains optional |
