...
| --- | --- | --- |
| Executable proof | Local invariants, planner rules, recovery classification, and refusal behavior | It still does not reach the live-source mutation boundary the release claim names |
| Lab/fixture proof | End-to-end route shape, journal flow, and storage guard smokes | It still runs under lab-scoped route profiles and does not clear the live production boundary |
| Docs-only proof | Naming the intended protocol, release flow, and safety matrix | It can describe the release bar without enforcing it |
| Missing proof | The single required gate and live-source evidence | It is the gap that still blocks release |
| Release blockers | `labBacked: true`, fixture-only scope, benchmark-only evidence, missing live-source proof | Any one of them keeps the release claim false |
