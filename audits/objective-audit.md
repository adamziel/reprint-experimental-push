...
The current evidence remains support-side and fail-closed. `351b6bbd` improves the durable-journal surface and narrows the gap, and `e725e749` plus `e7be9812` make the readiness failure report cleanly and fail fast, but neither closes the production release gate because the adapter is not yet consumed by the live release path and the auth/session/replay evidence is still not live production-backed proof. The verdict stays `0/4`.
