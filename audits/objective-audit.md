...
| CI or default enforcement | `origin/lane/reliable-executor` exposes `verify:release` on the current remote tip and ships a release-boundary contract; this checkout does not yet carry a checked, in-tree equivalent | A checked default entrypoint in this checkout that owns the release verdict | No enforced in-tree gate yet |
