...
| `Fast` | Explicitly unclaimed | Either a measured live-path throughput threshold or an intentional, enforced `speed unclaimed` decision surfaced by the required release gate; a green refusal-only test run is not enough, and optional smokes must never be allowed to stand in for that verdict |
