...
2. Preserve all WordPress data shapes that can be affected by a push, including related rows, files, plugin-owned data, and graph identity.
...
5. Prove the real remote/local topology, not just a local lab route shape, local Playground route, or fixture mount.
...
| Expose one required release command | Optional npm scripts and opt-in smokes exist: `npm test`, `npm run test:playground`, and separate scenario smokes. | A mandatory `verify:release`-style entrypoint that fails closed. | Yes: there is no required gate. |
...
- `test/recovery-journal.test.js` proves file-backed JSONL append/restart behavior, monotonic sequences, no raw journal values, and restart classification. It does not prove production storage durability, cross-process lease handling, or a real remote/local crash boundary, so it is proof of a journal model rather than proof of the live storage path.
- `test/performance-model.test.js` and `test/guarded-executor-benchmark.test.js` prove the benchmark model keeps proof obligations attached to the proposed fast paths and that unsupported throughput claims are blocked. They do not measure a production push path or establish a real runtime/memory threshold, so they support refusal of unsupported speed claims rather than a release-speed claim.
...
The repository currently has optional proof commands, not an enforced release gate. `package.json` confirms the split: the default suite is `npm test`, the bundled lab chain is `npm run test:playground`, and the stronger auth, journal, storage, recovery, plugin, and benchmark checks are only available as separate opt-ins. There is no `npm run release`, `npm run verify`, or `npm run verify:release`, and `find .github` returns no workflow files in this checkout, so there is no checked-in CI entrypoint to audit as a release gate.
