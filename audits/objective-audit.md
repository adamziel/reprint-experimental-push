...
1. `reliable-executor` now has tighter auth-session drift handling in `5271f45f`, but it still only proves fail-closed support behavior and boundary reporting. It does not establish production-backed auth/session lifecycle, canonical replay on a live source, or durable journal ownership on the release path.
