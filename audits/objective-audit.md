...
| Live mutation boundary | Unsupported-surface blocking in `no-data-loss-invariants`, including `a7ead953` classifying comments graph unsupported boundaries | A live production mutation boundary proving source changes are safe | Blocked |
...
3. `no-data-loss-invariants` now shows additional unsupported-surface blocking, but `a7ead953` still does not prove the live production mutation boundary.
