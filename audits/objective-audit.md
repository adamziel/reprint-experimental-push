...
| DB journal idempotency | `npm run test:playground:db-journal-idempotency` requires `X-Reprint-Push-Idempotency-Key`, rejects missing keys with `400 MISSING_IDEMPOTENCY_KEY`, records apply/replay/conflict events in `wp_reprint_push_lab_push_journal`, replays same key/body as `BATCH_ALREADY_COMMITTED` with no fresh mutation work or extra mutation events, and rejects same key/different body with `409 IDEMPOTENCY_KEY_CONFLICT`. | Production durable recovery or resumability. It is fixture-scoped local Playground DB evidence, not process-kill proof and not concurrency/race proof for duplicate first applies. Redaction checks are key-based plus fixture-value smoke checks, not a formal sanitizer for arbitrary future messages. |
...
2. **The failure test is weaker than the reliability claim.** It proves that a
   ...
   still not process-kill or production WordPress crash-boundary proof.
...
4. **The Playground endpoints are intentionally not production endpoints.** They
   ...
   inspection, and local-only REST routes under `reprint-push-lab/v1`.
   They do not provide production auth, sessions, durable journals, source-site
   capability checks, receipt signing, receipt expiry, process-kill safety, or
   duplicate first-apply concurrency proof.
...
3. Durable production apply journal, likely DB-table backed for WordPress
   ...
   boundaries. The current DB journal/idempotency slice is fixture-scoped local
   Playground evidence, and the JSONL lab journal has per-append `fsync`
   evidence, but neither is enough for this production gate.
...
6. Stale-plan and concurrency tests for remote changes before apply, during
   apply, between chunks, and duplicate first applies with the same idempotency
   key.
...
and fixture-scoped DB journal/idempotency evidence; it does not yet prove no
data loss, reliability, or speed for a live WordPress push.**
