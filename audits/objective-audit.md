...
- `Executable proof` exercises the claimed production boundary directly.
...
| --- | --- | --- | --- |
| No data loss | `test/push-planner.test.js`, recovery fixtures, and smoke tests prove selected planner rules, fixture-scoped protected writes, and replay refusal. The strongest route still labels itself `labBacked: true`, so the evidence is real but still lab/fixture proof. | It does not prove a live WordPress graph survives a failed push without losing or duplicating posts, postmeta, attachments, taxonomy links, menus, users, plugin-owned rows, or serialized plugin payloads. | Missing live crash coverage at every guarded DB/file/plugin boundary and missing a production-backed no-loss assertion. | Yes: indirect proof cannot clear the no-loss claim. |
| Reliability | The repo proves some journal, replay, stale-claim, and process-kill states are classified and blocked in local Playground fixtures, and the journal model rejects malformed states. | It does not prove restart safety, leases, fencing, rollback, or exactly-once behavior on a live source site across all mutation types and storage/transport failures. | Missing production-backed kill matrix plus durable journal evidence on the real path. | Yes: fixture recovery does not prove production reliability. |
