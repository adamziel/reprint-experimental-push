...
| --- | --- | --- | --- |
| Executable proof | `npm test` runs 89 Node tests, including planner, recovery-journal, benchmark-model, and guarded-benchmark checks | Local invariants, refusal discipline, redaction, monotonic journal shape, and benchmark guardrails | It stops at model and fixture boundaries; it does not exercise a live-source release path, so it can reject unsafe claims without approving a ship decision |
| Lab/fixture proof | `npm run test:playground`, `npm run test:playground:authenticated-http-push`, `npm run test:playground:authenticated-cli-push`, `npm run test:playground:production-shaped-push`, and the storage-guard, journal, process-kill, and stale-claim smokes | End-to-end route shape, journal flow, storage guard behavior, idempotency, and failure classification in Playground or fixture-backed routes | These checks are still lab-scoped or fixture-scoped, even when the URL or package name looks production-shaped, so they do not prove the real live boundary |
| Docs-only proof | `docs/supervised-lanes.md`, `progress.html`, [`audits/release-evidence-gap.md`](/home/claude/reprint-experimental-push-lanes/cycle-20260525-keep-busy-loop-2/independent-auditor/audits/release-evidence-gap.md), and this audit | The intended protocol, release flow, and safety gates are described clearly | Description does not enforce the gate, so it remains advisory only |
...
- `npm test` is therefore a valid sanity check, not a release approver, even when it passes 89 tests.
...
- The suite is evidence that release remains blocked, not evidence that release is safe, and it remains bypassable because there is no checked-in release gate.
