...
The current tests are strongest where they reject unsafe claims, and weakest where they are asked to prove production release safety on the live push path.
...
- `npm run test:playground:production-shaped-push` and `npm run test:playground:production-plugin-package` prove route shape and packaging behavior. They still report `labBacked: true`, so they are explicitly not production proof and cannot be used as release evidence.
...
The repository currently has optional proof commands, not an enforced release gate. `package.json` confirms the split: the default suite is `npm test`, the bundled lab chain is `npm run test:playground`, and the stronger auth, journal, storage, recovery, plugin, and benchmark checks are only available as separate opt-ins. There is no `npm run release`, `npm run verify`, or `npm run verify:release`, and this checkout has no checked-in `.github` workflow files, so there is no CI entrypoint to audit as a release gate.
