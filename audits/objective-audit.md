...
| Production auth/session lifecycle | `0bd0f4dffb57432dcd00a11ccd721c867e0fe457` now accepts the live release boundary with `production-auth-session` evidence on the checked path and no remaining `PRODUCTION_AUTH_SESSION_LIFECYCLE_REQUIRED` failure in the live verifier branch. | A remaining blocker, if any, is now outside this auth/session boundary. | Moved |
