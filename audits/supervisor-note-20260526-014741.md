...
- The current blocker is still with `reliable-executor`: it needs either the TAP rerun summary for `node --test test/protocol-fixtures.test.js` or the exact failing live `test/production-shaped-proof.test.js` command with the missing dependency named exactly.
