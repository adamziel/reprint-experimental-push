...
| Publish a measured speed claim or explicitly refuse to make one | Benchmark-model and guarded-benchmark tests refuse unsupported claims | No live-path throughput measurement is present | Audit notes refuse the claim | No production-speed threshold is verified | Yes |
| Expose one required release command that fails closed on any unsafe bucket | Not present | Optional smokes exist, but they are not mandatory | Audit notes describe the needed gate | `package.json` has no `verify`, `release`, or equivalent enforced command | Yes |
| Wire the release command into CI or another default entrypoint | No checked-in workflow exists in this checkout | None | Audit notes mention the absence | No enforced automation path exists | Yes |
