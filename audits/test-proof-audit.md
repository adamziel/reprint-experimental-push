...
The actionable next step is therefore not to expand the current refusal checks in place. It is to make the release decision explicit at the live-source boundary and keep the gate closed until production auth/session lifecycle plus durable journal semantics are proven there. If throughput remains unmeasured, that decision still needs to surface an enforced `speed unclaimed` verdict and fail closed. Optional smokes and refusal-only benchmarks are still useful, but they remain non-gating evidence until the mandatory live-boundary verdict exists.
