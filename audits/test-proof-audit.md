...
The actionable next step is therefore not to expand the current refusal checks in place. It is to add one mandatory command that reaches the live-source boundary and makes the release decision explicit. If throughput remains unmeasured, that command must still surface an enforced `speed unclaimed` verdict and fail closed; otherwise the suite is only refusing claims in a lab, not making a release decision. The missing command is the blocker, and the tests should be read as support evidence only until that command exists.
