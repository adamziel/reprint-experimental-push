...
The supervised reliable-executor lane now has material retained-source evidence: a `verify:release` lineage, `authSessionType`, minted session shape, `applyCommitted`, `durableJournal.rows: 17`, the `PRODUCTION_AUTH_SESSION_LIFECYCLE_REQUIRED` verdict at `889bd37a`, and the newer `2bdf01a8` proof-contract clarification. That is concrete progress, but it is still lab evidence. The gate stays closed until one rerunnable live boundary on a real local, Playground, or Docker `REPRINT_PUSH_SOURCE_URL` preserves the rejected remote after refusal, revalidates at apply time from fresh live hashes, and proves production WordPress auth/session lifecycle, durable journal storage plus lease/fencing on production-like storage, graph identity, and plugin-driver coverage on the same mutation. Until that exact boundary exists, the best available status is "lab-verified, production-unproven." The source notes only establish lineage from Reprint, ZS-Sync, and ForkPress; they are not a substitute for a rerunnable live push boundary on this branch.
