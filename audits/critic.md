...
The supervised reliable-executor lane now has material retained-source evidence: a `verify:release` lineage, `authSessionType`, minted session shape, `applyCommitted`, `durableJournal.rows: 17`, the `PRODUCTION_AUTH_SESSION_LIFECYCLE_REQUIRED` verdict at `889bd37a`, the newer `2bdf01a8` proof-contract clarification, and remote head `ee43e194` binding release verification to the supplied source URL. That is concrete progress, but it is still lab evidence and still not a live WordPress auth/session lifecycle on a preserved remote. The gate stays closed until one rerunnable live boundary on a real local, Playground, or Docker `REPRINT_PUSH_SOURCE_URL` preserves the rejected remote after refusal, revalidates at apply time from fresh live hashes, and proves production WordPress auth/session lifecycle, durable journal storage plus lease/fencing on production-like storage, graph identity, and plugin-driver coverage on the same mutation. Until that exact boundary exists, the best available status is "lab-verified, production-unproven." Reprint, ZS-Sync, and ForkPress only supply source-note lineage here; none of them replace a rerunnable live push boundary on this branch.
