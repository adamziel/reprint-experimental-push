...
The supervised reliable-executor lane now has material retained-source evidence: a `verify:release` lineage, `authSessionType`, minted session shape, `applyCommitted`, `durableJournal.rows: 17`, the `PRODUCTION_AUTH_SESSION_LIFECYCLE_REQUIRED` verdict at `889bd37a`, and the newer `2bdf01a8` proof-contract clarification. That is concrete progress, but it is still lab evidence. The next proof must be a rerunnable live boundary on one real local, Playground, or Docker `REPRINT_PUSH_SOURCE_URL`; it has to preserve the rejected remote after refusal, revalidate at apply time from fresh live hashes, and prove production WordPress auth/session lifecycle, durable journal storage plus lease/fencing on production-like storage, graph identity, and plugin-driver coverage on the same mutation. Until that exact boundary exists, the best available status is "lab-verified, production-unproven."
