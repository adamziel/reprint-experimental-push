...
The single strongest blocker is still the missing rerunnable live boundary on
this branch. The supervised reliable-executor lane already gives stronger
retained-source evidence, but that evidence only upgrades the lab harness, not
the release claim.

Until this branch can show preserved-remote auditability plus live
auth/session, journal, graph, and plugin-driver proof on the same mutation,
production-grade push support is not a defensible statement.
