...
export function buildDurableRecoveryJournalReleaseProof({
  ...
  const manualRecoveryAudit = resolveManualRecoveryAuditExportProof({
    releaseSummary,
    releaseProof,
    durableJournal,
    journal,
...
function resolveManualRecoveryAuditExportProof({
  releaseSummary,
  releaseProof,
  durableJournal,
  journal,
  ...
  const explicit = selectManualRecoveryAuditExportCandidate([
  ...
  if (explicit) {
    const legacyProof = withManualRecoveryAuditBoundary(
      manualRecoveryAuditExportProvesRecoveryGate(explicit, {
        planMutationCount,
        recovery,
      }),
      { releaseSummary },
    );
    if (legacyProof.proved) {
      return legacyProof;
    }

    return manualRecoveryAuditExportProof({
      explicitExport: explicit,
      releaseSummary,
      releaseProof,
      journal,
      ownership,
  ...
  const auditExport = buildManualRecoveryAuditExport({
    recovery,
  ...
  return withManualRecoveryAuditBoundary(
    manualRecoveryAuditExportProvesRecoveryGate(auditExport, {
      planMutationCount,
      recovery,
    }),
    { releaseSummary },
...
function withManualRecoveryAuditBoundary(proof, { releaseSummary }) {
  return {
    ...proof,
    sameReleaseBoundary: releaseSummary?.boundary?.verdict === 'LIVE_RELEASE_BOUNDARY_OK',
    sourceUrl: releaseSummary?.topology?.sourceUrl || null,
  };
}
