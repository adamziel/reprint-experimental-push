...
await withPlaygroundServer('ready-base', path.join(repoRoot, fixtures.base), async (server) => {
  ...
  const missingApply = await postLab(server, '/apply', { plan: readyPlan });
  ...
  const tamperedApply = await postLab(server, '/apply', {
    plan: readyPlan,
  ...
  const apply = await postLab(server, '/apply', { plan: readyPlan, receipt: readyReceipt });
...
await withPlaygroundServer('stale-remote', path.join(repoRoot, fixtures.remoteChanged), async (server) => {
  ...
  const staleApply = await postLab(server, '/apply', { plan: readyPlan, receipt: readyReceipt });
