...
function reprint_push_lab_rest_auth_evidence(WP_REST_Request $request): array
{
    ...
    if ($production_session && is_array($lifecycle_drift)) {
        ...
        if (($lifecycle_drift['mode'] ?? '') === 'warning-invalid') {
            $production_session_warning = ['lab-only-warning'];
        }
    }
    ...
    $session_cleanup = is_array($auth) && array_key_exists('cleanup', $auth)
        ? (bool) $auth['cleanup']
        : null;
    ...
    if ($production_session) {
        ...
        if (($lifecycle_drift['mode'] ?? '') === 'revoked-invalid') {
            $session_revoked = ['lab-revoked'];
        }
        if (($lifecycle_drift['mode'] ?? '') === 'cleaned-up-invalid') {
            $session_cleaned_up = ['lab-cleaned-up'];
        }
        if (($lifecycle_drift['mode'] ?? '') === 'cleanup-invalid') {
            $session_cleanup = ['lab-cleanup'];
        }
