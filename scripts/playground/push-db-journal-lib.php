...
function reprint_push_lab_db_journal_claim_id_from_key_hash($claim_key_hash): ?string
{
    return reprint_push_lab_db_journal_non_empty_string($claim_key_hash)
        ? 'psh_' . substr(hash('sha256', 'reprint-push-claim-id:' . (string) $claim_key_hash), 0, 27)
        : null;
}
...
function reprint_push_lab_db_journal_attach_checked_contract(
    array $summary,
    string $scope,
    string $supported_surface = 'claim-fenced-restart-readable'
): array {
...
function reprint_push_lab_db_journal_checked_boundary_contract_is_coherent($journal): bool
{
    ...
    $claim_identity_matches = !is_array($claim)
        || !reprint_push_lab_db_journal_non_empty_string($claim['activeClaimId'] ?? null)
        || (
...
function reprint_push_lab_db_journal_compact_result(array $result): array
{
    ...
    foreach (['mode', 'code', 'message', 'applied', 'verifiedKeys', 'verifiedPreconditions', 'applyRevalidation', 'rejectedRemoteEvidence', 'recovery', 'storageGuard', 'storageGuardCoverage'] as $key) {
...
function reprint_push_lab_db_journal_mutation_evidence(array $mutation, ?string $observed_hash = null): array
{
    ...
    if (isset($mutation['storageGuardCoverage']) && is_array($mutation['storageGuardCoverage'])) {
        $evidence['storageGuardCoverage'] = $mutation['storageGuardCoverage'];
    }
