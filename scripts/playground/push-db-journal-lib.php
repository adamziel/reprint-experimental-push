        ...
        $summary = array_merge(
            ...
            reprint_push_lab_db_journal_checked_boundary_contract(
                ...
                isset($summary['claim']['activeClaimId']) && is_string($summary['claim']['activeClaimId'])
                    ...
                    : null,
                isset($summary['claim']['activeClaimKeyHash']) && is_string($summary['claim']['activeClaimKeyHash'])
                    ? $summary['claim']['activeClaimKeyHash']
                    : null
            )
        );
    }
...
function reprint_push_lab_db_journal_claim_contract_matches($claim): bool
{
    ...
    return $status_matches_stale_claim
        ...
        && (string) ($claim['activeClaimId'] ?? '') === (string) ($claim['activeClaimKeyHash'] ?? '')
        && (!$has_previous_claim_identity || (
            ...
            && reprint_push_lab_db_journal_is_positive_int($claim['previousClaimSequence'] ?? null)
            && reprint_push_lab_db_journal_non_empty_string($claim['previousClaimEvent'] ?? null)
            && (string) ($claim['previousClaimId'] ?? '') === (string) ($claim['previousClaimKeyHash'] ?? '')
        ))
...
function reprint_push_lab_db_journal_claim_evidence_row_matches($row, array $expected): bool
{
    ...
    return (!reprint_push_lab_db_journal_is_positive_int($expected['sequence'] ?? null)
        ...
        && (!reprint_push_lab_db_journal_non_empty_string($expected['claimId'] ?? null)
            || (string) ($row['claimId'] ?? '') === (string) $expected['claimId'])
...
): array
{
    ...
    if (reprint_push_lab_db_journal_non_empty_string($claim_key_hash)) {
        $contract['claimKeyHash'] = $claim_key_hash;
    }
