...
const targetCoverageDefinitions = Object.freeze({
  ...
  blogVersionBlogIdReferenceVariant3: {
  ...
  commentPostReferenceVariant3: {
    family: 'comment-post-reference-variant3',
    tag: 'comment-post-reference-v3',
  },
  commentParentThreadReferenceVariant3: {
...
function buildGeneratedCase({ index, tier, rng }) {
  ...
  addBlogVersionBlogIdReferenceVariant3Target({
    family,
    base,
    local,
    remote,
  ...
  addCommentPostReferenceVariant3Target({
    family,
    base,
    local,
    remote,
  ...
  addCommentParentThreadReferenceVariant3Target({
    family,
    base,
    local,
    remote,
...
function addBlogVersionBlogIdReferenceVariant3Target({
  family,
  base,
  local,
  remote,
...
function addSitemetaSiteIdReferenceVariant3Target({
  family,
  base,
  local,
  remote,
  ...
  addSitemetaSiteIdReferenceVariant3(base, local, remote, allocator, tags, { staleTarget });
  tags.add(staleTarget ? 'expected-blocked' : 'ready-candidate');
}

function addCommentPostReferenceVariant3Target({
  family,
  base,
  local,
  remote,
  ...
  const readyTarget = family === 'same-plan-comment-graph';
  const staleTarget = family === 'stale-post-author-graph';
  if (!readyTarget && !staleTarget) {
    return;
  }

  addCommentPostReferenceVariant3(base, local, remote, allocator, tags, { staleTarget });
  tags.add(staleTarget ? 'expected-blocked' : 'ready-candidate');
}

function addCommentParentThreadReferenceVariant3Target({
  family,
  base,
  local,
  remote,
...
function addBlogVersionBlogIdReferenceVariant3(base, local, remote, allocator, tags, { staleTarget }) {
  const sourceBlogId = allocator.graphId();
  const sourceBlogRowId = `blog_id:${sourceBlogId}`;
  const blogVersionRow = {
  ...
  if (staleTarget) {
    const blogRow = {
      blog_id: sourceBlogId,
      site_id: 1,
      domain: `rpp-blog-version-reference-stale-blog-${sourceBlogId}.example.test`,
      path: '/generated-blog-version-reference/',
      public: 1,
      archived: 0,
      mature: 0,
      spam: 0,
    ...
    setRow(remote, 'wp_blogs', sourceBlogRowId, {
      ...blogRow,
      domain: `remote-private-rpp-blog-version-reference-stale-blog-${sourceBlogId}.example.test`,
    });
    setRow(local, 'wp_blog_versions', sourceBlogRowId, blogVersionRow);
  } else {
    ...
    const blogRow = {
      blog_id: sourceBlogId,
      site_id: 1,
      domain: `rpp-blog-version-reference-mapped-blog-${sourceBlogId}.example.test`,
      path: '/generated-blog-version-reference/',
      public: 1,
      archived: 0,
      mature: 0,
    ...
    tags.add('blog-version-blog-id-reference-v3-identity-map');
    tags.add('blog-version-blog-id-reference-v3-ready');
  }

  tags.add('blog-version-blog-id-reference-v3');
  tags.add('blog-version-blog-id-reference-v3-hash-only');
  tags.add('blog-version-blog');
  tags.add('same-plan-graph');
...
function addSitemetaSiteIdReferenceVariant3(base, local, remote, allocator, tags, { staleTarget }) {
  const sourceSiteId = allocator.graphId();
  const metaId = allocator.graphId();
  const sourceSiteRowId = `id:${sourceSiteId}`;
  const metaKey = `_rpp0902_sitemeta_site_id_reference_v3_${metaId}`;
  const sourceSitemetaRowId = `site_id:${sourceSiteId}:meta_key:${metaKey}`;
  const sitemetaRow = {
    site_id: sourceSiteId,
    meta_key: metaKey,
    meta_value: `rpp0902-${staleTarget ? 'stale' : 'ready'}-local-sitemeta-private-${metaId}`,
  };

  if (staleTarget) {
    const siteRow = {
      id: sourceSiteId,
      domain: `rpp-0902-stale-site-${sourceSiteId}.example.test`,
      path: '/',
    };
    ...
    setRow(remote, 'wp_site', sourceSiteRowId, {
      ...siteRow,
      domain: `rpp-0902-remote-stale-site-${sourceSiteId}.example.test`,
    });
    setRow(local, 'wp_sitemeta', sourceSitemetaRowId, sitemetaRow);
  } else {
    const targetSiteId = allocator.graphId();
    const targetSiteRowId = `id:${targetSiteId}`;
    const siteRow = {
      id: sourceSiteId,
      domain: `rpp-0902-mapped-site-${sourceSiteId}.example.test`,
      path: '/',
    };
    const targetSiteRow = {
      ...siteRow,
    ...
    tags.add('sitemeta-site-id-reference-v3-identity-map');
    tags.add('sitemeta-site-id-reference-v3-ready');
  }

  tags.add('sitemeta-site-id-reference-v3');
  tags.add('sitemeta-site-id-reference-v3-hash-only');
  tags.add('sitemeta-site');
  tags.add('same-plan-graph');

  if (staleTarget) {
    tags.add('stale-graph');
    tags.add('sitemeta-site-id-reference-v3-stale');
    tags.add('sitemeta-site-id-reference-v3-stale-target');
    tags.add('sitemeta-site-id-reference-v3-non-ready');
  }
}
...
function addCommentPostReferenceVariant3(base, local, remote, allocator, tags, { staleTarget }) {
  const sourcePostId = allocator.graphId();
  const commentId = allocator.graphId();
  const sourcePostRowId = `ID:${sourcePostId}`;
  const commentRowId = `comment_ID:${commentId}`;
  ...
  if (staleTarget) {
    const sourcePost = makePost(sourcePostId, `RPP-0345 generated stale comment post ${sourcePostId}`, {
      post_name: `rpp-0345-stale-comment-post-${sourcePostId}`,
      post_content: `rpp0345-stale-source-comment-post-private-${sourcePostId}`,
    });
    ...
    setRow(remote, 'wp_posts', sourcePostRowId, {
      ...sourcePost,
      post_title: `RPP-0345 generated remote stale comment post ${sourcePostId}`,
      post_content: `rpp0345-remote-stale-comment-post-private-${sourcePostId}`,
    });
    setRow(local, 'wp_comments', commentRowId, makeComment(commentId, {
      comment_post_ID: sourcePostId,
      comment_parent: 0,
      user_id: 1,
      comment_content: `RPP-0345 generated comment_post_ID identity-map comment stale ${commentId}`,
    }));
  } else {
    const targetPostId = allocator.graphId();
    const targetPostRowId = `ID:${targetPostId}`;
    const postTitle = `RPP-0345 generated mapped comment post ${sourcePostId}`;
    const postSlug = `rpp-0345-mapped-comment-post-${sourcePostId}`;
    const postContent = `rpp0345-ready-comment-post-private-${sourcePostId}`;
    const sourcePost = makePost(sourcePostId, postTitle, {
      post_name: postSlug,
      post_content: postContent,
    });
    ...
    setRow(local, 'wp_comments', commentRowId, makeComment(commentId, {
      ...
      comment_parent: 0,
      user_id: 1,
      comment_content: `RPP-0345 generated comment_post_ID identity-map comment ready ${commentId}`,
    }));

    tags.add('comment-post-reference-v3-ready');
    tags.add('comment-post-reference-v3-identity-map');
  }

  tags.add('comment-post-reference-v3');
  tags.add('comment-post-reference-v3-hash-only');
  ...
  if (staleTarget) {
    tags.add('stale-graph');
    tags.add('comment-post-reference-v3-stale');
    tags.add('comment-post-reference-v3-stale-target');
    tags.add('comment-post-reference-v3-non-ready');
  }
}

function addCommentParentThreadReferenceVariant3(base, local, remote, allocator, tags, { variant }) {
  const sourceParentId = allocator.graphId();
  const childId = allocator.graphId();
  const sourceParentRowId = `comment_ID:${sourceParentId}`;
  const childRowId = `comment_ID:${childId}`;
  const parentContent = `RPP-0346 generated comment_parent ${variant} parent private ${sourceParentId}`;
  const childContent = `RPP-0346 generated comment_parent ${variant} child private ${childId}`;

  if (variant === 'stable') {
    const parent = makeComment(sourceParentId, {
      comment_post_ID: 1,
      comment_parent: 0,
      user_id: 0,
    ...
    setRow(base, 'wp_comments', sourceParentRowId, parent);
    setRow(local, 'wp_comments', sourceParentRowId, parent);
    setRow(remote, 'wp_comments', sourceParentRowId, parent);
    setRow(local, 'wp_comments', childRowId, makeComment(childId, {
      comment_post_ID: 1,
      comment_parent: sourceParentId,
      user_id: 0,
      comment_content: childContent,
    }));

    tags.add('comment-parent-thread-reference-v3-stable');
    tags.add('comment-parent-thread-reference-v3-stable-ready');
  } else if (variant === 'identity-map') {
    const targetParentId = allocator.graphId();
    const targetParentRowId = `comment_ID:${targetParentId}`;
    const sourceParent = makeComment(sourceParentId, {
      comment_post_ID: 1,
      comment_parent: 0,
      user_id: 0,
      comment_content: parentContent,
    });
    const targetParent = makeComment(targetParentId, {
      comment_post_ID: 1,
      comment_parent: 0,
      user_id: 0,
      comment_content: parentContent,
    ...
    setRow(local, 'wp_comments', sourceParentRowId, sourceParent);
    setRow(remote, 'wp_comments', targetParentRowId, targetParent);
    addWordPressGraphIdentityMapRow(local, {
      table: 'wp_comments',
      localId: sourceParentRowId,
      remoteId: targetParentRowId,
    ...
    tags.add('comment-parent-thread-reference-v3-identity-map');
    tags.add('comment-parent-thread-reference-v3-identity-map-ready');
  } else {
    ...
    setRow(base, 'wp_comments', sourceParentRowId, parent);
    setRow(local, 'wp_comments', sourceParentRowId, parent);
    setRow(remote, 'wp_comments', sourceParentRowId, {
      ...parent,
      comment_content: `RPP-0346 generated comment_parent stale remote private ${sourceParentId}`,
    });
    setRow(local, 'wp_comments', childRowId, makeComment(childId, {
      comment_post_ID: 1,
      comment_parent: sourceParentId,
    ...
    tags.add('comment-parent-thread-reference-v3-stale');
    tags.add('comment-parent-thread-reference-v3-stale-target');
    tags.add('comment-parent-thread-reference-v3-non-ready');
  }

  tags.add('comment-parent-thread-reference-v3');
  tags.add('comment-parent-thread-reference-v3-hash-only');
  tags.add('comment-parent-graph');
  tags.add('comment-graph');
  tags.add('wp-comments-create');
}
